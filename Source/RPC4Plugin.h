...
/// \details This should not be used at the same time as RPC3. This is a less functional version of RPC3, and is here
/// for users that do not want the Boost dependency of RPC3.
...
public:
    ...
    /// \brief Queue a call to RPC4::RegisterSlot() globally. Actual call occurs once RPC4 is attached to an instance of
    /// RakPeer or TCPInterface.
    ...
    /// \brief Queue a call to RPC4::RegisterBlockingFunction() globally. Actual call occurs once RPC4 is attached to an
    /// instance of RakPeer or TCPInterface.
    RPC4GlobalRegistration(
        const char* uniqueID,
    ...
    /// \brief Queue a call to RPC4::RegisterLocalCallback() globally. Actual call occurs once RPC4 is attached to an
    /// instance of RakPeer or TCPInterface.
...
public:
    ...
    /// When a call is made to call this function from the \a Call() or CallLoopback() function, the function pointer
    /// will be invoked with the passed bitStream to Call() and the actual Packet that RakNet got.
    /// \sa RegisterPacketCallback()
    /// \param[in] uniqueID Identifier to be associated with \a functionPointer. If this identifier is already in use,
    ...
    /// Register a slot, which is a function pointer to one or more implementations that supports this function
    /// signature When a signal occurs, all slots with the same identifier are called.
    /// \param[in] sharedIdentifier A string to identify the slot. Recommended to be the same as the name of the
    /// function.
    /// \param[in] functionPtr Pointer to the function. For C, just pass the name of the function. For C++, use
    /// ARPC_REGISTER_CPP_FUNCTION
    /// \param[in] callPriority Slots are called by order of the highest callPriority first. For slots with the same
    /// priority, they are called in the order they are registered
    void RegisterSlot(
    ...
    /// \brief Same as \a RegisterFunction, but is called with CallBlocking() instead of Call() and returns a value to
    /// the caller
    ...
    /// When a RakNet Packet with the specified identifier is returned, execute CallLoopback() on a function previously
    /// registered with RegisterFunction() For example, you could call "OnClosedConnection" whenever you get
    /// ID_DISCONNECTION_NOTIFICATION or ID_CONNECTION_LOST
    /// \param[in] uniqueID Identifier passed to RegisterFunction()
    /// \param[in] messageId What RakNet packet ID to call on, for example ID_DISCONNECTION_NOTIFICATION or
    /// ID_CONNECTION_LOST
    ...
    /// \note This function does not return until the remote system responds, disconnects, or was never connected to
    /// begin with
    ...
    /// \return true if successfully called. False on disconnect, function not registered, or not connected to begin
    /// with
    bool CallBlocking(
        const char*         uniqueID,
        RakNet::BitStream*  bitStream,
