...
public:
    ...
    /// If the instance of RakPeer running NATPunchthroughServer was bound to two
    /// IP addresses, then you can call FindRouterPortStride() This will determine
    /// the stride that your router uses when assigning ports, if your router is
    /// full-cone This function is also called automatically when you call OpenNAT
    /// - however, calling it earlier when you are connected to the facilitator
    ...
    /*
    /// \deprecated See FullyConnectedMesh2::StartVerifiedJoin() which is more
    flexible
    /// Same as calling OpenNAT for a list of systems, but reply is delayed until
    all systems pass.
    /// This is useful for peer to peer games where you want to connect to every
    system in the remote session, not just one particular system
    /// \note For cloud computing, all systems in the group must be connected to
    the same facilitator since we're only specifying one
    ...
    /// However, if you lose connection to the facilitator, you may not
    necessarily get above bool OpenNATGroup(DataStructures::List<RakNetGUID>
    ...
    */
    ...
    /// Get the port mappings you should pass to UPNP (for miniupnpc-1.6.20120410,
    /// for the function UPNP_AddPortMapping)
...
protected:
    ...
    void UpdateGroupPunchOnNatResult(SystemAddress facilitator, RakNetGUID
    targetSystem, SystemAddress targetSystemAddress, int result); // 0=failed,
    1=success, 2=ignore
    */
};
