...
RakNet::TimeUS NormalizeTime(RakNet::TimeUS timeIn) {
    ...
    if (timeIn >= lastNormalizedInputValue) {
        ...
        if (diff > GET_TIME_SPIKE_LIMIT) lastNormalizedReturnedValue += GET_TIME_SPIKE_LIMIT;
        else lastNormalizedReturnedValue += diff;
    } else lastNormalizedReturnedValue += GET_TIME_SPIKE_LIMIT;
