...
/// \file
/// \brief \b [Internal] A class which stores a user message, and all information associated with sending and receiving
/// that message.
///
...
/// This is the counter used for holding packet numbers, so we can detect duplicate packets.  It should be large enough
/// that if the variables Internally assumed to be 4 bytes, but written as 3 bytes in
/// ReliabilityLayer::WriteToBitStreamFromInternalPacket
typedef uint24_t MessageNumberType;

/// This is the counter used for holding ordered packet numbers, so we can detect out-of-order packets.  It should be
/// large enough that if the variables were to wrap, the newly wrapped values would no longer be in use.  Warning: Too
/// large of a value wastes bandwidth!
...
struct InternalPacketFixedSizeTransmissionHeader {
    ...
    /// The ID of the split packet, if we have split packets.  This is the maximum number of split messages we can send
    /// simultaneously per connection.
...
struct InternalPacket : public InternalPacketFixedSizeTransmissionHeader {
    ...
    /// This fixes a bug where pre-determining message numbers and then sending a message on a different channel creates
    /// a huge gap. This causes performance problems and causes those messages to timeout.
    bool messageNumberAssigned;
    /// Was this packet number used this update to track windowing drops or increases?  Each packet number is only used
    /// once per update.
