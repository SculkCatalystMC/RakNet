...
int RakNet::ReadyEvent::RemoteSystemCompByGuid(const RakNetGUID& key, const RemoteSystem& data) {
    if (key < data.rakNetGuid) return -1;
    else if (key == data.rakNetGuid) return 0;
    else return 1;
}

int RakNet::ReadyEvent::ReadyEventNodeComp(const int& key, ReadyEvent::ReadyEventNode* const& data) {
    if (key < data->eventId) return -1;
    else if (key == data->eventId) return 0;
    else return 1;
}
...
bool ReadyEvent::IsEventCompletionProcessing(int eventId) const {
    ...
    if (objectExists) {
        ...
        for (unsigned i = 0; i < ren->systemList.Size(); i++) {
            if (ren->systemList[i].lastReceivedStatus == ID_READY_EVENT_ALL_SET) anyAllReady = true;
            else allAllReady = false;
        }
...
bool ReadyEvent::AddToWaitList(int eventId, RakNetGUID guid) {
    ...
    // Don't do this, otherwise if we are trying to start a 3 player game, it will not allow the 3rd player to hit ready
    // if the first two players have already done so
    // if (IsLocked(eventIndex))
    //	return false; // Not in the list, but event is already completed, or is starting to complete, and adding more
    // waiters would fail this.

    unsigned i;
    unsigned numAdded = 0;
    if (guid == UNASSIGNED_RAKNET_GUID) {
        for (i = 0; i < mRakPeerInterface->GetMaximumNumberOfPeers(); i++) {
            RakNetGUID firstGuid = mRakPeerInterface->GetGUIDFromIndex(i);
...
void ReadyEvent::OnReadyEventQuery(Packet* packet) {
    ...
    if (objectExists) {
        ...
        // Force the non-default send, because our initial send may have arrived at a system that didn't yet create the
        // ready event
...
bool ReadyEvent::SetEventByIndex(int eventIndex, bool isReady) {
    ...
    if (isReady) ren->eventStatus = ID_READY_EVENT_SET;
    else ren->eventStatus = ID_READY_EVENT_UNSET;
...
unsigned ReadyEvent::CreateNewEvent(int eventId, bool isReady) {
    ...
    ren->eventId        = eventId;
    if (isReady == false) ren->eventStatus = ID_READY_EVENT_UNSET;
    else ren->eventStatus = ID_READY_EVENT_SET;
...
void ReadyEvent::PushCompletionPacket(unsigned eventId) {
    ...
    // Not necessary
    /*
    // Pass a packet to the user that we are now completed, as setting ourselves to signaled was the last thing being
    waited on Packet *p = AllocatePacketUnified(sizeof(MessageID)+sizeof(int)); RakNet::BitStream bs(p->data,
    sizeof(MessageID)+sizeof(int), false); bs.SetWriteOffset(0); bs.Write((MessageID)ID_READY_EVENT_ALL_SET);
    bs.Write(eventId);
