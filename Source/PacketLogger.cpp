...
void PacketLogger::FormatLine(
    ...
    if (printId) {
        if (splitPacketCount > 0 && splitPacketCount != (unsigned int)-1) idToPrint = "(SPLIT PACKET)";
        else idToPrint = IDTOString(id);
    }
...
void PacketLogger::OnDirectSocketSend(const char* data, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress) {
    ...
    FormatLine(
        ...
        bitsUsed,
        RakNet::GetTimeMS(),
        mRakPeerInterface->GetExternalID(remoteSystemAddress),
        remoteSystemAddress,
        (unsigned int)-1,
        (unsigned int)-1,
        (unsigned int)-1,
...
void PacketLogger::OnDirectSocketReceive(
    ...
    FormatLine(
        ...
        bitsUsed,
        RakNet::GetTime(),
        mRakPeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS),
        remoteSystemAddress,
        (unsigned int)-1,
        (unsigned int)-1,
        (unsigned int)-1,
...
void PacketLogger::OnReliabilityLayerNotification(
    ...
    char  str[1024];
    char* type;
    if (isError) type = (char*)"RcvErr";
    else type = (char*)"RcvWrn";
    FormatLine(
        str,
        type,
        errorMessage,
...
void PacketLogger::OnAck(unsigned int messageNumber, SystemAddress remoteSystemAddress, RakNet::TimeMS time) {
    char          str[384];
    char          str1[64], str2[62];
    SystemAddress localSystemAddress = mRakPeerInterface->GetExternalID(remoteSystemAddress);
...
void PacketLogger::OnInternalPacket(
    ...
    else reliableMessageNumber = internalPacket->reliableMessageNumber;
...
void PacketLogger::WriteMiscellaneous(const char* type, const char* msg) {
    char          str[1024];
    char          str1[64];
    SystemAddress localSystemAddress = mRakPeerInterface->GetInternalID();
    localSystemAddress.ToString(true, str1);
    RakNet::TimeMS time = RakNet::GetTimeMS();
    char           localtime[128];
    GetLocalTime(localtime);
