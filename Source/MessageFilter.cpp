...
int RakNet::FilterSetComp(const int& key, FilterSet* const& data) {
    if (key < data->filterSetID) return -1;
    else if (key == data->filterSetID) return 0;
    else return 1;
}
...
int MessageFilter::GetSystemFilterSet(AddressOrGUID systemAddress) {
    ...
    if (index.IsInvalid()) return -1;
    else return systemList.ItemAtIndex(index).filter->filterSetID;
}
...
FilterSet* MessageFilter::GetFilterSetByID(int filterSetID) {
    ...
    unsigned index;
    index = filterList.GetIndexFromKey(filterSetID, &objectExists);
    if (objectExists) return filterList[index];
    else {
...
void MessageFilter::OnInvalidMessage(FilterSet* filterSet, AddressOrGUID systemAddress, unsigned char messageID) {
    ...
    if (filterSet->banOnDisallowedMessage && mRakPeerInterface) {
        char str1[64];
        systemAddress.systemAddress.ToString(false, str1);
        mRakPeerInterface->AddToBanList(str1, filterSet->disallowedMessageBanTimeMS);
    }
    if (filterSet->kickOnDisallowedMessage) {
        if (mRakPeerInterface) mRakPeerInterface->CloseConnection(systemAddress, true, 0);
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
        else tcpInterface->CloseConnection(systemAddress.systemAddress);
#endif
    }
}
    ...
    if (GreaterThan(curTime - 1000, whenLastTimeoutCheck)) {
        ...
        for (index = 0; index < itemList.Size(); index++) {
            if (itemList[index].filter && itemList[index].filter->maxMemberTimeMS > 0
                ...
                if (itemList[index].filter->banOnFilterTimeExceed && mRakPeerInterface) {
                    char str1[64];
                    keyList[index].ToString(false, str1);
                    mRakPeerInterface->AddToBanList(str1, itemList[index].filter->timeExceedBanTimeMS);
                }
                if (mRakPeerInterface) mRakPeerInterface->CloseConnection(keyList[index], true, 0);
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
                else tcpInterface->CloseConnection(keyList[index].systemAddress);
#endif
...
PluginReceiveResult MessageFilter::OnReceive(Packet* packet) {
    ...
    default:
        ...
        } else messageId = packet->data[0];
        // If this system is filtered, check if this message is allowed.  If not allowed, return
        // RR_STOP_PROCESSING_AND_DEALLOCATE index = systemList.GetIndexFromKey(packet->addressOrGUID, &objectExists);
