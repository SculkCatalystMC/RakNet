...
Packet* PacketizedTCP::Receive(void) {
    ...
    while (incomingPacket) {
        ...
        if (incomingPacket->deleteData == true) {
                ...
                if (bq->GetBytesWritten() >= dataLength + sizeof(PTCPHeader)) {
                    ...
                    if (newWritten / 65536 != oldWritten / 65536) {
                        ...
                        outgoingPacket->length =
                            sizeof(MessageID) + sizeof(unsigned int) * 2 + sizeof(unsigned int) + 65536;
                        ...
                        memcpy(
                            outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 1,
                            &totalParts,
                            sizeof(unsigned int)
                        );
                        memcpy(
                            outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 2,
                            &oneChunkSize,
                            sizeof(unsigned int)
                        );
                        bq->IncrementReadOffset(sizeof(PTCPHeader));
                        bq->ReadBytes(
                            (char*)outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 3,
                            oneChunkSize,
                            true
                        );
            ...
            DeallocatePacket(incomingPacket);
            incomingPacket = 0;
        } else waitingPackets.Push(incomingPacket, _FILE_AND_LINE_);
