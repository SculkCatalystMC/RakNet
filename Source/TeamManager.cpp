...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
    if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
        ...
        // - remote Host executes JoinNoTeam() and broadcasts event. This may cause may cause rebalance if team
        // balancing is on.
        // - - JoinNoTeam(): Remove from all current and requested teams. Set no-team category.

        if (teams.Size() == 0 && mNoTeamSubcategory == teamSelection.teamParameter.noTeamSubcategory) {
            // No change
            return false;
        }
        ...
        // - JoinAnyTeam(requesterGuid): Attempt to join any team immediately.  If fails, send to all except requestGuid
        // UpdateTeamsRequestedToAny(). Else sends out new team, including to caller. On remote host, execute
        // JoinAnyTeamCheck(). If fails, this was because you were added to a team simultaneously on host. This is OK,
        // just ignore the call. Assuming JoinAnyTeamCheck() passed on remote host, call UpdateTeamsRequestedToAny() for
        // this player. execute JoinAnyTeam(packet->guid).
        ...
        // - JoinSpecificTeam(requesterGuid): Attempt to join specific team immediately. If fails, send to all except
        // requesterGuid to execute AddSpecificToRequested(). Else sends out new team, including to caller. On remote
        // host, execute JoinSpecificTeamCheck(). If fails, just ignore. Assuming JoinSpecificTeamCheck() passed on
        // host, call AddSpecificToRequestedList(). Execute JoinSpecificTeam(packet->guid)
...
TeamSelection TM_TeamMember::GetRequestedTeam(void) const {
    ...
    else if (joinTeamType == JOIN_NO_TEAM) return TeamSelection::NoTeam(mNoTeamSubcategory);
...
bool TM_TeamMember::LeaveTeam(TM_Team* team, NoTeamId _noTeamSubcategory) {
    ...
    if (teams.Size() == 0) {
        mNoTeamSubcategory = _noTeamSubcategory;
        joinTeamType       = JOIN_NO_TEAM;
    }
    ...
    // On remote host, execute check. If check passes, execute non-host changes, followed by host changes. Relay to all
    // but sender.
    BitStream bsOut;
    bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
    ...
    bsOut.Write(mNoTeamSubcategory);
    world->BroadcastToParticipants(&bsOut, UNASSIGNED_RAKNET_GUID);

    if (world->GetHost() == world->GetTeamManager()->GetMyGUIDUnified()) {
        // Rebalance teams
        world->FillRequestedSlots();
        world->EnforceTeamBalance(mNoTeamSubcategory);
    }
...
bool TM_TeamMember::DeserializeConstruction(TeamManager* teamManager, BitStream* constructionBitstream) {
    ...
    TM_World* _world = teamManager->GetWorldWithId(worldId);
    RakAssert(_world);
    constructionBitstream->Read(networkId);
    _world->ReferenceTeamMember(this, networkId);
    ...
    for (unsigned int i = 0; i < teamsRequestedSize; i++) {
        ...
        if (hasTeamToLeave) {
            ...
            rt.teamToLeave = _world->GetTeamByNetworkID(teamToLeaveId);
        ...
        if (hasTeamRequested) {
            ...
            rt.requested = _world->GetTeamByNetworkID(teamRequestedId);
    ...
    if (success) _world->teamManager->ProcessTeamAssigned(constructionBitstream);
    return success;
}
...
NoTeamId TM_TeamMember::GetNoTeamId(void) const { return mNoTeamSubcategory; }
...
void TM_TeamMember::UpdateTeamsRequestedToAny(void) {
    ...
    joinAnyRequestIndex =
        world->teamRequestIndex++; // In case whenRequested is the same between two teams when sorting team requests
}
...
bool TM_Team::DeserializeConstruction(TeamManager* teamManager, BitStream* constructionBitstream) {
    ...
    RakAssert(b);
    if (b) {
        _world->ReferenceTeam(this, ID, balancingApplies);
    }
    return b;
}
...
void TM_World::DereferenceTeam(TM_Team* team, NoTeamId noTeamSubcategory) {
    ...
    for (i = 0; i < teams.Size(); i++) {
        if (teams[i] == team) {
            TM_Team* _team = teams[i];
            while (_team->teamMembers.Size()) {
                _team->teamMembers[_team->teamMembers.Size() - 1]->LeaveTeam(_team, noTeamSubcategory);
            }
            teams.RemoveAtIndex(i);

            teamsHash.Remove(_team->GetNetworkID(), _FILE_AND_LINE_);
...
void TM_World::KickExcessMembers(NoTeamId noTeamId) {
    ...
    // For each team that applies balancing, if the team is overfull, put on a team that is not overfull if the team has
    // ALLOW_JOIN_REBALANCING set If cannot move the player to another team, just take the player off the team and set
    // to noTeamId if they have no team at that point
...
void TeamManager::EncodeTeamAssigned(RakNet::BitStream* bitStream, TM_TeamMember* teamMember) {
    ...
    bitStream->Write(teamMember->mNoTeamSubcategory);
...
void TeamManager::ProcessTeamAssigned(RakNet::BitStream* bsIn) {
    ...
    if (teamMember) {
        ...
        teamMember->mNoTeamSubcategory = noTeamId;
