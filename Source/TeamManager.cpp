...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
    if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
        ...
        // - Execute JoinNoTeam() locally. Return ID_TEAM_BALANCER_TEAM_ASSIGNED
        // locally.
        // - If we are host, broadcast event. Done.
        // - Send to remote host event to call JoinNoTeam()
        // - remote Host executes JoinNoTeam() and broadcasts event. This may cause
        // may cause rebalance if team balancing is on.
        // - - JoinNoTeam(): Remove from all current and requested teams. Set
        // no-team category.
        ...
        // On local, call UpdateTeamsRequestedToAny(). Send event to also execute
        // this to remote host If we are host, execute JoinAnyTeam(myguid).
        // - JoinAnyTeam(requesterGuid): Attempt to join any team immediately.  If
        // fails, send to all except requestGuid UpdateTeamsRequestedToAny(). Else
        // sends out new team, including to caller. On remote host, execute
        // JoinAnyTeamCheck(). If fails, this was because you were added to a team
        // simultaneously on host. This is OK, just ignore the call. Assuming
        // JoinAnyTeamCheck() passed on remote host, call
        // UpdateTeamsRequestedToAny() for this player. execute
        ...
        // On local, call AddToRequestedTeams(). Send event to also execute this to
        // remote host If we are host, execute JoinSpecificTeam(myguid)
        // - JoinSpecificTeam(requesterGuid): Attempt to join specific team
        // immediately. If fails, send to all except requesterGuid to execute
        // AddSpecificToRequested(). Else sends out new team, including to caller.
        // On remote host, execute JoinSpecificTeamCheck(). If fails, just ignore.
        // Assuming JoinSpecificTeamCheck() passed on host, call
...
TeamSelection TM_TeamMember::GetRequestedTeam(void) const {
    if (teamsRequested.Size() > 0) return TeamSelection::SpecificTeam(teamsRequested[0].requested);
    else if (joinTeamType == JOIN_NO_TEAM) return TeamSelection::NoTeam(noTeamSubcategory);
    else return TeamSelection::AnyAvailable();
}
...
bool TM_TeamMember::LeaveTeam(TM_Team* team, NoTeamId noTeamCategory) {
    if (LeaveTeamCheck(team) == false) return false;

    RemoveFromSpecificTeamInternal(team);
    if (teams.Size() == 0) {
        noTeamSubcategory = noTeamCategory;
        joinTeamType      = JOIN_NO_TEAM;
    }
    ...
    // On local, call RemoteFromTeamsList(). Send event to also execute this to
    // remote host If we are host, execute OnLeaveTeamEvent(myGuid)
    ...
    // On remote host, execute RemoteFromTeamsList() followed by
    // OnLeaveTeamEvent(packet->guid)
    ...
    // On remote host, execute check. If check passes, execute non-host changes,
    // followed by host changes. Relay to all but sender.
    BitStream bsOut;
    bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
...
bool TM_TeamMember::LeaveAllTeams(NoTeamId noTeamCategory) {
    return RequestTeam(TeamSelection::NoTeam(noTeamCategory));
}
...
bool TM_TeamMember::DeserializeConstruction(TeamManager* teamManager, BitStream* constructionBitstream) {
    ...
    TM_World* teamWorld = teamManager->GetWorldWithId(worldId);
    RakAssert(teamWorld);
    constructionBitstream->Read(networkId);
    teamWorld->ReferenceTeamMember(this, networkId);
    ...
    for (unsigned int i = 0; i < teamsRequestedSize; i++) {
        ...
        if (hasTeamToLeave) {
            ...
            rt.teamToLeave = teamWorld->GetTeamByNetworkID(teamToLeaveId);
            RakAssert(rt.teamToLeave);
        } else rt.teamToLeave = 0;
        ...
        if (hasTeamRequested) {
            ...
            rt.requested = teamWorld->GetTeamByNetworkID(teamRequestedId);
            RakAssert(rt.requested);
        }
        rt.whenRequested = RakNet::GetTime();
        rt.requestIndex  = teamWorld->teamRequestIndex++; // In case whenRequested is the same between
...
bool TM_Team::DeserializeConstruction(TeamManager* teamManager, BitStream* constructionBitstream) {
    ...
    RakAssert(b);
    if (b) {
        teamWorld->ReferenceTeam(this, ID, balancingApplies);
    }
    return b;
}
...
void TM_World::ReferenceTeam(TM_Team* team, NetworkID networkId, bool applyBalancing) {
    ...
    // If autobalancing is on, and the team lock state supports it, then call
    // EnforceTeamBalancing()
...
void TM_World::DereferenceTeam(TM_Team* team, NoTeamId noTeamSubcategory) {
    ...
    for (i = 0; i < teams.Size(); i++) {
        if (teams[i] == team) {
            TM_Team* teamToRemove = teams[i];
            while (teamToRemove->teamMembers.Size()) {
                teamToRemove->teamMembers[teamToRemove->teamMembers.Size() - 1]->LeaveTeam(
                    teamToRemove,
                    noTeamSubcategory
                );
            }
            ...
            teamsHash.Remove(teamToRemove->GetNetworkID(), _FILE_AND_LINE_);
...
void TM_World::KickExcessMembers(NoTeamId noTeamId) {
    ...
    // For each team that applies balancing, if the team is overfull, put on a
    // team that is not overfull if the team has ALLOW_JOIN_REBALANCING set If
    // cannot move the player to another team, just take the player off the team
    // and set to noTeamId if they have no team at that point

    TeamMemberLimit balancedTeamLimit;
    if (balanceTeamsIsActive) balancedTeamLimit = GetBalancedTeamLimit();
    else balancedTeamLimit = (TeamMemberLimit)-1;
...
unsigned int
TM_World::GetAvailableTeamIndexWithFewestMembers(TeamMemberLimit secondaryLimit, JoinPermissions joinPermissions) {
...
int TM_World::JoinSpecificTeam(
    ...
    if (team->GetJoinPermissions() & ALLOW_JOIN_SPECIFIC_TEAM) {
        if (balanceTeamsIsActive == false || teamsWeAreLeaving.Size() == 0) {
            ...
            // We must be on one team, target must be on one team, and we want to
            // exchange teams
            if (teamsWeAreLeaving.Size() == 1) {
                ...
                if (j != (unsigned int)-1) {
                    ...
                    // Calling function sends ID_RUN_RemoveFromTeamsRequestedAndAddTeam
                    // which pushes ID_TEAM_BALANCER_TEAM_ASSIGNED for teamMember
...
void TeamManager::OnJoinAnyTeam(Packet* packet, TM_World* world) {
    ...
    if (teamMember) {
        ...
        if (resultCode == 1) {
            // Broadcast packet - remote systems should clear requested teams to none,
            // and add the team we joined. Broadcast includes non-host sender (all
            // participants)
            BitStream bsOut;
            bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
            bsOut.WriteCasted<MessageID>(ID_RUN_UpdateTeamsRequestedToNoneAndAddTeam);
            ...
            // SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->guid,
            // false);
            world->BroadcastToParticipants(&bsOut, UNASSIGNED_RAKNET_GUID);
...
void TeamManager::OnJoinRequestedTeam(Packet* packet, TM_World* world) {
    ...
    if (teamToJoin && teamMember) {
        if (isTeamSwitch) {
            if (teamMember->SwitchSpecificTeamCheck(teamToJoin, teamToLeave, packet->guid == GetMyGUIDUnified())
                == false)
                return;
        ...
        if (resultCode == 1) {
            // Broadcast packet - remote systems should remove from requested teams
            // and add the team we joined. Broadcast includes non-host sender (all
            // participants)
            BitStream bsOut;
            bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
...
void TeamManager::OnRemoveFromTeamsRequestedAndAddTeam(Packet* packet, TM_World* world) {
    ...
    if (team && teamMember) {
        teamMember->StoreLastTeams();
        if (teamToLeave) teamMember->RemoveFromSpecificTeamInternal(teamToLeave);
        else if (isTeamSwitch == true && switchSpecificTeam == false) teamMember->RemoveFromAllTeamsInternal();
...
void TeamManager::OnAddToRequestedTeams(Packet* packet, TM_World* world) {
    ...
    if (team && teamMember) {
        if (isTeamSwitch) teamMember->AddToRequestedTeams(team, teamToLeave);
        else teamMember->AddToRequestedTeams(team);
    }
}
...
void TeamManager::OnSetMemberLimit(Packet* packet, TM_World* world) {
    ...
    if (team) {
        ...
        if (world->GetHost() == world->GetTeamManager()->GetMyGUIDUnified()) {
            if (packet->guid == GetMyGUIDUnified())
                world->BroadcastToParticipants(packet->data, packet->length, packet->guid);
            else world->BroadcastToParticipants(packet->data, packet->length, UNASSIGNED_RAKNET_GUID);
            world->FillRequestedSlots();
...
void TeamManager::OnSetBalanceTeams(Packet* packet, TM_World* world) {
    ...
    if (world->GetHost() == world->GetTeamManager()->GetMyGUIDUnified()) {
        ...
        if (balanceTeams) world->EnforceTeamBalance(noTeamId);
        else world->FillRequestedSlots();
    }
}
