        ...
        if (_stricmp(*parameterList, "help") == 0 && numParameters <= 2) {
            ...
            } else // numParameters == 2, including the help tag
            {
                ...
                if (commandParsed == false) {
                    ...
                    RakNet::RegisteredCommand _rc;
                    for (i = 0; i < commandParserList.Size(); i++) {
                        if (commandParserList[i]->GetRegisteredCommand(parameterList[1], &_rc)) {
                            if (_rc.parameterCount == RakNet::CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS)
                                transport->Send(
                                    p->systemAddress,
                                    "(Variable parms): %s %s\r\n",
                                    _rc.command,
            ...
            if (numParameters >= 2) // At minimum <CommandParserName> <Command>
            {
                ...
                if (**parameterList >= '0' && **parameterList <= '9') {
                    ...
                    // // Prefixing with the name of a command parser directs to that parser.  See if the first word
                    // matches a parser
                    for (i = 0; i < commandParserList.Size(); i++) {
