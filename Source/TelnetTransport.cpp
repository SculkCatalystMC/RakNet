    ...
    */
    ...
    if (p->length == 3 && p->data[0] == 27 && p->data[1] == 91 && p->data[2] == 65) {
        if (remoteClient->lastSentTextInput[0]) {
            ...
            for (int j = 0; remoteClient->textInput[j]; j++) remoteClient->textInput[j] = 8;
...
SystemAddress TelnetTransport::HasNewIncomingConnection(void) {
    ...
    // 03/16/06 Can't force the stupid windows telnet to use line mode or local echo so now I have to track all the
    // remote players and their input buffer
