...
public:
    ...
    /// \brief Given a remote system identified by RakNetGUID, return the list of SystemAddresses and RakNetGUIDs they
    /// are connected to
    ...
    /// \param[in,out] outLength On input, the size of \a saOut and \a guidOut. On output, modified to reflect the
    /// number of elements actually written
    /// \return True if \a remoteSystemGuid was found. Otherwise false, and \a saOut, \a guidOut remain unchanged. \a
    ...
    /// Returns the average ping between two systems in the connection graph. Returns -1 if no connection exists between
    /// those systems
    ...
    /// If you need one system in the peer to peer group to relay data, have the FullyConnectedMesh2 host call this
    /// function after host migration, and use that system
    RakNetGUID GetLowestAveragePingSystem(void) const;

    /// \brief If called with false, then new connections are only added to the connection graph when you call
    /// ProcessNewConnection();
    /// \details This is useful if you want to perform validation before connecting a system to a mesh, or if you want a
    /// submesh (for example a server cloud)
    /// \param[in] b True to automatically call ProcessNewConnection() on any new connection, false to not do so.
    /// Defaults to true.
    ...
    /// ProcessNewConnection() on new connections
    ...
    struct RemoteSystem {
        DataStructures::
            OrderedList<SystemAddressAndGuid, SystemAddressAndGuid, ConnectionGraph2::SystemAddressAndGuidComp>
                   remoteConnections;
        RakNetGUID guid;
    };
    /// \internal
