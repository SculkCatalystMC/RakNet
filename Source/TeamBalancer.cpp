...
void TeamBalancer::RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam) {
    ...
    // Also flag that we have requested a team, and record desiredTeam in case the host changes and it needs to be
    // resent.
    BitStream bsOut;
    ...
    mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, hostGuid, false);
}
...
void TeamBalancer::RequestAnyTeam(NetworkID memberId) {
    ...
    for (unsigned int i = 0; i < myTeamMembers.Size(); i++) {
        if (myTeamMembers[i].memberId == memberId) {
            foundMatch = true;
            if (myTeamMembers[i].currentTeam != UNASSIGNED_TEAM_ID) return;
            else myTeamMembers[i].requestedTeam = UNASSIGNED_TEAM_ID;
            break;
        }
    }
...
PluginReceiveResult TeamBalancer::OnReceive(Packet* packet) {
    ...
    // - This user does not already have a team. Assign a team as if the user called RequestAnyTeam(), with a preference
    // for the requested team. Return ID_TEAM_BALANCER_TEAM_ASSIGNED once the team has been assigned. If teams are not
    // locked
    // - If even team balancing is on, only assign this user if this would not cause teams to be unbalanced. If teams
    // WOULD be unbalanced, then flag this user as wanting to join this team. Return
...
void TeamBalancer::OnAttach(void) { hostGuid = mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS); }
            ...
            if (teamMembers[droppedMemberIndex].memberGuid == rakNetGUID) {
                ...
                if (lockTeams == false) {
                    if (forceTeamsToBeEven) {
                        ...
                        // - teams were NOT forced to be even, and the team the dropped player on was full, then move
                        // users wanting to join that team (if any)
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet* packet) {
    ...
    for (uint8_t i = 0; i < requestedTeamChangeListSize; i++) {
        ...
        if (memberIndex == (unsigned int)-1) {
            ...
            // Do not process balancing on requested teams, since we don't necessarily have all data from all systems
            // yet and hopefully the state during the host migration was stable.
...
unsigned int TeamBalancer::AddTeamMember(const TeamMember& tm) {
    ...
    if (teamMemberCounts.Size() < tm.currentTeam) teamMemberCounts.Replace(1, 0, tm.currentTeam, _FILE_AND_LINE_);
    else teamMemberCounts[tm.currentTeam] = teamMemberCounts[tm.currentTeam] + 1;
...
void TeamBalancer::GetMinMaxTeamMembers(int& minMembersOnASingleTeam, int& maxMembersOnASingleTeam) {
    ...
    if ((teamMembers.Size() % teamLimits.Size()) == 0) maxMembersOnASingleTeam = minMembersOnASingleTeam;
    else maxMembersOnASingleTeam = minMembersOnASingleTeam + 1;
}
void TeamBalancer::EvenTeams(void) {
    // Ensure all teams are even. If not, pick players at random from overpopulated teams, and move to underpopulated
    // teams.
    ...
    // First select among players that have requested to switch teams, if any, before choosing players that did not want
    // to switch teams. Players that are moved should be notified of ID_TEAM_BALANCER_TEAM_ASSIGNED
    ...
    for (j = 0; j < sourceTeamNumbers.Size(); j++) {
        ...
        for (i = 0; i < teamMembers.Size(); i++) {
            if (teamMembers[i].currentTeam == sourceTeamNumbers[j]) {
                if (teamMembers[i].requestedTeam == targetTeamNumber) preferredSwapIndices.Push(i, _FILE_AND_LINE_);
                else potentialSwapIndices.Push(i, _FILE_AND_LINE_);
            }
        }
    }
...
void TeamBalancer::NotifyTeamAssigment(unsigned int teamMemberIndex) {
    ...
    mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, teamMembers[teamMemberIndex].memberGuid, false);
}
bool TeamBalancer::WeAreHost(void) const {
    return hostGuid == mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
...
void TeamBalancer::NotifyTeamsLocked(RakNetGUID target, TeamId requestedTeam) {
    ...
    mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, target, false);
}
