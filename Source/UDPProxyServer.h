...
/// \file
/// \brief A RakNet plugin performing networking to communicate with UDPProxyServer. It allows UDPProxyServer to control
/// our instance of UDPForwarder.
///
...
struct UDPProxyServerResultHandler {
    ...
    /// The coordinator operator set a different password in UDPProxyCoordinator::SetRemoteLoginPassword() than what we
    /// passed
    /// \param[out] usedPassword The password we passed to UDPProxyServer::LoginToCoordinator()
    /// \param[out] proxyServer The plugin calling this callback
...
public:
    ...
    /// If the server and coordinator are on the same LAN, you need to call SetServerPublicIP() to tell the client what
    /// address to connect to
    /// \param[in] ip IP address to report in UDPProxyClientResultHandler::OnForwardingSuccess() and
    ...
    /// UDPForwarder::Startup(), UDPForwarder::Shutdown(), and UDPForwarder::Update() are called automatically by the
    /// plugin
