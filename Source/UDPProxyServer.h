...
public:
    ...
    /// \brief The server IP reported to the client is the IP address from the
    /// server to the coordinator. If the server and coordinator are on the same
    /// LAN, you need to call SetServerPublicIP() to tell the client what address
    /// to connect to
    /// \param[in] ip IP address to report in
    /// UDPProxyClientResultHandler::OnForwardingSuccess() and
