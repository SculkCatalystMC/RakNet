...
destinationSystems, const SystemAddress &facilitator)
{
        ConnectionState cs = rakPeerInterface->GetConnectionState(facilitator);
        if (cs!=IS_CONNECTED)
                return false;
...
RakNet::OP_NEW<GroupPunchRequest>(_FILE_AND_LINE_);
...
void NatPunchthroughClient::Update(void) {
    ...
    if (sp.nextActionTime && sp.nextActionTime < time) {
        ...
        {
                // Send to unused port. We do not want the message to arrive, just
        to open our router's table SystemAddress sa=sp.targetAddress; int
        ttlSendIndex; for (ttlSendIndex=0; ttlSendIndex <=
        ...
        (sp.targetAddress.GetPort()+ttlSendIndex)); SendTTL(sa);
                ...
                // Wait 250 milliseconds for next stage. The delay is so that even
        with timing errors both systems send out the
        ...
        */
            ...
            if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
                if (natPunchthroughDebugInterface) {
                    ...
                    natPunchthroughDebugInterface->OnClientMessage(
                        RakNet::RakString(
                            "Likely bidirectional punchthrough failure to "
        ...
        if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
            ...
            if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
                if (natPunchthroughDebugInterface) {
                    ...
                    natPunchthroughDebugInterface->OnClientMessage(
                        RakNet::RakString(
                            "Likely unidirectional punchthrough failure to "
                            "guid %s, system address %s.",
                            guidString,
                            ipAddressString
                        )
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
    if (pc.retryOnFailure == false) {
        if (natPunchthroughDebugInterface) {
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "Failed punchthrough once. Returning failure to "
    ...
    for (i = 0; i < failedAttemptList.Size(); i++) {
        if (failedAttemptList[i].guid == sp.targetGuid) {
            if (natPunchthroughDebugInterface) {
                ...
                natPunchthroughDebugInterface->OnClientMessage(
                    RakNet::RakString(
                        "Failed punchthrough twice. Returning failure to "
                        "guid %s, system address %s to user.",
                        guidString,
                        ipAddressString
                    )
    ...
    if (rakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
        if (natPunchthroughDebugInterface) {
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "Not connected to facilitator, so cannot retry "
    ...
    if (natPunchthroughDebugInterface) {
        ...
        natPunchthroughDebugInterface->OnClientMessage(
            RakNet::RakString(
                "First punchthrough failure on guid %s, system "
...
PluginReceiveResult NatPunchthroughClient::OnReceive(Packet* packet) {
    ...
    case ID_OUT_OF_BAND_INTERNAL:
        if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
            ...
            if (natPunchthroughDebugInterface)
                natPunchthroughDebugInterface->OnClientMessage(RakString(
                    "HAS_PORT_STRIDE %i. First external port %i. Second "
                    "external port %i.",
                    portStride,
                    externalPort,
                    externalPort2
            ...
            if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL && sp.targetGuid == packet->guid) {
                ...
                if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
                    ...
                    if (natPunchthroughDebugInterface) {
                        ...
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString(
                                "Received ID_NAT_ESTABLISH_UNIDIRECTIONAL "
                                "from guid %s, system address %s.",
                                guidString,
                                ipAddressString
                            )
                ...
                if (mostRecentExternalPort == 0) {
                    ...
                    if (sp.testMode != SendPing::TESTING_INTERNAL_IPS
                        ...
                        // nextExternalPort += portStride *
                        // (pc.MAX_PREDICTIVE_PORT_RANGE+1);
                        mostRecentExternalPort = ourExternalPort;

                        if (natPunchthroughDebugInterface) {
                            natPunchthroughDebugInterface->OnClientMessage(RakString(
                                "ID_NAT_ESTABLISH_BIDIRECTIONAL: New "
                                "mostRecentExternalPort %i",
                                mostRecentExternalPort
                            ));
                        }
                ...
                if (natPunchthroughDebugInterface) {
                    ...
                    if (removedFromFailureQueue)
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString(
                                "Punchthrough to guid %s, system address %s "
    ...
    case ID_NAT_ALREADY_IN_PROGRESS: {
        ...
        if (natPunchthroughDebugInterface) {
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "Punchthrough retry to guid %s failed due to "
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
    ...
    if (natPunchthroughDebugInterface) {
        ...
        if (oobId == ID_NAT_ESTABLISH_UNIDIRECTIONAL)
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "%I64d: %s: OOB ID_NAT_ESTABLISH_UNIDIRECTIONAL to "
                    "guid %s, system address %s.\n",
                    serverTime,
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "%I64d: %s: OOB ID_NAT_ESTABLISH_BIDIRECTIONAL to "
                    "guid %s, system address %s.\n",
                    serverTime,
                    TestModeToString(sp.testMode),
                    guidString,
...
void NatPunchthroughClient::UpdateGroupPunchOnNatResult(SystemAddress
facilitator, RakNetGUID targetSystem, SystemAddress targetSystemAddress, int
result)
{
        GroupPunchRequest *gpr;
        unsigned long i,j,k;
...
_FILE_AND_LINE_); gpr->passedListAddress.Push(targetSystemAddress,
_FILE_AND_LINE_);
                                        }
                                        else
                                        {
...
_FILE_AND_LINE_);
                ...
                if (gpr->pendingList.Size()==0)
                {
                        ...
                        output.WriteCasted<unsigned
char>(gpr->passedListGuid.Size()); for (k=0; k < gpr->passedListGuid.Size();
k++)
                        {
                        ...
                        output.WriteCasted<unsigned
char>(gpr->ignoredList.Size()); for (k=0; k < gpr->ignoredList.Size(); k++)
                        {
                                output.Write(gpr->ignoredList[k]);
                        }
                        output.WriteCasted<unsigned
char>(gpr->failedList.Size()); for (k=0; k < gpr->failedList.Size(); k++)
                        {
                                output.Write(gpr->failedList[k]);
                        }

                        Packet *p =
AllocatePacketUnified(output.GetNumberOfBytesUsed());
                        p->systemAddress=gpr->facilitator;
                        p->systemAddress.systemIndex=(SystemIndex)-1;
                        p->guid=rakPeerInterface->GetGuidFromSystemAddress(gpr->facilitator);
                        p->wasGeneratedLocally=true;
                        memcpy(p->data, output.GetData(),
output.GetNumberOfBytesUsed()); rakPeerInterface->PushBackPacket(p, true);
