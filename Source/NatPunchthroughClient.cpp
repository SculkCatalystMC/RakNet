...
void NatPunchthroughClient::FindRouterPortStride(const SystemAddress& facilitator) {
    ConnectionState cs = mRakPeerInterface->GetConnectionState(facilitator);
    ...
    mRakPeerInterface->Send(&outgoingBs, HIGH_PRIORITY, RELIABLE_ORDERED, 0, facilitator, false);
}
...
/*
bool NatPunchthroughClient::OpenNATGroup(DataStructures::List<RakNetGUID> destinationSystems, const SystemAddress
&facilitator)
{
...
void NatPunchthroughClient::Update(void) {
    ...
    if (sp.nextActionTime && sp.nextActionTime < time) {
        ...
        else if (sp.testMode==SendPing::SEND_WITH_TTL)
        {
            ...
            // Wait 250 milliseconds for next stage. The delay is so that even with timing errors both systems send out
        the
        ...
        */
            ...
            SystemAddress sa = sp.targetAddress;
            if (sp.targetGuid < mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS))
                sa.SetPortHostOrder((unsigned short)(1024 + sp.attemptCount));
            else sa.SetPortHostOrder((unsigned short)(sa.GetPort() + sp.attemptCount));
            SendOutOfBand(sa, ID_NAT_ESTABLISH_UNIDIRECTIONAL);
        ...
        if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
            ...
            if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
                ...
                else sp.nextActionTime = time + pc.TIME_BETWEEN_PUNCH_ATTEMPTS_EXTERNAL - delta;
            }
        }
    }
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
    ...
    if (mRakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
        if (natPunchthroughDebugInterface) {
            char ipAddressString[32];
            sp.targetAddress.ToString(true, ipAddressString);
            char guidString[128];
            ...
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString(
                    "Not connected to facilitator, so cannot retry punchthrough after first failure. Returning failure "
                    "onj guid %s, system address %s to user.",
                    guidString,
                    ipAddressString
                )
            );
...
PluginReceiveResult NatPunchthroughClient::OnReceive(Packet* packet) {
    ...
    case ID_NAT_RESPOND_BOUND_ADDRESSES: {
        ...
        if (boundAddressCount < 2) {
            if (natPunchthroughDebugInterface)
                natPunchthroughDebugInterface->OnClientMessage(RakString(
                    ...
                    mRakPeerInterface->GetExternalID(packet->systemAddress).ToString()
                ));
        ...
        for (int i = 0; i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
            ...
            if (boundAddresses[i] != packet->systemAddress) {
                ...
                uint16_t externalPort = mRakPeerInterface->GetExternalID(packet->systemAddress).GetPort();
    ...
    case ID_OUT_OF_BAND_INTERNAL:
        if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
            ...
            // sp.targetGuid==packet->guid is because the internal IP addresses reported may include loopbacks not
            // reported by RakPeer::IsLocalIP()
            if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL && sp.targetGuid == packet->guid) {
                ...
                if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
                    ...
                    if (natPunchthroughDebugInterface) {
                        ...
                        natPunchthroughDebugInterface->OnClientMessage(
                            RakNet::RakString(
                                "PUNCHING_FIXED_PORT: Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, system "
                                "address %s.",
                                guidString,
                                ipAddressString
                            )
                ...
                if (mostRecentExternalPort == 0) {
                    ...
                    if (sp.testMode != SendPing::TESTING_INTERNAL_IPS
                        ...
                        if (hasPortStride != HAS_PORT_STRIDE) {
                            ...
                            if (natPunchthroughDebugInterface) {
                                natPunchthroughDebugInterface->OnClientMessage(RakString(
                                    "ID_NAT_ESTABLISH_BIDIRECTIONAL: Estimated port stride from incoming connection at "
                                    "%i. ourExternalPort=%i mostRecentExternalPort=%i",
                                    portStride,
                                    ourExternalPort,
                                    mostRecentExternalPort
                                ));
    ...
    case ID_NAT_TARGET_UNRESPONSIVE: {
        const char* reason;
        if (packet->data[0] == ID_NAT_TARGET_NOT_CONNECTED) reason = (char*)"ID_NAT_TARGET_NOT_CONNECTED";
        else if (packet->data[0] == ID_NAT_CONNECTION_TO_TARGET_LOST)
            reason = (char*)"ID_NAT_CONNECTION_TO_TARGET_LOST";
        else reason = (char*)"ID_NAT_TARGET_UNRESPONSIVE";
        ...
        for (i = 0; i < failedAttemptList.Size(); i++) {
            if (failedAttemptList[i].guid == targetGuid) {
                ...
                // If the retry target is not connected, or loses connection, or is not responsive, then previous
                // failures cannot be retried.
...
void NatPunchthroughClient::SendTTL(const SystemAddress& sa) {
    ...
    mRakPeerInterface->SendTTL(ipAddressString, sa.GetPort(), 2);
}
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
    ...
    if (natPunchthroughDebugInterface) {
        ...
        // server - diff = my time
        // server = myTime + diff
        RakNet::Time clockDifferential = mRakPeerInterface->GetClockDifferential(sp.facilitator);
...
void NatPunchthroughClient::OnNewConnection(
    ...
    SystemAddress ourExternalId = mRakPeerInterface->GetExternalID(systemAddress);
...
void NatPunchthroughClient::OnClosedConnection(
    ...
    if (sp.facilitator == systemAddress) {
        // If we lose the connection to the facilitator, all previous failures not currently in progress are returned as
        // such
        unsigned int i = 0;
...
void NatPunchthroughClient::GetUPNPPortMappings(
    ...
    if (mostRecentExternalPort == 0)
        mostRecentExternalPort = mRakPeerInterface->GetExternalID(natPunchthroughServerAddress).GetPort();
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet* packet) {
    ...
    if (mostRecentExternalPort == 0) {
        mostRecentExternalPort = mRakPeerInterface->GetExternalID(packet->systemAddress).GetPort();
    ...
    if (hasPortStride == HAS_PORT_STRIDE) portWithStride = mostRecentExternalPort + portStride;
    else portWithStride = mostRecentExternalPort;
...
void                       NatPunchthroughClient::OnReadyForNextPunchthrough(void) {
    if (mRakPeerInterface == 0) return;

    sp.nextActionTime = 0;

    RakNet::BitStream outgoingBs;
    outgoingBs.Write((MessageID)ID_NAT_CLIENT_READY);
    mRakPeerInterface->Send(&outgoingBs, HIGH_PRIORITY, RELIABLE_ORDERED, 0, sp.facilitator, false);
}
...
void NatPunchthroughClient::UpdateGroupPunchOnNatResult(SystemAddress facilitator, RakNetGUID targetSystem,
