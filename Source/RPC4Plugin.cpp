...
void RPC4::CallLoopback(const char* uniqueID, RakNet::BitStream* bitStream) {
    ...
    if (skhi.IsInvalid() == true) {
        if (mRakPeerInterface)
            ...
            p = tcpInterface->AllocatePacket(
                sizeof(MessageID) + sizeof(unsigned char) + (unsigned int)strlen(uniqueID) + 1
            );
#endif

        if (mRakPeerInterface) p->guid = mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
        else p->guid = UNASSIGNED_RAKNET_GUID;
#endif
    ...
    if (mRakPeerInterface) p = AllocatePacketUnified(out.GetNumberOfBytesUsed());
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
    else p = tcpInterface->AllocatePacket(out.GetNumberOfBytesUsed());
#endif
...
bool RPC4::CallBlocking(
    ...
    RakAssert(mRakPeerInterface);
    ConnectionState cs;
    cs = mRakPeerInterface->GetConnectionState(systemIdentifier);
    ...
    while (gotBlockingReturnValue == false) {
        ...
        packet = mRakPeerInterface->Receive();
        ...
        if (packet) {
            if ((packet->data[0] == ID_CONNECTION_LOST || packet->data[0] == ID_DISCONNECTION_NOTIFICATION)
                ...
                mRakPeerInterface->PushBackPacket(packet, true);
                while (packetQueue.Size()) mRakPeerInterface->PushBackPacket(packetQueue.Pop(), true);
                return false;
...
void RPC4::Signal(
    ...
    if (invokeLocal) {
        ...
        Packet p;
        p.guid                = mRakPeerInterface->GetMyGUID();
        p.systemAddress       = mRakPeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS);
...
void RPC4::OnAttach(void) {
    ...
    for (i = 0; i < globalRegistrationIndex; i++) {
        ...
        else RegisterLocalCallback(globalRegistrationBuffer[i].functionName, globalRegistrationBuffer[i].messageId);
    }
}
