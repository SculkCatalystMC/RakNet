...
/// \details ReplicaManager3 tracks your game objects and automates the networking for replicating them across the
/// network<BR> As objects are created, destroyed, or serialized differently, those changes are pushed out to other
/// systems.<BR> To use:<BR> <OL> <LI>Derive from Connection_RM3 and implement Connection_RM3::AllocReplica(). This is a
/// factory function where given a user-supplied identifier for a class (such as name) return an instance of that class.
/// Should be able to return any networked object in your game. <LI>Derive from ReplicaManager3 and implement
/// AllocConnection() and DeallocConnection() to return the class you created in step 1. <LI>Derive your networked game
/// objects from Replica3. All pure virtuals have to be implemented, however defaults are provided for
/// Replica3::QueryConstruction(), Replica3::QueryRemoteConstruction(), and Replica3::QuerySerialization() depending on
/// your network architecture. <LI>When a new game object is created on the local system, pass it to
/// ReplicaManager3::Reference(). <LI>When a game object is destroyed on the local system, and you want other systems to
/// know about it, call Replica3::BroadcastDestruction()
/// </OL>
/// <BR>
...
public:
    ...
    /// \details The connection object represents a remote system connected to you that is using the ReplicaManager3
    /// system.<BR> It has functions to perform operations per-connection.<BR> AllocConnection() and DeallocConnection()
    /// are factory functions to create and destroy instances of the connection object.<BR> It is used if autoCreate is
    /// true via SetAutoManageConnections() (true by default). Otherwise, the function is not called, and you will have
    /// to call PushConnection() manually<BR>
    /// \note If you do not want a new network connection to immediately download game objects,
    /// SetAutoManageConnections() and PushConnection() are how you do this.
    ...
    /// It is used if autoDestroy is true via SetAutoManageConnections() (true by default). Otherwise, the function is
    /// not called and you would then be responsible for deleting your own connection objects.
    ...
    /// \details ReplicaManager3 can automatically create and/or destroy Connection_RM3 as systems connect or disconnect
    /// from RakPeerInterface.<BR> By default this is on, to make the system easier to learn and setup.<BR> If you don't
    /// want all connections to take part in the game, or you want to delay when a connection downloads the game, set \a
    /// autoCreate to false.<BR> If you want to delay deleting a connection that has dropped, set \a autoDestroy to
    /// false. If you do this, then you must call PopConnection() to remove that connection from being internally
    /// tracked. You'll also have to delete the connection instance on your own.<BR>
    /// \param[in] autoCreate Automatically call ReplicaManager3::AllocConnection() for each new connection. Defaults to
    /// true. Also see AutoCreateConnectionList()
    /// \param[in] autoDestroy Automatically call ReplicaManager3::DeallocConnection() for each dropped connection.
    /// Defaults to true.
    ...
    /// \details If \a autoCreate is false for SetAutoManageConnections(), then you need this function to add new
    /// instances of Connection_RM3 yourself.<BR> You don't need to track this pointer yourself, you can get it with
    /// GetConnectionAtIndex(), GetConnectionByGUID(), or GetConnectionBySystemAddress().<BR>
    ...
    /// \details On call, for each replica returned by GetReplicasCreatedByGuid(), QueryActionOnPopConnection() will be
    /// called. Depending on the return value, this may delete the corresponding replica.<BR> If autoDestroy is true in
    /// the call to SetAutoManageConnections() (true by default) then this is called automatically when the connection
    /// is lost. In that case, the returned connection instance is deleted.<BR>
    ...
    /// \details Anytime you create a new object that derives from Replica3, and you want ReplicaManager3 to use it,
    /// pass it to Reference().<BR> Remote systems already connected will potentially download this object the next time
    /// ReplicaManager3::Update() is called, which happens every time you call RakPeerInterface::Receive().<BR> You can
    /// also call ReplicaManager3::Update() manually to send referenced objects right away
    ...
    /// This is called automatically from the destructor of Replica3, so you don't need to call it manually unless you
    /// want to stop tracking an object before it is destroyed.
    ...
    /// \details Originally created is defined as the value of Replica3::creatingSystemGUID, which is automatically
    /// assigned in ReplicaManager3::Reference().<BR> You do not have to be directly connected to that system to get the
    /// objects originally created by that system.<BR>
    /// \param[in] guid GUID of the system we are referring to. Originally passed as the \a guid parameter to
    /// ReplicaManager3::AllocConnection()
    /// \param[out] List of Replica3 instances to be returned
    /// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
    void
    GetReplicasCreatedByGuid(RakNetGUID guid, DataStructures::List<Replica3*>& replicaListOut, WorldId worldId = 0);
    ...
    /// \details This is all Replica3 instances passed to Reference, as well as instances we downloaded and created via
    /// Connection_RM3::AllocReference()
    /// \param[out] List of Replica3 instances to be returned
    /// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
    ...
    /// \details Returns the number of connections added with ReplicaManager3::PushConnection(), minus the number
    /// removed with ReplicaManager3::PopConnection()
    /// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
    ...
    /// \param[in] intervalMS How frequently to autoserialize all objects. This controls the maximum number of game
    /// object updates per second.
    ...
    /// \details This can be wrong, for example if that system locally deleted the outside the scope of ReplicaManager3,
    /// if QueryRemoteConstruction() returned false, or if DeserializeConstruction() returned false.
    /// \param[in] replica The replica to check against.
    /// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
    /// \param[out] connectionsThatHaveConstructedThisReplica Populated with connection instances that we believe have
    /// \a replica allocated
    ...
    /// \brief ReplicaManager3 can support multiple worlds, where each world has a separate NetworkIDManager, list of
    /// connections, replicas, etc A world with id 0 is created automatically. If you want multiple worlds, use this
    /// function, and ReplicaManager3::SetNetworkIDManager() to have a different NetworkIDManager instance per world
    ...
    /// Deallocating a world will also stop tracking and updating all connections and replicas associated with that
    /// world.
    ...
    /// \details WorldId 0 is created by default. Worlds will not necessarily be in the order added with AddWorld().
    /// Edit RemoveWorld() changing RemoveAtIndexFast() to RemoveAtIndex() to preserve order.
...
/// Connection_RM3::OnAutoserializeInterval() and Connection_RM3::SendSerializeIfChanged()
/// \ingroup REPLICA_MANAGER_GROUP3
struct LastSerializationResult {
    ...
    /// \note replica MUST be the first member of this struct because I cast from replica to LastSerializationResult in
    /// Update()
...
/// \brief Each remote system is represented by Connection_RM3. Used to allocate Replica3 and track which instances have
/// been allocated
...
public:
    ...
    /// Return 0 if unable to create the intended object. Note, in that case the other system will still think we have
    /// the object and will try to serialize object updates to us. Generally, you should not send objects the other
    /// system cannot create.<BR>
    /// \sa Replica3::WriteAllocationID().
    /// Sample implementation:<BR>
    /// {RakNet::RakString typeName; allocationIdBitstream->Read(typeName); if (typeName=="Soldier") return new Soldier;
    /// return 0;}<BR>
    ...
    /// \param[out] objectsTheyDoHave Destination list. Returned in sorted ascending order, sorted on the value of the
    /// Replica3 pointer.
    ...
    /// When a new connection connects, after constructing and serialization all objects, SerializeOnDownloadComplete()
    /// is called
    ...
    enum ConstructionMode {
        ...
        /// For every object that does not exist on the remote system, call Replica3::QueryConstruction() every tick.
        /// Based on the call, the object may be sent to the other system. For every object that does exist on the
        /// remote system, call Replica3::QueryDestruction() every tick. Based on the call, the object may be deleted on
        ...
        /// This can be faster than QUERY_REPLICA_FOR_CONSTRUCTION and QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION
        /// for large worlds See GridSectorizer.h under /Source for code that can help with this
        QUERY_CONNECTION_FOR_REPLICA_LIST
    };

    /// \brief Return whether or not downloads to our system should all be processed the same tick (call to
    /// RakPeer::Receive() )
    /// \details Normally the system will send ID_REPLICA_MANAGER_DOWNLOAD_STARTED, ID_REPLICA_MANAGER_CONSTRUCTION for
    /// all downloaded objects, ID_REPLICA_MANAGER_SERIALIZE for each downloaded object, and lastly
    /// ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE. This enables the application to show a downloading splash screen on
    /// ID_REPLICA_MANAGER_DOWNLOAD_STARTED, a progress bar, and to close the splash screen and activate all objects on
    /// ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE However, if the application was not set up for this then it would result in
    /// incomplete objects spread out over time, and cause problems If you return true from
    /// QueryGroupDownloadMessages(), then these messages will be returned all in one tick, returned only when the
    ...
    /// \details The default of calling QueryConstruction for every known object is easy to use, but not efficient,
    /// especially for large worlds where many objects are outside of the player's circle of influence.<BR>
    /// QueryDestruction is also not necessarily useful or efficient, as object destruction tends to happen in known
    /// cases, and can be accomplished by calling Replica3::BroadcastDestruction() QueryConstructionMode() allows you to
    /// specify more efficient algorithms than the default when overriden.
    /// \return How to get the list of objects that exist on the remote system. You should always return the same value
    ...
    /// \details This advantage of this callback is if that there are many objects that a particular connection does not
    /// have, then we do not have to iterate through those objects calling QueryConstruction() for each of them.<BR>
    ///< BR>
    /// See GridSectorizer in the Source directory as a method to find all objects within a certain radius in a fast
    /// way.<BR>
    ///< BR>
    ...
    /// \brief Override which replicas to serialize and in what order for a connection for a ReplicaManager3::Update()
    /// cycle
    /// \details By default, Connection_RM3 will iterate through queryToSerializeReplicaList and call
    /// QuerySerialization() on each Replica in that list queryToSerializeReplicaList is populated in the order in which
    /// ReplicaManager3::Reference() is called for those objects. If you write to to \a replicasToSerialize and return
    /// true, you can control in what order and for which replicas to call QuerySerialization() Example use case: We
    /// have more data to send then the bandwidth supports, so want to prioritize sends. For example enemies shooting
    /// are more important than animation effects When QuerySerializationList(), sort objects by priority, and write the
    /// list to \a replicasToSerialize, optionally skipping objects with a lower serialization frequency If you hit your
    /// bandwidth limit when checking SerializeParameters::bitsWrittenSoFar, you can return RM3SR_DO_NOT_SERIALIZE for
    /// all remaining items
    /// \note Only replicas written to replicasToSerialize are transmitted. Even if you returned RM3SR_SERIALIZED_ALWAYS
    /// a prior ReplicaManager3::Update() cycle, the replica will not be transmitted if it is not in replicasToSerialize
    /// \note If you do not know what objects are candidates for serialization, you can use queryToSerializeReplicaList
    /// as a source for your filtering or sorting operations
    /// \param[in] replicasToSerialize List of replicas to call QuerySerialization() on
    /// \return Return true to use replicasToSerialize (replicasToSerialize may be empty if desired). Otherwise return
    /// false.
    ...
    /// \internal This is used internally - however, you can also call it manually to send a data update for a remote
    /// replica.<BR>
    ...
    /// \internal
    /// \details Calls Connection_RM3::SendSerialize() if Replica3::Serialize() returns a different result than what is
    /// contained in \a lastSerializationResult.<BR> Used by autoserialization in
...
protected:
    ...
    */
    ...
    // A given Replica can be either in queryToConstructReplicaList or constructedReplicaList but not both at the same
    // time
...
private:
    ...
    ConstructionMode mConstructionMode;
};
...
enum RM3SerializationResult {
    ...
    /// An example of this is if every member variable has an accessor, changing a member sets a flag, and you check
    /// that flag in Replica3::QuerySerialization() The opposite of this is RM3SR_DO_NOT_SERIALIZE, in case the object
    /// did not change
    RM3SR_BROADCAST_IDENTICALLY_FORCE_SERIALIZATION,

    /// Either this object serializes differently depending on who we send to or we send it to some systems and not
    /// others. Inefficient for memory and speed, but efficient for bandwidth However, if you don't know what to return,
    /// return this
    ...
    /// Can send unique data per connection if desired. If same data is sent to all connections, use
    /// RM3SR_SERIALIZED_ALWAYS_IDENTICALLY for even better performance Efficient for memory and speed, but not
    /// necessarily bandwidth
...
enum RM3QuerySerializationResult {
    ...
    /// Never call Serialize() for this object and connection. This system will not serialize this object for this
    /// topology
...
/// \ingroup REPLICA_MANAGER_GROUP3
/// Used for Replica3::QueryConstruction_PeerToPeer() and Replica3::QuerySerialization_PeerToPeer() to describe how the
/// object replicates between hosts
enum Replica3P2PMode {
    ...
    /// Example: A pickup. When an avatar holds it, that avatar controls it. When it is on the ground, the host controls
    /// it.
    R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE,
    /// The Replica3 instance is constructed and/or serialized by different systems
    /// Another system is in charge of construction and/or serialization, but this system may be in charge at a later
    /// time Example: A pickup held by another player. That player sends creation of that object to new connections, and
    /// serializes it until it is dropped.
...
public:
    ...
    /// Before deleting a local instance of Replica3, call Replica3::BroadcastDestruction() for the deletion
    /// notification to go out on the network. It is not necessary to call ReplicaManager3::Dereference(), as this
    /// happens automatically in the destructor
    virtual ~Replica3();
    ...
    /// \details If ReplicaManager3::QueryConstructionMode() returns QUERY_CONNECTION_FOR_REPLICA_LIST or
    /// QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION (default), then QueyrConstruction() is called once per tick from
    /// ReplicaManager3::Update() to determine if an object should exist on a given system.<BR> Based on the return
    /// value, a network message may be sent to the other system to create the object.<BR> If QueryConstructionMode() is
    /// overriden to return QUERY_CONNECTION_FOR_REPLICA_LIST, this function is unused.<BR>
    /// \note Defaults are provided: QueryConstruction_PeerToPeer(), QueryConstruction_ServerConstruction(),
    /// QueryConstruction_ClientConstruction(). Return one of these functions for a working default for the relevant
    /// topology.
    ...
    QueryConstruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) = 0;

    /// \brief Ask if this object, which does exist on \a destinationConnection should be removed from the remote system
    /// \details If ReplicaManager3::QueryConstructionMode() returns QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION
    /// (default), then QueryDestruction() is called once per tick from ReplicaManager3::Update() to determine if an
    /// object that exists on a remote system should be destroyed for a given system.<BR> Based on the return value, a
    /// network message may be sent to the other system to destroy the object.<BR> Note that you can also destroy
    /// objects with BroadcastDestruction(), so this function is not useful unless you plan to delete objects for only a
    /// particular connection.<BR> If QueryConstructionMode() is overriden to return QUERY_CONNECTION_FOR_REPLICA_LIST,
    /// this function is unused.<BR>
    /// \param[in] destinationConnection Which system we will send to
    /// \param[in] replicaManager3 Plugin instance for this Replica3
    /// \return What action to take. Only RM3CS_SEND_DESTRUCTION does anything at this time.
    virtual RM3DestructionState
    QueryDestruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) {
        (void)destinationConnection;
        (void)replicaManager3;
        return RM3DS_DO_NOT_QUERY_DESTRUCTION;
    }

    /// \brief We're about to call DeserializeConstruction() on this Replica3. If QueryRemoteConstruction() returns
    /// false, this object is deleted instead.
    /// \details By default, QueryRemoteConstruction_ServerConstruction() does not allow clients to create objects. The
    /// client will get Replica3::DeserializeConstructionRequestRejected().<BR> If you want the client to be able to
    /// potentially create objects for client/server, override accordingly.<BR> Other variants of
    /// QueryRemoteConstruction_* just return true.
    /// \note Defaults are provided: QueryRemoteConstruction_PeerToPeer(), QueryRemoteConstruction_ServerConstruction(),
    /// QueryRemoteConstruction_ClientConstruction(). Return one of these functions for a working default for the
    /// relevant topology.
    /// \param[in] sourceConnection Which system sent us the object creation request message.
    /// \return True to allow the object to pass onto DeserializeConstruction() (where it may also be rejected), false
    /// to immediately reject the remote construction request
    ...
    /// For a peer in peer to peer, you can normally return false since the original destroying peer would have told all
    /// other peers about the destruction If a system gets a destruction command for an object that was already
    /// destroyed, the destruction message is ignored
    ...
    /// \details SerializeConstruction is used to write out data that you need to create this object in the context of
    /// your game, such as health, score, name. Use it for data you only need to send when the object is created.<BR>
    /// After SerializeConstruction() is called, Serialize() will be called immediately thereafter. However, they are
    /// sent in different messages, so Serialize() may arrive a later frame than SerializeConstruction() For that
    /// reason, the object should be valid after a call to DeserializeConstruction() for at least a short time.<BR>
    /// \note The object's NetworkID and allocation id are handled by the system automatically, you do not need to write
    /// these values to \a constructionBitstream
    ...
    virtual void
    SerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) = 0;
    ...
    /// \return true to accept construction of the object. false to reject, in which case the object will be deleted via
    /// Replica3::DeallocReplica()
    virtual bool
    DeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) = 0;
    ...
    /// \details Replica3::SerializeDestruction() will be called to write any object destruction specific data you want
    /// to send with this event.
    /// \a destructionBitstream can be read in DeserializeDestruction()
    /// \param[out] destructionBitstream Bitstream for you to write to
    /// \param[in] destinationConnection System that will receive this network message.
    virtual void
    SerializeDestruction(RakNet::BitStream* destructionBitstream, RakNet::Connection_RM3* destinationConnection) = 0;

    /// \brief Read data written by Replica3::SerializeDestruction()
    /// \details Return true to delete the object. BroadcastDestruction() will be called automatically, followed by
    /// ReplicaManager3::Dereference.<BR> Return false to not delete it. If you delete it at a later point, you are
    /// responsible for calling BroadcastDestruction() yourself.
    virtual bool
    DeserializeDestruction(RakNet::BitStream* destructionBitstream, RakNet::Connection_RM3* sourceConnection) = 0;
    ...
    /// <LI>ReplicaManager3::SetAutoManageConnections() was called autoDestroy true (which is the default setting), and
    /// a remote system that owns this object disconnected) OR
    /// <\OL>
    /// <BR>
    /// Override with {delete this;} to actually delete the object (and any other processing you wish).<BR>
    /// If you don't want to delete the object, just do nothing, however, the system will not know this. You may wish to
    /// call Dereference() if the object should no longer be networked, but remain in memory. You are responsible for
    /// deleting it yoruself later.<BR> destructionBitstream may be 0 if the object was deleted locally
    virtual void DeallocReplica(RakNet::Connection_RM3* sourceConnection) = 0;

    /// \brief Implement with QuerySerialization_ClientSerializable(), QuerySerialization_ServerSerializable(), or
    /// QuerySerialization_PeerToPeer()
    /// \details QuerySerialization() is a first pass query to check if a given object should serializable to a given
    ...
    /// This would give you feedback loops as the sender gets the same message back from the recipient it just sent
    /// to.<BR> If more than one system can serialize the same object then you will need to override to return true, and
    /// control the serialization result from Replica3::Serialize(). Be careful not to send back the same data to the
    /// system that just sent to you!
    ...
    /// If you want to do some kind of operation on the Replica objects that you own, just before Serialization(), then
    ...
    /// SerializationParamters::serializationBitstream.<BR> If data only needs to be written once, you can write it to
    /// SerializeConstruction() instead for efficiency.<BR> Transmitted over the network if it changed from the last
    /// time we called Serialize().<BR> Called every time the time interval to
    /// ReplicaManager3::SetAutoSerializeInterval() elapses and ReplicaManager3::Update is subsequently called.
    /// \param[in/out] serializeParameters Parameters controlling the serialization, including destination bitstream to
    /// write to
    ...
    /// Writes to PostDeserializeConstruction(), which is called after all objects are created for a given Construction
    /// tick(). Override to send data to PostDeserializeConstruction(), such as the NetworkID of other objects to
    /// resolve pointers to
    virtual void
    PostSerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        (void)constructionBitstream;
        (void)destinationConnection;
    }
    ...
    /// DeserializeConstruction, yet one had not been constructed yet In PostDeserializeConstruction(), you know that
    /// all objects have already been created, so can resolve NetworkIDs to pointers safely. You can also use it to
    /// trigger some sort of event when you know the object has completed deserialization.
    /// \param[in] constructionBitstream BitStream written in PostSerializeConstruction()
    /// \param[in] sourceConnection System that sent us this network message.
    virtual void
    PostDeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
        (void)constructionBitstream;
        (void)sourceConnection;
    }

    /// Same as PostSerializeConstruction(), but for objects that returned RM3CS_ALREADY_EXISTS_REMOTELY from
    /// QueryConstruction
    ...
    /// Same as PostDeserializeConstruction(), but for objects that returned RM3CS_ALREADY_EXISTS_REMOTELY from
    /// QueryConstruction
    ...
    /// Called after DeserializeDestruction completes for the object successfully, but obviously before the object is
    /// deleted.<BR> Override to trigger some sort of event when you know the object has completed destruction.
    /// \param[in] sourceConnection System that sent us this network message.
    ...
    virtual RM3ConstructionState
    QueryConstruction_ClientConstruction(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer);
    ...
    /// \param[in] isThisTheServer True if this system is the server, false if not.
    virtual bool
    QueryRemoteConstruction_ClientConstruction(RakNet::Connection_RM3* sourceConnection, bool isThisTheServer);
    ...
    /// \param[in] isThisTheServer True if this system is the server, false if not.
    virtual RM3ConstructionState
    QueryConstruction_ServerConstruction(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer);

    /// \brief Default call for QueryRemoteConstruction(). Allow the server to create this object, but not the client.
    /// \details Only the server is allowed to create this object. The network topology is client/server
    /// The code means if this is the server, and I got a command to create a Replica3 to ignore it. If this is the
    /// client, to allow it.
    /// \param[in] sourceConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] isThisTheServer True if this system is the server, false if not.
    virtual bool
    QueryRemoteConstruction_ServerConstruction(RakNet::Connection_RM3* sourceConnection, bool isThisTheServer);

    /// \brief Default call for QueryConstruction().
    /// \details All clients are allowed to create all objects. The object is not relayed when remotely created
    /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] p2pMode If controlled only by this system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass
    /// R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
    ...
    /// \details Use if the values you are serializing are generated by the client that owns the object. The
    /// serialization will be relayed through the server to the other clients.
    /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] isThisTheServer True if this system is the server, false if not.
    ...
    /// \brief Default call for QuerySerialization().
    /// \details Use if the values you are serializing are generated only by the server. The serialization will be sent
    /// to all clients, but the clients will not send back to the server.
    /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] isThisTheServer True if this system is the server, false if not.
    virtual RakNet::RM3QuerySerializationResult
    QuerySerialization_ServerSerializable(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer);
    /// \brief Default call for QuerySerialization().
    /// \details Use if the values you are serializing are on a peer to peer network. The peer that owns the object will
    /// send to all. Remote peers will not send.
    /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
    /// \param[in] p2pMode If controlled only by this system ever, pass R3P2PM_SINGLE_OWNER. Otherwise pass
    /// R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE or R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE
    ...
    /// Default: If we are a server, and the connection is lost, delete the client's objects and broadcast the
    /// destruction
    ...
    QueryActionOnPopConnection_PeerToPeer(RakNet::Connection_RM3* droppedConnection) const;
...
public:
    ...
    virtual void
    WriteAllocationID(RakNet::Connection_RM3* destinationConnection, RakNet::BitStream* allocationIdBitstream) const {
    ...
    virtual RakNet::RM3DestructionState
    QueryDestruction(RakNet::Connection_RM3* destinationConnection, RakNet::ReplicaManager3* replicaManager3) {
    ...
    virtual void
    SerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        r3CompositeOwner->SerializeConstruction(constructionBitstream, destinationConnection);
    }
    virtual bool
    DeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
    ...
    virtual void
    SerializeDestruction(RakNet::BitStream* destructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        r3CompositeOwner->SerializeDestruction(destructionBitstream, destinationConnection);
    }
    virtual bool
    DeserializeDestruction(RakNet::BitStream* destructionBitstream, RakNet::Connection_RM3* sourceConnection) {
