...
void StringCompressor::EncodeString(
    ...
    if (maxCharsToWrite <= 0 || (int)strlen(input) < maxCharsToWrite) charsToWrite = (int)strlen(input);
    else charsToWrite = maxCharsToWrite - 1;
...
bool StringCompressor::DecodeString(char* output, int maxCharsToWrite, RakNet::BitStream* input, uint8_t languageId) {
    ...
    if (bytesInStream < maxCharsToWrite) output[bytesInStream] = 0;
    else output[maxCharsToWrite - 1] = 0;
