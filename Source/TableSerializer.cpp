...
bool TableSerializer::DeserializeFilterQuery(RakNet::BitStream* out, DataStructures::Table::FilterQuery* query) {
    ...
    if (query->cellValue->isEmpty == false) {
        // HACK - cellValue->i is used for integer, character, and binary data. However, for character and binary c will
        // be 0. So use that to determine if the data was integer or not.
...
bool TableSerializer::DeserializeFilterQueryList(
    ...
    out->Read(anyQueries);
    if (anyQueries == false) {
        if (allocateExtraQueries <= 0) *query = 0;
        else *query = new DataStructures::Table::FilterQuery[allocateExtraQueries];
