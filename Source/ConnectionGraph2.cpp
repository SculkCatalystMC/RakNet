...
uint16_t ConnectionGraph2::GetPingBetweenSystems(RakNetGUID g1, RakNetGUID g2) const {
    ...
    if (g1 == mRakPeerInterface->GetMyGUID()) return (uint16_t)mRakPeerInterface->GetAveragePing(g2);
    if (g2 == mRakPeerInterface->GetMyGUID()) return (uint16_t)mRakPeerInterface->GetAveragePing(g1);
...
/// Returns the system with the lowest total ping among all its connections. This can be used as the 'best host' for a
...
RakNetGUID ConnectionGraph2::GetLowestAveragePingSystem(void) const {
    ...
    for (idx = 0; idx < remoteSystems.Size(); idx++) {
        ...
        ap = mRakPeerInterface->GetAveragePing(remoteSystems[idx]->guid);
        if (ap != -1) {
    ...
    if (lowestPingIdx == (unsigned int)-1) return mRakPeerInterface->GetMyGUID();
...
void ConnectionGraph2::OnClosedConnection(
    ...
    RakNet::BitStream bs;
    if (lostConnectionReason == LCR_CONNECTION_LOST) bs.Write((MessageID)ID_REMOTE_CONNECTION_LOST);
    else bs.Write((MessageID)ID_REMOTE_DISCONNECTION_NOTIFICATION);
...
void ConnectionGraph2::AddParticipant(const SystemAddress& systemAddress, RakNetGUID rakNetGUID) {
    ...
    bs.Write(systemAddress);
    bs.Write(rakNetGUID);
    bs.WriteCasted<uint16_t>(mRakPeerInterface->GetAveragePing(rakNetGUID));
    SendUnified(&bs, HIGH_PRIORITY, RELIABLE_ORDERED, 0, systemAddress, true);
    ...
    for (i = 0; i < addresses.Size(); i++) {
        ...
        bs.Write(guids[i]);
        bs.WriteCasted<uint16_t>(mRakPeerInterface->GetAveragePing(guids[i]));
        count++;
    }
    ...
    if (packet->data[0] == ID_REMOTE_CONNECTION_LOST || packet->data[0] == ID_REMOTE_DISCONNECTION_NOTIFICATION) {
        ...
        if (objectExists) {
            ...
            for (unsigned int idx2 = 0; idx2 < numAddresses; idx2++) {
                ...
                bool         _objectExists;
                unsigned int ii = remoteSystems[idx]->remoteConnections.GetIndexFromKey(saag, &_objectExists);
                if (_objectExists == false)
