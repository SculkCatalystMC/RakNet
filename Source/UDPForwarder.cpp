...
void UDPForwarder::RecvFrom(RakNet::TimeMS curTime, ForwardEntry* forwardEntry) {
    ...
    if (matchConfirmed1 == true || (matchConfirmed2 == false && confirmed1 == false && matchUnconfirmed1 == true)) {
        ...
        if (forwardEntry->addr2Confirmed != UNASSIGNED_SYSTEM_ADDRESS) forwardTarget = forwardEntry->addr2Confirmed;
        else forwardTarget = forwardEntry->addr2Unconfirmed;
        ...
        if (forwardEntry->addr1Confirmed != UNASSIGNED_SYSTEM_ADDRESS) forwardTarget = forwardEntry->addr1Confirmed;
        else forwardTarget = forwardEntry->addr1Unconfirmed;
    } else {
        return;
    }
    ...
    while (1) {
        ...
        if (GetUsedForwardEntries() > maxForwardEntries) {
            ...
            if (sfos.result == UDPFORWARDER_RESULT_COUNT) {
                ...
                else getaddrinfo(sfis->forceHostAddress.C_String(), "0", &hints, &servinfo);
                ...
                if (fe->socket == INVALID_SOCKET) sfos.result = UDPFORWARDER_BIND_FAILED;
                else sfos.result = UDPFORWARDER_SUCCESS;
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
    ...
    while (i < forwardListNotUpdated.Size()) {
        if (curTime > forwardListNotUpdated[i]->timeLastDatagramForwarded && // Account for timestamp wrap
            curTime
                > forwardListNotUpdated[i]->timeLastDatagramForwarded + forwardListNotUpdated[i]->timeoutOnNoDataMS) {
...
RAK_THREAD_DECLARATION(UpdateUDPForwarderGlobal) {
    ...
    while (udpForwarder->isRunning.GetValue() > 0) {
        ...
        if (udpForwarder->forwardListNotUpdated.Size() == 0) RakSleep(30);
        else RakSleep(0);
    }
