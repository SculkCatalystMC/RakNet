...
/// \file
/// \brief Message filter plugin. Assigns systems to FilterSets.  Each FilterSet limits what messages are allowed.  This
/// is a security related plugin.
///
...
/// \details The MessageFilter plugin is used for security where you limit what systems can send what kind of
/// messages.<BR> You implicitly define FilterSets, and add allowed message IDs to these FilterSets.<BR> You then add
/// systems to these filters, such that those systems are limited to sending what the filters allows.<BR> You can
/// automatically assign systems to a filter.<BR> You can automatically kick and possibly ban users that stay in a
...
public:
    ...
    /// Usually you specify a range to make it easier to add new enumerations without having to constantly refer back to
    /// this function.
    /// \param[in] allow True to allow this message ID, false to disallow. By default, all messageIDs except the noted
    /// types are disallowed.  This includes messages from other plugins!
    ...
    /// \param[in] filterSetID A user defined ID to represent a filter set.  If no filter with this ID exists, one will
    ...
    /// What action to take on a disallowed message.  You can kick or not.  You can add them to the ban list for some
    /// time By default no action is taken.  The message is simply ignored. param[in] 0 for permanent ban, >0 for ban
    ...
    /// Limit how long a connection can stay in a particular filterSetID. After this time, the connection is kicked and
    /// possibly banned. By default there is no limit to how long a connection can stay in a particular filter set.
    ...
    /// This function is used to change the filter set a system is using, to add it to a new filter set, or to remove it
    /// from all existin filter sets.
    /// \param[in] addressOrGUID The system we are referring to
    /// \param[in] filterSetID A user defined ID to represent a filter set.  If no filter with this ID exists, one will
    /// be created with default settings.  If -1, the system will be removed from all filter sets.
    ...
    /// \param[in] filterSetID The filter set to limit to.  Use -1 for none (just returns the total number of filter
