...
1. Sender calls ConnectInternal(). A ConnnectRequest structure is allocated and stored in the connectionRequests list,
containing a list of every system we are connected to. ID_ROUTER_2_QUERY_FORWARDING is sent to all connected systems.

2. Upon the router getting ID_ROUTER_2_QUERY_FORWARDING, ID_ROUTER_2_REPLY_FORWARDING is sent to the sender indicating
if that router is connected to the endpoint, along with the ping from the router to the endpoint.

3. Upon the sender getting ID_ROUTER_2_REPLY_FORWARDING, the connection request structure is looked up in
Router2::UpdateForwarding. The ping is stored in that structure. Once all systems have replied, the system continues to
the next state. If every system in step 1 has been exhausted, and routing has occured at least once, then
ID_CONNECTION_LOST is returned. If every system in step 1 has been exhausted and routing has never occured, then
ID_ROUTER_2_FORWARDING_NO_PATH is returned. Otherwise, the router with the lowest ping is chosen, and
RequestForwarding() is called with that system, which sends ID_ROUTER_2_REQUEST_FORWARDING to the router.

4. When the router gets ID_ROUTER_2_REQUEST_FORWARDING, a MiniPunchRequest structure is allocated and stored in the
miniPunchesInProgress list. The function SendOOBMessages() sends ID_ROUTER_2_REPLY_TO_SENDER_PORT from the routing
sockets to both the sender and endpoint. It also sends ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT through the regular RakNet
connection.

5. The sender and endpoint should get ID_ROUTER_2_REPLY_TO_SENDER_PORT and/or ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT
depending on what type of router they have. If ID_ROUTER_2_REPLY_TO_SENDER_PORT arrives, then this will reply back to
the routing socket directly. If ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT arrives, then the reply port is modified to be the
port specified by the router system. In both cases, ID_ROUTER_2_MINI_PUNCH_REPLY is sent. As the router has already
setup the forwarding, ID_ROUTER_2_MINI_PUNCH_REPLY will actually arrive to the endpoint from the sender, and from the
sender to the endpoint.

6. When ID_ROUTER_2_MINI_PUNCH_REPLY arrives, ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE will be sent to the router. This is to
tell the router that the forwarding has succeeded.

7. When ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE arrives on the router, the router will find the two systems in the
miniPunchesInProgress list, which was added in step 4 (See OnMiniPunchReplyBounce()). gotReplyFromSource or
gotReplyFromEndpoint will be set to true, depending on the sender. When both gotReplyFromSource and gotReplyFromEndpoint
have replied, then ID_ROUTER_2_REROUTE is sent to the endpoint, and ID_ROUTER_2_FORWARDING_ESTABLISHED is sent to the
sender.

8. When the endpoint gets ID_ROUTER_2_REROUTE, the system address is updated for the guid of the sender using
RakPeer::ChangeSystemAddress(). This happens in OnRerouted().

9. When the sender gets ID_ROUTER_2_FORWARDING_ESTABLISHED, then in OnForwardingSuccess() the endpoint is removed from
the connectionRequest list and moved to the forwardedConnectionList.

10. In OnClosedConnection(), for the sender, if the closed connection is the endpoint, then the endpoint is removed from
the forwardedConnectionList (this is a graceful disconnect). If the connection was instead lost to the router, then
ConnectInternal() gets called, which goes back to step 1. If instead this was a connection requset in progress, then
UpdateForwarding() gets called, which goes back to step 3.

11. When the user connects the endpoint and sender, then the sender will get ID_CONNECTION_REQUEST_ACCEPTED. The sender
will look up the endpoint in the forwardedConnectionList, and send ID_ROUTER_2_INCREASE_TIMEOUT to the endpoint. This
message will call SetTimeoutTime() on the endpoint, so that if the router disconnects, enough time is available for the
reroute to complete.
*/
...
bool Router2::ConnectInternal(RakNetGUID endpointGuid, bool returnConnectionLostOnFailure) {
    ...
    mRakPeerInterface->GetSystemList(addresses, guids);
    if (guids.Size() == 0) {
        char buff[512];
        if (debugInterface)
    ...
    for (unsigned int i = 0; i < guids.Size(); i++) {
        ...
        if (guids[i] != endpointGuid) {
            ...
            uint32_t pack_id = mRakPeerInterface->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, crs.guid, false);
...
void Router2::EstablishRouting(RakNetGUID endpointGuid) {
    ConnectionState cs = mRakPeerInterface->GetConnectionState(endpointGuid);
...
PluginReceiveResult Router2::OnReceive(Packet* packet) {
    ...
    if (packet->data[0] == ID_ROUTER_2_INTERNAL) {
        ...
        case ID_ROUTER_2_INCREASE_TIMEOUT: {
            ...
            mRakPeerInterface->SetTimeoutTime(
                mRakPeerInterface->GetTimeoutTime(packet->systemAddress) + 10000,
        ...
        case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
            ...
            if (debugInterface) {
                ...
                debugInterface->ShowDiagnostic(FormatStringTS(
                    buff,
                    "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on address %s, replying with ID_ROUTER_2_MINI_PUNCH_REPLY "
                    "at %s:%i\n",
                    sa.GetPort(),
                    buff2,
                    _FILE_AND_LINE_
        ...
        if (forwardingIndex < forwardedConnectionList.Size()) {
            ...
            mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE, 0, packet->guid, false);
            ...
            // See http://blog.delphi-jedi.net/2008/04/23/the-case-of-the-unexplained-dead-lock-in-a-single-thread/ for
            // details
            forwardedConnectionListMutex.Unlock();
        }
...
void Router2::OnClosedConnection(
    ...
    while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
        if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid == rakNetGUID) {
            ...
            // ID_CONNECTION_LOST. Also remove connection request if it already is in the list, to restart it
            connectionRequestsMutex.Lock();
            unsigned int pos =
                GetConnectionRequestIndex(forwardedConnectionList[forwardedConnectionIndex].endpointGuid);
        ...
        if (connectionRequestGuidIndex != (unsigned int)-1) {
            ...
            if (UpdateForwarding(cr) == false) // If returns false, no connection request systems left
            {
                ...
                RemoveConnectionRequest(connectionRequestIndex);
                   ...
                   // cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still waiting for other systems
                   // to respond.
            {
                if (debugInterface) {
                    char buff[512];
                ...
                //                 if(volatile bool is_my_fix_a_truth = true) { // A system in the list of potential
                //                 systems to try routing to dropped. There is no need to restart the whole process.
                //                     connectionRequestsMutex.Lock();
    ...
    if (connectionRequest->connectionRequestSystems.Size() == 0) {
        ...
        if (connectionRequest->returnConnectionLostOnFailure) {
            ReturnToUser(
                ...
                true
            ); // This is a connection which was previously established. Rerouting is not possible.
            ...
            //             rakPeerInterface->CloseConnection(%I64d)" , connectionRequest->endpointGuid.g);
        } else
            ReturnToUser(
...
void Router2::RequestForwarding(ConnnectRequest* connectionRequest) {
    ...
    mRakPeerInterface
        ->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, connectionRequest->lastRequestedForwardingSystem, false);
    ...
    if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            ...
            "(connectionRequest->lastRequestedForwardingSystem = %I64d, connectionRequest->endpointGuid = %I64d) at "
            "%s:%i\n",
...
void Router2::SendFailureOnCannotForward(RakNetGUID sourceGuid, RakNetGUID endpointGuid) {
    ...
    bsOut.Write(endpointGuid);
    bsOut.Write(false);
    mRakPeerInterface->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, sourceGuid, false);
}
int Router2::ReturnFailureOnCannotForward(RakNetGUID sourceGuid, RakNetGUID endpointGuid) {
    ...
    for (unsigned int i = 0; i < forwardedConnectionList.Size(); i++) {
        if ((forwardedConnectionList[i].endpointGuid == endpointGuid)
            || (forwardedConnectionList[i].endpointGuid
                == sourceGuid)) // sourceGuid is here so you do not respond to routing requests from systems you are
                                // already routing through.
        {
            forwardedConnectionListMutex.Unlock();
    ...
    int pingToEndpoint;
    pingToEndpoint = mRakPeerInterface->GetAveragePing(endpointGuid);
    if (pingToEndpoint == -1) {
        char buff[512];
        if (debugInterface)
            debugInterface->ShowFailure(FormatStringTS(
...
void Router2::OnQueryForwarding(Packet* packet) {
    ...
    mRakPeerInterface->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, packet->guid, false);
...
void Router2::OnQueryForwardingReply(Packet* packet) {
    ...
    if (canForward) {
        ...
        connectionRequests[connectionRequestIndex]
            ->connectionRequestSystems[connectionRequestGuidIndex]
            .pingToEndpoint = mRakPeerInterface->GetAveragePing(packet->guid) + pingToEndpoint;
    } else {
...
void Router2::SendOOBFromSpecifiedSocket(OutOfBandIdentifiers oob, SystemAddress sa, __UDPSOCKET__ socket) {
    RakNet::BitStream bs;
    mRakPeerInterface->WriteOutOfBandHeader(&bs);
...
void Router2::SendOOBMessages(Router2::MiniPunchRequest* mpr) {
    // Mini NAT punch
    // Send from srcToDestPort to packet->systemAddress (source). If the message arrives, the remote system should
    // reply.
    SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT, mpr->sourceAddress, mpr->forwardingSocket);

    // Send from destToSourcePort to endpointSystemAddress (destination). If the message arrives, the remote system
    // should reply.
...
void Router2::OnRequestForwarding(Packet* packet) {
    ...
    SystemAddress      endpointSystemAddress = mRakPeerInterface->GetSystemAddressFromGuid(endpointGuid);
    ...
    if (result == UDPFORWARDER_FORWARDING_ALREADY_EXISTS) {
        ...
        int ping1                             = mRakPeerInterface->GetAveragePing(packet->guid);
        int ping2                             = mRakPeerInterface->GetAveragePing(endpointGuid);
        if (ping1 > ping2) miniPunchRequest.timeout = RakNet::GetTimeMS() + ping1 * 8 + 300;
        else miniPunchRequest.timeout = RakNet::GetTimeMS() + ping2 * 8 + 300;
    ...
    while (i < miniPunchesInProgress.Size()) {
        if (miniPunchesInProgress[i].sourceGuid == packet->guid
            ...
            if (debugInterface) {
                ...
                debugInterface->ShowDiagnostic(FormatStringTS(
                    buff,
                    "Processing ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, gotReplyFromSource=%i gotReplyFromEndpoint=%i at "
                    "%s:%i\n",
...
void Router2::OnMiniPunchReply(Packet* packet) {
    ...
    SendOOBFromRakNetPort(
        ...
        0,
        mRakPeerInterface->GetSystemAddressFromGuid(routerGuid)
    );
    ...
    if (debugInterface) {
        ...
        mRakPeerInterface->GetSystemAddressFromGuid(routerGuid).ToString(true, buff2);
...
void Router2::OnRerouted(Packet* packet) {
    ...
    mRakPeerInterface->ChangeSystemAddress(endpointGuid, intermediaryAddress);
    ...
    if (forwardingIndex < forwardedConnectionList.Size()) {
        ...
        if (debugInterface) {
            ...
            debugInterface->ShowDiagnostic(FormatStringTS(
                buff,
                "Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED, Calling RakPeer::ChangeSystemAddress at "
                "%s:%i\n",
                __FILE__,
                __LINE__
            ));
...
int Router2::GetLargestPingAmongConnectedSystems(void) const {
    int          avePing;
    int          largestPing = -1;
    unsigned int maxPeers    = mRakPeerInterface->GetMaximumNumberOfPeers();
    if (maxPeers == 0) return 9999;
    unsigned int index;
    for (index = 0; index < mRakPeerInterface->GetMaximumNumberOfPeers(); index++) {
        RakNetGUID g = mRakPeerInterface->GetGUIDFromIndex(index);
        if (g != UNASSIGNED_RAKNET_GUID) {
            avePing = mRakPeerInterface->GetAveragePing(mRakPeerInterface->GetGUIDFromIndex(index));
...
void Router2::ReturnToUser(
    ...
    mRakPeerInterface->PushBackPacket(p, true);
}
