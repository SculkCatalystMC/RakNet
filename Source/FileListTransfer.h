...
/// \brief A plugin to provide a simple way to compress and incrementally send the files in the FileList structure.
/// \details Similar to the DirectoryDeltaTransfer plugin, except that it doesn't send deltas based on pre-existing
/// files or actually write the files to disk.
///
/// Usage:
...
/// It's up to you to transmit this value to the other system, along with information indicating what kind of files you
...
public:
    ...
    /// This value will be returned in the callback and is unique per file set.  Returns 65535 on failure (not connected
    /// to sender)
    ...
    /// \param[in] fileList A list of files.  The data contained in FileList::data will be sent incrementally and
    /// compressed among all files in the set
    /// \param[in] rakPeer The instance of RakNet to use to send the message. Pass 0 to use the instance the plugin is
    /// attached to
    ...
    /// \param[in] _chunkSize How large of a block of a file to read/send at once. Large values use more memory but
    /// transfer slightly faster.
    void Send(
        FileList*                 fileList,
