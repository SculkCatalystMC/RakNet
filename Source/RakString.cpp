...
void RakString::Realloc(SharedString* sharedStr, size_t bytes) {
    if (bytes <= sharedStr->bytesUsed) return;

    RakAssert(bytes > 0);
    size_t       oldBytes = sharedStr->bytesUsed;
    size_t       newBytes;
...
WCHAR* RakString::ToWideChar(void) {
    ...
    if ((sharedString->c_str == NULL) || (*sharedString->c_str == '\0')) {
        ...
        WCHAR* emptyWideString = RakNet::OP_NEW_ARRAY<WCHAR>(1, __FILE__, __LINE__);
        emptyWideString[0]     = 0;
...
void RakString::RemoveCharacter(char c) {
    ...
    for (readIndex = 0; sharedString->c_str[readIndex]; readIndex++) {
        if (sharedString->c_str[readIndex] != c) sharedString->c_str[writeIndex++] = sharedString->c_str[readIndex];
        else Clone();
    }
...
RakNet::RakString& RakString::URLDecode(void) {
    ...
    for (i = 0; i < strLen; i++) {
        ...
        if (c == '%') {
            ...
            if (hexDigits[0] >= 'a' && hexDigits[0] <= 'f') hexValues[0] = hexDigits[0] - 'a' + 10;
            else hexValues[0] = hexDigits[0] - '0';

            if (hexDigits[1] >= 'A' && hexDigits[1] <= 'F') hexValues[1] = hexDigits[1] - 'A' + 10;
            if (hexDigits[1] >= 'a' && hexDigits[1] <= 'f') hexValues[1] = hexDigits[1] - 'a' + 10;
            else hexValues[1] = hexDigits[1] - '0';
...
void RakString::SplitURI(RakNet::RakString& header, RakNet::RakString& domain, RakNet::RakString& path) {
    ...
    char         c;
    unsigned int i = 0;
    if (strncmp(sharedString->c_str, "http://", 7) == 0) i += (unsigned int)strlen("http://");
    else if (strncmp(sharedString->c_str, "https://", 8) == 0) i += (unsigned int)strlen("https://");
...
RakString
RakString::FormatForPOST(const char* uri, const char* contentType, const char* body, const char* extraHeaders) {
    return FormatForPUTOrPost("POST", uri, contentType, body, extraHeaders);
}
RakString
RakString::FormatForPUT(const char* uri, const char* contentType, const char* body, const char* extraHeaders) {
...
bool RakString::Deserialize(BitStream* bs) {
    ...
    if (l > 0) {
        ...
        if (b) sharedString->c_str[l] = 0;
        else Clear();
    } else bs->AlignReadToByteBoundary();
    return b;
}
...
bool RakString::DeserializeCompressed(BitStream* bs, bool readLanguageId) {
    uint8_t languageId;
    if (readLanguageId) bs->ReadCompressed(languageId);
    else languageId = 0;
