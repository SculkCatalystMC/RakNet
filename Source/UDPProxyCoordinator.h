...
/// \file
/// \brief Essentially maintains a list of servers running UDPProxyServer, and some state management for UDPProxyClient
/// to find a free server to forward datagrams
///
...
namespace RakNet {
/// When NAT Punchthrough fails, it is possible to use a non-NAT system to forward messages from us to the recipient,
/// and vice-versa The class to forward messages is UDPForwarder, and it is triggered over the network via the
/// UDPProxyServer plugin. The UDPProxyClient connects to UDPProxyCoordinator to get a list of servers running
...
public:
    ...
    /// For UDPProxyServers logging in remotely, they must pass a password to UDPProxyServer::LoginToCoordinator(). It
    /// must match the password set here. If no password is set, they cannot login remotely. By default, no password is
    /// set
