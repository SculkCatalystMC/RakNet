...
void CloudServer::OnPostRequest(Packet* packet) {
    ...
    for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
        if (queryFilters[filterIndex]->OnPostRequest(
                ...
                key,
                dataLengthBytes,
                (const char*)bsIn.GetData() + BITS_TO_BYTES(bsIn.GetReadOffset())
            )
            == false)
            return;
    }
    ...
    if (keyDataListExists == false) {
        ...
        if (forceAddress != UNASSIGNED_SYSTEM_ADDRESS) {
            ...
            cloudData->serverSystemAddress.SetPortHostOrder(
                mRakPeerInterface->GetExternalID(packet->systemAddress).GetPort()
            );
        } else {
            cloudData->serverSystemAddress = mRakPeerInterface->GetExternalID(packet->systemAddress);
            if (cloudData->serverSystemAddress.IsLoopback())
                cloudData->serverSystemAddress.FromString(mRakPeerInterface->GetLocalIP(0));
        }
        if (cloudData->serverSystemAddress.GetPort() == 0) {
            // Fix localhost port
            cloudData->serverSystemAddress.SetPortHostOrder(
                mRakPeerInterface->GetSocket(UNASSIGNED_SYSTEM_ADDRESS)->GetBoundAddress().GetPort()
            );
        }
        cloudData->clientSystemAddress = packet->systemAddress;
        cloudData->serverGUID          = mRakPeerInterface->GetMyGUID();
    ...
    if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
        ...
        for (keyIndex = 0; keyIndex < getRequest->cloudQueryWithAddresses.cloudQuery.keys.Size(); keyIndex++) {
            ...
            if (getRequest->cloudQueryWithAddresses.specificSystems.Size()) {
                ...
                unsigned int _specificSystemIndex;
                for (_specificSystemIndex = 0;
                     _specificSystemIndex < getRequest->cloudQueryWithAddresses.specificSystems.Size();
                     _specificSystemIndex++) {
                    RakNetGUID specificSystem =
                        getRequest->cloudQueryWithAddresses.specificSystems[_specificSystemIndex];
                ...
                if (subscribedKeysIndexExists) {
                    KeySubscriberID* _keySubscriberId;
                    _keySubscriberId = remoteCloudClient->subscribedKeys[subscribedKeysIndex];
                    unsigned int _specificSystemIndex;
                    for (_specificSystemIndex = 0;
                         _specificSystemIndex < _keySubscriberId->specificSystemsSubscribedTo.Size();
                         _specificSystemIndex++) {
                        bool         keyDataExists;
                        unsigned int keyDataIndex = cloudDataList->keyData.GetIndexFromKey(
                            _keySubscriberId->specificSystemsSubscribedTo[_specificSystemIndex],
                            &keyDataExists
                        );
                        if (keyDataExists) {
...
void CloudServer::OnUnsubscribeRequest(Packet* packet) {
    ...
    for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
        if (queryFilters[filterIndex]
                ->OnUnsubscribeRequest(packet->guid, packet->systemAddress, cloudKeys, specificSystems)
            == false)
            return;
    }
    ...
    for (index = 0; index < keyCount; index++) {
        CloudKey _cloudKey = cloudKeys[index];

        //	dataRepositoryIndex =
        dataRepository.GetIndexFromKey(_cloudKey, &dataRepositoryExists);
        if (dataRepositoryExists == false) continue;
        //		cloudDataList = dataRepository[dataRepositoryIndex];

        unsigned int keySubscriberIndex;
        bool         hasKeySubscriber;
        keySubscriberIndex = remoteCloudClient->subscribedKeys.GetIndexFromKey(_cloudKey, &hasKeySubscriber);

        if (hasKeySubscriber == false) continue;

        UnsubscribeFromKey(remoteCloudClient, packet->guid, keySubscriberIndex, _cloudKey, specificSystems);
    }
...
void CloudServer::AddServer(RakNetGUID systemIdentifier) {
    ConnectionState cs = mRakPeerInterface->GetConnectionState(systemIdentifier);
    ...
    if (specificSystems.Size() == 0) {
        ...
        for (j = 0; j < specificSystems.Size(); j++) {
            ...
            specificSystemsSubscribedToIndex = keySubscriberId->specificSystemsSubscribedTo.GetIndexFromKey(
                specificSystems[j],
                &hasSpecificSystemsSubscribedTo
            );
