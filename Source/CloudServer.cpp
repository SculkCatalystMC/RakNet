    ...
    if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
        ...
        for (keyIndex = 0; keyIndex < getRequest->cloudQueryWithAddresses.cloudQuery.keys.Size(); keyIndex++) {
            ...
            if (getRequest->cloudQueryWithAddresses.specificSystems.Size()) {
                ...
                unsigned int specificSystemLoopIndex;
                for (specificSystemLoopIndex = 0;
                     specificSystemLoopIndex < getRequest->cloudQueryWithAddresses.specificSystems.Size();
                     specificSystemLoopIndex++) {
                    RakNetGUID specificSystem =
                        getRequest->cloudQueryWithAddresses.specificSystems[specificSystemLoopIndex];
                ...
                if (subscribedKeysIndexExists) {
                    KeySubscriberID* subscribedKeySubscriberId;
                    subscribedKeySubscriberId = remoteCloudClient->subscribedKeys[subscribedKeysIndex];
                    unsigned int specificSystemLoopIndex;
                    for (specificSystemLoopIndex = 0;
                         specificSystemLoopIndex < subscribedKeySubscriberId->specificSystemsSubscribedTo.Size();
                         specificSystemLoopIndex++) {
                        bool         keyDataExists;
                        unsigned int keyDataIndex = cloudDataList->keyData.GetIndexFromKey(
                            subscribedKeySubscriberId->specificSystemsSubscribedTo[specificSystemLoopIndex],
                            &keyDataExists
                        );
                        if (keyDataExists) {
...
void CloudServer::OnUnsubscribeRequest(Packet* packet) {
    ...
    for (index = 0; index < keyCount; index++) {
        CloudKey unsubscribeKey = cloudKeys[index];

        //	dataRepositoryIndex =
        dataRepository.GetIndexFromKey(unsubscribeKey, &dataRepositoryExists);
        if (dataRepositoryExists == false) continue;
        //		cloudDataList = dataRepository[dataRepositoryIndex];

        unsigned int keySubscriberIndex;
        bool         hasKeySubscriber;
        keySubscriberIndex = remoteCloudClient->subscribedKeys.GetIndexFromKey(unsubscribeKey, &hasKeySubscriber);

        if (hasKeySubscriber == false) continue;

        UnsubscribeFromKey(remoteCloudClient, packet->guid, keySubscriberIndex, unsubscribeKey, specificSystems);
    }
