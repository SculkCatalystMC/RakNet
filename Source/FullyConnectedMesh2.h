...
/// It will also calculate which system has been running longest, to find out who should be host, if you need one system
/// to act as a host
...
public:
    ...
    /// should be false and you should connect manually. It is here for legacy reasons.
    /// \param[in] attemptConnection If true, we try to connect to any systems we are notified about with
    /// ID_REMOTE_NEW_INCOMING_CONNECTION, which comes from the ConnectionGraph2 plugin. Defaults to true.
    ...
    /// \details Will return UNASSIGNED_RAKNET_GUID if we are not connected to anyone, or if we are connected and are
    /// calculating the host If includeCalculating is true, will return the estimated calculated host as long as the
    /// calculation is nearly complete includeCalculating should be true if you are taking action based on another
    /// system becoming host, because not all host calculations may complete at the exact same time
    /// \sa ConnectionGraph2::GetLowestAveragePingSystem() . If you need one system in the peer to peer group to relay
    /// data, have the host call this function after host migration, and use that system
    ...
    /// \return System address of whichever system is host. Always returns something, even though it may be our own
    /// system.
    ...
    /// \param[in] includeCalculating If true, and we are currently calculating a new host, return the new host if the
    ...
    /// SetAutoparticipateConnections() or by calling this function
    ...
    /// \brief if SetAutoparticipateConnections() is called with false, then you need to use AddParticipant before these
    /// systems will be added to the mesh FullyConnectedMesh2 will track who is the who host among a fully connected
    /// mesh of participants Each remote system that you want to check should be added as a participant, either through
    /// SetAutoparticipateConnections() or by calling this function
    /// \param[in] participant The new participant
    /// \param[in] userContext Static data to be passed around with each participant, which can be queried with
    /// GetParticipantData().
    ...
    /// When there is a risk of failure, such as if relying on NATPunchthroughClient, you may not want to call
    /// AddParticipant() until are connections have completed to all other particpants StartVerifiedJoin() can manage
    /// the overhead of the negotiation involved so the programmer only has to deal with overall success or failure
    ///
    /// Processing:
    /// 1. Send the RakNetGUID and SystemAddress values of GetParticipantList() to the client with
    /// ID_FCM2_VERIFIED_JOIN_START
    /// 2. The client, on ID_FCM2_VERIFIED_JOIN_START, can execute NatPunchthroughClient::OpenNAT() (optional), followed
    /// by RakPeerInterface::Connect() if punchthrough success, for each system returned from
    /// GetVerifiedJoinRequiredProcessingList()
    /// 3. After all participants in step 2 have connected, failed to connect, or failed NatPunchthrough, the client
    /// automatically sends the results to the server.
    /// 4. The server compares the results of the operations in step 2 with the values from GetParticpantList().
    /// 4A. If the client failed to connect to a current participant, return ID_FCM2_VERIFIED_JOIN_FAILED to the client.
    /// CloseConnection() is automatically called on the client for the failed participants. 4B. If AddParticipant() was
    /// called between steps 1 and 4, go back to step 1, transmitting new participants. 4C. If the client successfully
    /// connected to all participants, the server gets ID_FCM2_VERIFIED_JOIN_CAPABLE. The server programmer, on the same
    /// frame, should execute RespondOnVerifiedJoinCapable() to either accept or reject the client.
    /// 5. If the client got ID_FCM2_VERIFIED_JOIN_ACCEPTED, AddParticipant() is automatically called for each system in
    /// the mesh.
    /// 6. If the client got ID_FCM2_VERIFIED_JOIN_REJECTED, CloseConnection() is automatically called for each system
    /// in the mesh. The connection is NOT automatically closed to the original host that sent StartVerifiedJoin().
    /// 7. If the client's connection to the server was lost before getting ID_FCM2_VERIFIED_JOIN_ACCEPTED or
    /// ID_FCM2_VERIFIED_JOIN_REJECTED, return to the programmer ID_FCM2_VERIFIED_JOIN_FAILED and call
    /// RakPeerInterface::CloseConnection()
    ///
    /// \brief Notify the client of GetParticipantList() in order to connect to each of those systems until the mesh has
    /// been completed
    ...
    /// \param[in] accept True to accept, and thereby automatically call AddParticipant() on all systems on the mesh.
    /// False to reject, and call CloseConnection() to all mesh systems on the target
    /// \param[in] additionalData Any additional data you want to add to the ID_FCM2_VERIFIED_JOIN_ACCEPTED or
    /// ID_FCM2_VERIFIED_JOIN_REJECTED messages
    /// \sa WriteVJCUserData()
    ...
    /// \param[out] addresses SystemAddress values of systems to connect to. List has the same number and order as \a
    /// guids
    ...
    /// fullyConnectedMesh->GetVerifiedJoinAcceptedAdditionalData(packet, &thisSystemAccepted, systemsAccepted,
    /// &additionalData);
    /// \endcode
    /// \param[in] packet Packet containing the ID_FCM2_VERIFIED_JOIN_ACCEPTED message
    /// \param[out] thisSystemAccepted If true, it was this instance of RakPeerInterface that was accepted. If false,
    /// this is notification for another system
    /// \param[out] systemsAccepted Which system(s) were added with AddParticipant(). If \a thisSystemAccepted is false,
    /// this list will only have length 1
    ...
    /// \param[in] userContext The data set with SetMyContext() for that system. May be empty. To properly write
    /// userContext, you will need to first write userContext->GetNumberOfBitsUsed(), followed by
    /// bsOut->Write(userContext);
    // virtual void WriteVJSUserData(RakNet::BitStream *bsOut, RakNetGUID userGuid, BitStream *userContext) {(void)
    // bsOut; (void) userGuid; (void) userContext;}
    ...
    /// \internal
    virtual void
    OnFailedConnectionAttempt(Packet* packet, PI2_FailedConnectionAttemptReason failedConnectionAttemptReason);
...
protected:
    ...
    unsigned int
         GetVerifiedJoinInProgressMemberIndex(const AddressOrGUID systemIdentifier, VerifiedJoinInProgress* vjip);
    ...
    // totalConnectionCount is roughly maintained across all systems, and increments by 1 each time a new system
    // connects to the mesh It is always kept at the highest known value It is used as the high 4 bytes for new
    // FCMGuids. This causes newer values of FCM2Guid to be higher than lower values. The lowest value is the host.
    unsigned int totalConnectionCount;

    // Our own ourFCMGuid. Starts at unassigned (0). Assigned once we send ID_FCM2_REQUEST_FCMGUID and get back
...
{
    // They added us as a participant, but we didn't add them. This can be caused by lag where both participants are not
