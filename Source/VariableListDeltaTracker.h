...
namespace RakNet {
/// Class to write a series of variables, copy the contents to memory, and return if the newly written value is
/// different than what was last written Can also encode the reads, writes, and results directly to/from a bitstream
...
public:
    ...
    /// Records the passed value of the variable to memory, and returns true if the value is different from the write
    /// before that (or if it is the first write)
    /// \pre Call StartWrite() before doing the first of a series of calls to WriteVar or other functions that call
    /// WriteVar
    ...
    bool WriteVar(const VarType& varData) {
        ...
        if (temp.GetNumberOfBytesUsed() != variableList[nextWriteIndex].byteLength) {
            variableList[nextWriteIndex].lastData = (char*)
                rakRealloc_Ex(variableList[nextWriteIndex].lastData, temp.GetNumberOfBytesUsed(), _FILE_AND_LINE_);
        ...
        if (variableList[nextWriteIndex].isDirty == false
            && memcmp(temp.GetData(), variableList[nextWriteIndex].lastData, variableList[nextWriteIndex].byteLength)
                   == 0) {
            nextWriteIndex++;
    ...
    bool WriteVarToBitstream(
        ...
        if (WriteVarToBitstream(varData, bitStream) == true) {
            ...
            if (numberOfBitsMod8 == 0) bArray[writeOffset >> 3] = 0x80;
            else bArray[writeOffset >> 3] |= 0x80 >> (numberOfBitsMod8); // Set the bit to 1
    ...
    /// Paired with a call to WriteVarToBitstream(), will read a variable if it had changed. Otherwise the values
    /// remains the same.
    template <class VarType>
    ...
    /// This updates all the variables in the list, where in each index \a varsWritten is true, so will the variable at
