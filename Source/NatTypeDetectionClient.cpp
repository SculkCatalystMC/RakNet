...
void NatTypeDetectionClient::DetectNATType(SystemAddress _serverAddress) {
    ...
    mRakPeerInterface->Send(&bs, MEDIUM_PRIORITY, RELIABLE, 0, serverAddress, false);
}
void NatTypeDetectionClient::OnCompletion(NATTypeDetectionResult result) {
    ...
    p->guid                      = mRakPeerInterface->GetGuidFromSystemAddress(serverAddress);
    ...
    if (result != NAT_TYPE_PORT_RESTRICTED && result != NAT_TYPE_SYMMETRIC) {
        ...
        bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_REQUEST);
        bs.Write(false); // Done
        mRakPeerInterface->Send(&bs, HIGH_PRIORITY, RELIABLE, 0, serverAddress, false);
    }
...
void NatTypeDetectionClient::OnTestPortRestricted(Packet* packet) {
    ...
    bsOut.Write(mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS));
    //	SocketLayer::SendTo_PC( sockets[0], (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), s3p4Addr,
    //__FILE__, __LINE__ );
