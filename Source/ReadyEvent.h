...
/// \file
/// \brief Ready event plugin.  This enables a set of systems to create a signal event, set this signal as ready or
/// unready, and to trigger the event when all systems are ready
///
...
enum ReadyEventSystemStatus {
    ...
    RES_WAITING,
    /// The remote system called SetEvent(thisEvent,true), but it still waiting for other systems before completing the
    /// ReadyEvent.
    RES_READY,
...
/// For example, if A is connected to B and C, A may see that B and C are ready, but does not know if B is ready to C,
/// or vice-versa.<BR> This plugin uses two stages to solve that problem, first, everyone I know about is ready. Second,
/// everyone I know about is ready to everyone they know about.<BR> The user will get ID_READY_EVENT_SET and
/// ID_READY_EVENT_UNSET as the signal flag is set or unset<BR> The user will get ID_READY_EVENT_ALL_SET when all
/// systems are done waiting for all other systems, in which case the event is considered complete, and no longer
/// tracked.<BR>
...
public:
    ...
    /// If eventId was previously used and you want to reuse it, call DeleteEvent first, or else you will keep the same
    /// event signals from before Systems previously or later added through AddToWaitList() with the same \a eventId
    /// when isReady=true will get ID_READY_EVENT_SET Systems previously added through AddToWaitList with the same \a
    /// eventId will get ID_READY_EVENT_UNSET For both ID_READY_EVENT_SET and ID_READY_EVENT_UNSET, eventId is encoded
    /// in bytes 1 through 1+sizeof(int)
    /// \param[in] eventId A user-defined identifier to wait on. This can be a sequence counter, an event identifier, or
    /// anything else you want.
    /// \param[in] isReady True to signal we are ready to proceed with this event, false to unsignal
    /// \return False if event status is ID_READY_EVENT_FORCE_ALL_SET, or if we are setting to a status we are already
    /// in (no change). Otherwise true
    bool SetEvent(int eventId, bool isReady);

    /// When systems can call SetEvent() with isReady==false, it is possible for one system to return true from
    /// IsEventCompleted() while the other systems return false This can occur if a system SetEvent() with
    /// isReady==false while the completion message is still being transmitted. If your game has the situation where
    /// some action should be taken on all systems when IsEventCompleted() is true for any system, then call
    /// ForceCompletion() when the action begins. This will force all systems to return true from IsEventCompleted().
    /// \param[in] eventId A user-defined identifier to immediately set as completed
    void ForceCompletion(int eventId);

    /// Deletes an event.  We will no longer wait for this event, and any systems that we know have set the event will
    /// be forgotten. Call this to clear memory when events are completed and you know you will never need them again.
    ...
    /// If you are waiting for a specific number of systems (such as players later connecting), also check
    /// GetRemoteWaitListSize(eventId) to be equal to 1 less than the total number of participants.
    /// \param[in] eventId A user-defined identifier
    ...
    /// Events may be known even if we never ourselves referenced them with SetEvent, because other systems created them
    /// via ID_READY_EVENT_SET.
    /// \param[in] eventId A user-defined identifier
    ...
    /// Adds a system to wait for to signal an event before considering the event complete and returning
    /// ID_READY_EVENT_ALL_SET. As we add systems, if this event was previously set to true with SetEvent, these systems
    /// will get ID_READY_EVENT_SET. As these systems disconnect (directly or indirectly through the router) they are
    /// removed.
    /// \note If the event completion process has already started, you cannot add more systems, as this would cause the
    /// completion process to fail
    /// \param[in] eventId A user-defined number previously passed to SetEvent that has not yet completed
    /// \param[in] guid An address to wait for event replies from.  Pass UNASSIGNED_SYSTEM_ADDRESS for all currently
    /// connected systems. Until all systems in this list have called SetEvent with this ID and true, and have this
    /// system in the list, we won't get ID_READY_EVENT_COMPLETE
    ...
    /// \param[in] guid The system to remove from the wait list. Pass UNASSIGNED_RAKNET_GUID for all currently connected
    /// systems.
    /// \return True on success, false on unknown eventId (this should be considered an error)
    ...
    /// \internal
    /// An event, with a set of systems we are waiting for, a set of systems that are signaled, and a set of systems
