...
enum MultilistType {
    /// Removing from the middle of the list will swap the end of the list rather than shift the elements. Push and Pop
    /// operate on the tail.
    ML_UNORDERED_LIST,
    ...
    ML_QUEUE,
    /// A list that is always kept in order. Elements must be unique, and compare against each other consistently using
    /// <, ==, and >
    ML_ORDERED_LIST,
...
/// MLKeyRef is a helper class to turn a native type into a class, so you can compare that native type against a pointer
/// to a different class Used for he Multilist, when _DataType != _KeyType
template <class templateType>
class MLKeyRef {
public:
...
/// This is non-trivial due to the need to use MLKeyRef in case the type held is a pointer to a structure or class and
/// the key type is not a class For convenience, this macro will implement the comparison operators under the following
/// conditions
...
/// \param[in] _KeyType If a function takes a key to sort on, what type of key this is. The comparison operator between
/// _DataType and _KeyType must be defined
...
public:
    ...
    void
    PushOpposite(const _DataType& d, const _KeyType& key, const char* file = __FILE__, unsigned int line = __LINE__);
    ...
    bool
    RemoveAtKey(_KeyType key, bool assertIfDoesNotExist, const char* file = __FILE__, unsigned int line = __LINE__);
    ...
    /// \brief Finds the index of \a key. Return 0 if the key is not found. Useful if _DataType is always non-zero
    /// pointers.
    _DataType GetPtr(_KeyType key) const;

    /// \brief Iterate over the list, calling the function pointer on each element.
    void
    ForEach(void (*func)(_DataType& item, const char* file, unsigned int line), const char* file, unsigned int line);
    ...
    /// \brief Sorts the list unless it is an ordered list, in which it does nothing as the list is assumed to already
    /// be sorted.
    /// \details However, if \a force is true, it will also resort the ordered list, useful if the comparison operator
    /// between _KeyType and _DataType would now return different results Once the list is sorted, further operations to
    /// lookup by key will be log2(n) until the list is modified
...
Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Multilist() {
    ...
    if (_MultilistType == ML_ORDERED_LIST) sortState = ML_SORTED_ASCENDING;
    else sortState = ML_UNSORTED;
...
_DataType& Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::operator[](const _IndexType position) const {
    ...
    if (GetMultilistType() == ML_QUEUE) {
        if (queueHead + position >= allocationSize) return data[queueHead + position - allocationSize];
        else return data[queueHead + position];
    }
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
_DataType&
Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::PopOpposite(const char* file, unsigned int line) {
    RakAssert(IsEmpty() == false);
    if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK
        || GetMultilistType() == ML_ORDERED_LIST) {
        ...
        if (queueTail == 0) queueTail = allocationSize - 1;
        else --queueTail;
...
_DataType& Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::PeekOpposite(void) const {
    ...
    if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK
        ...
        if (queueTail == 0) priorIndex = allocationSize - 1;
        else priorIndex = queueTail - 1;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::InsertAtIndex(
    ...
    if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK
        ...
        while (readIndex >= index) {
            if (queueHead + writeIndex >= allocationSize) trueWriteIndex = queueHead + writeIndex - allocationSize;
            else trueWriteIndex = queueHead + writeIndex;

            if (queueHead + readIndex >= allocationSize) trueReadIndex = queueHead + readIndex - allocationSize;
            else trueReadIndex = queueHead + readIndex;
        ...
        if (queueHead + index >= allocationSize) trueWriteIndex = queueHead + index - allocationSize;
        else trueWriteIndex = queueHead + index;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::RemoveAtIndex(
    ...
    if (GetMultilistType() == ML_UNORDERED_LIST) {
        ...
        if (queueHead + position >= allocationSize) index = queueHead + position - allocationSize;
        else index = queueHead + position;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Clear(
    ...
    if (GetMultilistType() == ML_ORDERED_LIST)
        if (ascendingSort) sortState = ML_SORTED_ASCENDING;
        else sortState = ML_SORTED_DESCENDING;
    else sortState = ML_UNSORTED;
    queueHead = 0;
    queueTail = 0;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Reallocate(
    ...
    if (size < dataSize) newAllocationSize = dataSize;
    else newAllocationSize = size;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Sort(bool force) {
    ...
    if (dataSize > 1) {
        if (ascendingSort) QSortAscending(0, dataSize - 1);
        else QSortDescending(0, dataSize - 1);
    }
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ReallocateIfNeeded(
    ...
    _IndexType newAllocationSize;
    if (allocationSize < 16) newAllocationSize = 32;
    else if (allocationSize > 65536) newAllocationSize = allocationSize + 65536;
    else {
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
    ...
    while (1) {
        if (MLKeyRef<_KeyType>(key) > operator[](index)) {
            if (ascendingSort) lowerBound = index + 1;
            else upperBound = index - 1;
        } else if (MLKeyRef<_KeyType>(key) < operator[](index)) {
            if (ascendingSort) upperBound = index - 1;
            else lowerBound = index + 1;
        } else {
            // ==
            *objectExists = true;
            return index;
