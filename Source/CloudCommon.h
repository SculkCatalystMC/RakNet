...
struct RAKNET_API CloudKey {
    ...
    /// Identifies the secondary key. This is intended to be a subcategory enumeration, such as PLAYER_LIST or
    /// RUNNING_SCORES
...
struct RAKNET_API CloudQuery {
    ...
    /// This query is run on uploads from all clients, and those that match the combination of primaryKey and
    /// secondaryKey are potentially returned If you pass more than one key at a time, the results are concatenated so
    /// if you need to differentiate between queries then send two different queries
    DataStructures::List<CloudKey> keys;

    /// If limiting the number of rows to return, this is the starting offset into the list. Has no effect unless
    /// maxRowsToReturn is > 0
...
struct RAKNET_API CloudQueryResult {
    ...
    /// If there were multiple keys in CloudQuery::keys, then each key is processed in order and the result concatenated
    /// to rowsReturned The starting index of each query is written to resultKeyIndices For example, if CloudQuery::keys
    /// had 4 keys, returning 3 rows, 0, rows, 5 rows, and 12 rows then resultKeyIndices would be 0, 3, 3, 8
    ...
    /// \internal
    void
    SerializeCloudQueryRows(bool writeToBitstream, uint32_t& numRows, BitStream* bitStream, CloudAllocator* allocator);
};
