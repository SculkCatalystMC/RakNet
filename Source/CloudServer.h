...
public:
    ...
    /// \return true to allow, false to reject
    virtual bool
    OnReleaseRequest(RakNetGUID clientGuid, SystemAddress clientAddress, DataStructures::List<CloudKey>& cloudKeys) = 0;
...
public:
    ...
    /// \brief Max bytes returned by a download. If the number of bytes would exceed this amount, the returned list is
    /// truncated However, if this would result in no rows downloaded, then one row will be returned.
    ...
    /// \brief Add a server, which is assumed to be connected in a fully connected mesh to all other servers and also
    /// running the CloudServer plugin The other system must also call AddServer before getting the subscription data,
    /// or it will be rejected. Sending a message telling the other system to call AddServer(), followed by calling
    /// AddServer() locally, would be sufficient for this to work.
    ...
    /// \param[in] forcedAddress The systmeAddress to return in queries. Use UNASSIGNED_SYSTEM_ADDRESS (default) to use
    /// what RakPeer returns
    void ForceExternalSystemAddress(SystemAddress forcedAddress);

    /// \brief Adds a callback called on each query. If all filters returns true for an operation, the operation is
    /// allowed. If the filter was already added, the function silently fails
    /// \param[in] filter An externally allocated instance of CloudServerQueryFilter. The instance must remain valid
    /// until it is removed with RemoveQueryFilter() or RemoveAllQueryFilters()
...
protected:
    ...
    struct CloudData {
        ...
        unsigned char* allocatedData; // Uses allocatedData instead of stackData if length of data exceeds
    ...
    struct GetRequest {
        ...
        DataStructures::
            OrderedList<RakNetGUID, BufferedGetResponseFromServer*, CloudServer::BufferedGetResponseFromServerComp>
    ...
    GetOrAllocateCloudDataList(CloudKey key, bool* dataRepositoryExists, unsigned int& dataRepositoryIndex);
...
// B. If no one else is subscribing to this key for any system, notify remote servers we no longer need subscription
// updates
//
// Internal operations:
//
