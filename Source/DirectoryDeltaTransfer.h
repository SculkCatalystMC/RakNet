...
/// \brief Simple class to send changes between directories.  In essence, a simple autopatcher that can be used for
/// transmitting levels, skins, etc.
/// \details
...
/// To allow other systems to download files, call AddUploadsFromSubdirectory, where the parameter is a path
/// relative<BR> to the path to your application.  This includes subdirectories.<BR> For example:<BR>
...
public:
    ...
    /// \details So you need an instance of that plugin registered with RakPeerInterface, and a pointer to that
    /// interface should be passed here.
    ...
    /// Therefore,
    /// AddUploadsFromSubdirectory("Levels/Level1/"); would allow you to download using
    /// DownloadFromSubdirectory("Levels/Level1/Textures/"... but it would NOT allow you to download from
    /// DownloadFromSubdirectory("Levels/"... or DownloadFromSubdirectory("Levels/Level2/"...
    /// \pre You must call SetFileListTransferPlugin with a valid FileListTransfer plugin
    /// \note Blocking. Will block while hashes of the local files are generated
    /// \param[in] subdir A directory passed to AddUploadsFromSubdirectory on the remote system.  The passed dir can be
    /// more specific than the remote dir.
    /// \param[in] outputSubdir The directory to write the output to.  Usually this will match \a subdir but it can be
    /// different if you want.
    /// \param[in] prependAppDirToOutputSubdir True to prepend outputSubdir with pathToApplication when determining the
    /// final output path.  Usually you want this to be true.
    /// \param[in] host The address of the remote system to send the message to.
    /// \param[in] onFileCallback Callback to call per-file (optional).  When fileIndex+1==setCount in the callback then
    ...
    /// \param[in] localFiles Hashes of local files already on the harddrive. Populate with GenerateHashes(), which you
    /// may wish to call from a thread
    /// \param[in] subdir A directory passed to AddUploadsFromSubdirectory on the remote system.  The passed dir can be
    /// more specific than the remote dir.
    /// \param[in] outputSubdir The directory to write the output to.  Usually this will match \a subdir but it can be
    /// different if you want.
    ...
    /// Hash files already on the harddrive, in preparation for a call to DownloadFromSubdirectory(). Passed to second
    /// version of DownloadFromSubdirectory() This is slow, and it is exposed so you can call it from a thread before
    /// calling DownloadFromSubdirectory()
    ...
    /// \param[in] _incrementalReadInterface If a file in \a fileList has no data, filePullInterface will be used to
    /// read the file in chunks of size \a chunkSize
