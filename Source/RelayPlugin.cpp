...
// Send a message to a server running RelayPlugin, to forward a message to the
// system identified by \a key
...
PluginReceiveResult RelayPlugin::OnReceive(Packet* packet) {
    if (packet->data[0] == ID_RELAY_PLUGIN) {
        ...
        case RPE_ADD_CLIENT_REQUEST_FROM_CLIENT: {
            ...
            BitStream bsOut;
            bsOut.WriteCasted<MessageID>(ID_RELAY_PLUGIN);
            if (acceptAddParticipantRequests) bsOut.WriteCasted<MessageID>(AddParticipantOnServer(key, packet->guid));
            else bsOut.WriteCasted<MessageID>(RPE_ADD_CLIENT_NOT_ALLOWED);
            bsOut.WriteCompressed(key);
...
void RelayPlugin::SendMessageToRoom(StrAndGuidAndRoom** strAndGuidSender, BitStream* message) {
    ...
    for (unsigned int i = 0; i < chatRooms.Size(); i++) {
        if (chatRooms[i]->roomName == (*strAndGuidSender)->currentRoom) {
            ...
            RP_Group* room = chatRooms[i];
            for (unsigned int roomUserIndex = 0; roomUserIndex < room->usersInRoom.Size(); roomUserIndex++) {
                if (room->usersInRoom[roomUserIndex].guid != (*strAndGuidSender)->guid)
                    SendUnified(
                        &bsOut,
                        HIGH_PRIORITY,
                        RELIABLE_ORDERED,
                        0,
                        room->usersInRoom[roomUserIndex].guid,
                        false
                    );
            }
