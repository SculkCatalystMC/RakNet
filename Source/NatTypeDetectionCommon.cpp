...
RakNetSocket2* RakNet::CreateNonblockingBoundSocket(
    ...
    /*
    #ifdef __native_client__
    RakNetSocket2 *s = SocketLayer::CreateBoundSocket( 0, 0, false, bindAddr,
    true, 0, AF_INET, chromeInstance ); #else RakNetSocket2 *s =
    SocketLayer::CreateBoundSocket( 0, 0, false, bindAddr, true, 0, AF_INET, 0 );
    #endif
    ...
    defined(_PS4) || defined(SN_TARGET_PSP2) int sock_opt=1;
            s->SetSockOpt(SOL_SOCKET, SO_NBIO, ( char * ) & sock_opt, sizeof (
    sock_opt ) ); #elif defined(__native_client__)
            // Nop
    #else
...
/*
int RakNet::NatTypeRecvFrom(char *data, RakNetSocket2* socket, SystemAddress
&sender, RNS2EventHandler *eventHandler)
{
#if defined(__native_client__)
        RakAssert("TODO" && 0);
#elif defined(WINDOWS_STORE_RT)
