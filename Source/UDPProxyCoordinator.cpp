...
// bool operator<( const DataStructures::MLKeyRef<unsigned short> &inputKey, const UDPProxyCoordinator::ServerWithPing
// &cls ) {return inputKey.Get() < cls.ping;} bool operator>( const DataStructures::MLKeyRef<unsigned short> &inputKey,
// const UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() > cls.ping;} bool operator==( const
// DataStructures::MLKeyRef<unsigned short> &inputKey, const UDPProxyCoordinator::ServerWithPing &cls ) {return
...
//
// bool operator<( const DataStructures::MLKeyRef<UDPProxyCoordinator::SenderAndTargetAddress> &inputKey, const
// UDPProxyCoordinator::ForwardingRequest *cls )
// {
// 	return inputKey.Get().senderClientAddress < cls->sata.senderClientAddress ||
// 		(inputKey.Get().senderClientAddress == cls->sata.senderClientAddress && inputKey.Get().targetClientAddress <
...
// UDPProxyCoordinator::ForwardingRequest *cls )
// {
// 	return inputKey.Get().senderClientAddress == cls->sata.senderClientAddress && inputKey.Get().targetClientAddress ==
// cls->sata.targetClientAddress;
// }
...
void UDPProxyCoordinator::OnForwardingRequestFromClientToCoordinator(Packet* packet) {
    ...
    if (usesAddress) {
        ...
        targetGuid = mRakPeerInterface->GetGuidFromSystemAddress(targetAddress);
    } else {
        incomingBs.Read(targetGuid);
        targetAddress = mRakPeerInterface->GetSystemAddressFromGuid(targetGuid);
    }
    ...
    sata.senderClientGuid    = mRakPeerInterface->GetGuidFromSystemAddress(sourceAddress);
    ...
    if (mRakPeerInterface->GetConnectionState(targetAddress) != IS_CONNECTED && usesAddress == false) {
        outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
    ...
    if (serverList.Size() > 1) {
        ...
        mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, sourceAddress, false);
        mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, targetAddress, false);
...
void UDPProxyCoordinator::SendForwardingRequest(
    ...
    mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, serverAddress, false);
}
...
void UDPProxyCoordinator::OnForwardingReplyFromServerToCoordinator(Packet* packet) {
    ...
    if (success == UDPFORWARDER_SUCCESS) {
        ...
        outgoingBs.Write(serverPublicIp);
        outgoingBs.Write(forwardingPort);
        mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, fw->requestingAddress, false);
        ...
        outgoingBs.Write(serverPublicIp);
        outgoingBs.Write(forwardingPort);
        mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, sata.targetClientAddress, false);

        // 05/18/09 Keep the entry around for some time after success, so duplicates are reported if attempting
        // forwarding from the target system before notification of success
...
void UDPProxyCoordinator::SendAllBusy(
    ...
    mRakPeerInterface->Send(&outgoingBs, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, requestingAddress, false);
}
...
void UDPProxyCoordinator::ForwardingRequest::OrderRemainingServersToTry(void) {
    ...
    for (idx = 0; idx < remainingServersToTry.Size(); idx++) {
        ...
        swp.ping          = 0;
        if (sourceServerPings.Size()) swp.ping += (unsigned short)(sourceServerPings[idx].ping);
        else swp.ping += (unsigned short)(DEFAULT_CLIENT_UNRESPONSIVE_PING_TIME);
        if (targetServerPings.Size()) swp.ping += (unsigned short)(targetServerPings[idx].ping);
        else swp.ping += (unsigned short)(DEFAULT_CLIENT_UNRESPONSIVE_PING_TIME);
