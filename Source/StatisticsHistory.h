...
/// \brief Input numerical values over time. Get sum, average, highest, lowest,
/// standard deviation on recent or all-time values
...
public:
    ...
    enum SHDataCategory {
        ...
        /// Add values from one set to values from the other set, at corresponding
        /// times If value at time t does not exist in the other set, linearly
        /// extrapolate value for other set based on nearest two data points
    ...
    bool        GetHistorySorted(
               ...
               DataStructures::List<TimeAndValueQueue*>& values
           ) const;
    ...
    struct TimeAndValueQueue {
        ...
        // DC_DISCRETE will produce a histogram (sum) while DC_CONTINUOUS will
        // produce an average
...
protected:
    ...
    // 	virtual void OnDirectSocketSend(const char *data, const BitSize_t
    // bitsUsed, SystemAddress remoteSystemAddress); 	virtual void
    // OnDirectSocketReceive(const char *data, const BitSize_t bitsUsed,
