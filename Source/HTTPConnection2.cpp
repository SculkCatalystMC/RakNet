...
#else
            RakAssert(
                "HTTPConnection2::TransmitRequest needs define  "
                "RAKNET_SUPPORT_IPV6"
                && 0
            );
#endif
...
void ReadChunkBlock(size_t& currentChunkSize, size_t& bytesReadSoFar, char* txtIn, RakString& txtOut) {
    ...
    do {
        ...
        txtOut.AppendBytes(txtIn, static_cast<unsigned int>(bytesToRead));
        txtIn          += bytesToRead;
            ...
            if (isFirstChunk) {
                ...
                if (sentRequest->contentLength == -1) {
                    ...
                    if (length_header) {
                        ...
                        for (clLength = 0; length_header[clLength] && length_header[clLength] >= '0'
                ...
                if (sentRequest->contentLength != -1) {
                        ...
                        if (body_header) {
                            ...
                            if (slen >= (size_t)sentRequest->contentLength) {
                                sentRequest->contentOffset =
                                    static_cast<int>(body_header - sentRequest->stringReceived.C_String());
                                completedRequestsMutex.Lock();
                                completedRequests.Push(sentRequest, _FILE_AND_LINE_);
                                completedRequestsMutex.Unlock();
                    ...
                    if (firstNewlineSet != 0) {
                        int offset = static_cast<int>(firstNewlineSet - sentRequest->stringReceived.C_String());
                        if (sentRequest->stringReceived.C_String()[offset + 4] == 0) sentRequest->contentOffset = -1;
                        else sentRequest->contentOffset = offset + 4;
                        completedRequestsMutex.Lock();
                        completedRequests.Push(sentRequest, _FILE_AND_LINE_);
                        completedRequestsMutex.Unlock();
...
void HTTPConnection2::SendPendingRequestToConnectedSystem(SystemAddress sa) {
    ...
    // Search through requests to find a match for this instance of TCPInterface
    // and SystemAddress
    unsigned int i;
    i = 0;
    pendingRequestsMutex.Lock();
...
#endif // #if _RAKNET_SUPPORT_HTTPConnection2==1 &&
       // _RAKNET_SUPPORT_TCPInterface==1
