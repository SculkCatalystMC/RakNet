...
public:
    ...
    /// \param[in] stringToTransmit What string to transmit. See RakString::FormatForPOST(), RakString::FormatForGET(),
    /// RakString::FormatForDELETE()
    /// \param[in] host The IP address to connect to
    /// \param[in] port The port to connect to
    /// \param[in] useSSL If to use SSL to connect. OPEN_SSL_CLIENT_SUPPORT must be defined to 1 in RakNetDefines.h or
    ...
    /// HTTPConnection2 will store Packet::data and return the response to you either when the connection to the
    /// webserver is lost, or enough data has been received() This will only potentially return true after a call to
    /// ProcessTCPPacket() or OnLostConnection()
    ...
    /// \param[out] contentOffset The offset from the start of responseReceived to the data body. Equivalent to
    /// searching for \r\n\r\n in responseReceived.
