...
public:
    ...
    /// \param[in] tcp An instance of TCPInterface that previously had
    /// TCPInterface::Start() called
    /// \param[in] stringToTransmit What string to transmit. See
    /// RakString::FormatForPOST(), RakString::FormatForGET(),
    /// RakString::FormatForDELETE()
    /// \param[in] host The IP address to connect to
    /// \param[in] port The port to connect to
    /// \param[in] useSSL If to use SSL to connect. OPEN_SSL_CLIENT_SUPPORT must
    /// be defined to 1 in RakNetDefines.h or RakNetDefinesOverrides.h
    /// \param[in] ipVersion 4 for IPV4, 6 for IPV6
    /// \param[in] useAddress Assume we are connected to this address and send to
    /// it, rather than do a lookup
    /// \param[in] userData
    ...
    /// \brief Check for and return a response from a prior call to
    /// TransmitRequest() As TCP is stream based, you may get a webserver reply
    /// over several calls to TCPInterface::Receive() HTTPConnection2 will store
    /// Packet::data and return the response to you either when the connection to
    /// the webserver is lost, or enough data has been received() This will only
    /// potentially return true after a call to ProcessTCPPacket() or
    /// OnLostConnection()
    /// \param[out] stringTransmitted The original string transmitted
    /// \param[out] hostTransmitted The parameter of the same name passed to
    /// TransmitRequest()
    /// \param[out] responseReceived The response, if any
    /// \param[out] hostReceived The SystemAddress from ProcessTCPPacket() or
    /// OnLostConnection()
    /// \param[out] contentOffset The offset from the start of responseReceived to
    /// the data body. Equivalent to searching for \r\n\r\n in responseReceived.
