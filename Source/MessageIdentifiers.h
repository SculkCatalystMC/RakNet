...
enum DefaultMessageIDTypes {
    ...
    ID_UNCONNECTED_PING,
    /// Ping from an unconnected system.  Only reply if we have open connections. Do not update timestamps. (internal
    /// use only)
    ...
    /// C2S: Header(1), OfflineMesageID(16), Cookie(4, if HasSecurity is true on the server), clientSupportsSecurity(1
    /// bit), handshakeChallenge (if has security on both server and client), remoteBindingAddress(6), MTU(2), client
    /// GUID(8) Connection slot allocated if cookie is valid, server is not full, GUID and IP not already in use.
    ID_OPEN_CONNECTION_REQUEST_2,
    /// S2C: Header(1), OfflineMesageID(16), server GUID(8), mtu(2), doSecurity(1 bit), handshakeAnswer (if do security
    /// is true)
    ...
    /// RakPeer - We passed a public key to RakPeerInterface::Connect(), but the other system did not have security
    /// turned on
    ...
    /// Second byte indicates type. Used currently for NAT punchthrough for receiver port advertisement. See
    /// ID_NAT_ADVERTISE_RECIPIENT_PORT
    ID_OUT_OF_BAND_INTERNAL,
    /// If RakPeerInterface::Send() is called where PacketReliability contains _WITH_ACK_RECEIPT, then on a later call
    /// to RakPeerInterface::Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS. The message will be 5
    /// bytes long, and bytes 1-4 inclusive will contain a number in native order containing a number that identifies
    /// this message. This number will be returned by RakPeerInterface::Send() or RakPeerInterface::SendList().
    /// ID_SND_RECEIPT_ACKED means that the message arrived
    ID_SND_RECEIPT_ACKED,
    /// If RakPeerInterface::Send() is called where PacketReliability contains UNRELIABLE_WITH_ACK_RECEIPT, then on a
    /// later call to RakPeerInterface::Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS. The message
    /// will be 5 bytes long, and bytes 1-4 inclusive will contain a number in native order containing a number that
    /// identifies this message. This number will be returned by RakPeerInterface::Send() or
    /// RakPeerInterface::SendList(). ID_SND_RECEIPT_LOSS means that an ack for the message did not arrive (it may or
    /// may not have been delivered, probably not). On disconnect or shutdown, you will not get ID_SND_RECEIPT_LOSS for
    /// unsent messages, you should consider those messages as all lost.
    ID_SND_RECEIPT_LOSS,
    ...
    /// RakPeer - The system specified in Packet::systemAddress has disconnected from us.  For the client, this would
    /// mean the server has shutdown.
    ID_DISCONNECTION_NOTIFICATION,
    /// RakPeer - Reliable packets cannot be delivered to the system specified in Packet::systemAddress.  The connection
    /// to that system has been closed.
    ID_CONNECTION_LOST,
    /// RakPeer - We are banned from the system we attempted to connect to.
    ID_CONNECTION_BANNED,
    /// RakPeer - The remote system is using a password and has refused our connection because we did not set the
    /// correct password.
    ID_INVALID_PASSWORD,
    ...
    /// RakPeer - The sizeof(RakNetTime) bytes following this byte represent a value which is automatically modified by
    /// the difference in system times between the sender and the recipient. Requires that you call SetOccasionalPing.
    ID_TIMESTAMP,
    /// RakPeer - Pong from an unconnected system.  First byte is ID_UNCONNECTED_PONG, second sizeof(RakNet::TimeMS)
    /// bytes is the ping, following bytes is system specific enumeration data. Read using bitstreams
    ID_UNCONNECTED_PONG,
    /// RakPeer - Inform a remote system of our IP/Port. On the recipient, all data past ID_ADVERTISE_SYSTEM is whatever
    /// was passed to the data parameter
    ID_ADVERTISE_SYSTEM,
    ...
    /// partLength (unsigned int), first part data (length <= MAX_MTU_SIZE). See the three parameters partCount,
    /// partTotal
    ///  and partLength in OnFileProgress in FileListTransferCBInterface.h
    ID_DOWNLOAD_PROGRESS,

    /// ConnectionGraph2 plugin - In a client/server environment, a client other than ourselves has disconnected
    /// gracefully.
    ///   Packet::systemAddress is modified to reflect the systemAddress of this client.
    ID_REMOTE_DISCONNECTION_NOTIFICATION,
    /// ConnectionGraph2 plugin - In a client/server environment, a client other than ourselves has been forcefully
    /// dropped.
    ///  Packet::systemAddress is modified to reflect the systemAddress of this client.
    ...
    /// NATPunchthrough plugin: Destination system is not connected to the server. Bytes starting at offset 1 contains
    /// the
    ///  RakNetGUID destination field of NatPunchthroughClient::OpenNAT().
    ID_NAT_TARGET_NOT_CONNECTED,
    /// NATPunchthrough plugin: Destination system is not responding to ID_NAT_GET_MOST_RECENT_PORT. Possibly the plugin
    /// is not installed.
    ...
    /// NATPunchthrough plugin: Punchthrough succeeded. See packet::systemAddress and packet::guid. Byte 1 contains 1 if
    /// you are the sender,
    ...
    /// Informs user of a new host GUID. Packet::Guid contains this new host RakNetGuid. The old host can be read out
    /// using BitStream->Read(RakNetGuid) starting on byte 1 This is not returned until connected to a remote system If
    /// the oldHost is UNASSIGNED_RAKNET_GUID, then this is the first time the host has been determined
    ID_FCM2_NEW_HOST,
    ...
    /// A remote system (not necessarily the host) called FullyConnectedMesh2::StartVerifiedJoin() with our system as
    /// the client Use FullyConnectedMesh2::GetVerifiedJoinRequiredProcessingList() to read systems For each system,
    /// attempt NatPunchthroughClient::OpenNAT() and/or RakPeerInterface::Connect() When this has been done for all
    /// systems, the remote system will automatically be informed of the results
    ...
    /// ID_FCM2_VERIFIED_JOIN_FAILED as that system may have been shutting down from the host too
    ...
    /// ID_FCM2_VERIFIED_JOIN_START Programmer should inform the player via the UI that they cannot join this session,
    /// and to choose a different session
    /// \note Server normally sends us this message, however if connection to the server was lost, message will be
    /// returned locally
    /// \note Only the designated client gets this message
    ...
    /// RespondOnVerifiedJoinCapable() with true AddParticipant() has automatically been called for this system Use
    /// GetVerifiedJoinAcceptedAdditionalData() to read any additional data passed to RespondOnVerifiedJoinCapable()
    /// \note All systems in the mesh get this message
    /// \sa RespondOnVerifiedJoinCapable()
    ID_FCM2_VERIFIED_JOIN_ACCEPTED,
    /// The system that called StartVerifiedJoin() got ID_FCM2_VERIFIED_JOIN_CAPABLE from the client and then called
    /// RespondOnVerifiedJoinCapable() with false CloseConnection() has been automatically called for each system
    /// connected to since ID_FCM2_VERIFIED_JOIN_START. The connection is NOT automatically closed to the original host
    /// that sent StartVerifiedJoin() Use GetVerifiedJoinRejectedAdditionalData() to read any additional data passed to
    ...
    /// For TeamBalancer: Byte 1 contains the team you requested to join. Following bytes contain NetworkID of which
    /// member
    ...
    /// TwoWayAuthentication::AddPassword() with the same password we passed to TwoWayAuthentication::Challenge() You
    /// can read the identifier used to challenge as follows: RakNet::BitStream bs(packet->data, packet->length, false);
    ...
    /// RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString
    /// password; bs.Read(password);
    ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE,
    /// The other system did not respond within a timeout threshhold. Either the other system is not running the plugin
    /// or the other system was blocking on some operation for a long time. You can read the identifier used to
    /// challenge as follows: RakNet::BitStream bs(packet->data, packet->length, false);
