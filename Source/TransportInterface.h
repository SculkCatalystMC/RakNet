...
public:
    ...
    /// If your transport method requires particular formatting of the outgoing data (e.g. you don't just send strings)
    /// you can do it here and parse it out in Receive().
    ...
    /// The byte length should be written to Packet::length .  The player/address should be written to
    /// Packet::systemAddress If your transport protocol adds special formatting to the data stream you should parse it
    /// out before returning it in the packet and thus only return a string in Packet::data
    ...
    /// If a new system connects to you, you should queue that event and return the systemAddress/address of that player
    /// in this function.
    /// \return The SystemAddress/address of the system
    virtual SystemAddress HasNewIncomingConnection(void) = 0;

    /// If a system loses the connection, you should queue that event and return the systemAddress/address of that
    ...
    /// For example, your transport layer may have a password which you want remote users to be able to set or you may
