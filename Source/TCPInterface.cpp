...
void TCPInterface::CloseConnection(SystemAddress systemAddress) {
    ...
    if (systemAddress.systemIndex < remoteClientsLength
        ...
        for (int clientIndex = 0; clientIndex < remoteClientsLength; clientIndex++) {
            remoteClients[clientIndex].isActiveMutex.Lock();
            if (remoteClients[clientIndex].isActive && remoteClients[clientIndex].systemAddress == systemAddress) {
                remoteClients[systemAddress.systemIndex].SetActive(false);
                remoteClients[clientIndex].isActiveMutex.Unlock();
                break;
            }
            remoteClients[clientIndex].isActiveMutex.Unlock();
        }
...
void TCPInterface::PushBackPacket(Packet* packet, bool pushAtHead) {
    if (pushAtHead) headPush.Push(packet, _FILE_AND_LINE_);
    else tailPush.Push(packet, _FILE_AND_LINE_);
}
        ...
        while (1) {
            ...
            for (i = 0; i < (unsigned int)sts->remoteClientsLength; i++) {
                ...
                if (sts->remoteClients[i].isActive) {
                    // calling FD_ISSET with -1 as socket (that�s what 0 is set to)
                    // produces a bus error under Linux 64-Bit
                ...
                if (newSock != 0) {
                    ...
                    for (newRemoteClientIndex = 0; newRemoteClientIndex < sts->remoteClientsLength;
                        ...
                        if (sts->remoteClients[newRemoteClientIndex].isActive == false) {
                            ...
                            sts->remoteClients[newRemoteClientIndex].systemAddress.SetPortNetworkOrder(
                                sockAddr.sin_port
                            );
                            ...
                            if (sockAddr.ss_family == AF_INET) {
                                memcpy(
                                    &sts->remoteClients[newRemoteClientIndex].systemAddress.address.addr4,
                                    (sockaddr_in*)&sockAddr,
                                    sizeof(sockaddr_in)
                                );
                                ...
                                memcpy(
                                    &sts->remoteClients[newRemoteClientIndex].systemAddress.address.addr6,
                        ...
                        *lostConnectionSystemAddress               = sts->remoteClients[i].systemAddress;
                        ...
                        if (FD_ISSET(socketCopy, &readFD)) {
                            ...
                            if (len > 0) {
                                ...
                                incomingMessage->data[len] = 0; // Null terminate this so we can print it out as regular
                                                                // strings.  This is different from RakNet which does
                                                                // not do this.
                                ...
                                */
                                ...
                                incomingMessage->deleteData = true; // actually means came from SPSC, rather than
                                                                    // AllocatePacket
                        ...
                        if (FD_ISSET(socketCopy, &writeFD)) {
                            ...
                            if (bytesInBuffer > 0) {
                                ...
                                if (contiguousLength < (unsigned int)BUFF_SIZE && contiguousLength < bytesInBuffer) {
                                    if (bytesInBuffer > BUFF_SIZE) bytesAvailable = BUFF_SIZE;
                                    else bytesAvailable = bytesInBuffer;
    ...
    for (; parameterIndex < numParameters; parameterIndex++) {
        ...
        if (ALLOW_SEND_FROM_USER_THREAD && outgoingData.GetBytesWritten() == 0) {
            ...
            if (bytesSent < (int)lengths[parameterIndex]) {
                ...
                outgoingDataMutex.Lock();
                outgoingData
                    .WriteBytes(data[parameterIndex] + bytesSent, lengths[parameterIndex] - bytesSent, _FILE_AND_LINE_);
                outgoingDataMutex.Unlock();
            }
        } else {
...
bool RemoteClient::InitSSL(SSL_CTX* ctx, SSL_METHOD* meth) {
    ...
    if (res < 0) {
        ...
        // by the specifications of the TLS/SSL protocol. Call SSL_get_error() with
        ...
        case SSL_ERROR_SYSCALL: {
            ...
            if (ege == 0 && res == 0) printf("SSL_ERROR_SYSCALL EOF in violation of the protocol\n");
            else if (ege == 0 && res == -1) printf("SSL_ERROR_SYSCALL %s\n", strerror(errno));
            else printf("SSL_ERROR_SYSCALL %s\n", ERR_error_string(ege, buff));
        } break;
        case SSL_ERROR_SSL:
...
int RemoteClient::Send(const char* data, unsigned int length) {
    if (ssl) return SSL_write(ssl, data, length);
    else return send__(socket, data, length, 0);
}
int RemoteClient::Recv(char* data, const int dataSize) {
    if (ssl) return SSL_read(ssl, data, dataSize);
    else return recv__(socket, data, dataSize, 0);
}
#else
int RemoteClient::Send(const char* data, unsigned int length) {
#ifdef __native_client__
