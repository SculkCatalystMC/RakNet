...
int Connection_RM3::Replica3LSRComp(Replica3* const& replica3, LastSerializationResult* const& data) {
    ...
    // 7/28/2013 - If GetNetworkID chagned during runtime, the list would be out of order and lookup would always fail
    // or go out of bounds I remember before that I could not directly compare
    ...
    for (unsigned int index = 0; index < participantListIn.Size(); index++) {
        if (!GetConnectionByGUID(participantListIn[index], worldId)) {
            Connection_RM3* connection = AllocConnection(
                mRakPeerInterface->GetSystemAddressFromGuid(participantListIn[index]),
...
bool ReplicaManager3::PushConnection(RakNet::Connection_RM3* newConnection, WorldId worldId) {
    ...
    // Was this intended?
    RakAssert(newConnection->GetRakNetGUID() != mRakPeerInterface->GetMyGUID());
    ...
    if (index == (unsigned int)-1) {
        ...
        newConnection->SendValidation(mRakPeerInterface, worldId);
...
RakNet::Connection_RM3* ReplicaManager3::PopConnection(unsigned int index, WorldId worldId) {
    ...
    connection->ClearDownloadGroup(mRakPeerInterface);
...
unsigned int ReplicaManager3::ReferenceInternal(RakNet::Replica3* replica3, WorldId worldId) {
    ...
    if (index == (unsigned int)-1) {
        ...
        if (replica3->creatingSystemGUID == UNASSIGNED_RAKNET_GUID)
            replica3->creatingSystemGUID = mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
...
void ReplicaManager3::GetReplicasCreatedByMe(DataStructures::List<Replica3*>& replicaListOut, WorldId worldId) {
    ...
    GetReplicasCreatedByGuid(
        mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS),
        replicaListOut,
        worldId
    );
}
...
void ReplicaManager3::RM3World::Clear(ReplicaManager3* replicaManager3) {
    if (replicaManager3->GetAutoDestroyConnections()) {
        ...
        // Clear out downloadGroup even if not auto destroying the connection, since the packets need to go back to
        // RakPeer
...
PluginReceiveResult ReplicaManager3::OnReceive(Packet* packet) {
    ...
    if ((unsigned char)packet->data[0] == ID_TIMESTAMP) {
        ...
        } else return RR_STOP_PROCESSING_AND_DEALLOCATE;
    } else {
    ...
    case ID_REPLICA_MANAGER_SCOPE_CHANGE: {
        ...
        if (connection && connection->isValidated == false) {
            ...
            connection->SendValidation(mRakPeerInterface, incomingWorldId);
        }
    }
    }
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3* replicaManager3, WorldId worldId) {
    ...
    ConstructionMode _constructionMode = QueryConstructionMode();
    ...
    if (_constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION
        || _constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
        while (index < queryToConstructReplicaList.Size()) {
            ...
            if (constructionState == RM3CS_ALREADY_EXISTS_REMOTELY
                    ...
                    res!=RM3SR_SERIALIZED_UNIQUELY)
                {
                    ...
                    if (SendSerialize(replica, allIndices, sp.outputBitstream, sp.messageTimestamp, sp.pro,
                replicaManager3->GetRakPeerInterface(), replicaManager3->GetWorldID())==SSICR_SENT_DATA)
        ...
        if (_constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
    ...
    } else if (_constructionMode == QUERY_CONNECTION_FOR_REPLICA_LIST) {
    ...
    SendConstruction(
        ...
        replicaManager3->mRakPeerInterface,
        worldId,
        replicaManager3
    );
}
void ReplicaManager3::Update(void) {
    ...
    if (time - lastAutoSerializeOccurance >= autoSerializeInterval) {
        for (index3 = 0; index3 < worldsList.Size(); index3++) {
            ...
            index2 = 0;
            for (_index = 0; _index < world->connectionList.Size(); _index++) {
                connection               = world->connectionList[_index];
                sp.bitsWrittenSoFar      = 0;
                index2                   = 0;
                ...
                if (connection->QuerySerializationList(replicasToSerialize)) {
                    ...
                    while (index2 < replicasToSerialize.Size()) {
                        ...
                        ssicr =
                            connection->SendSerializeIfChanged(lsr, &sp, GetRakPeerInterface(), worldId, this, time);
...
PluginReceiveResult ReplicaManager3::OnConstruction(
    ...
    for (index = 0; index < constructionObjectListSize; index++) {
        ...
        if (actuallyCreateObject) {
            ...
            // Go past the bitStream written to with WriteAllocationID(). Necessary in case the user didn't read out the
            // bitStream the same way it was written bitOffset2 is already aligned
    ...
    for (index = 0; index < constructionObjectListSize; index++) {
        ...
        if (pdcWritten) {
            ...
            if (constructionTickStack[index] != 0) {
                ...
                else constructionTickStack[index]->PostDeserializeConstructionExisting(&bsIn, connection);
            }
            bsIn.SetReadOffset(streamEnd);
        } else {
            if (constructionTickStack[index] != 0) {
                ...
                else constructionTickStack[index]->PostDeserializeConstructionExisting(&empty, connection);
            }
        }
    }
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(
    ...
    if (connection->QueryGroupDownloadMessages() &&
        // ID_DOWNLOAD_STARTED will be processed twice, being processed the second time once ID_DOWNLOAD_COMPLETE
        // arrives. However, the second time groupConstructionAndSerialize will be set to true so it won't be processed
        // a third time
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(
    ...
    if (connection->groupConstructionAndSerialize == true && connection->downloadGroup.Size() > 0) {
        ...
        for (i = 0; i < connection->downloadGroup.Size(); i++)
            mRakPeerInterface->PushBackPacket(connection->downloadGroup[i], false);
        ...
        mRakPeerInterface->PushBackPacket(packet, false);
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(
    ...
    for (int i = 0; i < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; i++) {
        ...
        if (lsr->lastSerializationResultBS) sp->lastSentBitstream[i] = &lsr->lastSerializationResultBS->bitStream[i];
        else sp->lastSentBitstream[i] = &replica->lastSentSerialization.bitStream[i];
    }
                ...
                && (serializationResult == RM3SR_BROADCAST_IDENTICALLY_FORCE_SERIALIZATION
                    || ((
                        ...
                        || memcmp(
                               ...
                               sp->outputBitstream[z].GetNumberOfBytesUsed()
                           ) != 0
                    )))) {
...
void Connection_RM3::SendConstruction(
    ...
    // If the object was serialized identically, and does not change later on, then the new connection never gets the
    // data
    SerializeParameters sp;
    sp.whenLastSerialized = 0;
    RakNet::BitStream emptyBs;
...
Replica3::QueryConstruction_ClientConstruction(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer) {
...
RM3ConstructionState
Replica3::QueryConstruction_ServerConstruction(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer) {
    (void)destinationConnection;
...
RM3ConstructionState
Replica3::QueryConstruction_PeerToPeer(RakNet::Connection_RM3* destinationConnection, Replica3P2PMode p2pMode) {
    (void)destinationConnection;
    ...
    if (p2pMode == R3P2PM_SINGLE_OWNER) {
        ...
        // RM3CS_ALREADY_EXISTS_REMOTELY will not send the object, but WILL call QuerySerialization() and Serialize() on
        // it.
        return RM3CS_ALREADY_EXISTS_REMOTELY;
    }
}
...
RM3QuerySerializationResult
Replica3::QuerySerialization_ClientSerializable(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer) {
...
RM3QuerySerializationResult
Replica3::QuerySerialization_ServerSerializable(RakNet::Connection_RM3* destinationConnection, bool isThisTheServer) {
    (void)destinationConnection;
    // Server sends to all
...
RM3QuerySerializationResult
Replica3::QuerySerialization_PeerToPeer(RakNet::Connection_RM3* destinationConnection, Replica3P2PMode p2pMode) {
    (void)destinationConnection;
...
Replica3::QueryActionOnPopConnection_PeerToPeer(RakNet::Connection_RM3* droppedConnection) const {
    (void)droppedConnection;
    return RM3AOPC_DELETE_REPLICA;
}
