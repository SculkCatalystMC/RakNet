...
int Connection_RM3::Replica3LSRComp(Replica3* const& replica3, LastSerializationResult* const& data) {
    ...
    // 7/28/2013 - If GetNetworkID chagned during runtime, the list would be out
    // of order and lookup would always fail or go out of bounds I remember before
    // that I could not directly compare
...
RakNet::Connection_RM3* ReplicaManager3::PopConnection(unsigned int index, WorldId worldId) {
    ...
    for (index2 = 0; index2 < destructionList.Size(); index2++) {
        // Do lookup in case DeallocReplica destroyed one of of the later Replica3
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3* replicaManager3, WorldId worldId) {
    ...
    ConstructionMode queryConstructionMode = QueryConstructionMode();
    ...
    if (queryConstructionMode == QUERY_REPLICA_FOR_CONSTRUCTION
        || queryConstructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
        ...
        if (queryConstructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
    ...
    } else if (queryConstructionMode == QUERY_CONNECTION_FOR_REPLICA_LIST) {
...
void ReplicaManager3::Update(void) {
    ...
    if (time - lastAutoSerializeOccurance >= autoSerializeInterval) {
        for (index3 = 0; index3 < worldsList.Size(); index3++) {
            ...
            index2 = 0;
            for (connectionIndex = 0; connectionIndex < world->connectionList.Size(); connectionIndex++) {
                connection               = world->connectionList[connectionIndex];
                sp.bitsWrittenSoFar      = 0;
                index2                   = 0;
...
void Connection_RM3::OnLocalReference(Replica3* replica3, ReplicaManager3* replicaManager) {
    ConstructionMode queryConstructionMode = QueryConstructionMode();
    RakAssert(
        queryConstructionMode == QUERY_REPLICA_FOR_CONSTRUCTION
        || queryConstructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION
    );
    RakAssert(replica3);
    (void)replicaManager;
    (void)queryConstructionMode;
    ...
    if (constructedReplicaList.HasData(replica3) == true) {
        RakAssert(
            "replica added to queryToConstructReplicaList when already in "
...
void Connection_RM3::OnDownloadFromThisSystem(Replica3* replica3, ReplicaManager3* replicaManager) {
    ...
    if (constructedReplicaList.Insert(lsr->replica, lsr, true, _FILE_AND_LINE_) != (unsigned)-1) {
        // assert(queryToSerializeReplicaList.GetIndexOf(replica3)==(unsigned
        // int)-1);
