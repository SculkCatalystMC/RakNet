...
// FCM2Guid theirFCMGuid, BitStream *userContext )
...
void FullyConnectedMesh2::AddParticipant(RakNetGUID rakNetGuid) {
    ...
    // Need to query other system for userdata before calling
    // AddParticipantInternal But maybe I can call with no data, and piggyback on
...
/*
bool FullyConnectedMesh2::GetParticipantContext(RakNetGUID participantGuid,
BitStream *userContext)
{
        ...
        for (i=0; i < fcm2ParticipantList.Size(); i++)
        {
                ...
                {
                        if
(fcm2ParticipantList[i]->userContext.GetNumberOfBitsUsed() > 0)
                        {
...
void FullyConnectedMesh2::AssignOurFCMGuid(void) {
    ...
    randomNumber ^=
        (unsigned int)(rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g & 0xFFFFFFFF);
...
void         FullyConnectedMesh2::StartVerifiedJoin(RakNetGUID client) {
    // Assert is because there is no point calling StartVerifiedJoin() if this
    // client is already a participant
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet* packet, bool accept, BitStream* additionalData) {
    ...
    if (accept) {
        ...
        // Tell client to call AddParticipant() for
        // participatingMembersOnClientSucceeded
        ...
        // Process immediately
        // This is so if another ID_FCM2_VERIFIED_JOIN_CAPABLE is buffered, it
        // responds with ID_FCM2_VERIFIED_JOIN_START
        ...
        // Tell client rejected, otherwise process the same as
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet* packet) {
    ...
    if (curIndex != (unsigned int)-1) {
        ...
        // RakAssert(vjip->sentResults!=false); 			return
        ...
        for (unsigned short i = 0; i < listSize; i++) {
            ...
            if (j == (unsigned int)-1) {
                ...
                // 11/13/2013 - ReadVerifiedJoinInProgressMember already sets
                // joinInProgressState
    ...
    // 11/13/2013 - ReadVerifiedJoinInProgressMember may set JIPS_CONNECTED, so
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinCapable(Packet* packet) {
    ...
    // If this assert hits, AddParticipant() was called on this system, or another
    // system, which it should not have been.
