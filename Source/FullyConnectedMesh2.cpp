...
SystemAddress FullyConnectedMesh2::GetConnectedHostAddr(void) const {
    ...
    return mRakPeerInterface->GetSystemAddressFromGuid(hostRakNetGuid);
}
RakNetGUID FullyConnectedMesh2::GetHostSystem(void) const {
    if (ourFCMGuid == 0) return mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);

    return hostRakNetGuid;
}
bool FullyConnectedMesh2::IsHostSystem(void) const {
    return GetHostSystem() == mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
void FullyConnectedMesh2::GetHostOrder(DataStructures::List<RakNetGUID>& hostList) {
    ...
    if (ourFCMGuid == 0 || fcm2ParticipantList.Size() == 0) {
        hostList.Push(mRakPeerInterface->GetMyGUID(), _FILE_AND_LINE_);
        return;
    }

    FCM2Participant fcm2;
    fcm2.fcm2Guid   = ourFCMGuid;
    fcm2.rakNetGuid = mRakPeerInterface->GetMyGUID();
...
bool FullyConnectedMesh2::IsConnectedHost(void) const {
    return GetConnectedHost() == mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
...
// bool FullyConnectedMesh2::AddParticipantInternal( RakNetGUID rakNetGuid, FCM2Guid theirFCMGuid, BitStream
// *userContext )
...
void FullyConnectedMesh2::AddParticipant(RakNetGUID rakNetGuid) {
    if (mRakPeerInterface->GetConnectionState(mRakPeerInterface->GetSystemAddressFromGuid(rakNetGuid))
        != IS_CONNECTED) {
#ifdef DEBUG_FCM2
...
void FullyConnectedMesh2::OnAttach(void) {
    ...
    if (mRakPeerInterface->IsActive()) startupTime = RakNet::GetTimeUS();
}
        ...
        if (fcm2ParticipantList[idx]->rakNetGuid == rakNetGUID) {
            ...
            if (rakNetGUID == hostRakNetGuid && ourFCMGuid != 0) {
                if (fcm2ParticipantList.Size() == 0) {
                    hostRakNetGuid = mRakPeerInterface->GetMyGUID();
...
RakNet::TimeUS FullyConnectedMesh2::GetElapsedRuntime(void) {
    ...
    if (curTime > startupTime) return curTime - startupTime;
    else return 0;
}
...
void FullyConnectedMesh2::PushNewHost(const RakNetGUID& guid, RakNetGUID oldHost) {
    ...
    bs.Write(oldHost);
    p->systemAddress             = mRakPeerInterface->GetSystemAddressFromGuid(guid);
    p->systemAddress.systemIndex = (SystemIndex)-1;
    p->guid                      = guid;
    p->wasGeneratedLocally       = true;
    mRakPeerInterface->PushBackPacket(p, true);
...
void FullyConnectedMesh2::SendFCMGuidRequest(RakNetGUID rakNetGuid) {
    if (rakNetGuid == mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS)) return;
    ...
    bsOut.Write(myContext);
    myContext.ResetReadPointer();
    mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, rakNetGuid, false);
}
void FullyConnectedMesh2::SendOurFCMGuid(SystemAddress addr) {
    ...
    bsOut.Write(myContext);
    myContext.ResetReadPointer();
    mRakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, addr, false);
}
...
void FullyConnectedMesh2::AssignOurFCMGuid(void) {
    ...
    randomNumber ^=
        (unsigned int)(mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g & 0xFFFFFFFF);
...
void FullyConnectedMesh2::CalculateHost(RakNetGUID* rakNetGuid, FCM2Guid* fcm2Guid) {
    ...
    RakNetGUID associatedRakNetGuid = mRakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
    ...
    */
    ...
    if (ourFCMGuid == 0) {
        if (hasRemoteFCMGuid == false) {
            ...
            unsigned int idx;
            for (idx = 0; idx < fcm2ParticipantList.Size(); idx++)
                SendOurFCMGuid(mRakPeerInterface->GetSystemAddressFromGuid(fcm2ParticipantList[idx]->rakNetGuid));
        }
    } else {
        if (hasRemoteFCMGuid == false) {
    ...
    if (AddParticipantInternal(packet->guid, theirFCMGuid)) {
        ...
        for (idx = 0; idx < fcm2ParticipantList.Size(); idx++) {
            if (packet->guid != fcm2ParticipantList[idx]->rakNetGuid)
                mRakPeerInterface
                    ->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, fcm2ParticipantList[idx]->rakNetGuid, false);
        }
    }
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(Packet* packet) {
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        mRakPeerInterface
            ->Connect(str, remoteAddress.GetPort(), connectionPassword.C_String(), (int)connectionPassword.GetLength());
    }
}
...
void         FullyConnectedMesh2::StartVerifiedJoin(RakNetGUID client) {
    ...
    RakAssert(client != mRakPeerInterface->GetMyGUID());
    ...
    for (i = 0; i < fcm2ParticipantList.Size(); i++) {
        ...
        bsOut.Write(mRakPeerInterface->GetSystemAddressFromGuid(fcm2ParticipantList[i]->rakNetGuid));
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet* packet, bool accept, BitStream* additionalData) {
    ...
    if (participatingMembersNotOnClient.Size() > 0) {
        ...
        for (i = 0; i < participatingMembersNotOnClient.Size(); i++) {
            ...
            bsOut.Write(mRakPeerInterface->GetSystemAddressFromGuid(participatingMembersNotOnClient[i]));
...
void FullyConnectedMesh2::GetVerifiedJoinAcceptedAdditionalData(
    ...
    *thisSystemAccepted = systemToAddGuid == mRakPeerInterface->GetMyGUID();
    unsigned short listSize;
    bsIn.Read(listSize);
    bsIn.IgnoreBytes(listSize * RakNetGUID::size());
    bsIn.Read(listSize);
    if (systemToAddGuid == mRakPeerInterface->GetMyGUID()) {
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet* packet) {
    ...
    if (curIndex != (unsigned int)-1) {
        ...
        for (unsigned short i = 0; i < listSize; i++) {
            ...
            unsigned int j;
            if (vjipm.guid != UNASSIGNED_RAKNET_GUID) j = GetVerifiedJoinInProgressMemberIndex(vjipm.guid, vjip);
            else j = GetVerifiedJoinInProgressMemberIndex(vjipm.systemAddress, vjip);
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinCapable(Packet* packet) {
    ...
    // If this assert hits, AddParticipant() was called on this system, or another system, which it should not have
    // been.
    ...
    if (callCloseConnection) {
        ...
        for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
            if (vjip->vjipMembers[j].joinInProgressState != JIPS_FAILED) {
                mRakPeerInterface->CloseConnection(vjip->vjipMembers[j].guid, true);
            }
...
void FullyConnectedMesh2::OnVerifiedJoinAccepted(Packet* packet) {
    ...
    if (systemToAddGuid == mRakPeerInterface->GetMyGUID()) {
        ...
        for (unsigned short i = 0; i < listSize; i++) {
            ...
            RakNetGUID guid;
            bsIn.Read(guid);
            mRakPeerInterface->CloseConnection(guid, true);
        }
        ...
        // Another system
        ConnectionState cs = mRakPeerInterface->GetConnectionState(systemToAddGuid);
...
void FullyConnectedMesh2::ReadVerifiedJoinInProgressMember(
    ...
    ConnectionState cs = mRakPeerInterface->GetConnectionState(vjipm->guid);
    if (cs == IS_CONNECTED) vjipm->joinInProgressState = JIPS_CONNECTED;
    else if (cs == IS_DISCONNECTING || cs == IS_SILENTLY_DISCONNECTING) vjipm->joinInProgressState = JIPS_FAILED;
    else vjipm->joinInProgressState = JIPS_PROCESSING;
...
void FullyConnectedMesh2::CategorizeVJIP(
    ...
    for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) {
        ...
        if (j == (unsigned int)-1) {
            ...
            else participatingMembersOnClientSucceeded.Push(fcm2ParticipantList[i]->rakNetGuid, _FILE_AND_LINE_);
            vjip->vjipMembers[j].workingFlag = true;
        }
    }
    ...
    for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
        if (vjip->vjipMembers[j].workingFlag == false) {
            ...
            else clientMembersNotParticipatingSucceeded.Push(vjip->vjipMembers[j].guid, _FILE_AND_LINE_);
        }
    }
}
