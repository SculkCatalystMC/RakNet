...
void TwoWayAuthentication::OnClosedConnection(
    ...
    if (rakNetGUID != UNASSIGNED_RAKNET_GUID) nonceGenerator.ClearByAddress(rakNetGUID);
    else nonceGenerator.ClearByAddress(systemAddress);
}
    ...
    for (i = 0; i < outgoingChallenges.Size(); i++) {
        if (outgoingChallenges[i].remoteSystem == aog && outgoingChallenges[i].sentHash == false) {
            ...
            if (skhi.IsInvalid() == false) {
                ...
                bsOut.Write(requestId);
                bsOut.Write(
                    outgoingChallenges[i].identifier
                ); // Identifier helps the other system lookup the password quickly.
