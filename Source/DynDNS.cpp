...
void DynDNS::Update(void) {
    ...
    if (serverAddress != UNASSIGNED_SYSTEM_ADDRESS) {
        if (connectPhase == CP_CONNECTING_TO_CHECKIP) {
            ...
            tcp->Send(
                ...
                serverAddress,
                false
            ); // Needs 2 newlines! This is not documented and wasted a
               // lot of my time
        } else {
    ...
    if (packet) {
        if (connectPhase == CP_WAITING_FOR_DYNDNS_RESPONSE) {
            ...
            char* resultString;
            resultString = strstr((char*)packet->data, "Connection: close");
            if (resultString != 0) {
                resultString += strlen("Connection: close");
                while (*resultString && ((*resultString == '\r') || (*resultString == '\n') || (*resultString == ' ')))
                    resultString++;
                for (i = 0; i < 13; i++) {
                    if (strncmp(resultTable[i].code, resultString, strlen(resultTable[i].code)) == 0) {
                        if (resultTable[i].resultCode == RC_SUCCESS) {
                            // Read my external IP into myIPStr
                            // Advance until we hit a number
                            while (*resultString && ((*resultString < '0') || (*resultString > '9'))) resultString++;
                            if (*resultString) {
                                SystemAddress parser;
                                parser.FromString(resultString);
            ...
            */
            ...
            char* resultString;
            resultString = strstr((char*)packet->data, "Current IP Address: ");
            if (resultString != 0) {
                resultString += strlen("Current IP Address: ");
                SystemAddress myIp;
                myIp.FromString(resultString);
