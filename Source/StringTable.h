...
/// \file
/// \brief A simple class to encode and decode known strings based on a lookup table.  Similar to the StringCompressor
/// class.
///
...
/// StringTableType should be the smallest type possible, or else it defeats the purpose of the StringTable class, which
...
/// must match on all systems - hence you must add all the strings in the same order on all systems.<BR> Furthermore,
/// this must be done before sending packets that use this class, since the strings are ordered for fast lookup.  Adding
...
public:
    ...
    /// \param[in] copyString true to make a copy of the passed string (takes more memory), false to not do so (if your
    ...
    /// Relies on the StringCompressor class, which is automatically reference counted in the constructor and destructor
    /// in RakPeer.  You can call the reference counting functions yourself if you wish too.
