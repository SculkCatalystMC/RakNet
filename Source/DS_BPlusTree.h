...
bool BPlusTree<KeyType, DataType, order>::Delete(const KeyType key, DataType& out) {
    ...
    if (root == leftmostLeaf) {
        ...
        return true;
    } else if (FindDeleteRebalance(key, root, &underflow, root->keys[0], &returnAction, out) == false) return false;
...
bool BPlusTree<KeyType, DataType, order>::FindDeleteRebalance(
    ...
    if (GetIndexOf(key, cur, &childIndex)) branchIndex = childIndex + 1;
    else branchIndex = childIndex;
    ...
    if (cur->children[branchIndex]->isLeaf == false) {
        ...
        if (cur->children[branchIndex]->size < order / 2) *underflow = true;
...
Page<KeyType, DataType, order>* BPlusTree<KeyType, DataType, order>::InsertIntoNode(
    ...
    if (cur->size < order) {
        ...
        if (cur->isLeaf) cur->data[insertionIndex] = leafData;
        else cur->children[insertionIndex + 1] = nodeData;
...
#pragma warning(disable : 4127) // warning C4127: conditional expression is constant
    ...
    while (1) {
        if (key == page->keys[index]) {
            *out = index;
            return true;
        } else if (key < page->keys[index]) upperBound = index - 1;
