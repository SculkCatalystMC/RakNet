...
void StatisticsHistoryPlugin::Update(void) {
    ...
    mRakPeerInterface->GetStatisticsList(addresses, guids, stats);
    ...
    for (unsigned int idx = 0; idx < guids.Size(); idx++) {
        ...
        if (objectIndex != (unsigned int)-1) {
            ...
            statistics.AddValueByIndex(
                objectIndex,
                "RN_lastPing",
                (SHValueType)mRakPeerInterface->GetLastPing(guids[idx]),
                curTime,
                false
            );
...
/*
void StatisticsHistoryPlugin::OnDirectSocketSend(const char *data, const BitSize_t bitsUsed, SystemAddress
remoteSystemAddress)
{
    // Would have to use GetGuidFromSystemAddress for every send
}
void StatisticsHistoryPlugin::OnDirectSocketReceive(const char *data, const BitSize_t bitsUsed, SystemAddress
remoteSystemAddress)
{
}
*/
