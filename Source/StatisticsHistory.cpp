...
bool StatisticsHistory::GetHistorySorted(
    ...
    for (unsigned int i = 0; i < itemList.Size(); i++) {
        ...
        else tavq->sortValue = tavq->GetLongTermLowest();
        ...
        else sortedQueues.Insert(tavq, tavq, false, _FILE_AND_LINE_, TimeAndValueQueueCompDesc);
    }
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetRecentAverage(void) const {
    if (values.Size() > 0) return recentSum / (SHValueType)values.Size();
    else return 0;
}
...
void StatisticsHistory::TimeAndValueQueue::MergeSets(
    ...
    // 1A. Sums are just added. If two values have the same time, just put in
    // queue twice 1B. longTermLowest and longTermHighest are the lowest and
    // highest of the two sets
    //
    // 2. Add by time. If time for the other set is missing, calculate slope to
    // extrapolate 2A. Have to recalculate recentSum, recentSumOfSquares. 2B.
    // longTermSum, longTermCount, longTermLowest, longTermHighest are unknown
    ...
    if (lhsDataCategory == StatisticsHistory::DC_DISCRETE && rhsDataCategory == StatisticsHistory::DC_DISCRETE) {
        ...
        if (lhs->longTermLowest < rhs->longTermLowest) output->longTermLowest = lhs->longTermLowest;
        else output->longTermLowest = rhs->longTermLowest;
        if (lhs->longTermHighest > rhs->longTermHighest) output->longTermHighest = lhs->longTermHighest;
        else output->longTermHighest = rhs->longTermHighest;
    } else {
        ...
        if (histogram.Peek().time > timeClipEnd) {
            ...
            do {
                ...
                if (histogram.PeekTail().time == timeClipEnd) {
                    ...
                    StatisticsHistory::TimeAndValue tav2;
                    tav2.val =
                        StatisticsHistory::TimeAndValueQueue::Interpolate(tav, histogram.PeekTail(), timeClipEnd);
...
SHValueType StatisticsHistory::TimeAndValueQueue::Interpolate(
    ...
    // 	else
    // 	{
    // 		SHValueType slope = (t1.val - t2.val) / (SHValueType) (t1.time -
    // t2.time); 		return t2.val + slope * (SHValueType) (time - t2.time);
    // 	}
}
...
void StatisticsHistoryPlugin::Update(void) {
    ...
    for (unsigned int idx = 0; idx < guids.Size(); idx++)
    {
            ...
            bool AddValue(uint64_t objectId, RakString key, SHValueType val, Time
    curTime);
...
/*
void StatisticsHistoryPlugin::OnDirectSocketSend(const char *data, const
BitSize_t bitsUsed, SystemAddress remoteSystemAddress)
{
        // Would have to use GetGuidFromSystemAddress for every send
}
void StatisticsHistoryPlugin::OnDirectSocketReceive(const char *data, const
BitSize_t bitsUsed, SystemAddress remoteSystemAddress)
{
}
*/
