...
void ByteQueue::WriteBytes(const char* in, unsigned length, const char* file, unsigned int line) {
    ...
    if (length <= lengthAllocated - writeOffset) memcpy(data + writeOffset, in, length);
    else {
        // Wrap
...
char* ByteQueue::PeekContiguousBytes(unsigned int* outLength) const {
    if (writeOffset >= readOffset) *outLength = writeOffset - readOffset;
    else *outLength = lengthAllocated - readOffset;
...
unsigned ByteQueue::GetBytesWritten(void) const {
    ...
    else return writeOffset + (lengthAllocated - readOffset);
}
...
void ByteQueue::DecrementReadOffset(unsigned length) {
    if (length > readOffset) readOffset = lengthAllocated - (length - readOffset);
    else readOffset -= length;
}
