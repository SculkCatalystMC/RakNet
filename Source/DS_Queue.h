...
void Queue<queue_type>::PushAtHead(const queue_type& input, unsigned index, const char* file, unsigned int line) {
    ...
    while (readIndex >= index) {
        if (head + writeIndex >= allocation_size) trueWriteIndex = head + writeIndex - allocation_size;
        else trueWriteIndex = head + writeIndex;

        if (head + readIndex >= allocation_size) trueReadIndex = head + readIndex - allocation_size;
        else trueReadIndex = head + readIndex;
    ...
    if (head + index >= allocation_size) trueWriteIndex = head + index - allocation_size;
    else trueWriteIndex = head + index;
...
inline queue_type Queue<queue_type>::PeekTail(void) const {
    ...
    RakAssert(head != tail);
#endif
    if (tail != 0) return (queue_type)array[tail - 1];
    else return (queue_type)array[allocation_size - 1];
}
...
void Queue<queue_type>::ClearAndForceAllocation(int size, const char* file, unsigned int line) {
    RakNet::OP_DELETE_ARRAY(array, file, line);
    if (size > 0) array = RakNet::OP_NEW_ARRAY<queue_type>(size, file, line);
    else array = 0;
...
inline queue_type& Queue<queue_type>::operator[](unsigned int position) const {
    ...
    if (head + position >= allocation_size) return array[head + position - allocation_size];
    else return array[head + position];
}
...
void Queue<queue_type>::RemoveAtIndex(unsigned int position) {
    ...
    if (head + position >= allocation_size) index = head + position - allocation_size;
    else index = head + position;
    ...
    // Move the tail back
    if (tail == 0) tail = allocation_size - 1;
    else --tail;
}
} // namespace DataStructures
