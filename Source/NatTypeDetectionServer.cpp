...
void NatTypeDetectionServer::Update(void) {
    ...
    bufferedPacketsMutex.Lock();
    if (bufferedPackets.Size() > 0) recvStruct = bufferedPackets.Pop();
    else recvStruct = 0;
    bufferedPacketsMutex.Unlock();
    while (recvStruct) {
        ...
        if (data[0] == NAT_TYPE_PORT_RESTRICTED && recvStruct->socket == s3p4) {
            ...
            if (readSuccess) {
                ...
                if (j != (unsigned int)-1) {
                    ...
                    mRakPeerInterface
                        ->Send(&bs, HIGH_PRIORITY, RELIABLE, 0, natDetectionAttempts[j].systemAddress, false);

                    // Done
                    natDetectionAttempts.RemoveAtIndexFast(j);
                } else {
                    //		RakAssert("i==0 in Update when looking up GUID in NatTypeDetectionServer.cpp. Either a bug
                    // or a late resend" && 0);
                }
            } else {
    ...
    // Only socket that receives messages is s3p4, to see if the external address is different than that of the
    connection to rakPeerInterface char data[ MAXIMUM_MTU_SIZE ]; int len; SystemAddress senderAddr;
    ...
    while (len>0 && data[0]==NAT_TYPE_PORT_RESTRICTED)
    {
        ...
        if (readSuccess)
        {
            ...
            {
        //		RakAssert("i==0 in Update when looking up GUID in NatTypeDetectionServer.cpp. Either a bug or a late
    resend" && 0);
            }
        }
        else
    ...
    while (i < (int)natDetectionAttempts.Size()) {
        if (time > natDetectionAttempts[i].nextStateTime) {
            ...
            case STATE_TESTING_FULL_CONE_2:
                ...
                // S2P3 sends to C1 (Different address, different port, to previously used port on client). If received,
                // Full-cone nat. Done.  (Else S2P3 potentially banned, do not use again).
                saOut = natDetectionAttempts[i].systemAddress;
                saOut.SetPortHostOrder(natDetectionAttempts[i].systemAddress.GetPort());
                // SocketLayer::SendTo_PC( s2p3, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), saOut, __FILE__,
                // __LINE__  );
            ...
            case STATE_TESTING_ADDRESS_RESTRICTED_2:
                ...
                // S1P2 sends to C1 (Same address, different port, to previously used port on client). If received,
                // address-restricted cone nat. Done.
                saOut = natDetectionAttempts[i].systemAddress;
                saOut.SetPortHostOrder(natDetectionAttempts[i].systemAddress.GetPort());
                // SocketLayer::SendTo_PC( s1p2, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), saOut, __FILE__,
                // __LINE__  );
                bsp.data          = (char*)bs.GetData();
                bsp.length        = bs.GetNumberOfBytesUsed();
                bsp.systemAddress = saOut;
            ...
            case STATE_TESTING_PORT_RESTRICTED_2:
                // C1 sends to S3P4. If address of C1 as seen by S3P4 is the same as the address of C1 as seen by S1P1,
                // then port-restricted cone nat. Done
                ...
                mRakPeerInterface->Send(&bs, HIGH_PRIORITY, RELIABLE, 0, natDetectionAttempts[i].systemAddress, false);
                break;
            default:

#ifdef NTDS_VERBOSE
                printf(
                    "Warning, exceeded final check STATE_TESTING_PORT_RESTRICTED_2.\nExpected that client would have "
                    "sent NAT_TYPE_PORT_RESTRICTED on s3p4.\nDefaulting to Symmetric\n"
                );
#endif
                bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_RESULT);
                bs.Write((unsigned char)NAT_TYPE_SYMMETRIC);
...
void NatTypeDetectionServer::OnDetectionRequest(Packet* packet) {
    ...
    if (isRequest) {
        ...
        nda.nextStateTime       = 0;
        nda.timeBetweenAttempts = mRakPeerInterface->GetLastPing(nda.systemAddress) * 3 + 50;
