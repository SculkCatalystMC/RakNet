...
void NatTypeDetectionServer::Update(void) {
    ...
    while (recvStruct) {
        ...
        if (data[0] == NAT_TYPE_PORT_RESTRICTED && recvStruct->socket == s3p4) {
            ...
            if (readSuccess) {
                unsigned int detectionAttemptIndex = GetDetectionAttemptIndex(senderGuid);
                if (detectionAttemptIndex != (unsigned int)-1) {
                    bs.Reset();
                    bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_RESULT);
                    // If different, then symmetric
                    if (senderAddr != natDetectionAttempts[detectionAttemptIndex].systemAddress) {
                    ...
                    rakPeerInterface->Send(
                        ...
                        RELIABLE,
                        0,
                        natDetectionAttempts[detectionAttemptIndex].systemAddress,
                        false
                    );

                    // Done
                    natDetectionAttempts.RemoveAtIndexFast(detectionAttemptIndex);
                } else {
    ...
    is different than that of the connection to rakPeerInterface char data[
    MAXIMUM_MTU_SIZE ]; int len; SystemAddress senderAddr;
    len=NatTypeRecvFrom(data, s3p4, senderAddr);
    // Client is asking us if this is port restricted. Only client requests of
    this type come in on s3p4 while (len>0 && data[0]==NAT_TYPE_PORT_RESTRICTED)
    {
    ...
    ID_NAT_TYPE_DETECTION_RESULT);
                            ...
                            else
                            {
                                    ...
                                    #ifdef NTDS_VERBOSE
                                    printf("Determined client is port
    restricted\n"); #endif bs.Write((unsigned char) NAT_TYPE_PORT_RESTRICTED);
                            }
