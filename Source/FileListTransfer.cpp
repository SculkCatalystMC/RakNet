...
FileListTransfer::SetupReceive(FileListTransferCBInterface* handler, bool deleteHandler, SystemAddress allowedSender) {
    if (mRakPeerInterface && mRakPeerInterface->GetConnectionState(allowedSender) != IS_CONNECTED)
        return (unsigned short)-1;
    FileListReceiver* receiver;

    if (fileListReceivers.Has(mSetId)) {
        receiver = fileListReceivers.Get(mSetId);
        ...
        fileListReceivers.Delete(mSetId);
    }
    ...
    receiver->setID                 = mSetId;
    fileListReceivers.Set(mSetId, receiver);
    oldId = mSetId;
    if (++mSetId == (unsigned short)-1) mSetId = 0;
    return oldId;
}
...
void FileListTransfer::Send(
    ...
    // Write the chunk header, which contains the frequency table, the total number of files, and the total number of
    // bytes
    ...
    if (anythingToWrite) {
        ...
        if (filesToPush.IsEmpty() == false) {
            ...
            for (unsigned int j = 0; j < fileToPushRecipientList.Size(); j++) {
                if (fileToPushRecipientList[j]->systemAddress == recipient
                    && fileToPushRecipientList[j]->setId == mSetId) {
                    // 					ftpr=fileToPushRecipientList[j];
                    // 					ftpr->AddRef();
                    // 					break;
...
bool FileListTransfer::DecodeFile(Packet* packet, bool isTheFullFile) {
    ...
    if (isTheFullFile) {
        ...
        // Remote system is sending a complete file, but the file is large enough that we get ID_PROGRESS_NOTIFICATION
...
void FileListTransfer::Clear(void) {
    ...
    fileToPushRecipientListMutex.Lock();
    for (unsigned int j = 0; j < fileToPushRecipientList.Size(); j++) {
        FileToPushRecipient* ftpr = fileToPushRecipientList[j];
...
void FileListTransfer::OnReferencePush(Packet* packet, bool isTheFullFile) {
    ...
    if (isTheFullFile == false) {
        ...
        // 2/16/2012 I care because a reference push is 16 megabytes by default. Also, if it is the last file "if
        // (ftpr->filesToPush.Size()<2)" or total file size exceeds smallFileTotalSize it always sends a reference push.
        //		return;
    }
    ...
    if (isTheFullFile) {
        if (mb.flrMemoryBlock) {
            // Either the very first block, or a subsequent block and allocateIrIDataChunkAutomatically was true for the
            // first block
            memcpy(
            ...
            // This means the first block explicitly deallocated the memory, and no blocks will be permanently held by
            // RakNet
    ...
    for (ftpIndex = 0; ftpIndex < fileListTransfer->fileToPushRecipientList.Size(); ftpIndex++) {
        ...
        if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
            ...
            while (done && ftp->currentOffset == 0 && smallFileTotalSize < ftp->chunkSize) {
                ////ftpr->filesToPushMutex.Lock();
                // The reason for 2 is that ID_FILE_LIST_REFERENCE_PUSH gets ID_FILE_LIST_REFERENCE_PUSH_ACK. WIthout
                // ID_FILE_LIST_REFERENCE_PUSH_ACK, SendIRIToAddressCB would not be called again
                ...
                // Send all small files at once, rather than wait for ID_FILE_LIST_REFERENCE_PUSH. But at least one
