...
unsigned short
FileListTransfer::SetupReceive(FileListTransferCBInterface* handler, bool deleteHandler, SystemAddress allowedSender) {
...
void FileListTransfer::Send(
    ...
    if (anythingToWrite) {
        ...
        if (rakPeer) rakPeer->Send(&outBitstream, priority, RELIABLE_ORDERED, orderingChannel, recipient, false);
        else SendUnified(&outBitstream, priority, RELIABLE_ORDERED, orderingChannel, recipient, false);
        ...
        if (filesToPush.IsEmpty() == false) {
            ...
            for (unsigned int recipientIndex = 0; recipientIndex < fileToPushRecipientList.Size(); recipientIndex++) {
                if (fileToPushRecipientList[recipientIndex]->systemAddress == recipient
                    && fileToPushRecipientList[recipientIndex]->setId == setID) {
...
void FileListTransfer::Clear(void) {
    ...
    fileToPushRecipientListMutex.Lock();
    for (unsigned int recipientIndex = 0; recipientIndex < fileToPushRecipientList.Size(); recipientIndex++) {
        FileToPushRecipient* ftpr = fileToPushRecipientList[recipientIndex];
...
void FileListTransfer::CancelReceive(unsigned short receiverSetId) {
    if (fileListReceivers.Has(receiverSetId) == false) {
#ifdef _DEBUG
        RakAssert(0);
#endif
        return;
    }
    FileListReceiver* fileListReceiver = fileListReceivers.Get(receiverSetId);
    fileListReceiver->downloadHandler->OnDereference();
    if (fileListReceiver->deleteDownloadHandler) RakNet::OP_DELETE(fileListReceiver->downloadHandler, _FILE_AND_LINE_);
    RakNet::OP_DELETE(fileListReceiver, _FILE_AND_LINE_);
    fileListReceivers.Delete(receiverSetId);
}
...
bool FileListTransfer::IsHandlerActive(unsigned short receiverSetId) { return fileListReceivers.Has(receiverSetId); }
...
void FileListTransfer::OnReferencePush(Packet* packet, bool isTheFullFile) {
    ...
    if (isTheFullFile == false) {
        ...
        // 2/16/2012 I care because a reference push is 16 megabytes by default.
        // Also, if it is the last file "if (ftpr->filesToPush.Size()<2)" or total
        // file size exceeds smallFileTotalSize it always sends a reference push.
        //		return;
    }
    ...
    if (isTheFullFile) amountToRead = chunkLength;
    else amountToRead = unreadBytes;
    ...
    if (chunkLength == 0 || chunkLength == onFileStruct.byteLengthOfThisFile) totalNotifications = 1;
    else totalNotifications = onFileStruct.byteLengthOfThisFile / chunkLength + 1;

    if (chunkLength == 0) currentNotificationIndex = 0;
    else currentNotificationIndex = offset / chunkLength;
    ...
    if (finished) {
        ...
        if (isTheFullFile) {
            ...
            else currentNotificationIndex = (offset + chunkLength) / fileListReceiver->partLength;
            unreadBytes =
                onFileStruct.byteLengthOfThisFile - ((currentNotificationIndex + 1) * fileListReceiver->partLength);
            fps.partCount = currentNotificationIndex;
            fps.partTotal = totalNotifications;

            // 2/19/2013 Why was this check here? It prevent smaller progress
            // notifications
            //			if (rakPeerInterface)
            {
...
2. Send ID_FILE_LIST_TRANSFER_FILE for each small file in the queue of ifles to
be sent
...
Recipient:
send ID_FILE_LIST_REFERENCE_PUSH_ACK sent from OnReferencePush() when 2nd
parameter is true.

File sender:
Got ID_FILE_LIST_REFERENCE_PUSH_ACK. Calls OnReferencePushAck, calls
SendIRIToAddress, calls SendIRIToAddressCB
*/
    ...
    for (ftpIndex = 0; ftpIndex < fileListTransfer->fileToPushRecipientList.Size(); ftpIndex++) {
        ...
        if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
            ...
            while (done && ftp->currentOffset == 0 && smallFileTotalSize < ftp->chunkSize) {
                ...
                // ID_FILE_LIST_REFERENCE_PUSH_ACK. WIthout
                // ID_FILE_LIST_REFERENCE_PUSH_ACK, SendIRIToAddressCB would not be
                // called again
            ...
            if (done) {
                ...
                if (ftpr->filesToPush.Size() == 0) {
                    ...
                    for (unsigned int flpcIndex = 0; flpcIndex < fileListTransfer->fileListProgressCallbacks.Size();
                         flpcIndex++)
                        fileListTransfer->fileListProgressCallbacks[flpcIndex]->OnFilePushesComplete(
                            systemAddress,
                            setId
                        );
            ...
            // See
            // http://www.jenkinssoftware.com/forum/index.php?topic=4768.msg19738#msg19738
            fileListTransfer->SendListUnified(
                dataBlocks,
                lengths,
                2,
...
void FileListTransfer::SendIRIToAddress(SystemAddress systemAddress, unsigned short recipientSetId) {
    ThreadData threadData;
    threadData.fileListTransfer = this;
    threadData.systemAddress    = systemAddress;
    threadData.setId            = recipientSetId;
...
void FileListTransfer::OnReferencePushAck(Packet* packet) {
    RakNet::BitStream inBitStream(packet->data, packet->length, false);
    inBitStream.IgnoreBits(8);
    unsigned short ackSetId;
    inBitStream.Read(ackSetId);
    SendIRIToAddress(packet->systemAddress, ackSetId);
}
