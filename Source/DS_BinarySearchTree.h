...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::BalanceTree(
    ...
    while (current) {
        ...
        if (right_height - left_height == 2) {
            if (RightHigher(current->right)) RotateLeft(current->right);
            else DoubleRotateLeft(current);

            if (rotateOnce) break;
        }

        else if (right_height - left_height == -2) {
            if (LeftHigher(current->left)) RotateRight(current->left);
            else DoubleRotateRight(current);
...
unsigned int BinarySearchTree<BinarySearchTreeType>::Height(typename BinarySearchTree::node* starting_node) {
    if (BinarySearchTree_size == 0 || starting_node == 0) return 0;
    else return HeightRecursive(starting_node);
}
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node*
BinarySearchTree<BinarySearchTreeType>::Del(const BinarySearchTreeType& input, const char* file, unsigned int line) {
                      ...
                      == 0) // Node has only one child, delete it and cause the parent to point to that child
    {
        ...
        if (parent) {
            if (direction == RIGHT) parent->right = current->right;
                      ...
                      != 0) // Node has only one child, delete it and cause the parent to point to that child
    {
        ...
        if (parent) {
            if (direction == RIGHT) parent->right = current->left;
    ...
    } else // Go right, then as left as far as you can
    {
        ...
        *(node_to_delete->item) = *(current->item);
        ...
        if (current->right == 0) {
            if (direction == RIGHT) parent->right = 0;
            else parent->left = 0;
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node*
BinarySearchTree<BinarySearchTreeType>::Add(const BinarySearchTreeType& input, const char* file, unsigned int line) {
...
void BinarySearchTree<BinarySearchTreeType>::DisplayInorder(BinarySearchTreeType* return_array) {
    ...
    if (BinarySearchTree_size == 0) return; // Do nothing for an empty tree
...
BinarySearchTree<BinarySearchTreeType>::operator=(const BinarySearchTree& original_copy) {
    typename BinarySearchTree::node* current;
