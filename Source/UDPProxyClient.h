...
/// \file
/// \brief A RakNet plugin performing networking to communicate with UDPProxyCoordinator. Ultimately used to tell
/// UDPProxyServer to forward UDP packets.
...
struct UDPProxyClientResultHandler {
    ...
    /// Called when our forwarding request was completed. We can now connect to \a targetAddress by using \a
    /// proxyAddress instead
    ...
    /// \param[out] proxyCoordinator \a proxyCoordinator parameter originally passed to
    /// UDPProxyClient::RequestForwarding
    /// \param[out] sourceAddress \a sourceAddress parameter passed to UDPProxyClient::RequestForwarding. If it was
    /// UNASSIGNED_SYSTEM_ADDRESS, it is now our external IP address.
    ...
    /// \param[out] proxyCoordinator \a proxyCoordinator parameter originally passed to
    /// UDPProxyClient::RequestForwarding
    /// \param[out] sourceAddress \a sourceAddress parameter passed to UDPProxyClient::RequestForwarding. This is
    /// originating source IP address of the remote system that will be sending to us.
    /// \param[out] targetAddress \a targetAddress parameter originally passed to UDPProxyClient::RequestForwarding.
    ...
    /// Called when our forwarding request failed, because all UDPProxyServers that are connected to UDPProxyCoordinator
    /// are at their capacity Either add more servers, or increase capacity via UDPForwarder::SetMaxForwardEntries()
    /// \param[out] proxyCoordinator \a proxyCoordinator parameter originally passed to
    /// UDPProxyClient::RequestForwarding
    /// \param[out] sourceAddress \a sourceAddress parameter passed to UDPProxyClient::RequestForwarding. If it was
    /// UNASSIGNED_SYSTEM_ADDRESS, it is now our external IP address.
...
/// \details When NAT Punchthrough fails, it is possible to use a non-NAT system to forward messages from us to the
/// recipient, and vice-versa.<BR> The class to forward messages is UDPForwarder, and it is triggered over the network
/// via the UDPProxyServer plugin.<BR> The UDPProxyClient connects to UDPProxyCoordinator to get a list of servers
/// running UDPProxyServer, and the coordinator will relay our forwarding request
...
public:
    ...
    /// Sends a request to proxyCoordinator to find a server and have that server setup UDPForwarder::StartForwarding()
    /// on our address to \a targetAddressAsSeenFromCoordinator The forwarded datagrams can be from any UDP source, not
    /// just RakNet
    /// \pre Must be connected to \a proxyCoordinator
    /// \pre Systems running UDPProxyServer must be connected to \a proxyCoordinator and logged in via
    /// UDPProxyCoordinator::LoginServer() or UDPProxyServer::LoginToCoordinator()
    /// \note May still fail, if all proxy servers have no open connections.
    /// \note RakNet's protocol will ensure a message is sent at least every 5 seconds, so if routing RakNet messages,
    /// it is a reasonable value for timeoutOnNoDataMS, plus an extra few seconds for latency.
    /// \param[in] proxyCoordinator System we are connected to that is running the UDPProxyCoordinator plugin
    /// \param[in] sourceAddress External IP address of the system we want to forward messages from. This does not have
    /// to be our own system. To specify our own system, you can pass UNASSIGNED_SYSTEM_ADDRESS which the coordinator
    /// will treat as our external IP address.
    /// \param[in] targetAddressAsSeenFromCoordinator External IP address of the system we want to forward messages to.
    /// If this system is connected to UDPProxyCoordinator at this address using RakNet, that system will ping the
    /// server and thus open the router for incoming communication. In any other case, you are responsible for doing
    /// your own network communication to have that system ping the server. See also targetGuid in the other version of
    /// RequestForwarding(), to avoid the need to know the IP address to the coordinator of the destination.
    /// \param[in] timeoutOnNoData If no data is sent by the forwarded systems, how long before removing the forward
    /// entry from UDPForwarder? UDP_FORWARDER_MAXIMUM_TIMEOUT is the maximum value. Recommended 10 seconds.
    /// \param[in] serverSelectionBitstream If you want to send data to UDPProxyCoordinator::GetBestServer(), write it
    /// here
    ...
    /// If requesting forwarding to a RakNet enabled system, then it is easier to use targetGuid instead of
