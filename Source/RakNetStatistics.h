...
enum RNSPerSecondMetrics {
    ...
    /// How many user message bytes were sent via a call to RakPeerInterface::Send(). This is less than or equal to
    /// USER_MESSAGE_BYTES_PUSHED. A message would be pushed, but not yet sent, due to congestion control
    USER_MESSAGE_BYTES_SENT,

    /// How many user message bytes were resent. A message is resent if it is marked as reliable, and either the message
    /// didn't arrive or the message ack didn't arrive.
...
struct RAKNET_API RakNetStatistics {
    ...
    /// How many messages are waiting in the resend buffer? This includes messages waiting for an ack, so should
    /// normally be a small value If the value is rising over time, you are exceeding the bandwidth capacity. See
