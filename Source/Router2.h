...
/// \file
/// \brief Router2 plugin. Allows you to connect to a system by routing packets through another system that is connected
/// to both you and the destination. Useful for getting around NATs.
///
...
/// \brief Part of the NAT punchthrough solution, allowing you to connect to systems by routing through a shared
/// connection.
/// \details Router2 routes datagrams between two systems that are not directly connected by using the bandwidth of a
/// third system, to which the other two systems were connected It is of benefit when a fully connected mesh topology is
/// desired, but could not be completely established due to routers and/or firewalls As the system address of a remote
/// system will be the system address of the intermediary, it is necessary to use the RakNetGUID object to refer to
/// systems, including with other plugins
...
public:
    ...
    ///
    /// \note The SystemAddress for a connection should not be used - always use RakNetGuid as the address can change at
    /// any time. When the address changes, you will get ID_ROUTER_2_REROUTED
