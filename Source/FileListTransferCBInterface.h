...
public:
    ...
    struct OnFileStruct {
        ...
        /// \details However, on error, this is instead changed to one of the enumerations in the PatchContext
        /// structure.
    ...
    struct FileProgressStruct {
        /// \param[out] onFileStruct General information about this file, such as the filename and the first \a
        /// partLength bytes. You do NOT need to save this data yourself. The complete file will arrive normally.
        OnFileStruct* onFileStruct;
        /// \param[out] partCount The zero based index into partTotal. The percentage complete done of this file is 100
        /// * (partCount+1)/partTotal
        unsigned int partCount;
        /// \param[out] partTotal The total number of parts this file was split into. Each part will be roughly the MTU
        ...
        /// \param[out] firstDataChunk The first \a partLength of the final file. If you store identifying information
        /// about the file in the first \a partLength bytes, you can read them while the download is taking place. If
        /// this hasn't arrived yet, firstDataChunk will be 0
        char* firstDataChunk;
        /// \param[out] iriDataChunk If the remote system is sending this file using IncrementalReadInterface, then this
        /// is the chunk we just downloaded. It will not exist in memory after this callback. You should either store
        /// this to disk, or in memory. If it is 0, then the file is smaller than one chunk, and will be held in memory
        /// automatically
        char* iriDataChunk;
        /// \param[out] iriWriteOffset Offset in bytes from the start of the file for the data pointed to by
        /// iriDataChunk
        ...
        RakNetGUID senderGuid;
        /// \param[in] allocateIrIDataChunkAutomatically If true, then RakNet will hold iriDataChunk for you and return
        /// it in OnFile. Defaults to true
    ...
    /// ID_PROGRESS_NOTIFICATION, or you got a block via IncrementalReadInterface If the transport layer is returning
    /// ID_PROGRESS_NOTIFICATION (see RakPeer::SetSplitMessageProgressInterval()) then FileProgressStruct::iriDataChunk
    /// will be 0. If this is a block via IncrementalReadInterface, then iriDataChunk will point to the block just
    /// downloaded. If not using IncrementalReadInterface, then you only care about partCount and partTotal to tell how
    /// far the download has progressed. YOu can use firstDataChunk to read the first part of the file if desired. The
    /// file is usable when you get the OnFile callback. If using IncrementalReadInterface and you let RakNet buffer the
    /// files in memory (default), then it is the same as above. The file is usable when you get the OnFile callback. If
    /// using IncrementalReadInterface and you do not let RakNet buffer the files in memory, then set
    /// allocateIrIDataChunkAutomatically to false. Write the file to disk whenever you get OnFileProgress and
    ...
    /// At that point OnDereference will be called and the class will no longer be maintained by the FileListTransfer
    /// plugin.
    ...
    /// At that point OnDereference will be called and the class will no longer be maintained by the FileListTransfer
    /// plugin. Otherwise return true, and Update will continue to be called.
