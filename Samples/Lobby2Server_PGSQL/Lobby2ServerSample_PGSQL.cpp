...
void main(void) {
    ...
    for (int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) ipList[i].ToString(false, ipListStr[i]);
    ...
    // Normally the proxy server is on a different computer. Here, we login to our
    // own IP address since the plugin is on the same system
    ...
    while (1) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_NEW_INCOMING_CONNECTION:
                ...
                printf(
                    "Allowing all operations from remote client for testing "
                    "(insecure)\n"
                );
