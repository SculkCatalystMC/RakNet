...
FMODVoiceAdapter* FMODVoiceAdapter::Instance() { return &instance; }
...
bool FMODVoiceAdapter::SetupAdapter(FMOD::System* fmodSystem, RakVoice* rakVoice) {
    ...
    if (fmodErr != FMOD_OK) return false;
...
void FMODVoiceAdapter::SetMute(bool mute) { this->mute = mute; }
...
void FMODVoiceAdapter::UpdateSound(bool isRec) {
    ...
    if (((!isRec) || (isRec && !mute)) && (currPos != lastPos)) {
        ...
        //  Since the length and current position are both a multiple of
        //  bufferSizeBytes
