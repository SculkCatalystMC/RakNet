...
int                           main(void) {
    ...
    for (i = 0; i < NUM_PEERS; i++) rakPeer[i] = RakNet::RakPeerInterface::GetInstance();

    printf(
        "This project tests and demonstrates the two way authentication "
                                  "plugin.\n"
    );
            ...
            if (packet) {
                ...
                case ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_SUCCESS: {
                    ...
                    if (packet->data[0] == ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS)
                        printf(
                            "ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS with "
                                                      "%s from %s\n",
                            password.C_String(),
                        ...
                        printf(
                            "ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_SUCCESS with "
                                                      "%s from %s\n",
                            password.C_String(),
                            packet->systemAddress.ToString(true)
                        );
                    if (++passCount == 2) {
                        if (stage <= 2) {
                            if (stage == 0) {
                                printf(
                                    "Stage 1, instance 1 challenges instance 0 (should "
                                                              "pass)\n"
                                );
                                ...
                                printf(
                                    "Stage 2, instance 1 re-challenges instance 0 (should "
                                                              "pass)\n"
                                );
                                twoWayAuthenticationPlugin[1]->Challenge("PWD0", rakPeer[1]->GetGUIDFromIndex(0));
                                passCount = 0;
                                ...
                                printf(
                                    "Stage 3, instance 0 challenges with bad password (call "
                                                              "should fail)\n"
                                );
                                if (twoWayAuthenticationPlugin[0]->Challenge("PWD3", rakPeer[0]->GetGUIDFromIndex(0))
                                    == false) {
                                    printf("Passed stage 3\n");
                                    stage++;
                                    printf(
                                        "Stage 4, instance 0 challenges with unknown password "
                                                                  "(should fail twice)\n"
                                    );
                ...
                case ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILURE: {
                    printf(
                        "ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILED from %s\n",
                        packet->systemAddress.ToString(true)
                    );
                    if (stage != 4) {
                        printf("Failed stage %i\n", stage);
                    } else {
                        if (++stage4FailureCount == 2) quit = true;
                    }
                } break;
                case ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT: {
                    ...
                    bs.Read(password);
                    printf(
                        "ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT with %s "
                                                  "from %s\n",
                        password.C_String(),
                        packet->systemAddress.ToString(true)
                    );
                ...
                case ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE: {
                    ...
                    bs.Read(password);
                    printf(
                        "ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILED with %s "
                                                  "from %s\n",
                        password.C_String(),
                        packet->systemAddress.ToString(true)
                    );
    ...
    for (i = 0; i < NUM_PEERS; i++) RakNet::RakPeerInterface::DestroyInstance(rakPeer[i]);
