    ...
    while (1) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_CLOUD_GET_RESPONSE: {
                ...
                else printf("Downloaded client list. %i clients.\n", cloudQueryResult.rowsReturned.Size());
                ...
                for (rowIndex = 0; rowIndex < cloudQueryResult.rowsReturned.Size(); rowIndex++) {
                    ...
                    if (wasCallToGetServers) {
                        ...
                        if (row->clientGUID == rakPeer->GetMyGUID()) printf(" (Ourselves)");
                        RakNet::BitStream bsIn(row->data, row->length, false);
                ...
                if (didRebalance == false && wasCallToGetServers && cloudQueryResult.rowsReturned.Size() > 0
                    && connectionsOnOurServer > lowestConnectionsServer) {
                    ...
                    // slots Alternatively, just call Startup() with 2 slots instead of 1
                    RakSleep(500);
            ...
            case ID_CLOUD_SUBSCRIPTION_NOTIFICATION: {
                ...
                if (wasUpdated) printf("New client at %s\n", cloudQueryRow.clientSystemAddress.ToString(true));
                else printf("Lost client at %s\n", cloudQueryRow.clientSystemAddress.ToString(true));
