...
int main(void) {
    ...
    printf(
        "Hit 'c' to run as a client.  Hit 's' to run as a server. Hit 'q' to "
        "quit\n"
    );
    char buff[256];
    ...
    while (1) {
        ...
        if (ch == 'c') {
            // Run as a client.  If you don't have another machine, just run 2
            // instances of this program and use "127.0.0.1"
    ...
    printf(
        "Entering main loop.  Press 'q' to quit\n'c' to send from the "
    ...
    while (1) {
        ...
        if (packet && packet->data[0] == ID_TIMESTAMP) {
            // Write the bytes after the first to a variable.  That is the time the
            // packet was sent.
        ...
        if (packet) {
            if (packetFromServer) rakServer->DeallocatePacket(packet);
            else rakClient->DeallocatePacket(packet);
        }
    ...
    // Shutdown stuff.  It's ok to call disconnect on the server if we are a
