...
int main(void) {
    ...
    printf(
        "No data is actually sent so it's mostly a sample of how to use a "
        "plugin.\n"
    );
    ...
    for (peerIndex = 0; peerIndex < NUM_PEERS; peerIndex++) {
        ...
        // The fully connected mesh relies on the connection graph plugin also being
        // attached
        ...
        if (ch == ' ') {
            ...
            for (i = 0; i < NUM_PEERS; i++) {
                if (NUM_PEERS <= 10) {
                    ...
                    for (j=0; j < (int)fullyConnectedMeshPlugin[i].GetMeshPeerListSize();
                    j++)
                    {
                    ...
                    */
                    ...
                    for (j = 0; j < NUM_PEERS; j++) {
                        ...
                        if (systemAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("%i ", systemAddress.GetPort());
                    }
                    ...
                    for (connCount = 0, j = 0; j < NUM_PEERS; j++) {
                        systemAddress = rakPeer[i]->GetSystemAddressFromIndex(j);
                        if (systemAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS) connCount++;
                    }
                    /*
                    for (meshCount=0, j=0; j <
                    (int)fullyConnectedMeshPlugin[i].GetMeshPeerListSize(); j++)
                    {
                            systemAddress=fullyConnectedMeshPlugin[i].GetPeerIDAtIndex(j);
                            if (systemAddress!=RakNet::UNASSIGNED_SYSTEM_ADDRESS)
                                    meshCount++;
