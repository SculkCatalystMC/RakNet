...
int main(void) {
    ...
    printf(
        "This is a test I use to test the packet splitting capabilities of "
        "RakNet\n"
    );
    ...
    printf(
        "Enter 's' to run as server, 'c' to run as client, space to run "
        "local.\n"
    );
    ch = ' ';
    ...
    if (ch == 'c') {
        ...
        Gets(text, BIG_PACKET_SIZE);
        if (text[0] == 0) strcpy(text, "natpunch.jenkinssoftware.com"); // dx in Europe
    } else if (ch == 's') {
    ...
    // Always apply the network simulator on two systems, never just one, with
    // half the values on each. Otherwise the flow control gets confused.
    // 	if (client)
            ...
            for (packet = server->Receive(); packet; server->DeallocatePacket(packet), packet = server->Receive()) {
                if (packet->data[0] == ID_NEW_INCOMING_CONNECTION || packet->data[0] == 253) {
                    ...
                    if (BIG_PACKET_SIZE <= 100000) {
                        for (int i = 0; i < BIG_PACKET_SIZE; i++) text[i] = 255 - (i & 255);
                    } else text[0] = (unsigned char)255;
                    server->Send(
                        text,
                        BIG_PACKET_SIZE,
                        LOW_PRIORITY,
                    ...
                    // Keep the stat from updating until the messages move to the thread
                    // or it quits right away
        ...
        if (client) {
            ...
            while (packet) {
                if (packet->data[0] == ID_DOWNLOAD_PROGRESS) {
                    ...
                    // Disable endian swapping on reading this, as it's generated locally
                    // in ReliabilityLayer.cpp
                    ...
                    printf(
                        "Progress: msgID=%i Progress %i/%i Partsize=%i\n",
    ...
    printf(
        "%i bytes per second (%.2f seconds). Press enter to quit\n",
