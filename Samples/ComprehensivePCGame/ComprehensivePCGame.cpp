...
// This is used to make it easier to parse the JSON returned from the master
// server
#include "jansson.h"
...
// Required?: No, but provides a flexible and full-featured implementation that
// supports host migration, mid-game joins, and advanced team management.
TeamManager* teamManager;

// Purpose: Game object replication
// Required?: No, but manages object replication automatically. Some form of
// this is required for almost every game
ReplicaManager3* replicaManager3;

// Purpose: Lookup game objects given ID. Used by ReplicaManager3
// Required?: Required to use ReplicaManager3, and some form of this is required
// for almost every game
NetworkIDManager* networkIDManager;

// Purpose: Connect to RakNet's hosted master server
// Required?: Steam and consoles provide server solutions, so usually not if you
// are releasing only on those platforms.
TCPInterface* tcp;

// Purpose: If UPNP fails, used to connect across routers.
// Required?: Steam and consoles provide server solutions that do this
// automatically. Otherwise, if UPNP fails you need this. Note: See the project
// NAT Punchthrough / NATCompleteServer for a sample server implementation
NatPunchthroughClient* natPunchthroughClient;

#ifdef NAT_TYPE_DETECTION_SERVER
// Purpose: Determine what type of router we are behind, if any
// Required?: No, but game sessions that are impossible to join can be filtered
// out from game listings
...
// Purpose: Used for players to ready / unready in the lobby in a way that is
// properly synchronized under peer to peer Required?: No, but provides a more
// robust way to ready/unready in a peer to peer game
ReadyEvent* readyEvent;

// Purpose: Automatically determine and migrate the host of a peer to peer
// session. Avoid partial connection failures when joining peer-to-peer games.
// Required?: Yes, for peer to peer games that need a session host. Not
// necessary for client/server
FullyConnectedMesh2* fullyConnectedMesh2;

// Purpose: Helper class to parse http connection events
// Required?: Technically no since you could parse the strings yourself. But
// it's a lot of work otherwise.
...
// Game contains the list of objects, and serializes if the game is locked, the
// name of the game, and if it is in gameplay or in the lobby Game is created on
// startup, rather than over the network, so is replicated as a static object.
// see Help/replicamanager3.html under the topic Static Objects for more details
// about how this class is Replicated
...
public:
    ...
    virtual ~Game() {
        if (masterServerQueryResult) json_decref(masterServerQueryResult);
    }
    virtual void
    ...
    QueryConstruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) {
        if (fullyConnectedMesh2->IsConnectedHost())
            ...
            return QueryConstruction_PeerToPeer(
    ...
    virtual RakNet::RM3QuerySerializationResult QuerySerialization(RakNet::Connection_RM3* destinationConnection) {
        if (fullyConnectedMesh2->IsConnectedHost())
            ...
            return QuerySerialization_PeerToPeer(
                destinationConnection,
                R3P2PM_STATIC_OBJECT_NOT_CURRENTLY_AUTHORITATIVE
            );
    }
    ...
    virtual void Deserialize(RakNet::DeserializeParameters* deserializeParameters) {
        if (deserializeParameters->bitstreamWrittenTo[0]) {
            ...
            if (b != lockGame) {
                lockGame = b;
                if (lockGame) printf("Game is no longer locked\n");
                else printf("Game is now locked\n");
            }
            deserializeParameters->serializationBitstream[0].Read(b);
            if (b != gameInLobby) {
                gameInLobby = b;
    ...
    void EnterPhase(Phase newPhase) {
        ...
        case CONNECTING_TO_SERVER: {
            char port[256];
            printf(
                "Enter address of server running the NATCompleteServer "
                "project.\nEnter for default: "
            );
            Gets(game->serverIPAddr, 256);
            if (game->serverIPAddr[0] == 0) strcpy(game->serverIPAddr, DEFAULT_SERVER_ADDRESS);
            printf("Enter server port, or enter for default: ");
            Gets(port, 256);
            if (port[0] == 0) strcpy(port, DEFAULT_SERVER_PORT);
    ...
    // This is serialized so if the host migrates, the new host takes over that
    // row. Otherwise, a new row would be written for the same game.
    int masterServerRow;
    ...
    Phase                          phase;
    // NAT punchthrough server runs RakNet project NatCompleteServer with
    // NAT_TYPE_DETECTION_SERVER, and NAT_PUNCHTHROUGH_SERVER
    ...
    json_t* GetMasterServerQueryResult(void) {
        if (masterServerQueryResult == 0) return 0;
    ...
    // The GET request returns a string. I use http://www.digip.org/jansson/ to
    // parse the string, and store the results.
...
// It uses TM_Team from the TeamManager plugin to do actual team functionality,
// store which players are on which teams, and networking It derives from
// Replica3 in order to replicate the teams across the network
...
public:
    ...
    virtual ~Team() { game->teams.RemoveAtIndex(game->teams.GetIndexOf(this)); }
    virtual void
    WriteAllocationID(RakNet::Connection_RM3* destinationConnection, RakNet::BitStream* allocationIdBitstream) const {
    ...
    QueryConstruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) {
        ...
        // If the original host disconnects, the new host as determined by
        // FullyConnectedMesh2 takes over replication duties
        if (fullyConnectedMesh2->IsConnectedHost())
            return QueryConstruction_PeerToPeer(destinationConnection, R3P2PM_MULTI_OWNER_CURRENTLY_AUTHORITATIVE);
        else return QueryConstruction_PeerToPeer(destinationConnection, R3P2PM_MULTI_OWNER_NOT_CURRENTLY_AUTHORITATIVE);
    }
    virtual bool QueryRemoteConstruction(RakNet::Connection_RM3* sourceConnection) { return true; }
    virtual void
    SerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
    ...
    DeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
        ...
        // When ReplicaManager3 creates the team from a network command, the
        // TeamManager class has to be informed of the new TM_Team instance
    ...
    virtual RakNet::RM3ActionOnPopConnection
    QueryActionOnPopConnection(RakNet::Connection_RM3* droppedConnection) const {
        // Do not destroy the object when the connection that created it
        // disconnects.
        return RM3AOPC_DO_NOTHING;
    }
    virtual void DeallocReplica(RakNet::Connection_RM3* sourceConnection) { delete this; }
...
public:
    ...
    virtual ~User() { game->users.RemoveAtIndex(game->users.GetIndexOf(this)); }
    virtual void
    WriteAllocationID(RakNet::Connection_RM3* destinationConnection, RakNet::BitStream* allocationIdBitstream) const {
    ...
    virtual bool
    DeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
        // The TeamManager plugin has to be informed of TM_TeamMember instances
    ...
    virtual void
    PostSerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        // TeamManager requires that TM_Team was created before TM_TeamMember that
        // uses it. PostSerializeConstruction and PostDeserializeConstruction ensure
        // that all objects have been created before serialization
    ...
    PostDeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
        ...
        if (tmTeamMember.GetCurrentTeam() == 0) printf(" not on a team\n");
        ...
        if (fullyConnectedMesh2->IsConnectedHost()) PostRoomToMaster();
    }
...
// Required by ReplicaManager3. Acts as a class factory for Replica3 derived
// instances
class SampleConnectionRM3 : public Connection_RM3 {
public:
    SampleConnectionRM3(const SystemAddress& _systemAddress, RakNetGUID _guid)
...
// Required by ReplicaManager3. Acts as a class factory for Connection_RM3
// derived instances
...
// Register the function where it is defined, which is easier than maintaining a
// bunch of RegisterSlot() calls in main()
...
void SerializeToJSON(
    RakString&                                    outputString,
    RakString&                                    roomName,
    DataStructures::List<NATTypeDetectionResult>& natTypes
) {
    outputString
        .Set("'roomName': '%s', 'guid': '%s', 'natTypes' : [ ", roomName.C_String(), rakPeer->GetMyGUID().ToString());
...
// Register this system with the plugins that need to know about new
// participants This operation happens after FullyConnectedMesh2 has told us
...
void RegisterGameParticipant(RakNetGUID guid) {
    ...
    if (replicaManager3->PushConnection(connection) == false) replicaManager3->DeallocConnection(connection);
...
// If that system disconnects, the new host, as determined by
// FullyConnectedMesh2 will reupload the room
void PostRoomToMaster(void) {
    BitStream                                    bsOut;
    RakString                                    jsonSerializedRoom;
    DataStructures::List<NATTypeDetectionResult> natTypes;
    for (unsigned int i = 0; i < game->users.Size(); i++) natTypes.Push(game->users[i]->natType, _FILE_AND_LINE_);
    SerializeToJSON(jsonSerializedRoom, game->gameName, natTypes);

    RakString rowStr;
    if (game->masterServerRow != -1) rowStr.Set("\"__rowId\": %i,", game->masterServerRow);
    ...
    RakString rsRequest = RakString::FormatForPOST(
        ...
        RakString(
            "{'__gameId': 'comprehensivePCGame', '__clientReqId': '0', %s "
            "'__timeoutSec': '30', %s }",
            rowStr.C_String(),
            jsonSerializedRoom.C_String()
        )
    );
...
void CreateRoom(void) {
    size_t arraySize;
    if (game->GetMasterServerQueryResult()) arraySize = json_array_size(game->GetMasterServerQueryResult());
    else arraySize = 0;
    ...
    // So that time spent in single player does not count towards which system has
    // been running the longest in multiplayer
...
RAK_THREAD_DECLARATION(UPNPOpenWorker) {
    ...
    if (devlist) {
        ...
        for (device = devlist; device; device = device->pNext) {
            ...
            if (args->progressCallback) args->progressCallback(args->buff, args->userData);
        }
        ...
        if (UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr)) == 1) {
            ...
            int r =
                UPNP_AddPortMapping(urls.controlURL, data.first.servicetype, eport, iport, lanaddr, 0, "UDP", 0, "0");
            ...
            if (r != UPNPCOMMAND_SUCCESS) {
                ...
                if (args->progressCallback) args->progressCallback("UPNP success.\n", args->userData);
    ...
    if (args->resultCallback) args->resultCallback(success, args->portToOpen, args->userData);
...
void UPNPProgressCallback(const char* progressMsg, void* userData) { printf(progressMsg); }
void UPNPResultCallback(bool success, unsigned short portToOpen, void* userData) {
    if (success) game->myNatType = NAT_TYPE_SUPPORTS_UPNP;
...
int main(void) {
    printf(
        "Demonstrates networking elements for a P2P game on the PC, "
    ...
    */
    ...
    // ---------------------------------------------------------------------------------------------------------------------
    // Setup plugins: Disable automatically adding new connections. Allocate
    // initial objects and register for replication
    // ---------------------------------------------------------------------------------------------------------------------
    ...
    // Tell ReplicaManager3 which networkIDManager to use for object lookup, used
    // for automatic serialization
    replicaManager3->SetNetworkIDManager(networkIDManager);
    // Do not automatically count new connections, but do drop lost connections
    // automatically
    ...
    while (game->phase != Game::EXIT_SAMPLE) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_NEW_INCOMING_CONNECTION: {
                printf(
                    "ID_NEW_INCOMING_CONNECTION from %s. guid=%s.\n",
            ...
            case ID_CONNECTION_REQUEST_ACCEPTED: {
                printf(
                    "ID_CONNECTION_REQUEST_ACCEPTED from %s,guid=%s\n",
                    packet->systemAddress.ToString(true),
                    packet->guid.ToString()
                );
                ...
                if (game->phase == Game::CONNECTING_TO_SERVER) {
                    ...
                    // ---------------------------------------------------------------------------------------------------------------------
                    // PC self-hosted servers only: Use the NAT punch server to determine
                    // NAT type. Attempt to open router if needed.
                    // ---------------------------------------------------------------------------------------------------------------------
                    ...
                    // So time in single player does not count towards which system has
                    // been running multiplayer the longest
                    fullyConnectedMesh2->ResetHostCalculation();

                    // Custom message to ask to join the game
                    // We first connect to the game host, and the game host is responsible
                    // for calling StartVerifiedJoin() for us to join the session
                    BitStream bsOut;
            ...
            case ID_DISCONNECTION_NOTIFICATION:
                if (game->phase == Game::DETERMINE_NAT_TYPE) {
                    printf(
                        "Lost connection during NAT type detection. Reason %s. "
                        "Retrying...\n",
                    ...
                    printf(
                        "Lost connection during NAT punch to game host. Reason %s.\n",
                        PacketLogger::BaseIDTOString(packet->data[0])
                    );
                    game->EnterPhase(Game::SEARCH_FOR_GAMES);
                } else {
                    if (packet->guid == game->natPunchServerGuid) {
                        printf(
                            "Server connection lost. Reason %s.\nGame session is no "
                            "longer searchable.\n",
                            PacketLogger::BaseIDTOString(packet->data[0])
                        );
                    } else {
            ...
            case ID_INCOMPATIBLE_PROTOCOL_VERSION:
                // Note: Failing to connect to another system does not automatically
                // mean we cannot join a session, since that system may be disconnecting
                // from the host simultaneously FullyConnectedMesh2::StartVerifiedJoin()
                // internally handles success or failure and notifies the client through
                // ID_FCM2_VERIFIED_JOIN_FAILED if needed.
                printf(
                ...
                if (game->phase == Game::CONNECTING_TO_SERVER) game->EnterPhase(Game::EXIT_SAMPLE);
                break;
            ...
            case ID_FCM2_NEW_HOST: {
                ...
                if (packet->guid == rakPeer->GetMyGUID()) {
                    if (oldHost != UNASSIGNED_RAKNET_GUID) {
                        if (game->phase == Game::IN_LOBBY_WAITING_FOR_HOST) game->phase = Game::IN_LOBBY_WITH_HOST;
                        PostRoomToMaster();
                        printf(
                            "ID_FCM2_NEW_HOST: Taking over as host from the old "
                            "host.\nNew options:\n"
                        );
                    } else {
                        // Room not hosted if we become host the first time since this was
                        // done in CreateRoom() already
                        printf(
                            "ID_FCM2_NEW_HOST: We have become host for the first time. "
                            "New options:\n"
                        );
                    }
                    ...
                    if (oldHost != UNASSIGNED_RAKNET_GUID)
                        printf(
                            "ID_FCM2_NEW_HOST: A new system %s has become host, GUID=%s\n",
                            packet->systemAddress.ToString(true),
                            packet->guid.ToString()
                        );
                    else
                        printf(
                            "ID_FCM2_NEW_HOST: System %s is host, GUID=%s\n",
                            packet->systemAddress.ToString(true),
                            packet->guid.ToString()
                        );
                }
                ...
                if (oldHost == UNASSIGNED_RAKNET_GUID) {
                    ...
                    // Reference previously created replicated objects, which cannot be
                    // serialized until host is known the first time
                    if (packet->guid == rakPeer->GetMyGUID()) {
                    ...
                    for (unsigned int i = 0; i < game->users.Size(); i++) replicaManager3->Reference(game->users[i]);
                }
            } break;
            case ID_TEAM_BALANCER_TEAM_ASSIGNED: {
                ...
                if (teamMember->GetCurrentTeam() == 0) printf(" not on team\n");
            ...
            case ID_NAT_PUNCHTHROUGH_FAILED: {
                // As with connection failed, this does not automatically mean we cannot
                // join the session We only fail on ID_FCM2_VERIFIED_JOIN_FAILED
                printf(
                    "NAT punch to %s failed. Reason %s\n",
                    packet->guid.ToString(),
                    PacketLogger::BaseIDTOString(packet->data[0])
                );

                if (game->phase == Game::NAT_PUNCH_TO_GAME_HOST) game->EnterPhase(Game::SEARCH_FOR_GAMES);
            }
                ...
                if (game->phase == Game::NAT_PUNCH_TO_GAME_HOST || game->phase == Game::VERIFIED_JOIN) {
                    ...
                    if (car != RakNet::CONNECTION_ATTEMPT_STARTED) {
                        ...
                        if (game->phase == Game::NAT_PUNCH_TO_GAME_HOST) {
                            printf(
                                "NAT punch completed. Connecting to %s (game host)...\n",
                                packet->systemAddress.ToString(true)
                            );
                            game->EnterPhase(Game::CONNECTING_TO_GAME_HOST);
                        } else {
                            printf(
                                "NAT punch completed. Connecting to %s (game client)...\n",
                                packet->systemAddress.ToString(true)
                            );
                        }
                    }
            ...
            case ID_NAT_TYPE_DETECTION_RESULT: {
                ...
                if (game->myNatType == RakNet::NAT_TYPE_PORT_RESTRICTED
                    || game->myNatType == RakNet::NAT_TYPE_SYMMETRIC) {
                    printf(
                        "Note: Your router must support UPNP or have the user "
            ...
            // ID_USER_PACKET_ENUM is used by this sample as a custom message to ask
            // to join a game
            case ID_USER_PACKET_ENUM:
                if (game->phase > Game::SEARCH_FOR_GAMES) {
                    printf(
                        "Got request from client to join session.\nExecuting "
                        "StartVerifiedJoin()\n"
                    );
                ...
                break;
            // ID_USER_PACKET_ENUM+1 is used by this sample as a custom message to
            // reject a join game request Requests may also be later rejected through
            // FullyConnectedMesh2::RespondOnVerifiedJoinCapable() to send
            ...
            case ID_FCM2_VERIFIED_JOIN_START: {
                ...
                // This message means the session host sent us a list of systems in the
                // session Once we connect to, or fail to connect to, each of these
                // systems we will get ID_FCM2_VERIFIED_JOIN_FAILED,
            ...
            case ID_FCM2_VERIFIED_JOIN_CAPABLE:
                ...
                } else fullyConnectedMesh2->RespondOnVerifiedJoinCapable(packet, true, 0);
                break;
            ...
            case ID_FCM2_VERIFIED_JOIN_ACCEPTED: {
                ...
                fullyConnectedMesh2
                    ->GetVerifiedJoinAcceptedAdditionalData(packet, &thisSystemAccepted, systemsAccepted, 0);
                if (thisSystemAccepted) printf("Game join request accepted\n");
                else printf("System %s joined the mesh\n", systemsAccepted[0].ToString());

                // Add the new participant to the game if we already know who the host
                // is. Otherwise do this once ID_FCM2_NEW_HOST arrives
                if (fullyConnectedMesh2->GetConnectedHost() != UNASSIGNED_RAKNET_GUID) {
                    // FullyConnectedMesh2 already called AddParticipant() for each
                    // accepted system Still need to add those systems to the other
                    // plugins though
                    for (unsigned int i = 0; i < systemsAccepted.Size(); i++)
                        RegisterGameParticipant(systemsAccepted[i]);

                    if (thisSystemAccepted) game->EnterPhase(Game::IN_LOBBY_WITH_HOST);
                } else {
                    if (thisSystemAccepted) game->EnterPhase(Game::IN_LOBBY_WAITING_FOR_HOST);
                }
            ...
            case ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE: {
                if (replicaManager3->GetAllConnectionDownloadsCompleted() == true) {
                    ...
                    if (game->gameInLobby) game->EnterPhase(Game::IN_LOBBY_WITH_HOST);
                    else game->EnterPhase(Game::IN_GAME);
                }
        ...
        int           contentOffset;
        if (httpConnection2
                ->GetResponse(stringTransmitted, hostTransmitted, responseReceived, hostReceived, contentOffset)) {
            if (responseReceived.IsEmpty() == false) {
                if (contentOffset == -1) {
                    ...
                    json_error_t error;
                    json_t*      root =
                        json_loads(responseReceived.C_String() + contentOffset, JSON_REJECT_DUPLICATES, &error);
                    if (!root) {
                        ...
                        while (iter) {
                            ...
                            if (stricmp(firstKey, "GET") == 0) {
                                ...
                                for (unsigned int i = 0; i < arraySize; i++) {
                                    ...
                                    printf(
                                        "Room name: %s. Players: %i\n",
                                        json_string_value(roomNameVal),
                                        natTypesSize
                                    );
                                }

                                if (arraySize == 0) printf("No rooms.\n");
        ...
        if (kbhit()) {
            ...
            if (game->phase == Game::SEARCH_FOR_GAMES) {
                ...
                if (game->phase == Game::IN_GAME) {
                    if (ch == 'c' || ch == 'C') {
                        ...
                        if (participantList.Size() > 0) {
                            ...
                            Gets(str, 256);
                            RakString rs;
                            // Don't use RakString constructor to assign str, or will process
                            // % escape characters
                            rs = str;
                            BitStream bsOut;
                            bsOut.Write(rs);
                ...
                if (ch == '1') {
                    ...
                    if (readyEvent->SetEvent(0, true)) printf("We are ready to start.\n");
                } else if (ch == 'u' || ch == 'U') {
                    if (readyEvent->SetEvent(0, false)) printf("We are no longer ready to start.\n");
                } else if (ch == 'l' || ch == 'L') {
                    if (fullyConnectedMesh2->IsConnectedHost()) {
                        if (game->lockGame) {
                    ...
                    // However, teams are not deleted since the Team class can migrate
                    // between systems. So delete Team instances manually
                    while (game->teams.Size()) delete game->teams[game->teams.Size() - 1];

                    // If we were the host, no longer list this session
                    // The new host will call PostRoomToCloud to reupload under a new IP
                    // address on ID_FCM2_NEW_HOST
        ...
        if ((fullyConnectedMesh2->IsConnectedHost() || game->users.Size() == 1)
            ...
            && (game->phase == Game::IN_LOBBY_WITH_HOST || game->phase == Game::IN_GAME
    ...
    while (game->teams.Size()) delete game->teams[game->teams.Size() - 1];
    while (game->users.Size()) delete game->users[game->users.Size() - 1];
    delete game;
