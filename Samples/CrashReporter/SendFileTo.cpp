...
bool CSendFileTo::SendMail(
    HWND        hWndParent,
    const char* strAttachmentFilePath,
    const char* strAttachmentFileName,
    const char* strSubject,
    const char* strBody,
    const char* strRecipient
) {
    ...
    if (strAttachmentFileName) strcpy(szName, (LPCTSTR)strAttachmentFileName);
    ...
    if (strAttachmentFileName && strAttachmentFilePath) {
        if (strlen(strAttachmentFilePath) < 3 || strAttachmentFilePath[1] != ':'
            || (strAttachmentFilePath[2] != '\\' && strAttachmentFilePath[2] != '/')) {
            // Make relative paths absolute
            getcwd(fullPath, _MAX_PATH);
            strcat(fullPath, "/");
            strcat(fullPath, strAttachmentFilePath);
        } else strcpy(fullPath, strAttachmentFilePath);
