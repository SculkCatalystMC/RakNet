...
// The callstack in release is the same as usual, which means it isn't all that
// accurate.
#ifdef WIN32
...
LONG ProcessException(struct _EXCEPTION_POINTERS* ExceptionInfo) {
    ...
    sprintf(
        appDescriptor,
        "%s %s - %s %s",
        CrashReporter::controls.appName,
        CrashReporter::controls.appVersion,
        __DATE__,
        __TIME__
    );
    ...
    if ((CrashReporter::controls.actionToTake & AOC_EMAIL_WITH_ATTACHMENT)
        ...
        if (CrashReporter::controls.actionToTake & AOC_WRITE_TO_DISK) {
            ...
            // Write to a temporary directory if the user doesn't want the dump on the
            // harddrive.
            if (!GetTempPath(_MAX_PATH, dumpFilepath)) dumpFilepath[0] = 0;
        }
        ...
        HANDLE hFile =
            CreateFile(dumpFilepath, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (CrashReporter::controls.actionToTake & AOC_EMAIL_NO_ATTACHMENT) {
        ...
        if (CrashReporter::controls.actionToTake & AOC_SILENT_MODE) {
            sprintf(
                silentModeEmailBody,
                "%s%s version %s has crashed.\r\nIt was compiled on %s %s.\r\n",
                CrashReporter::controls.emailBody,
                CrashReporter::controls.appName,
                CrashReporter::controls.appVersion,
                __DATE__,
                __TIME__
            ...
            emailSender.Send(
                CrashReporter::controls.SMTPServer,
                25,
            ...
            sendFile
                .SendMail(0, 0, 0, subject, CrashReporter::controls.emailBody, CrashReporter::controls.emailRecipient);
        }
...
LONG WINAPI CrashExceptionFilter(struct _EXCEPTION_POINTERS* ExceptionInfo) {
    // Mutex here due to
    // http://www.jenkinssoftware.com/raknet/forum/index.php?topic=2305.0;topicseen
