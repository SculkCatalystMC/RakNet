...
void ShowHelp(void) {
    printf(
        ...
        "random client\n(R)andom silent disconnects and connects for all "
...
// Buffer for input (an ugly hack to keep *nix happy)
    ...
    printf(
        "This is a project I use internally to test if dropped connections "
        "are detected\n"
    );
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (ch == 'd' || ch == 'D') {
                ...
                if (numberOfSystems == 0) printf("Client %i silently closing inactive connection.\n", index);
                else printf("Client %i silently closing active connection.\n", index);
            } else if (ch == 'c' || ch == 'C') {
                index = randomMT() % NUMBER_OF_CLIENTS;

                clients[index]->GetConnectionList(0, &numberOfSystems);
                clients[index]->Connect("127.0.0.1", serverPort, 0, 0);
                if (numberOfSystems == 0) printf("Client %i connecting to same existing connection.\n", index);
                else printf("Client %i connecting to closed connection.\n", index);
                ...
                for (index = 0; index < NUMBER_OF_CLIENTS; index++) {
                    if (NUMBER_OF_CLIENTS == 1 || (randomMT() % 2) == 0) {
                        if (clients[index]->IsActive()) {
                            if (ch == 'r' || ch == 'R') clients[index]->CloseConnection(serverID, false, 0);
                ...
                for (index = 0; index < NUMBER_OF_CLIENTS; index++) {
                    clients[index]->GetConnectionList(0, &numberOfSystems);
                    if (numberOfSystems > 1) printf("Bug: Client %i has %i connections\n", index, numberOfSystems);
    ...
    for (index = 0; index < NUMBER_OF_CLIENTS; index++) RakNet::RakPeerInterface::DestroyInstance(clients[index]);
    return 1;
}
