    ...
    MakePatch(const char* oldFile, const char* newFile, char** patch, unsigned int* patchLength, int* patchAlgorithm) {
        ...
        if ((contentLengthOld < 33554432 && contentLengthNew < 33554432) || PATH_TO_XDELTA_EXE[0] == 0) {
            ...
            *patchAlgorithm = 1;
            ...
            // r+ instead of r, because I want exclusive access in case xdelta is
            // still working
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        if (notificationAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("ID_CONNECTION_REQUEST_ACCEPTED\n");
        ...
        p = rakPeer->Receive();
        while (p) {
            if (p->data[0] == ID_NEW_INCOMING_CONNECTION) printf("ID_NEW_INCOMING_CONNECTION\n");
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') break;
                ...
                if (appDir[0] == 0) strcpy(appDir, "D:\\temp");
