    ...
    virtual PatchContext ApplyPatchBase(
        ...
        if (patchAlgorithm == 0) {
            ...
            _snprintf(
                ...
                "-d -f -s %s patchClient_%s.tmp newFile_%s.tmp",
                oldFilePath,
                buff,
                buff
            );
...
int main(int argc, char** argv) {
    ...
    // Plugin will send us downloading progress notifications if a file is split
    // to fit under the MTU 10 or more times
    ...
    } else strcpy(appDir, argv[2]);
    char appName[512];
    if (argc < 4) {
        printf("Enter application name: ");
        Gets(appName, sizeof(appName));
        if (appName[0] == 0) strcpy(appName, "TestApp");
    } else strcpy(appName, argv[3]);
    ...
    if (patchImmediately == false) {
        ...
        if (PATH_TO_XDELTA_EXE[0] == 0) strcpy(PATH_TO_XDELTA_EXE, "c:/xdelta3-3.0.6-win32.exe");
        ...
        if (PATH_TO_XDELTA_EXE[0]) {
            ...
            if (WORKING_DIRECTORY[0] == 0) GetTempPath(MAX_PATH, WORKING_DIRECTORY);
        ...
        printf(
            "Hit 'q' to quit, 'p' to patch, 'f' to full scan. 'c' to cancel the "
    ...
    while (1) {
        ...
        while (p) {
            if (p->data[0] == ID_AUTOPATCHER_REPOSITORY_FATAL_ERROR) {
                ...
                printf(
                    "Launch \"AutopatcherClientRestarter.exe "
                    "autopatcherRestart.txt\"\nQuit this application immediately "
                    "after to unlock files.\n"
                );
        ...
        p = rakPeer->Receive();
        while (p) {
            if (p->data[0] == ID_DISCONNECTION_NOTIFICATION) printf("ID_DISCONNECTION_NOTIFICATION\n");
