...
/// \file
/// \brief Tests connecting two peers at the same time with the internet
/// simulator running.
...
int main() {
    ...
    while (1) {
        ...
        for (packet = rakPeer1->Receive(); packet; rakPeer1->DeallocatePacket(packet), packet = rakPeer1->Receive()) {
            if (packet->data[0] == ID_NEW_INCOMING_CONNECTION) gotNewIncomingConnection[0]++;
            else if (packet->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) gotConnectionRequestAccepted[0]++;
                ...
                printf(
                    "Got ID_ALREADY_CONNECTED on rakPeer1, (not necessarily a "
                    "failure)\n"
                );
        }
        for (packet = rakPeer2->Receive(); packet; rakPeer2->DeallocatePacket(packet), packet = rakPeer2->Receive()) {
            if (packet->data[0] == ID_NEW_INCOMING_CONNECTION) gotNewIncomingConnection[1]++;
            else if (packet->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) gotConnectionRequestAccepted[1]++;
            else if (packet->data[0] == ID_CONNECTION_ATTEMPT_FAILED)
                printf("Error on rakPeer2, got ID_CONNECTION_ATTEMPT_FAILED\n");
            else if (packet->data[0] == ID_ALREADY_CONNECTED)
                printf(
                    "Got ID_ALREADY_CONNECTED on rakPeer2, (not necessarily a "
                    "failure)\n"
                );
        }
        ...
        } else if (gotConnectionRequestAccepted[0] + gotConnectionRequestAccepted[1] + gotNewIncomingConnection[0]
