...
bool Authenticate(RakNet::Rackspace* rackspaceApi, RakNet::TCPInterface* tcpInterface) {
    ...
    // This is found when you log in, under Your Account, expanded to API Access.
    // Then click Show Key under Enable API Access.
    ...
    if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("Failed to connect to %s\n", authenticationURL);
...
class DisplayHTMLPage : public RakNet::RackspaceEventCallback_Default {
    virtual void
    ExecuteDefault(const char* callbackName, RakNet::RackspaceEventType eventType, const char* htmlAdditionalInfo) {
        /*
        ...
        if (fp)
        {
                ...
                ShellExecute(NULL, "open", fileName.C_String(), NULL, szDirectory,
        SW_SHOWNORMAL);
        }
        */
    ...
    virtual void OnConnectionAttemptFailure(RakNet::RackspaceOperationType operationType, const char* url) {
        printf("OnConnectionAttemptFailure\noperationType=%i\n%s\n", operationType, url);
    }
};
...
int main() {
    ...
    while (1) {
        for (packet = tcpInterface.Receive(); packet;
