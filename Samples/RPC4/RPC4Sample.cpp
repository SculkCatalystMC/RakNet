...
int main(void) {
    ...
    if (ip[0]) {
        ...
        //	rpc.Signal("Event1", &testBs,
        // HIGH_PRIORITY,RELIABLE_ORDERED,0,rakPeer->GetSystemAddressFromIndex(0),false,
        // true);

        RakSleep(100);
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), rakPeer->Receive());

        // Needs 2 program instances, because while the call is blocking
        // rakPeer2->Receive() isn't getting called
        ...
        rpc.CallBlocking(
            ...
            RELIABLE_ORDERED,
            0,
            rakPeer->GetSystemAddressFromIndex(0),
            &testBlockingReturn
        );
