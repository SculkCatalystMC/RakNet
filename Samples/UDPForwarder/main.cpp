...
int main() {
    ...
    // RakNet will send a message at least every 5 seconds. Add another second to
    // account for thread latency
    const RakNet::TimeMS timeoutOnNoDataMS = 6000;

    // Address is probably 192.168.0.1. Fix it to be 127.0.0.1.
    // Only necessary to do this when connecting through the loopback on the local
    // system. In a real system we'd stick with the external IP
    ...
    if (!udpForwarder.StartForwarding(
            peer0Addr,
            peer1Addr,
            timeoutOnNoDataMS,
            "127.0.0.1",
    ...
    // Send a connect message to the forwarder, on the port to forward to
    // rakPeer[1]
