...
// Keeps a record of the last 20 calls, to give a faster response to traffic
// change
void LogStats() {
    ...
    printf(
        "avgKbpsSent=%02.1f avgKbpsRec=%02.1f kbpsSent=%02.1f kbpsRec=%02.1f  "
        "  \r",
        avgBpsSent / 1000,
        avgBpsRec / 1000,
        bpsSent / 1000,
        bpsRec / 1000,
        rakVoice.GetBufferedBytesToReturn(RakNet::UNASSIGNED_RAKNET_GUID)
    );
    // printf("MsgBuf=%6i SndBuf=%10i RcvBuf=%10i    \r",
    // rakVoice.GetRakPeerInterface()->GetStatistics(RakNet::UNASSIGNED_SYSTEM_ADDRESS)->messageSendBuffer[HIGH_PRIORITY],
    // rakVoice.GetBufferedBytesToSend(RakNet::UNASSIGNED_SYSTEM_ADDRESS),
    // rakVoice.GetBufferedBytesToReturn(RakNet::UNASSIGNED_SYSTEM_ADDRESS));
}
...
#define MY_BUFSIZE 1024                  // Buffer size for console window titles.
    ...
    wsprintf(pszNewWindowTitle, TEXT("%d/%d"), GetTickCount(), GetCurrentProcessId());
...
int main(void) {
    ...
    printf(
        "(+/-)encoder complexity.  (N)oise filter on/off. (V)AD on/off. "
        "(B)vbr on/off.\n"
    );
#else
