...
DSoundVoiceAdapter* DSoundVoiceAdapter::Instance() { return &instance; }
...
bool DSoundVoiceAdapter::SetupAdapter(RakVoice* rakVoice, HWND hwnd, DWORD dwCoopLevel) {
    ...
    // Check if the rest of the initialization fails, and if so, release any
    // allocated resources
...
bool DSoundVoiceAdapter::SetupAdapter(RakVoice* rakVoice) {
    ...
    if (!SetupIncomingBuffer()) return false;
...
bool DSoundVoiceAdapter::SetupIncomingBuffer() {
    ...
    dsbdesc.dwFlags = DSBCAPS_CTRLVOLUME | DSBCAPS_GLOBALFOCUS | DSBCAPS_CTRLPOSITIONNOTIFY
                    | DSBCAPS_LOCSOFTWARE; // Create in software, because DX documentation says
                                           // its the best option when using notifications
    dsbdesc.dwBufferBytes = rakVoice->GetBufferSizeBytes() * FRAMES_IN_SOUND;
    dsbdesc.lpwfxFormat   = &wfx;
    // Create buffer.
    if (FAILED(hr = ds->CreateSoundBuffer(&dsbdesc, &pDsb, NULL))) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSound8::CreateSoundBuffer, when creating "
            L"buffer for incoming sound )",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr)) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundBuffer::QueryInterface, when getting "
            L"IDirectSoundBuffer8 interface for incoming sound",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr = dsbIncoming->QueryInterface(IID_IDirectSoundNotify8, (LPVOID*)&dsbNotify))) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundBuffer8::QueryInterface, when getting "
            L"IDirectSoundNotify8 interface for incoming sound",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr)) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundNotify8::SetNotificationPositions, when "
            L"setting notifications for incoming sound",
            hr
        );
        return false;
    }
...
bool DSoundVoiceAdapter::SetupOutgoingBuffer() {
    ...
    if (FAILED(hr = dsC->CreateCaptureBuffer(&dscbd, &pDscb, NULL))) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundCapture8::CreateCaptureBuffer, when "
            L"creating buffer for outgoing sound )",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr)) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundBuffer::QueryInterface, when getting "
            L"IDirectSoundCaptureBuffer8 interface for outgoing sound",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr = dsbOutgoing->QueryInterface(IID_IDirectSoundNotify8, (LPVOID*)&dsbNotify))) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundCaptureBuffer8::QueryInterface, when getting "
            L"IDirectSoundNotify8 interface for outgoing sound",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr)) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundNotify8::SetNotificationPositions, when "
            L"setting notifications for outgoing sound",
            hr
        );
        return false;
    }
    ...
    if (FAILED(hr = dsbOutgoing->Start(DSCBSTART_LOOPING))) {
        DXTRACE_ERR_MSGBOX(
            L"IDirectSoundCaptureBuffer8::Start, when starting "
            L"outgoing sound buffer",
            hr
        );
        return false;
    }
...
void DSoundVoiceAdapter::Release() {
    ...
    for (int i = 0; i < FRAMES_IN_SOUND; i++) {
        if (incomingBufferNotifications[i].hEventNotify != 0
            && CloseHandle(incomingBufferNotifications[i].hEventNotify) == 0) {
            DXTRACE_ERR_MSGBOX(L"CloseHandle", GetLastError());
        }
        if (outgoingBufferNotifications[i].hEventNotify != 0
            && CloseHandle(outgoingBufferNotifications[i].hEventNotify) == 0) {
            DXTRACE_ERR_MSGBOX(L"CloseHandle", GetLastError());
        }
    }
    memset(incomingBufferNotifications, 0, sizeof(incomingBufferNotifications));
...
IDirectSound8* DSoundVoiceAdapter::GetDSDeviceObject() { return ds; }
...
void DSoundVoiceAdapter::Update() {
    ...
    for (int i = 0; i < FRAMES_IN_SOUND; i++) {
        ...
        if (WaitForSingleObject(incomingBufferNotifications[i].hEventNotify, 0) == WAIT_OBJECT_0) {
            ...
            DWORD dwOffset = (i == 0) ? incomingBufferNotifications[FRAMES_IN_SOUND - 1].dwOffset
                                      : incomingBufferNotifications[i - 1].dwOffset;
        ...
        if (WaitForSingleObject(outgoingBufferNotifications[i].hEventNotify, 0) == WAIT_OBJECT_0) {
             ...
             */
            if (!mute) {
                // The lock offset is the buffer right before the one the event refers
                // to
                DWORD dwOffset = (i == 0) ? outgoingBufferNotifications[FRAMES_IN_SOUND - 1].dwOffset
                                          : outgoingBufferNotifications[i - 1].dwOffset;
                hr             = dsbOutgoing
                         ->Lock(dwOffset, rakVoice->GetBufferSizeBytes(), &audioPtr, &audioPtrbytes, NULL, NULL, 0);
                RakAssert(audioPtrbytes == rakVoice->GetBufferSizeBytes());
                if (SUCCEEDED(hr)) {
...
void DSoundVoiceAdapter::SetMute(bool mute) { this->mute = mute; }
