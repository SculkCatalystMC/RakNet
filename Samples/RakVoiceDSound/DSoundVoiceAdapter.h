...
Instead of using IDirectSoundBuffer::GetCurrentPosition /
IDirectSoundCaptureBuffer::GetCurrentPosition to keep track of the play/write
cursors, notifications are used instead. Speex encodes/decodes sound in blocks
(frames), so the size of the used DirectSound buffers is multiple of the Speex's
frame size. This makes it easier to implement DirectSound notifications at the
end of each frame.
...
// If you get:
// Error	1	fatal error C1083: Cannot open include file: 'dsound.h':
// No such file or directory It is because this project depends on Directx SDK.
// You must have the DirectX SDK installed.
//  Also, check if you have the DXSDK_DIR environment variable point to the
//  right DX SDK path,
// or change the project settings to point to the right include and lib
// directories
#include "dsound.h"
#include "dxerr.h"
...
public:
    ...
    ///   This function initializes all the required DirectSound objects for you.
    ///   If you already have
    ///  initialized DirectSound on your own, use the other supplied SetupAdater
    ///  function, where you can pass your own DirectSound device object
    /// \param[in] rakVoice RakVoice object to use, fully Initialized AND attached
    /// to a RakPeerInterface.
    /// \param[in] hwnd Your Window Handle. Required for DirectSound
    /// initialization
    /// \param[in] dwCoopLevel DirectSound cooperative level. Required for
    ...
    /// \pre IMPORTANT : Don't forget to initialized and attach rakVoice, before
    /// calling this method.
...
private:
    ...
    IDirectSoundCaptureBuffer8* dsbOutgoing; // DirectSound buffer used for outgoing sound (capture from
                                             // your microphone and send to the other players)
    bool mute;
