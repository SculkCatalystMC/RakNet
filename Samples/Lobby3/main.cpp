...
void main_RakNet(void) {
    ...
    // json_object_set(jsonObject, "__autoFields",
    // json_string("svrTimestamp,svrIP,svrSerial,svrGeoIP"));
    json_object_set(
        jsonObject,
        "__fieldMetadata",
        json_string(
            "sampleField1Key(_ownerRW,_putMin),"
            "sampleField2Key(_userRW,_putSum)"
        )
    );
    ...
    RakString rspost = RakString::FormatForPOST(URI, RakString("text/plain; charset=UTF-8"), jsonStr);

    RakSleep(100);
    SystemAddress serverAddr = tcp->HasCompletedConnectionAttempt();
    RakAssert(serverAddr != UNASSIGNED_SYSTEM_ADDRESS);
    if (useSSL) tcp->StartSSLClient(serverAddr);
