...
// Used by TeamManager to call SetHost() automatically when the current host
// peer drops Also used to determine who the host is for the purposes of
// serializing objects to new systems
FullyConnectedMesh2* fullyConnectedMesh2;
// UDP network communication
RakPeerInterface* rakPeer;
// Maintains pointers to TM_Team and TM_TeamMember, which contain team related
// functionality
...
// A static object is one that already exists on systems before connection, as
// opposed to being created on demand. see Help/replicamanager3.html under the
// topic Static Objects for what to return from the implemented interfaces
...
public:
    ...
    QueryConstruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) {
        if (fullyConnectedMesh2->IsConnectedHost()) return RM3CS_ALREADY_EXISTS_REMOTELY;
    ...
    virtual RakNet::RM3QuerySerializationResult QuerySerialization(RakNet::Connection_RM3* destinationConnection) {
        if (fullyConnectedMesh2->IsConnectedHost()) return RM3QSR_CALL_SERIALIZE;
...
public:
    ...
    virtual void
    SerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        // teamMember must be serialized later than teams. This is accomplished by
        // registering teams first with ReplicaManager3
...
// Required to derive from ReplicaManager3 as a class factory pattern to create
// SampleConnectionRM3
...
// Helper function
// ReplicaManager3 does not add connections until we know who the host is via
// FullyConnectedMesh2 This function takes all FullyConnectedMesh2 connections
// and registers them with ReplicaManager3 once the host is determined.
    ...
    if (fullyConnectedMesh2->GetConnectedHost() != UNASSIGNED_RAKNET_GUID) {
        ...
        for (unsigned int i = 0; i < participantList.Size(); i++) {
            Connection_RM3* connection = replicaManager3->AllocConnection(
                rakPeer->GetSystemAddressFromGuid(participantList[i]),
                participantList[i]
            );
...
int main(void) {
    printf(
        "This project demonstrates an in-game lobby using the team manager "
        "plugin.\n"
    );
    ...
    */
    ...
    // Tell ReplicaManager3 and TeamManager to not automatically add new
    // connections, because we wait for host calculation to complete from
    // FullyConnectedMesh2 first
    ...
    for (unsigned int i = 0; i < TEAM_TYPES_COUNT; i++) {
        ...
        // We serialize teams before team members, this is required by TeamManager
        // during remote object construction. Serialization occurs in the order that
        // Reference() is called on the object
        ...
        teamManager->GetWorldAtIndex(0)
            ->ReferenceTeam(&teams[i].tmTeam, teams[i].GetNetworkID(), balancingAppliesToThisTeam);
        if (i == REFEREE_TEAM) teams[i].tmTeam.SetMemberLimit(1, 0);
        else teams[i].tmTeam.SetMemberLimit(2, 0);
    }
    ...
    sd.port         = 60000;
    while (IRNS2_Berkley::IsPortInUse(sd.port, sd.hostAddress, sd.socketFamily, SOCK_DGRAM) == true) sd.port++;
    ...
    while (!quit) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_NEW_INCOMING_CONNECTION: {
                ...
                // Add mid-game joins to ReplicaManager3 as long as we know who the host
                // is
                if (fullyConnectedMesh2->GetConnectedHost() != UNASSIGNED_RAKNET_GUID) {
            ...
            case ID_FCM2_NEW_HOST: {
                if (packet->guid == rakPeer->GetMyGUID()) printf("Got new host (ourselves)");
                else printf("Got new host %s, GUID=%s", packet->systemAddress.ToString(true), packet->guid.ToString());
                RakNet::BitStream bs(packet->data, packet->length, false);
                bs.IgnoreBytes(1);
                RakNetGUID oldHost;
                bs.Read(oldHost);
                // If the old host is different, then this message was due to losing
                // connection to the host.
                if (oldHost != packet->guid) printf(". Oldhost Guid=%s\n", oldHost.ToString());
                else printf("\n");
        ...
        if (kbhit()) {
            ...
            if (ch == 'B' || ch == 'b') {
                ...
                if (buff1[0] != 0 && buff1[0] >= '0' && buff1[0] <= '2') {
                    success =
                        user->tmTeamMember.RequestTeam(TeamSelection::SpecificTeam(&(teams[buff1[0] - '0'].tmTeam)));
                    printf("Success=%i\n", success);
                } else {
                    printf("Aborted\n");
                }
            ...
            if (ch == 'C' || ch == 'c') {
                ...
                if (buff1[0] != 0 && buff1[0] >= '0' && buff1[0] <= '2'
                    ...
                    if (buff2[0])
                        success = user->tmTeamMember.RequestTeamSwitch(
                            &(teams[buff1[0] - '0'].tmTeam),
                            &teams[buff2[0] - '0'].tmTeam
                        );
                    else success = user->tmTeamMember.RequestTeamSwitch(&(teams[buff1[0] - '0'].tmTeam), 0);
                    printf("Success=%i\n", success);
                } else {
                    printf("Aborted\n");
                }
            ...
            if (ch == 'D' || ch == 'd') {
                ...
                if ((buff1[0] != 0 && buff1[0] >= '0' && buff1[0] <= '2') || buff1[0] == 0) {
                    if (buff1[0]) success = user->tmTeamMember.CancelTeamRequest(&(teams[buff1[0] - '0'].tmTeam));
                    else success = user->tmTeamMember.CancelTeamRequest(0);
                    printf("Success=%i\n", success);
                } else {
                    printf("Aborted\n");
                }
            ...
            if (ch == 'G' || ch == 'g') {
                ...
                gets(buff2);
                if (buff1[0] != 0 && buff1[0] >= '0' && buff1[0] <= '2' && buff2[0] != 0 && buff2[0] >= '0'
                    && buff2[0] <= '9') {
            ...
            if (ch == ' ') {
                if (teamManager->GetWorldAtIndex(0)->GetBalanceTeams()) printf("Team balancing is on\n");
                else printf("Team balancing is off\n");

                for (unsigned int i = 0; i < TEAM_TYPES_COUNT; i++) {
                    printf(
                        "Team %i. %s %i/%i members ",
                        i + 1,
                        teams[i].teamName.C_String(),
                        teams[i].tmTeam.GetTeamMembersCount(),
                        teams[i].tmTeam.GetMemberLimit()
                    );
    ...
    for (unsigned int i = 0; i < TEAM_TYPES_COUNT; i++) {
        // Teams are globally deallocated after NetworkIDManager, so prevent crash
        // on automatic dereference
