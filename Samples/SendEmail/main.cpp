...
int main() {
    ...
    printf(
        "TLS support (such as for Gmail) requires OPEN_SSL_CLIENT_SUPPORT to "
        "be defined\nin RakNetDefines.h.\n"
    );
    ...
    const char*         quote = "Man is distinguished, not only by his reason, but by this singular "
    ...
    if (mailServer[0] == 0) strcpy(mailServer, "smtp.gmail.com");
    printf("Enter email account username: ");
    Gets(senderUsername, sizeof(senderUsername));
    if (senderUsername[0] == 0) strcpy(senderUsername, "subspacegod@gmail.com");
    printf("Enter receiver email address: ");
    Gets(receiver, sizeof(receiver));
    if (receiver[0] == 0) strcpy(receiver, "rakkar@rakkar.org");
    printf("Enter password needed to send: ");
    Gets(password, sizeof(password));

    // http://mail.google.com/support/bin/answer.py?hl=en&answer=13287
    unsigned short hostPort;
    if (strcmp(mailServer, "smtp.gmail.com") == 0) hostPort = 465;
    else hostPort = 25;
    ...
    fileList.AddFile(
        ...
        "quote.txt",
        quote,
        (const unsigned int)strlen(quote),
        (const unsigned int)strlen(quote),
        FileListNodeContext(0, 0, 0, 0),
        false
    );
    const char* sendResult = emailSender.Send(
        mailServer,
        hostPort,
        senderUsername,
        receiver,
    ...
    if (sendResult != 0) printf("Send Failed! %s", sendResult);
    else printf("Success (probably).\n");
