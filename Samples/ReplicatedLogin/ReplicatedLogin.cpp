...
public:
    ...
    DeserializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* sourceConnection) {
        ...
        else printf("Another user created with name %s and score %i\n", username.C_String(), score);
        return true;
    }
    ...
    QueryActionOnPopConnection(RakNet::Connection_RM3* droppedConnection) const {
        if (runningAsServer) return QueryActionOnPopConnection_Server(droppedConnection);
        else return QueryActionOnPopConnection_Client(droppedConnection);
    }
    ...
    virtual void PreDestruction(RakNet::Connection_RM3* sourceConnection) {
        ...
        else printf("Another user destroyed with name %s and score %i\n", username.C_String(), score);
    }
...
int main(void) {
    ...
    char ch = getch();
    if (ch == 's' || ch == 'S') return serverMain();
    else return clientMain();
}
    ...
    while (1) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_CONNECTION_LOST:
                printf(
                    "ID_CONNECTION_LOST from %s,guid=%s\n",
                    packet->systemAddress.ToString(true),
                    packet->guid.ToString()
                );
            ...
            case ID_DISCONNECTION_NOTIFICATION:
                printf(
                    "ID_DISCONNECTION_NOTIFICATION from %s,guid=%s\n",
                    packet->systemAddress.ToString(true),
                    packet->guid.ToString()
                );
                User::DeleteUserWithGuid(packet->guid);
