...
int main(int argc, char** argv) {
    ...
    printf(
        "Important: Requires that you first set the DB schema and the max "
        "packet size on the server.\n"
    );
    ...
    printf(
        "WARNING: MySQL is an order of magnitude slower than "
    ...
    // So only one thread runs per connection, we create an array of connection
    // objects, and tell the autopatcher server to use one thread per item
    ...
    // MySQL is extremely slow in AutopatcherMySQLRepository::GetFilePart so
    // running tho incremental reads in a thread allows multiple concurrent users
    // to read Without this, only one user would be sent files at a time basically
    ...
    // To create the schema, go to the command line client and type create schema
    // autopatcher; You also have to add max_allowed_packet=128M Where 128 is the
    // maximum size file in megabytes you'll ever add to MySQL\MySQL
    // Server 5.1\my.ini in the [mysqld] section Be sure to restart the service
    // after doing so
    Gets(db, sizeof(db));
    if (db[0] == 0) strcpy(db, "autopatcher");
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') break;
                ...
                if (appDir[0] == 0) strcpy(appDir, "C:/temp");
