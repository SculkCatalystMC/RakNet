...
int main(void) {
    ...
    printf(
        "2. Raw (direct socket send) OR Ack (Acknowledgement) OR\nTms "
        "(Timestamped packet),\n"
    );
    printf("3. Message number,\n");
    printf(
        "4. Packet Number (Independent for send & receive).\n(Each Packet may "
        "contain multiple messages),\n"
    );
    printf("5. Packet ID (or a string for RPC calls),\n");
    printf(
        "6. Bits used by the message (does not include 2-4 byte RakNet "
        "header),\n"
    );
