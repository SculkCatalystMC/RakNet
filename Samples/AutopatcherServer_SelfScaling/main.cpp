...
// 2. Connect to the new (or any existing) instance through remote desktop.
// Immediately set max concurrent users to 0. Wait for users to finish patching,
// if any started.
// 3. Update the patch
// 4. Image the server
// 5. Wait for the image to complete (press l to check, or use Rackspace control
// panel. Takes like half an hour).
// 6. When the image completes, set the image spawn from the 'l' option to the
// image that was created in step 4
// 6. Send terminate command. Servers running the old patch will shutdown
// automatically.
...
// Replace this repository with your own implementation if you don't want to use
// PostgreSQL
...
public:
    ...
    ~CloudServerHelper_RackspaceCloudDNS() { RakNet::OP_DELETE(rackspace2, _FILE_AND_LINE_); }
    ...
    void CopyServerDetails(json_t* arrayElement, const char* publicIPV4) {
        ...
        strcpy(
            thisServerDetail.flavorId,
            json_string_value(json_object_get(json_object_get(arrayElement, "flavor"), "id"))
        );
        strcpy(thisServerDetail.hostId, json_string_value(json_object_get(arrayElement, "hostId")));
        strcpy(thisServerDetail.id, json_string_value(json_object_get(arrayElement, "id")));
        strcpy(
            thisServerDetail.imageId,
            json_string_value(json_object_get(json_object_get(arrayElement, "image"), "id"))
        );
    ...
    virtual void OnTransmissionFailed(HTTPConnection2* httpConnection2, RakString postStr, RakString authURLDomain) {
        printf(
            "--ERROR--: OnTransmissionFailed().\npostStr=%s\nauthURLDomain=%s\n",
            postStr.C_String(),
            authURLDomain.C_String()
        );
    ...
    virtual void
    OnMessage(const char* message, RakString responseReceived, RakString stringTransmitted, int contentOffset) {
    ...
    virtual void OnResponse(Rackspace2ResponseCode r2rc, RakString responseReceived, int contentOffset) {
        if (r2rc == R2RC_AUTHENTICATED) {
            ...
            json_t*      root = json_loads(
                strstr(responseReceived.C_String() + contentOffset, "{"),
            ...
            for (i = 0; i < arraySize; i++) {
                ...
                if (j == endpointArraySize) printf("My URL=<unknown>");
                if (myPublicURLStr) {
                    if (strcmp(elementNameStr, "cloudDNS") == 0) strcpy(rackspaceDNSURL, myPublicURLStr);
            ...
            for (i = 0; i < arraySize && cshState == GETTING_SERVERS; i++) {
                ...
                // json_t *publicAddressesArray =
                // json_object_get(json_object_get(arrayElement, "addresses"),"public");
                // size_t publicAddressesArraySize =
                // json_array_size(publicAddressesArray); for (size_t j=0; j <
                // publicAddressesArraySize; j++)
                //{
                for (int k = 0; k < rakPeer->GetNumberOfAddresses(); k++) {
                    if (strcmp(rakPeer->GetLocalIP(k), json_string_value(json_object_get(arrayElement, "accessIPv4")))
                        == 0) {
            ...
            if (cshState == GETTING_SERVERS) {
                // 				printf("Could not find a server with
                // this IP address\n"); 				printf("Continue
                // anyway (y/n)?\n"); 				if (getch()=='y')
                // 				{
            ...
            if (arraySize > 0) {
                ...
                if (str[0]) {
                    ...
                    if (idx >= 1 && idx <= arraySize) {
                        ...
                        // Use spawningFlavorId of whatever it was when the server was
                        // started, I don't care about flavor
                        printf("Using image id %s for new servers\n", spawningImageId);
                    }
                } else printf("Aborted.\n");
            }
                ...
                if (strcmp(elementNameStr, patcherHostSubdomainURL) == 0) {
                    ...
                    if (appState == AP_TERMINATE_IF_NOT_DNS_HOST) {
                        if (strcmp(dnsHostIP, rakPeer->GetLocalIP(0)) == 0) {
                            ...
                            if (timeSinceZeroUsers != 0) Terminate();
                            else appState = AP_TERMINATE_WHEN_ZERO_USERS_REACHED;
                        }
                    }
    ...
    void GetCustomImages(void) {
        // curl -k
        // https://ord.servers.api.rackspacecloud.com/v2/570016/images?type=SNAPSHOT
        // --header "X-Auth-Token: 97d8335b-7b9e-4a74-96a1-2a3a043c1000" --header
    ...
    bool AuthenticateWithRackspaceBlocking(void) {
        ...
        if (cshState != AUTHENTICATED) return false;
        ...
        if (cshState != GOT_RECORDS) return false;

        return true;
    }
    void UpdateHostIPAsynch(SystemAddress sa) {
        if (appState != AP_RUNNING) return;
    ...
    virtual void PrintHelp(void) {
        printf(
            "Distributed authenticated CloudServer using DNS based host "
            "migration.\n"
        );
        ...
        printf("Example:\n");
        printf(
            "AutopatcherServer_SelfScaling s2sPassword 60000 1024 128 "
            "game1.mycompanypatcher.com mycompanypatcher.com "
            "https://identity.api.rackspacecloud.com/v2.0 rackspaceUsername "
            "rackspacePw dbPassword 3 6 1"
        );
    }
    virtual bool ParseCommandLineParameters(int argc, char** argv) {

        if (argc != 14) PrintHelp();
    ...
    virtual int OnJoinCloudResult(
        ...
        if (packet->data[0] == ID_CONNECTION_ATTEMPT_FAILED) {
            ...
            for (unsigned int i = 0; i < rakPeer->GetNumberOfAddresses(); i++) {
                ...
                if (sa.IsLANAddress() == false) {
                    printf(
                        "Connected to self-LAN address. Changing DNS to point to this "
                        "system.\n"
                    );

                    UpdateHostIPAsynch(sa);
                    return CloudServerHelper::OnJoinCloudResult(
                        packet,
                        rakPeer,
                        cloudServer,
                        cloudClient,
...
class AutopatcherLoadNotifier : public RakNet::AutopatcherServerLoadNotifier {
    void AutopatcherLoadNotifier::OnQueueUpdate(
        ...
        // AutopatcherServerLoadNotifier_Printf::OnQueueUpdate(remoteSystem,
        // requestType, queueOperation, autopatcherState);
    }
    ...
    void AutopatcherLoadNotifier::OnGetChangelistCompleted(
        ...
        // AutopatcherServerLoadNotifier_Printf::OnGetChangelistCompleted(remoteSystem,
        // getChangelistResult, autopatcherState);
    }

    void AutopatcherLoadNotifier::OnGetPatchCompleted(
        SystemAddress                                    remoteSystem,
        ...
        // AutopatcherServerLoadNotifier_Printf::OnGetPatchCompleted(remoteSystem,
        // patchResult, autopatcherState);
    }
};

void SetMaxConcurrentUsers(int i) { autopatcherServer->SetMaxConurrentUsers(i); }

// char WORKING_DIRECTORY[MAX_PATH];
// char PATH_TO_XDELTA_EXE[MAX_PATH];

// The default AutopatcherPostgreRepository2 uses bsdiff which takes too much
// memory for large files. I override MakePatch to use XDelta in this case
class AutopatcherPostgreRepository2_WithXDelta : public RakNet::AutopatcherPostgreRepository2 {
    bool
    MakePatch(const char* oldFile, const char* newFile, char** patch, unsigned int* patchLength, int* patchAlgorithm) {
        ...
        if (shortContent || PATH_TO_XDELTA_EXE[0] == 0) {
            if (shortContent == false) {
                printf(
                    "Warning: PATH_TO_XDELTA_EXE needed but not "
                    "set.\ncontentLengthOld=%i\ncontentLengthNew=%i\n",
                    contentLengthOld,
                    contentLengthNew
                );
            }
            // Use bsdiff, which does a good job but takes a lot of memory based on
            ...
            *patchAlgorithm = 1;
            ...
            // ShellExecute is blocking, but if it writes a file to disk that file is
            // not always immediately accessible after it returns. And this only
            // happens in release, and only when not running in the debugger
            // ShellExecute(NULL, "open", PATH_TO_XDELTA_EXE, commandLine,
...
#error RakNet must be compiled with OPEN_SSL_CLIENT_SUPPORT=1 to connect to Rackspace
    ...
    connectionObject =
        RakNet::OP_NEW_ARRAY<AutopatcherPostgreRepository2_WithXDelta>(sqlConnectionObjectCount, _FILE_AND_LINE_);
    // RakNet::AutopatcherRepositoryInterface
    // **connectionObjectAddresses[sqlConnectionObjectCount];
    AutopatcherRepositoryInterface** connectionObjectAddresses =
        RakNet::OP_NEW_ARRAY<AutopatcherRepositoryInterface*>(sqlConnectionObjectCount, _FILE_AND_LINE_);

    for (int i = 0; i < sqlConnectionObjectCount; i++) connectionObjectAddresses[i] = &connectionObject[i];
    autopatcherServer->SetFileListTransferPlugin(&fileListTransfer);
    // PostgreSQL is fast, so this may not be necessary, or could use fewer
    // threads This is used to read increments of large files concurrently,
    // thereby serving users downloads as other users read from the DB
    ...
    RakNet::CloudClient cloudClient;
    // Used to determine the host of the server fully connected mesh, as well as
    // to connect servers automatically
    RakNet::FullyConnectedMesh2 fullyConnectedMesh2;
    // Used for servers to verify each other - otherwise any system could pose as
    // a server Could also be used to verify and restrict clients if paired with
    // the MessageFilter plugin
    RakNet::TwoWayAuthentication twoWayAuthentication;
    ...
    if (!cloudServerHelper->StartRakPeer(rakPeer)) return 1;

    RakNet::CloudServerHelperFilter sampleFilter; // Keeps clients from updating stuff to the server they are
                                                  // not supposed to
    sampleFilter.serverGuid = rakPeer->GetMyGUID();
    cloudServerHelper->SetupPlugins(
        &cloudServer,
        &sampleFilter,
        &cloudClient,
        &fullyConnectedMesh2,
        &twoWayAuthentication,
        &connectionGraph2,
        cloudServerHelper->serverToServerPassword
    );

    int ret;
    do {
        ret = cloudServerHelper->JoinCloud(
            rakPeer,
            &cloudServer,
            &cloudClient,
            &fullyConnectedMesh2,
            &twoWayAuthentication,
            &connectionGraph2,
            cloudServerHelper->GetCloudHostAddress()
        );
    } while (ret == 2);
    if (ret == 1) return 1;
    ...
    while (connectionSuccess == false) {
        ...
        if (conIdx == sqlConnectionObjectCount) connectionSuccess = true;
    }

    printf("Database connection suceeded.\n");
    printf("Starting threads\n");
    // 4 Worker threads, which is CPU intensive
    // A greater number of SQL connections, which read files incrementally for
    // large downloads
    ...
    printf(
        "(D)rop database\n(C)reate database.\n(U)pdate revision.\n(S)pawn a "
        "clone of this server\n(M)ax concurrent users (this server "
    ...
    while (appState != AP_TERMINATED) {
        ...
        if (autopatcherLoad == 0) {
            ...
            if (timeSinceZeroUsers == 0) timeSinceZeroUsers = RakNet::GetTime();
        } else {
        ...
        while (p) {
            ...
            p->systemAddress.ToString(true)); else if
            (p->data[0]==ID_DISCONNECTION_NOTIFICATION)
                    printf("ID_DISCONNECTION_NOTIFICATION (TCP) from %s\n",
            p->systemAddress.ToString(true)); else if (p->data[0]==ID_CONNECTION_LOST)
                    printf("ID_CONNECTION_LOST (TCP) from %s\n",
            p->systemAddress.ToString(true)); else
            */
            if (p->data[0] == ID_FCM2_NEW_HOST) {
                if (appState == AP_RUNNING) {
                    ...
                    if (p->guid == rakPeer->GetMyGUID()) {
                        if (oldHost != UNASSIGNED_RAKNET_GUID) {
                            ...
                            printf(
                                "ID_FCM2_NEW_HOST: We have become host for the first "
                                "time.\n"
                            );
                        }
                    }
                ...
                if (incomingPw == cloudServerHelper->serverToServerPassword) {
                    ...
                    if (appState == AP_RUNNING) {
                        if (timeSinceZeroUsers != 0) cloudServerHelper->Terminate();
                        else appState = AP_TERMINATE_WHEN_ZERO_USERS_REACHED;
                    }

                    // Disconnect from the cloud, and do not accept new connections from
                    // users The autopatcher will still run since it is on TCP
                ...
                for (rowIndex = 0; rowIndex < cloudQueryResult.rowsReturned.Size(); rowIndex++) {
                    ...
                    bsIn.Read(connCount);
                    printf(
                        "%i. Server found at %s with %i connections\n",
                        rowIndex + 1,
                        row->serverSystemAddress.ToString(true),
                        connCount
                    );
                ...
                if (
                    available < 0
                    && cloudQueryResult.rowsReturned.Size() < MAX_SERVERS_EVER // no more than MAX_SERVERS_EVER servers
                                                                               // ever, to control costs
                ) {
                    int newServersNeeded = -available / sqlConnectionObjectCount;
                    if (newServersNeeded > 4) newServersNeeded = 4; // Do not start more than 4 servers at a time
                    ...
                    if (newServersNeeded > 0) cloudServerHelper->SpawnServers(newServersNeeded);
                }
        ...
        if (timeSinceZeroUsers != 0 && appState == AP_RUNNING && autopatcherServer->GetMaxConurrentUsers() > 0) {
            ...
            if (diff > 0) {
                if (fullyConnectedMesh2.IsHostSystem() == false) {
                    // 2. Shutdown automatically if no users for 12 hours, except if we
                    // are the host
                    if (diff > 1000 * 60 * 60 * 12) cloudServerHelper->Terminate();
                } else {
                    // We are host according to RakPeer. But no users for 24 hours
                    // Verify we are the host according to Rackspace DNS records. If not,
                    // shutdown
                    if (diff > 1000 * 60 * 60 * 24) {
                        // No users for 24 hours, verify if we are the host according to
                        // Rackspace DNS records. If not, shutdown
                        RakNet::Time diff2;
        ...
        // 4. If we are the host, every 1 hour check load among all servers
        // (including self). It takes like 30 minutes to build a new server.
        if (fullyConnectedMesh2.IsHostSystem() && autopatcherServer->GetMaxConurrentUsers() > 0) {
            ...
            if (curTime > timeForNextLoadCheck) {
                ...
                // If the load exceeds such that >=1 new fully loaded server is needed,
                // add that many servers See ID_CLOUD_GET_RESPONSE
                RakNet::CloudQuery cloudQuery;
                cloudQuery.keys.Push(
                    RakNet::CloudKey("CloudConnCount", 0),
                    _FILE_AND_LINE_
                ); // CloudConnCount is defined at the top of
                   // CloudServerHelper.cpp
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') break;
                ...
                printf(
                    "Image this server?\nWill clone this server so that future "
                    "servers made from this image will use its current "
                    "state.\nPress 'y' to image.\n"
                );
                if (getch() == 'y') {
                ...
                if (appDir[0] == 0) strcpy(appDir, "D:/temp");
