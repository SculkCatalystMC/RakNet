...
/*
What is being done here is having 8 peers all connect to eachother over the max
defined connection.
...
int MaximumConnectTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    for (int i = 0; i < peerNum; i++) {
        ...
        if (connReturn != maxConnections) {
            if (isVerbose) {
                printf(
                    "Getmaxconnections wrong for peer %i, %i should be the value "
                    "but the value is %i.Fail\n",
                    i,
                    maxConnections,
                    connReturn
                );
...
RakString MaximumConnectTest::GetTestName() { return "MaximumConnectTest"; }
