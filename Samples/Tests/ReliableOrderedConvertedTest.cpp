...
void* ReliableOrderedConvertedTest::LoggedMalloc(size_t size, const char* file, unsigned int line) {
    ...
    if (fp) fprintf(fp, "Alloc %s:%i %i bytes %i total\n", file, line, size, memoryUsage);
...
void ReliableOrderedConvertedTest::LoggedFree(void* p, const char* file, unsigned int line) {
    ...
    if (fp) fprintf(fp, "Free %s:%i %i bytes %i total\n", file, line, allocatedSize, memoryUsage);
    free(realP);
}
void* ReliableOrderedConvertedTest::LoggedRealloc(void* p, size_t size, const char* file, unsigned int line) {
    ...
    memcpy(p, &size, sizeof(size));
    if (fp) fprintf(fp, "Realloc %s:%i %i to %i bytes %i total\n", file, line, allocatedSize, size, memoryUsage);
...
int ReliableOrderedConvertedTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    */
    ...
    if (isVerbose) printf("Connecting...\n");
    ...
    if (isVerbose) printf("Waiting for connections...\n");
    ...
    // while (1)
    {
        ...
        while (packet) {
            ...
            case ID_CONNECTION_REQUEST_ACCEPTED:
                if (isVerbose) printf("ID_CONNECTION_REQUEST_ACCEPTED\n");
                doSend   = true;
                nextSend = currentTime;
                break;
            case ID_NO_FREE_INCOMING_CONNECTIONS:
                if (isVerbose) printf("ID_NO_FREE_INCOMING_CONNECTIONS\n");
                break;
            case ID_DISCONNECTION_NOTIFICATION:
                if (isVerbose) printf("ID_DISCONNECTION_NOTIFICATION\n");
                break;
            case ID_CONNECTION_LOST:
                if (isVerbose) printf("ID_CONNECTION_LOST\n");
                break;
            case ID_CONNECTION_ATTEMPT_FAILED:
                if (isVerbose) printf("Connection attempt failed\n");
                break;
            }
        ...
        while (doSend && currentTime > nextSend) {
            ...
            // if (sender->Send(&bitStream, HIGH_PRIORITY, (PacketReliability)
            // (RELIABLE + (randomMT() %2)) ,streamNumber, UNASSIGNED_SYSTEM_ADDRESS,
            // true)==false)
            if (sender->Send(
                    &bitStream,
                    HIGH_PRIORITY,
        ...
        while (packet) {
            ...
            case ID_NEW_INCOMING_CONNECTION:
                if (isVerbose) printf("ID_NEW_INCOMING_CONNECTION\n");
                break;
            case ID_DISCONNECTION_NOTIFICATION:
                if (isVerbose) printf("ID_DISCONNECTION_NOTIFICATION\n");
                break;
            ...
            case ID_USER_PACKET_ENUM + 1:
                ...
                if (receivedPacketNumberReceiver != packetNumberReceiver[streamNumberReceiver]) {
                    ...
                    // WARNING: If you modify the below code make sure the whole string
                    // remains in bounds, sprintf will NOT do it for you. The error string
                    // is 512 in length

                    // Note: Removed buffer checking because chance is insignificant, left
                    // code if wanted in future. Needs limits.h ISO C standard.
                    ...
                    if (strlen(lastError)>maxIntCharLen* 3 +27)//512 should be a good len
                    for now
                    {*/
                    ...
                    {
                    sprintf(lastError,"Did not get what was expected. More details can be
                    given if the error string buffer size is increased.");
...
RakString ReliableOrderedConvertedTest::GetTestName() { return "ReliableOrderedConvertedTest"; }
