...
/*
Description: Tests what happens if two instances of RakNet connect to each other
...
int CrossConnectionConvertTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    while (GetTimeMS() - entryTime < 10000) // Run for 10 Secoonds
    {
        ...
        for (p = server->Receive(); p; server->DeallocatePacket(p), p = server->Receive()) {
            ...
            if (p->data[0] == ID_NEW_INCOMING_CONNECTION) {
                ...
                if (isVerbose) printf("ID_UNCONNECTED_PONG\n");
                TimeMS    sendPingTime;
                BitStream bs(p->data, p->length, false);
                bs.IgnoreBytes(1);
                bs.Read(sendPingTime);
                TimeMS rtt = GetTimeMS() - sendPingTime;
                if (rtt / 2 <= 500) connectionAttemptTime = GetTimeMS() + 1000 - rtt / 2;
                else connectionAttemptTime = GetTimeMS();
                gotNotification = false;
            }
        }
        ...
        if (connectionAttemptTime != 0 && GetTimeMS() >= connectionAttemptTime) {
            ...
            if (isVerbose) printf("Attemping connection\n");
    ...
    if (isVerbose) printf("Test succeeded.\n");

    return 0;
}

RakString CrossConnectionConvertTest::GetTestName() { return "CrossConnectionConvertTest"; }
