...
int main(int argc, char* argv[]) {
    ...
    DataStructures::List<int>            testsToRunIndexes; // A list of tests to run by
                                                            // index
    ...
    if (testsToRun.Size() == 0 && testsToRunIndexes.Size() == 0) {
        ...
        for (int i = 0; i < testListSize; i++) {
            ...
            if (returnVal == 0) {
                ...
                printf(
                    "Test %s returned with error %s",
                    testList[i]->GetTestName().C_String(),
                    testList[i]->ErrorCodeToString(returnVal).C_String()
                );
            }
        }
    }

    if (testsToRun.Size() != 0) // If string arguments convert to indexes.Suggestion: if speed becoms
                                // an issue keep a sorted list and binary search it
    {
            ...
            if (testsToRunIndexes[i] < testListSize) {
                ...
                if (returnVal == 0) {
                    ...
                    printf(
                        "Test %s returned with error %s",
                        testList[testsToRunIndexes[i]]->GetTestName().C_String(),
                        testList[testsToRunIndexes[i]]->ErrorCodeToString(returnVal).C_String()
                    );
                }
            }
        }
