...
/*
Description: Does a little bit of everything forever. This is an internal sample
just to see if RakNet crashes or leaks memory over a long period of time.
...
int ComprehensiveConvertTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    //	DebugTools::ShowError("Note: The conversion of this is on hold until the
    // original sample's problem is known.",!noPauses &&
    // isVerbose,__LINE__,__FILE__);
    ...
    while (GetTimeMS() < endTime) {
        ...
        if (nextAction < .04f) {
            ...
            if ((randomMT() % NUM_PEERS) == 0) target = UNASSIGNED_SYSTEM_ADDRESS;
            else target = peers[peerIndex]->GetSystemAddressFromIndex(randomMT() % NUM_PEERS);
            ...
            sprintf(
                data + 3,
                "dataLength=%i priority=%i reliability=%i orderingChannel=%i "
                "target=%i broadcast=%i\n",
                dataLength,
                priority,
                reliability,
                orderingChannel,
            ...
            // peers[peerIndex]->RPC(RPCName, data, dataLength*8, priority,
            // reliability, orderingChannel, target, broadcast, 0,
            ...
            if (rss) {
                ...
                if (isVerbose) printf("Statistics for local system %i:\n%s", mySystemAddress.port, data);
            }

            rss = peers[peerIndex]->GetStatistics(target);
            if (rss) {
                StatisticsToString(rss, data, 0);
                if (isVerbose) printf("Statistics for target system %i:\n%s", target.port, data);
            }
        }

        for (i = 0; i < NUM_PEERS; i++) peers[i]->DeallocatePacket(peers[i]->Receive());
...
void ComprehensiveConvertTest::DestroyPeers() {
    ...
    for (int i = 0; i < NUM_PEERS; i++) RakPeerInterface::DestroyInstance(peers[i]);
}

RakString ComprehensiveConvertTest::GetTestName() { return "ComprehensiveConvertTest"; }
