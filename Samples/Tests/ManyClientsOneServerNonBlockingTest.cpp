...
/*
What is being done here is having 256 clients connect to one server, disconnect,
connect again.
...
int ManyClientsOneServerNonBlockingTest::RunTest(
    ...
    while (GetTimeMS() - entryTime < 10000) // Run for 10 Secoonds
    {
        ...
        if (isVerbose && packet) printf("For server\n");
        ...
        for (int i = 0; i < clientNum; i++) // Receive for all peers
        {
            ...
            if (isVerbose && packet) printf("For peer %i\n", i);
    ...
    for (int i = 0; i < clientNum; i++) {
        ...
        if (connNum != 1)              // Did we connect to all?
        {
            ...
            if (isVerbose) DebugTools::ShowError("", !noPauses && isVerbose, __LINE__, __FILE__);
...
RakString ManyClientsOneServerNonBlockingTest::GetTestName() { return "ManyClientsOneServerNonBlockingTest"; }
