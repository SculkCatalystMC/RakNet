...
bool CommonFunctions::WaitAndConnect(
    ...
    while (!CommonFunctions::ConnectionStateMatchesOptions(peer, connectToAddress, true)
...
void CommonFunctions::DisconnectAndWait(RakPeerInterface* peer, char* ip, unsigned short int port) {
    ...
    while (
        CommonFunctions::ConnectionStateMatchesOptions(peer, targetAddress, true, true, true, true)
    ) // disconnect client
    {
