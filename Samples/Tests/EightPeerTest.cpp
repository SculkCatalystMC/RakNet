...
connected. Then it check if they all connect. If so send data in ordered
reliable mode for 100 loops.
...
shouldn't be significant at this number but the recieve speed it part of the
test.
...
int EightPeerTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    int               connectionAmount[peerNum];      // Counter for me to keep track of connection
                                                      // requests and accepts
    int recievedFromList[peerNum][peerNum];           // Counter for me to keep track of
                                                      // packets received
    int lastNumberReceivedFromList[peerNum][peerNum]; // Counter for me to keep track of
    ...
    for (int k = 0;
         k < numPackets || GetTimeMS() - finishTimer < 5000;) // Quit after we send 100 messages while connected,
                                                              // if not all connected and not failure, otherwise
                                                              // fail after 20 seconds and exit
    {
        ...
        if (allConnected) {
            if (!initialConnectOver) initialConnectOver = true;
            if (k < numPackets) {
        ...
        if (k >= numPackets - 3) // This is our last 3 packets, give it time to send packet
                                 // and arrive on interface, 2 seconds is more than enough
        {
            RakSleep(300);
            if (k == numPackets) {
        ...
        {
            if (allConnected) // If all connected try to make the data more visually
                              // appealing by bunching it in one receive
            {
                int waittime = 0;
                do {
                    ...
                    if (waittime > 1000) // Check for packet every millisec and if one second has
                                         // passed move on, don't block execution
                    {
                        break;
                    }
                } while (!packet); // For testing purposes wait for packet a little
                                   // while, go if not recieved
            } else // Otherwise just keep recieving quickly until connected
            {
                packet = peerList[i]->Receive();
            }
            if (isVerbose) printf("For peer %i with %i connected peers.\n", i, connectionAmount[i]);
            while (packet) {
                ...
                case ID_REMOTE_NEW_INCOMING_CONNECTION:
                    if (isVerbose) printf("Another client has connected.\n");
                    break;
                case ID_CONNECTION_REQUEST_ACCEPTED:
                    if (isVerbose) printf("Our connection request has been accepted.\n");
                    connectionAmount[i]++;
                ...
                case ID_NEW_INCOMING_CONNECTION:
                    if (isVerbose) printf("A connection is incoming.\n");
                    connectionAmount[i]++; // For this test assume connection. Test will
                                           // fail if connection fails.
                    break;
                case ID_NO_FREE_INCOMING_CONNECTIONS: // Should not happend
                    if (isVerbose) {
                        printf(
                            "The server is full. This shouldn't happen in this test "
                            "ever.\n"
                        );
                ...
                case ID_ALREADY_CONNECTED:
                    if (isVerbose) printf("Already connected\n"); // Shouldn't happen
                ...
                default:
                    ...
                    if (packet->data[0] == ID_USER_PACKET_ENUM + 1) {
                        ...
                        if (isVerbose) printf("Message %i from %i\n", sequenceNum, thePeerNum);
    ...
    for (int i = 0; i < peerNum; i++) {
        ...
        for (int j = 0; j < peerNum; j++) {
            if (i != j) {
                if (isVerbose) printf("%i recieved %i packets from %i\n", i, recievedFromList[i][j], j);
                if (recievedFromList[i][j] != numPackets) {
                    if (isVerbose) {
                        printf(
                            "Not all packets recieved. it was in reliable ordered mode "
                            "so that means test failed or wait time needs increasing\n"
                        );
...
RakString EightPeerTest::GetTestName() { return "EightPeerTest"; }
