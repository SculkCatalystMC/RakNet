...
void PeerConnectDisconnectTest::WaitAndPrintResults(RakPeerInterface** peerList, int peerNum, bool isVerbose) {
    ...
    {
        if (isVerbose) printf("For peer %i\n", i);
...
/*
What is being done here is having 8 peers all connect to eachother, disconnect,
connect again.
...
int PeerConnectDisconnectTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    const int peerNum        = 8;
    const int maxConnections = peerNum * 3; // Max allowed connections for test set to times 3 to
                                            // eliminate problem variables
    ...
    for (int i = 0; i < peerNum; i++) {
        ...
        for (int j = i + 1; j < peerNum; j++) // Start at i+1 so don't connect two of the same together.
        {
                ...
                )) // Are we connected or is there a pending operation ?
            {
                ...
                if (peerList[i]->Connect("127.0.0.1", 60000 + j, 0, 0) != CONNECTION_ATTEMPT_STARTED) {
                    peerList[i]->GetSystemList(systemList, guidList); // Get
                                                                      // connectionlist
                    int len = systemList.Size();
                ...
                if (CommonFunctions::ConnectionStateMatchesOptions(
                    ...
                    == false)
                    printf(
                        "Not calling Connect() for peer %i to peer %i because it is "
                        "disconnecting.\n",
                        i,
                        j
                    );
                else if (CommonFunctions::ConnectionStateMatchesOptions(peerList[i], currentSystem, false, true, true)
                         == false)
                    printf(
                        "Not calling Connect() for peer %i to peer %i because it is "
                        "connecting.\n",
...
RakString PeerConnectDisconnectTest::GetTestName() { return "PeerConnectDisconnectTest"; }
