...
Tests:
virtual void RakPeerInterface::AddToSecurityExceptionList  	(  	const
char *   	 ip  	 ) virtual void RakPeerInterface::AddToBanList (
const char *   	 IP, 		TimeMS  	milliseconds = 0 ) virtual void
RakPeerInterface::GetIncomingPassword  	(  	char *   	 passwordData,
int *  	passwordDataLength	  	) virtual void
RakPeerInterface::InitializeSecurity  	(  	const char *   	 pubKeyE,
const char *  	pubKeyN, 		const char *  	privKeyP,
const char *  	privKeyQ	  	) virtual bool
RakPeerInterface::IsBanned  	(  	const char *   	 IP  	 ) virtual bool
RakPeerInterface::IsInSecurityExceptionList  	(  	const char *   	 ip )
virtual void RakPeerInterface::RemoveFromSecurityExceptionList  	(
const char *   	 ip  	 ) virtual void RakPeerInterface::RemoveFromBanList (
const char *   	 IP  	 ) virtual void RakPeerInterface::SetIncomingPassword (
const char *   	 passwordData, 		int  	passwordDataLength ) virtual
void 	ClearBanList (void)=0
...
int SecurityFunctionsTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    returnedPass[returnedLen] = 0; // Password is a data block convert to null
                                   // terminated string to make the test easier
    ...
    if (isVerbose) printf("Testing if  no password is rejected\n");
    ...
    if (isVerbose) printf("Testing if incorrect password is rejected\n");
    ...
    if (isVerbose) printf("Testing if correct password is accepted\n");
    ...
    while (
        CommonFunctions::ConnectionStateMatchesOptions(client, serverAddress, true, true, true, true)
    ) // disconnect client
    {
    ...
    if (isVerbose) printf("Testing if connection is rejected after adding to ban list\n");
    ...
    if (isVerbose)
        printf(
            "Testing if connection is accepted after ban removal by "
            "RemoveFromBanList\n"
        );
    ...
    (client,serverAddress,true)&&GetTimeMS()-entryTime<5000)
            {
    ...
    (client,serverAddress,true,true,true,true))
            ...
            client->Send(str2,(int) strlen(str2)+1, HIGH_PRIORITY,
    RELIABLE_ORDERED ,0, UNASSIGNED_SYSTEM_ADDRESS, true); client->Send(str2,(int)
    strlen(str2)+1, HIGH_PRIORITY, RELIABLE_ORDERED ,0, UNASSIGNED_SYSTEM_ADDRESS,
    true);
            ...
            {
                    for (packet=server->Receive();
    packet;server->DeallocatePacket(packet), packet=server->Receive())
                    {
            ...
            if (rss->encryptionBitsSent<=0)//If we did connect encrypted we should
    see encryptionBitsSent
            {
                    if (isVerbose)
                            DebugTools::ShowError("Client did not connect
    encrypted\n",!noPauses && isVerbose,__LINE__,__FILE__); return 10;
            }
    ...
    (client,serverAddress,true,true,true,true))//disconnect client
            ...
            if (isVerbose)
                    printf("Testing AddToSecurityExceptionList client should
    connect without encryption\n");
            ...
            {
                    if (isVerbose)
                            DebugTools::ShowError("IsInSecurityExceptionList does
    not register localhost addition\n",!noPauses && isVerbose,__LINE__,__FILE__);
                    return 12;
            }
    ...
    see encryptionBitsSent
            {
                    if (isVerbose)
                            DebugTools::ShowError("Client connected encrypted but
    shouldn't have\n",!noPauses && isVerbose,__LINE__,__FILE__); return 11;
            }
...
RakString SecurityFunctionsTest::GetTestName() { return "SecurityFunctionsTest"; }
