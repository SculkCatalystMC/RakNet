...
void ManyClientsOneServerDeallocateBlockingTest::WaitForConnectionRequestsToComplete(
...
Connect function returns false and peer is not connected to anything,pending a
connection, or disconnecting.
...
int ManyClientsOneServerDeallocateBlockingTest::RunTest(
    ...
    if (isVerbose) printf("Entering disconnect loop \n");
...
void ManyClientsOneServerDeallocateBlockingTest::DestroyPeers() {
    ...
    for (int i = 0; i < clientNum; i++) RakPeerInterface::DestroyInstance(clientList[i]);
