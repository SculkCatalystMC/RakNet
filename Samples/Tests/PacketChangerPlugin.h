...
public:
    ...
    void OnFailedConnectionAttempt(Packet* packet, PI2_FailedConnectionAttemptReason failedConnectionAttemptReason) {
    ...
    /// \param[in] bitsUsed How many bits long \a data is
    /// \param[in] remoteSystemAddress Which system this message is being sent to
    void OnDirectSocketReceive(const char* data, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress) {
        (void)data;
        (void)bitsUsed;
        (void)remoteSystemAddress;
    }
    ...
    void OnInternalPacket(
        ...
        TimeMS          time,
        int             isSend
    ); // {(void) internalPacket; (void) frameNumber; (void)
       // remoteSystemAddress; (void) time; (void) isSend;}
    ...
    /// \param[in] remoteSystemAddress The player we sent or got this packet from
    /// \param[in] time The current time as returned by GetTimeMS()
    void OnAck(unsigned int messageNumber, SystemAddress remoteSystemAddress, TimeMS time) {
        (void)messageNumber;
        (void)remoteSystemAddress;
        (void)time;
    }
    ...
    /// \param[in] bitsUsed How many bits long \a data is
    /// \param[in] remoteSystemAddress The player we sent or got this packet from
    void OnPushBackPacket(const char* data, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress) {
        (void)data;
        (void)bitsUsed;
        (void)remoteSystemAddress;
    }
