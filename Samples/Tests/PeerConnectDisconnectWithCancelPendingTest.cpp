...
Also tests nonblocking connects, the simpler one PeerConnectDisconnect tests
without it
...
int PeerConnectDisconnectWithCancelPendingTest::RunTest(
    ...
    while (GetTimeMS() - entryTime < 10000) // Run for 10 Secoonds
    {
        ...
        for (int i = 0; i < peerNum; i++) {
            ...
            for (int j = i + 1; j < peerNum; j++) // Start at i+1 so don't connect two of the same together.
            {
                ...
                peerList[i]->CancelConnectionAttempt(currentSystem); // Make sure a connection is not pending before
                                                                     // trying to connect.
            }
        }
            ...
            for (int j = i + 1; j < peerNum; j++) // Start at i+1 so don't connect two of the same together.
            {
                ...
                if (peerList[i]->Connect("127.0.0.1", 60000 + j, 0, 0) != CONNECTION_ATTEMPT_STARTED) {
                    ...
                    if (CommonFunctions::ConnectionStateMatchesOptions(
                            ...
                            true,
                            true
                        )) // Did we drop the pending connection?
                    {
                        if (isVerbose)
    ...
    for (int i = 0; i < peerNum; i++) {
        ...
        for (int j = i + 1; j < peerNum; j++) // Start at i+1 so don't connect two of the same together.
        {
            ...
            peerList[i]->CancelConnectionAttempt(currentSystem); // Make sure a connection is not pending before trying
                                                                 // to connect.
