...
Ping is tested in CrossConnectionConvertTest,SetOfflinePingResponse and
GetOfflinePingResponse tested in OfflineMessagesConvertTest
...
RakPeerInterface Functions used, tested indirectly by its use, not all
encompassing list: Startup SetMaximumIncomingConnections Receive
DeallocatePacket
...
int PingTestsTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    sender2->SetOccasionalPing(false); // Test the lowest ping and such without
                                       // occassionalping,occasional ping comes later
    RakTimer timer(1500);
    ...
    if (isVerbose) printf("Average Ping time %i\n", averagePing);

    lastPing   = sender2->GetLastPing(currentSystem);
    lowestPing = sender2->GetLowestPing(currentSystem);

    if (isVerbose) printf("Last Ping time %i\n", lastPing);

    if (isVerbose) printf("Lowest Ping time %i\n", lowestPing);
    ...
    if (lowestPing > 10) // The lowest ping for localhost should drop below 10MS at least once
    {
        ...
        if (isVerbose)
            DebugTools::ShowError(
                "The lowest ping for localhost should drop below "
                "10MS at least once\n",
                !noPauses && isVerbose,
                __LINE__,
                __FILE__
    ...
    if (lastPing < lowestPing) {
        ...
        if (isVerbose)
            DebugTools::ShowError(
                "There is a problem if the lastping is lower than "
                "the lowestping stat\n",
                !noPauses && isVerbose,
                __LINE__,
                __FILE__
    ...
    CommonFunctions::DisconnectAndWait(sender2, "127.0.0.1",
                                       60000); // Eliminate variables.
...
int PingTestsTest::TestAverageValue(int averagePing, int line, bool noPauses, bool isVerbose) {
    ...
    if (averagePing < 0) {
        ...
        if (isVerbose)
            DebugTools::ShowError(
                "Problem with the average ping time,should never "
                "be less than zero in this test\n",
                !noPauses && isVerbose,
                line,
                __FILE__
            );
    ...
    if (averagePing > 10) // Average Ping should not be greater than 10MS for
                          // localhost. Command line pings typically give < 1ms
    {

        if (isVerbose)
            DebugTools::ShowError(
                "Average Ping should not be greater than 10MS for localhost. Command "
                "line pings typically give < 1ms\n",
                !noPauses && isVerbose,
                line,
                __FILE__
            );
...
RakString PingTestsTest::GetTestName() { return "PingTestsTest"; }
...
RakString PingTestsTest::ErrorCodeToString(int errorCode) {
    ...
    case 1:
        return "Problem with the average ping time,should never be less than 0 in "
               "this test";
        break;
    ...
    case 5:
        return "There is a problem if the lastping is lower than the lowestping "
               "stat";
        break;

    case 6:
        return "Average Ping should not be greater than 10MS for localhost. "
               "Command line pings typically give < 1ms";
        break;
