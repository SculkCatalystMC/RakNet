...
void ManyClientsOneServerBlockingTest::WaitAndPrintResults(
    ...
    if (isVerbose) printf("For server\n");

    for (packet = server->Receive(); packet; server->DeallocatePacket(packet), packet = server->Receive()) {
        switch (packet->data[0]) {
        case ID_REMOTE_DISCONNECTION_NOTIFICATION:
            if (isVerbose) printf("Another client has disconnected.\n");

            break;
        case ID_REMOTE_CONNECTION_LOST:
            if (isVerbose) printf("Another client has lost the connection.\n");
        ...
        case ID_CONNECTION_ATTEMPT_FAILED:
            if (isVerbose) printf("A connection has failed.\n");
        ...
        case ID_NO_FREE_INCOMING_CONNECTIONS:
            if (isVerbose) printf("The server is full.\n");
        ...
        case ID_DISCONNECTION_NOTIFICATION:
            if (isVerbose) printf("We have been disconnected.\n");
            break;
        case ID_CONNECTION_LOST:
            if (isVerbose) printf("Connection lost.\n");
    ...
    {
        if (isVerbose) printf("For client %i\n", i);

        for (packet = clientList[i]->Receive(); packet;
             clientList[i]->DeallocatePacket(packet), packet = clientList[i]->Receive()) {
            switch (packet->data[0]) {
            case ID_REMOTE_DISCONNECTION_NOTIFICATION:
                if (isVerbose) printf("Another client has disconnected.\n");
...
/*
What is being done here is having 256 clients connect to a server, disconnect,
connect again.

Do this for about 10 seconds. Then allow them all to connect for one last time.

This version waits for connect and such in a loop, blocking execution so it is a
blocking test.
...
Connect function returns false and peer is not connected to anything and does
not have anything pending.
...
int ManyClientsOneServerBlockingTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    for (int i = 0; i < clientNum; i++) {
            ...
            )) // Are we connected or is there a pending operation ?
        {
            ...
            if (clientList[i]->Connect("127.0.0.1", 60000, 0, 0) != CONNECTION_ATTEMPT_STARTED) {
                clientList[i]->GetSystemList(systemList, guidList); // Get
                                                                    // connectionlist
                int len = systemList.Size();
            ...
            if (CommonFunctions::ConnectionStateMatchesOptions(clientList[i], currentSystem, false, false, false, true)
                == false)
                printf(
                    "Not calling Connect() for client %i because it is "
                    "disconnecting.\n",
                    i
                );
            else if (CommonFunctions::ConnectionStateMatchesOptions(clientList[i], currentSystem, false, true, true)
                     == false)
    ...
    if (isVerbose) printf("Pass\n");
    return 0;
}

RakString ManyClientsOneServerBlockingTest::GetTestName() { return "ManyClientsOneServerBlockingTest"; }
