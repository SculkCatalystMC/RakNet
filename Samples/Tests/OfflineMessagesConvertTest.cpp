...
/*
Description:
Description: Tests / Demonstrates sending messages to systems you are not
connected to.
...
int OfflineMessagesConvertTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    if (isVerbose) printf("Peer 1 guid = %s\n", peer1->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).ToString());
    if (isVerbose) printf("Peer 2 guid = %s\n", peer2->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).ToString());
    if (isVerbose) printf("Systems started.  Waiting for advertise system packet\n");
    ...
    while (nextTest != 2 && GetTimeMS() - entryTime < 10000) // run for 10 seconds
    {
        ...
        if (packet) {
            if (packet->data[0] == ID_ADVERTISE_SYSTEM) {
                if (packet->length > 1) {
                    if (isVerbose) printf("Got Advertise system with data: %s\n", packet->data + 1);
                ...
                if (isVerbose) printf("Was sent from GUID %s\n", packet->guid.ToString());
                if (isVerbose)
                    printf(
                        "Sending ping from %s\n",
                        peer2->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).ToString()
                ...
                if (peer2->IsLocalIP(packet->systemAddress.ToString(false))) {
                    if (isVerbose) printf("ID_UNCONNECTED_PONG from our own");
                } else {
                    if (isVerbose) printf("ID_UNCONNECTED_PONG from");
                }
                if (isVerbose) {
                    printf(
...
RakString OfflineMessagesConvertTest::GetTestName() { return "OfflineMessagesConvertTest"; }
