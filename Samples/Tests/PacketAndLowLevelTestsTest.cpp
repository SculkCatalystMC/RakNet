...
virtual int RakPeerInterface::GetSplitMessageProgressInterval  	(  	void
)   	 const " virtual void RakPeerInterface::PushBackPacket  	(
Packet *   	 packet, 		bool  	pushAtHead	  	)
" virtual bool RakPeerInterface::SendList  	(  	char **   	 data,
const int *  	lengths, 		const int  	numParameters,
PacketPriority  	priority, 		PacketReliability
reliability, 		char  	orderingChannel, 		SystemAddress
systemAddress, 		bool  	broadcast	  	) " virtual void
RakPeerInterface::SetSplitMessageProgressInterval 	( 	int  	interval
) virtual void RakPeerInterface::SetUnreliableTimeout  	(  	TimeMS
timeoutMS  	 ) virtual Packet* RakPeerInterface::AllocatePacket  	(
unsigned   	 dataSize  	 ) AttachPlugin (PluginInterface2 *plugin)=0
...
int PacketAndLowLevelTestsTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    if (!(packet = CommonFunctions::WaitAndReturnMessageWithID(server, ID_USER_PACKET_ENUM + 1, 1000))) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[9 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (packet->length != 25) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[13], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    /*//Couldn't find a good cross platform way for allocated memory so skipped
    this check if (somemalloccheck<3000000)
    {}
    */
    ...
    while (
        CommonFunctions::WaitForMessageWithID(server, ID_DOWNLOAD_PROGRESS, 500)
    ) // Clear out the rest before next test
    {}
    ...
    if (!TestHelpers::WaitForTestPacket(server, 5000)) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[12], !noPauses && isVerbose, __LINE__, __FILE__);
...
RakString PacketAndLowLevelTestsTest::GetTestName() { return "PacketAndLowLevelTestsTest"; }
...
PacketAndLowLevelTestsTest::PacketAndLowLevelTestsTest(void) {
    ...
    errorList.Push(
        "Large packet did not split or did not properly get ID_DOWNLOAD_PROGRESS "
        "after SetSplitMessageProgressInterval is set to 1 millisecond",
        _FILE_AND_LINE_
    );
    errorList.Push(
        "Did not recieve and put on packet made with AllocatePacket "
        "and put on recieve stack with PushBackPacket",
        _FILE_AND_LINE_
    );
    errorList.Push("Client failed to connect to server", _FILE_AND_LINE_);
