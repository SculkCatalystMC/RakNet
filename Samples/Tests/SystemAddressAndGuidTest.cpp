...
Description:
Tests:
virtual unsigned short RakPeerInterface::NumberOfConnections  	(  	void
)   	 const virtual void RakPeerInterface::GetSystemList  	(
DataStructures::List< SystemAddress > &   	 addresses,
DataStructures::List< RakNetGUID > &  	guids	  	) virtual bool
RakPeerInterface::IsActive  	(  	void   	  	 )   	 const virtual
SystemAddress RakPeerInterface::GetSystemAddressFromIndex  	(  	int
index  	 ) virtual SystemAddress RakPeerInterface::GetSystemAddressFromGuid (
const RakNetGUID   	 input  	 )   	 const virtual const RakNetGUID&
RakPeerInterface::GetGuidFromSystemAddress  	(  	const SystemAddress
input  	 )   	 const pure virtual  virtual RakNetGUID
RakPeerInterface::GetGUIDFromIndex  	(  	int   	 index  	 )
virtual SystemAddress RakPeerInterface::GetExternalID  	(  	const
SystemAddress   	 target  	 )   	 const
...
Both SystemList and Number of connections have problems and report different
results Both SystemList and Number of connections have problems and report same
results Undefined Error System address from list is wrong. Guid from list is
wrong GetSystemAddressFromIndex failed to return correct values
...
RakPeerInterface Functions used, tested indirectly by its use. List may not be
complete: Startup SetMaximumIncomingConnections Receive DeallocatePacket Send
IsConnected
...
int SystemAddressAndGuidTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    if (client->IsActive()) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[1 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (!client->IsActive()) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[2 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (!TestHelpers::WaitAndConnectTwoPeersLocally(client, server, 5000)) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[3 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (len2 != len) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[4 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (conNum != len) {
        ...
        if (conNum == 1 || len == 1) {
            ...
            if (conNum != 1) {
                ...
                if (isVerbose) DebugTools::ShowError(errorList[5 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
            ...
            if (len != 1) {
                ...
                if (isVerbose) DebugTools::ShowError(errorList[6 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
            ...
            if (isVerbose) DebugTools::ShowError(errorList[7 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
        ...
        if (conNum != 1) {
            ...
            if (isVerbose) DebugTools::ShowError(errorList[8 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (!compareSystemAddresses(systemList[0], serverAddress)) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[10 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (guidList[0] != serverGuid) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[11 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (!compareSystemAddresses(client->GetSystemAddressFromIndex(0), serverAddress)) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[12 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (!compareSystemAddresses(client->GetSystemAddressFromGuid(serverGuid), serverAddress)) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[13 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (client->GetGuidFromSystemAddress(serverAddress) != serverGuid) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[14 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    if (client->GetGUIDFromIndex(0) != serverGuid) {
        ...
        if (isVerbose) DebugTools::ShowError(errorList[15 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
    ...
    printf(
        "Test GetExternalID, automatic testing is not only required for "
        "this\nbecause of it's nature\nShould be supplemented by internet "
        "tests\n"
    );

    if (!compareSystemAddresses(client->GetExternalID(serverAddress), clientAddress)) {

        if (isVerbose) DebugTools::ShowError(errorList[16 - 1], !noPauses && isVerbose, __LINE__, __FILE__);
...
RakString SystemAddressAndGuidTest::GetTestName() { return "SystemAddressAndGuidTest"; }
...
SystemAddressAndGuidTest::SystemAddressAndGuidTest(void) {
    ...
    errorList.Push(
        "Both SystemList and Number of connections have problems and "
...
void SystemAddressAndGuidTest::DestroyPeers() {
    ...
    for (int i = 0; i < theSize; i++) RakPeerInterface::DestroyInstance(destroyList[i]);
}