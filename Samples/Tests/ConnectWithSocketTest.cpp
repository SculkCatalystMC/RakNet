...
/*
Description:
virtual bool RakPeerInterface::ConnectWithSocket  	(  	const char *
host, 		unsigned short  	remotePort, 		const char *
passwordData, 		int  	passwordDataLength, 		RakNetSmartPtr<
RakNetSocket >  	socket, 		unsigned
sendConnectionAttemptCount = 7, 		unsigned
timeBetweenSendConnectionAttemptsMS = 500, 		TimeMS
timeoutTime = 0	  	)

virtual void RakPeerInterface::GetSockets  	(  	DataStructures::List<
RakNetSmartPtr< RakNetSocket > > &   	 sockets  	 ) virtual
RakNetSmartPtr<RakNetSocket> RakPeerInterface::GetSocket  	(  	const
SystemAddress   	 target  	 )   	 [pure virtual]
...
RakString ConnectWithSocketTest::GetTestName() { return "ConnectWithSocketTest"; }
...
ConnectWithSocketTest::ConnectWithSocketTest(void) {
    ...
    errorList.Push(
        "Client did not connect after 5 secods Using ConnectWithSocket, could be "
        "GetSockets or ConnectWithSocket problem",
        _FILE_AND_LINE_
    );
    errorList.Push("Server did not recieve test packet from client", _FILE_AND_LINE_);
    errorList.Push(
        "Client did not connect after 5 secods Using ConnectWithSocket, could be "
        "GetSocket or ConnectWithSocket problem",
        _FILE_AND_LINE_
    );
    errorList.Push("Server did not recieve test packet from client", _FILE_AND_LINE_);
}
