...
Tests silently dropping multiple instances of RakNet. This is used to test that
...
Connect function fails and there is no pending operation and there is no current
connection with server. Random timout detection fails.
...
int DroppedConnectionConvertTest::RunTest(DataStructures::List<RakString> params, bool isVerbose, bool noPauses) {
    ...
    for (index = 0; index < NUMBER_OF_CLIENTS; index++) {
        ...
        RakSleep(1000);
        if (isVerbose) printf("%i. ", index);
    }

    TimeMS entryTime = GetTimeMS(); // Loop entry time

    int seed = 12345;
    if (isVerbose) printf("Using seed %i\n", seed);
    ...
    while (GetTimeMS() - entryTime < 30000) // run for 30 seconds.
    {
        ...
        case 0: {
            ...
            if (numberOfSystems == 0) {
                if (isVerbose) printf("Client %i silently closing inactive connection.\n", index);
            } else {
                if (isVerbose) printf("Client %i silently closing active connection.\n", index);
            }
        }
        ...
        case 1: {
            ...
            if (!CommonFunctions::ConnectionStateMatchesOptions(
                    ...
                    true,
                    true
                )) // Are we connected or is there a pending operation ?
            {
                if (clients[index]->Connect("127.0.0.1", serverPort, 0, 0) != CONNECTION_ATTEMPT_STARTED) {
            ...
            if (numberOfSystems == 0) {
                if (isVerbose) printf("Client %i connecting to same existing connection.\n", index);

            } else {
                if (isVerbose) printf("Client %i connecting to closed connection.\n", index);
            }
        }
        ...
        case 2: {
            ...
            if (isVerbose) printf("Randomly connecting and disconnecting each client\n");
            for (index = 0; index < NUMBER_OF_CLIENTS; index++) {
                if (NUMBER_OF_CLIENTS == 1 || (randomMT() % 2) == 0) {
                    if (clients[index]->IsActive()) {

                        int randomTest2 = randomMT() % 2;
                        if (randomTest2) clients[index]->CloseConnection(serverID, false, 0);
                        else clients[index]->CloseConnection(serverID, true, 0);
                    }
                } else {
                    if (!CommonFunctions::ConnectionStateMatchesOptions(
                            clients[index],
        ...
        case 3: {
            ...
            if (isVerbose) printf("Testing if clients dropped after timeout.\n");
            timeoutWaitTimer.Start();
            // Wait half the timeout time, the other half after receive so we don't
            // drop all connections only missing ones, Active ait so the threads run
            // on linux
        ...
        server->GetConnectionList(0, &numberOfSystems);
        numberOfSystems2 = numberOfSystems;
        if (isVerbose) printf("The server thinks %i clients are connected.\n", numberOfSystems);
        connectionCount = 0;
        for (index = 0; index < NUMBER_OF_CLIENTS; index++) {
            clients[index]->GetConnectionList(0, &numberOfSystems);
            if (numberOfSystems > 1) {
        ...
        if (isVerbose) printf("%i clients are actually connected.\n", connectionCount);
        if (isVerbose) printf("server->NumberOfConnections==%i.\n", server->NumberOfConnections());
        ...
        while (1) {
            ...
            if (p) {
                ...
                case ID_NEW_INCOMING_CONNECTION:
                    ...
                    if (isVerbose) printf("%i: ID_NEW_INCOMING_CONNECTION from %i.\n", sender, p->systemAddress.port);
                    break;

                case ID_CONNECTION_LOST:
                    // Couldn't deliver a reliable packet - i.e. the other system was
                    // abnormally terminated
                    if (isVerbose) printf("%i: ID_CONNECTION_LOST from %i.\n", sender, p->systemAddress.port);
                    break;
            ...
            if (sender == NUMBER_OF_CLIENTS) server->DeallocatePacket(p);
            else clients[sender]->DeallocatePacket(p);
        }
        if (dropTest) {
        ...
        ch=255;
        server->Send((char*)&ch, 1, HIGH_PRIORITY, RELIABLE, 0,
        UNASSIGNED_SYSTEM_ADDRESS, true);

        for (index=0; index < NUMBER_OF_CLIENTS; index++)
        clients[index]->Send((char*)&ch, 1, HIGH_PRIORITY, RELIABLE, 0,
        UNASSIGNED_SYSTEM_ADDRESS, true);
        */
...
RakString DroppedConnectionConvertTest::GetTestName() { return "DroppedConnectionConvertTest"; }
