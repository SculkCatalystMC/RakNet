...
void main_sockets(void) {
    ...
    const char* postRequest = "POST /testServer HTTP/1.1\r\n"
                              "Content-Length: 83\r\n"
                              "Content-Type: text/plain; charset=UTF-8\r\n"
                              "Host: masterserver2.raknet.com\r\n"
                              "Connection: Keep-Alive\r\n"
                              "\r\n"
                              "{'__gameId': 'myGame','__clientReqId': "
                              "'0','__timeoutSec': '60','mapname': 'myMap'}\r\n";
...
void main_RakNet_Post(void) {
    ...
    // json_object_set(jsonObject, "mapname",
    // json_string(RakString::NonVariadic("Joué-lés-tours").URLEncode().C_String())
    // );
...
void main_RakNet_Get(void) {
    ...
    RakString rsRequest = RakString::FormatForGET(RakString(MASTER_SERVER_ADDRESS "/testServer?__gameId=MotoGP_13"));
                        ...
                        while (iter) {
                            ...
                            if (stricmp(firstKey, "GET") == 0) {
                                ...
                                for (unsigned int i = 0; i < arraySize; i++) {
                                    ...
                                    // RakString val =
                                    // RakString::NonVariadic(json_string_value(cityNameVal)).URLDecode();

                                    printf("gameId: %i.\n", json_integer_value(gameIdVal));
                                }

                                if (arraySize == 0) printf("No results.\n");
