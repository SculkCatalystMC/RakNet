...
int main(void) {
    ...
    TestCommandServer(&tt, 23, rakPeer); // Uncomment to use Telnet as a client.
                                         // Telnet uses port 23 by default.

    // 	RakNet::RakNetTransport2 rt2;
    // 	rakPeer->AttachPlugin(&rt2);
    // 	TestCommandServer(&rt2, 60000,rakPeer); // Uncomment to use RakNet as a
    // client
...
void TestCommandServer(RakNet::TransportInterface* ti, unsigned short port, RakNet::RakPeerInterface* rakPeer) {
    ...
    printf(
        "a standalone application or part of your game server.  It allows you "
        "to\n"
    );
    printf(
        "easily parse text strings sent from a client using either secure "
        "RakNet\n"
    );
    printf("or Telnet.\n");
