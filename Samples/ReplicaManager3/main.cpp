...
// Demonstrates ReplicaManager 3: A system to automatically create, destroy, and
// serialize objects
...
enum { CLIENT, SERVER, P2P } topology;
...
struct SampleReplica : public Replica3 {
    ...
    SerializeConstruction(RakNet::BitStream* constructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        ...
        // variableDeltaSerializer is a helper class that tracks what variables were
        // sent to what remote system This call adds another remote system to track
    ...
    SerializeDestruction(RakNet::BitStream* destructionBitstream, RakNet::Connection_RM3* destinationConnection) {
        ...
        // variableDeltaSerializer is a helper class that tracks what variables were
        // sent to what remote system This call removes a remote system
    ...
    virtual RM3SerializationResult Serialize(SerializeParameters* serializeParameters) {
        ...
        // Put all variables to be sent unreliably on the same channel, then specify
        // the send type for that channel
        serializeParameters->pro[0].reliability = UNRELIABLE_WITH_ACK_RECEIPT;
        // Sending unreliably with an ack receipt requires the receipt number, and
        // that you inform the system of ID_SND_RECEIPT_ACKED and
        // ID_SND_RECEIPT_LOSS
        ...
        // Same as above, all variables to be sent with a particular reliability are
        // sent in a batch We use BeginIdenticalSerialize instead of BeginSerialize
        // because the reliable variables have the same values sent to all systems.
        // This is memory-saving optimization
        ...
        // This return type makes is to ReplicaManager3 itself does not do a memory
        // compare. we entirely control serialization ourselves here. Use
        // RM3SR_SERIALIZED_ALWAYS instead of RM3SR_SERIALIZED_ALWAYS_IDENTICALLY to
        // support sending different data to different system, which is needed when
        // using unreliable and dirty variable resends
    ...
    virtual void Deserialize(RakNet::DeserializeParameters* deserializeParameters) {
        ...
        // Note that the Serialize() call above uses two different reliability
        // types. This results in two separate Send calls So Deserialize is
        // potentially called twice from a single Serialize
        variableDeltaSerializer.BeginDeserialize(
        ...
        variableDeltaSerializer.BeginDeserialize(
            &deserializationContext,
            &deserializeParameters->serializationBitstream[1]
        );
    ...
    void NotifyReplicaOfMessageDeliveryStatus(RakNetGUID guid, uint32_t receiptId, bool messageArrived) {
        // When using UNRELIABLE_WITH_ACK_RECEIPT, the system tracks which variables
        // were updated with which sends So it is then necessary to inform the
        // system of messages arriving or lost Lost messages will flag each variable
        // sent in that update as dirty, meaning the next Serialize() call will
        // resend them with the current values
    ...
    // We manually test each variable to the last synchronized value and only send
    // those values that change
    int var1Unreliable, var2Unreliable, var3Reliable, var4Reliable;

    // Class to save and compare the states of variables this Serialize() to the
    // last Serialize() If the value is different, true is written to the
    // bitStream, followed by the value. Otherwise false is written. It also
    // tracks which variables changed which Serialize() call, so if an unreliable
    // message was lost (ID_SND_RECEIPT_LOSS) those variables are flagged 'dirty'
    // and resent
...
struct ClientCreatible_ServerSerialized : public SampleReplica {
    ...
    virtual RM3SerializationResult Serialize(SerializeParameters* serializeParameters) {
        if (topology == CLIENT) return RM3SR_DO_NOT_SERIALIZE;
        return SampleReplica::Serialize(serializeParameters);
    }
    virtual RM3ConstructionState
    QueryConstruction(RakNet::Connection_RM3* destinationConnection, ReplicaManager3* replicaManager3) {
...
public:
    ...
    // See documentation - Makes all messages between
    // ID_REPLICA_MANAGER_DOWNLOAD_STARTED and
    ...
    while (!quit) {
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_ADVERTISE_SYSTEM:
                // The first conditional is needed because ID_ADVERTISE_SYSTEM may be
                // from a system we are connected to, but replying on a different
                // address. The second conditional is because AdvertiseSystem also sends
                // to the loopback
            ...
            case ID_SND_RECEIPT_ACKED: {
                ...
                for (idx = 0; idx < replicaListOut.Size(); idx++) {
                    ((SampleReplica*)replicaListOut[idx])
                        ->NotifyReplicaOfMessageDeliveryStatus(
                            packet->guid,
                            msgNumber,
        ...
        if (kbhit()) {
            ...
            if (ch == 'd' || ch == 'D') {
                ...
                DataStructures::List<Replica3*> replicaListOut;
                // The reason for ClearPointers is that in the sample, I don't track
                // which objects have and have not been allocated at the application
                // level. So ClearPointers will call delete on every object in the
                // returned list, which is every object that the application has
                // created. Another way to put it is 	A. Send a packet to tell other
                // systems to delete these objects 	B. Delete these objects on my
                // own system
                replicaManager.GetReplicasCreatedByMe(replicaListOut);
