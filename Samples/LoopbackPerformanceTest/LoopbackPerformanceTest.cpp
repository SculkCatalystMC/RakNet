...
int main(void) {
    ...
    if (byteBlock[0] == 0) {
        ...
        quitTime = 90;
    } else quitTime = atoi((char*)byteBlock);
    ...
    if (byteBlock[0] == 0) {
        ...
        verbosityLevel = 1;
    } else verbosityLevel = atoi((char*)byteBlock);
    ...
    } else showStatsInterval = atoi((char*)byteBlock) * 1000;
    ...
    if (systemType == 0) {
        ...
        printf("Initializing Raknet...\n");
        // Relay.  Accept one connection, initiate outgoing connection, wait for
        ...
        printf(
            "Initialization complete. Relay system started.\nConnecting to "
            "destination and waiting for sender...\n"
        );
    } else {
...
#endif
        } else packetsPerSecond = atoi((char*)byteBlock);
        ...
        if (byteBlock[0] == 0) {
            ...
            if (bytesPerPacket > 4096) {
                printf(
                    "Increase the array size of byteBlock to send more than 4096 "
                    "bytes.\n"
                );
                bytesPerPacket = 4096;
            }
        ...
        printf(
            "Initialization complete. Sender system started. Connecting to "
            "relay...\n"
        );
    }
    ...
    while (time < quitTime || (connectionCompleted == false && incomingConnectionCompleted == false)) {
        ...
        while (1) {
            ...
            if (p) {
                ...
                case ID_CONNECTION_REQUEST_ACCEPTED:
                    ...
                    if (systemType == 1 || systemType == 2) quitTime = quitTime * 1000 + time;
                    break;
                ...
                case ID_NEW_INCOMING_CONNECTION:
                    ...
                    if (systemType == 0) quitTime = quitTime * 1000 + time;
                    if (systemType == 1 && connectionCompleted == false)
                        printf(
                            "Warning, relay connection to destination has not completed "
                            "yet.\n"
                        );
                    break;
        ...
        // As the destination, we don't care if the connection is completed.  Do
        // nothing As the relay, we relay packets if the connection is completed.
        ...
        if (systemType == 2 && connectionCompleted) {
            ...
            // Number of packets to send is (float)(packetsPerSecond * (time -
            // lastSendTime)) / 1000.0f;
