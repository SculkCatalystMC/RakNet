...
// pathToGame should be something like "C:\Games\mygame", whatever the
// installation path was gameName should be patcherHostSubdomainURL found in
// AutopatcherServer_SelfScaling patchImmediately should be 1 portToStartOn
// should be 0 serverPort should be 60000, see LISTEN_PORT_TCP_PATCHER in
// AutopatcherServer_SelfScaling fullScan should be '1' to fully scan all files.
// 0 to use the last patch.
...
public:
    virtual bool OnFile(OnFileStruct* onFileStruct) {
        if (onFileStruct->context.op == PC_HASH_1_WITH_PATCH || onFileStruct->context.op == PC_HASH_2_WITH_PATCH)
            printf("Patched: ");
        else if (onFileStruct->context.op == PC_WRITE_FILE) printf("Written: ");
        else if (onFileStruct->context.op == PC_ERROR_FILE_WRITE_FAILURE) printf("Write Failure: ");
        else if (onFileStruct->context.op == PC_ERROR_PATCH_TARGET_MISSING) printf("Patch target missing: ");
        else if (onFileStruct->context.op == PC_ERROR_PATCH_APPLICATION_FAILURE) printf("Patch process failure: ");
        else if (onFileStruct->context.op == PC_ERROR_PATCH_RESULT_CHECKSUM_FAILURE) printf("Patch checksum failure: ");
        else if (onFileStruct->context.op == PC_NOTICE_WILL_COPY_ON_RESTART) printf("Copy pending restart: ");
        else if (onFileStruct->context.op == PC_NOTICE_FILE_DOWNLOADED) printf("Downloaded: ");
        else if (onFileStruct->context.op == PC_NOTICE_FILE_DOWNLOADED_PATCH) printf("Downloaded Patch: ");
        else RakAssert(0);
    ...
    virtual void OnFileProgress(FileProgressStruct* fps) {
        printf(
            "Downloading: %i. (%i%%) %i/%i %s %ib/%ib %ib/%ib total\n",
            fps->onFileStruct->setID,
            (int)(100.0 * (double)fps->onFileStruct->bytesDownloadedForThisFile
                  / (double)fps->onFileStruct->byteLengthOfThisFile),
    ...
    virtual PatchContext ApplyPatchBase(
        const char*   oldFilePath,
        char**        newFileContents,
        unsigned int* newFileSize,
        char*         patchContents,
        unsigned int  patchSize,
        uint32_t      patchAlgorithm
    ) {
        if (patchAlgorithm == 0) {
            ...
            if (fpPatch == 0) return PC_ERROR_PATCH_TARGET_MISSING;
            ...
            _snprintf(
                ...
                "-d -f -s %s %s/patchClient_%s.tmp %s/newFile_%s.tmp",
                oldFilePath,
                WORKING_DIRECTORY,
                buff,
                WORKING_DIRECTORY,
            ...
            // // ShellExecute is blocking, but if it writes a file to disk that file
            // is not always immediately accessible after it returns. And this only
            // happens in release, and only when not running in the debugger
            // ShellExecute(NULL, "open", "xdelta3-3.0.6-win32.exe", commandLine,
            // NULL, SW_SHOWNORMAL);
            ...
            *newFileContents = (char*)rakMalloc_Ex(*newFileSize, _FILE_AND_LINE_);
            ...
            while ((unlinkRes1 != 0 || unlinkRes2 != 0) && RakNet::GetTimeUS() < stopWaiting) {
                RakSleep(1000);
                if (unlinkRes1 != 0) unlinkRes1 = _unlink(pathToPatch1);
                if (unlinkRes2 != 0) unlinkRes2 = _unlink(pathToPatch2);
            }
...
int main(int argc, char** argv) {
    if (argc < 8) {
        printf(
            "Arguments: serverIP, pathToGame, gameName, patchImmediately, "
            "localPort, serverPort, fullScan"
        );
        return 0;
    }
    ...
    bool didRebalance = false; // So we only reconnect to a lower load server
                               // once, for load balancing
    ...
    while (1) {
        ...
        if (notificationAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("ID_NEW_INCOMING_CONNECTION\n");
        notificationAddress = packetizedTCP.HasLostConnection();
        if (notificationAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("ID_CONNECTION_LOST\n");
        ...
        while (p) {
            ...
            // this application immediately after to unlock files.\n");
            ...
            if (p->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) {
                ...
                for (rowIndex = 0; rowIndex < cloudQueryResult.rowsReturned.Size(); rowIndex++) {
                    ...
                    if (wasCallToGetServers) {
                        ...
                        else connectionsExcludingOurselves = connCount;
                ...
                if (didRebalance == false && wasCallToGetServers) {
                    if (cloudQueryResult.rowsReturned.Size() > 0 && connectionsOnOurServer > lowestConnectionsServer) {
                        ...
                        // Wait for the thread to close, otherwise will immediately get back
                        // ID_CONNECTION_ATTEMPT_FAILED because no free outgoing connection
                        // slots Alternatively, just call Startup() with 2 slots instead of
                        // 1
                        RakSleep(500);

                        rakPeer
                            ->Connect(lowestConnectionAddress.ToString(false), lowestConnectionAddress.GetPort(), 0, 0);
        ...
        if (TCPServerAddress != RakNet::UNASSIGNED_SYSTEM_ADDRESS && patchImmediately == true) {
            ...
            if (fullScan == false) {
                ...
                if (fp) {
                    ...
                    fclose(fp);
                } else lastUpdateDate = 0;
            } else lastUpdateDate = 0;

            if (autopatcherClient.PatchApplication(
                    appName,
                    appDir,
                    lastUpdateDate,
                    TCPServerAddress,
...
void UploadInstanceToCloud(RakNet::CloudClient* cloudClient, RakNet::RakNetGUID serverGuid) {
    ...
    RakNet::BitStream bs;
    bs.Write("Hello World"); // This could be anything such as player list, game
                             // name, etc.
...
void GetServers(RakNet::CloudClient* cloudClient, RakNet::RakNetGUID serverGuid) {
    ...
    cloudQuery.keys.Push(
        ...
        _FILE_AND_LINE_
    ); // CloudConnCount is defined at the top
       // of CloudServerHelper.cpp
    cloudQuery.subscribeToResults = false;
    cloudClient->Get(&cloudQuery, serverGuid);
}
