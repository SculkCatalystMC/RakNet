...
class DBResultHandler : public RankingServerCBInterface {
    virtual void SubmitMatch_CB(SubmitMatch_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("SubmitMatch call canceled:\n");
    ...
    virtual void
    GetRatingForParticipant_CB(GetRatingForParticipant_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("GetRatingForParticipant call cancelled:\n");
        else if (callResult->dbQuerySuccess == false) {
            ...
            if (callResult->participantFound) printf("[out] rating=%f\n", callResult->rating);
        }

        printf("\n");
    }
    virtual void
    GetRatingForParticipants_CB(GetRatingForParticipants_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("GetRatingForParticipants call canceled:\n");
        else if (callResult->dbQuerySuccess == false) {
            ...
            for (i = 0; i < callResult->outputList.Size(); i++) {
                printf(
                    "[out] %i. participantDbId.primaryKey=%i\n",
                    i + 1,
                    callResult->outputList[i].participantDbId.primaryKey
                );
                printf(
                    "[out] %i. participantDbId.secondaryKey=%i\n",
                    i + 1,
                    callResult->outputList[i].participantDbId.secondaryKey
                );
    ...
    virtual void
    GetHistoryForParticipant_CB(GetHistoryForParticipant_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("GetHistoryForParticipant call cancelled:\n");
        else if (callResult->dbQuerySuccess == false) {
            ...
            for (i = 0; i < callResult->matchHistoryList.Size(); i++) {
                printf(
                    "[out] %i. participantBDbId.primaryKey (opponent) =%i\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantBDbId.primaryKey
                );
                printf(
                    "[out] %i. participantBDbId.secondaryKey (opponent) =%i\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantBDbId.secondaryKey
                );
                printf(
                    "[out] %i. participantAScore (us) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantAScore
                );
                printf(
                    "[out] %i. participantBScore (opponent) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantBScore
                );
                printf(
                    "[out] %i. participantAOldRating (us) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantAOldRating
                );
                printf(
                    "[out] %i. participantANewRating (us) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantANewRating
                );
                printf(
                    "[out] %i. participantBOldRating (opponent) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantBOldRating
                );
                printf(
                    "[out] %i. participantBNewRating (opponent) =%f\n",
                    i + 1,
                    callResult->matchHistoryList[i]->participantBNewRating
                );
                printf("[out] %i. Match Notes=%s\n", i + 1, callResult->matchHistoryList[i]->matchNotes.c_str());
                printf(
                    "[out] %i. matchBinaryDataLength=%i\n",
                    i + 1,
                    callResult->matchHistoryList[i]->matchBinaryDataLength
                );
...
void main(int argc, char** argv) {
    ...
    printf(
        ...
        "history of matches for a given participant and game ID\n"
    );
    ...
    // The real functionality of the RankingServer is contained in the functors
    // defined in RankingServer_PostgreSQL.h/.cpp. However, this class contains
    // some utility functions and members, such as the functionThread and a
    // pointer to the postgreSQL interface.
    RankingServer_PostgreSQL rankingServer;

    // RankingServer_PostgreSQL internally uses a functionThread so that database
    ...
    // from the *_PostgreSQLImpl functors found in RankingServer_PostgreSQL.h/.cpp
    ...
    while (1) {
        if (kbhit()) {
            ...
            if (ch == 'q') break;
                ...
                if (rankingServer.CreateRankingServerTables() == false) printf("%s", rankingServer.GetLastError());
                else printf("Tables created.\n");
            } else if (ch == 'd') {
                if (rankingServer.DestroyRankingServerTables() == false) printf("%s", rankingServer.GetLastError());
                ...
                /// The primary ID can be used to reference a database primary key
                /// referring to another table holding all participant The secondary ID
                /// can be used to indicate type such as indicating the type of
                /// participant (player, squad, guild, clan, etc).
                ...
                // Score is tracked in case you want to check the database for cheaters,
                // or to modify ratings, etc. If you don't care about it, just use any
                // number.
                ...
                // The rating of the player before the match started. Elo.h/.cpp is
                // provided as one way to calculate ratings
                ...
                /// Games are identified by a pair of integers the same way participants
                /// are The primary key could be the game title (such as Asteroids)
                /// while the secondary key could Indicate a submode (such as
                /// cooperative or deathmatch)
                ...
                if (inputStr[0] != 'n') {
                    ...
                    // As before, calling a function in the external code in case it was
                    // built to a DLL
