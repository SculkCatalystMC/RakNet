...
int main(void) {
    ...
    printf(
        "A client / server sample showing how clients can broadcast offline "
        "packets\n"
    );
    ...
    printf(
        ...
        "client and it will get pongs from those servers.\n"
    );
    ...
    if (str[0] == 's' || str[0] == 'S') {
        ...
        if (serverPort[0] == 0) strcpy(serverPort, "60001");
        ...
        if (b) printf("Server started, waiting for connections.\n");
        else {
        ...
        if (clientPort[0] == 0) strcpy(clientPort, "60000");
    ...
    } else quitTime = RakNet::GetTimeMS() + atoi(str) * 1000;

    // Loop for input
    while (RakNet::GetTimeMS() < quitTime) {
        if (server) p = server->Receive();
        else p = client->Receive();

        if (p == 0) {
            RakSleep(30);
            continue;
        }
        if (server) server->DeallocatePacket(p);
        else {
    ...
    if (server) RakNet::RakPeerInterface::DestroyInstance(server);
    if (client) RakNet::RakPeerInterface::DestroyInstance(client);
