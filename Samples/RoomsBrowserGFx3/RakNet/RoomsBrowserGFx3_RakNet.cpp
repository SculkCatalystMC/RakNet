...
void ServerAndRoomBrowserData::WriteToTable(DataStructures::Table* table) {
    ...
    ); // "Room name" from RoomTypes.cpp, write
       // this here?
    cells[3].Set(roomName.C_String());
    // The rooms plugin uses this table, and the rooms plugin automatically
    // assigns a room id
    //	table->AddColumn(DefaultRoomColumns::GetColumnName(DefaultRoomColumns::TC_ROOM_ID),
    // DataStructures::Table::NUMERIC); 	cells[4].Set((double) roomId.g);
    RakAssert(table->GetColumnCount() == 4);
    DataStructures::List<DataStructures::Table::Cell*> initialCellValues;
    for (int i = 0; i < 4; i++) initialCellValues.Push(&cells[i], _FILE_AND_LINE_);
...
RoomsBrowserGFx3_RakNet::~RoomsBrowserGFx3_RakNet() { Shutdown(); }
void RoomsBrowserGFx3_RakNet::Init(
    RakNet::Lobby2Client*         _lobby2Client,
...
void RoomsBrowserGFx3_RakNet::Shutdown(void) { RoomsBrowserGFx3::Shutdown(); }
...
void RoomsBrowserGFx3_RakNet::LoadProperty(const char* propertyId, RakNet::RakString& propertyOut) {
    XMLNode xMainNode = XMLNode::openFileHelper(pathToXMLPropertyFile.C_String(), "");
    if (xMainNode.isEmpty()) return;
    XMLNode xNode = xMainNode.getChildNode(propertyId);
    LPCTSTR attr  = xNode.getAttribute("value", 0);
    if (attr) propertyOut = attr;
    else propertyOut.Clear();
}
void RoomsBrowserGFx3_RakNet::OnClosedConnection(
...
void RoomsBrowserGFx3_RakNet::OnNewConnection(
    const SystemAddress& systemAddress,
    RakNetGUID           rakNetGUID,
    bool                 isIncoming
) {
...
void RoomsBrowserGFx3_RakNet::OnFailedConnectionAttempt(
    Packet*                           packet,
    ...
    else FxDelegate::Invoke2(movie, "c2f_NotifyFailedConnectionAttempt", rargs);
}
PluginReceiveResult RoomsBrowserGFx3_RakNet::OnReceive(Packet* packet) {
    switch (packet->data[0]) {
    case ID_UNCONNECTED_PONG:
        if (
            packet->systemAddress.GetPort() >= lanServerPort && packet->systemAddress.GetPort() < lanServerPort + 8
            && packet->length > sizeof(RakNet::TimeMS) + sizeof(MessageID)
            // &&
            ...
            // This was set in RakPeerInterface::SetOfflinePingResponse, by using
            // ServerWANUpdate::SetAsOfflinePingResponse()
...
ACTIONSCRIPT_CALLABLE_FUNCTION(RoomsBrowserGFx3_RakNet, f2c_RegisterAccount) {
    RakNet::Client_RegisterAccount* arg =
        (RakNet::Client_RegisterAccount*)msgFactory->Alloc(L2MID_Client_RegisterAccount);
    arg->userName                         = pparams[0].GetString();
    loginUsername                         = arg->userName;
...
ACTIONSCRIPT_CALLABLE_FUNCTION(RoomsBrowserGFx3_RakNet, f2c_UpdateRoomsList) {
    ...
    if (isNetUpdate) {
        ...
        // Ping out looking for game servers. We can't find servers unless they all
        // use the same port Up to 8 instances on the same machine is reasonable
        for (int i = 0; i < 8; i++) {
...
ACTIONSCRIPT_CALLABLE_FUNCTION(RoomsBrowserGFx3_RakNet, f2c_JoinByFilter) {
    if (pparams.GetArgCount() < 1) return;
...
ACTIONSCRIPT_CALLABLE_FUNCTION(RoomsBrowserGFx3_RakNet, f2c_CreateRoom) {
    ...
    if (pparams[4].GetBool() == true) // Room members can invite
        func.networkedRoomCreationParameters.inviteToRoomPermission =
            NetworkedRoomCreationParameters::INVITE_MODE_ANYONE_CAN_INVITE;
    else
        func.networkedRoomCreationParameters.inviteToRoomPermission =
            NetworkedRoomCreationParameters::INVITE_MODE_MODERATOR_CAN_INVITE;
    func.networkedRoomCreationParameters.inviteToSpectatorSlotPermission =
        NetworkedRoomCreationParameters::INVITE_MODE_ANYONE_CAN_INVITE;
    ...
    browserData.maxPlayers = func.networkedRoomCreationParameters.slots.publicSlots
                           + func.networkedRoomCreationParameters.slots.reservedSlots;
...
ACTIONSCRIPT_CALLABLE_FUNCTION(RoomsBrowserGFx3_RakNet, f2c_SetRoomLockState) {
    ...
    if (lockState == "NOT_LOCKED") func.roomLockState = RLS_NOT_LOCKED;
    else if (lockState == "PLAYERS_LOCKED") func.roomLockState = RLS_PLAYERS_LOCKED;
    else if (lockState == "ALL_LOCKED") func.roomLockState = RLS_ALL_LOCKED;
    else {
...
void RoomsBrowserGFx3_RakNet::CreateRoom_Callback(
    const SystemAddress&     senderAddress,
    RakNet::CreateRoom_Func* callResult
) {
    if (senderAddress != lobby2Client->GetServerAddress()) return;
...
void RoomsBrowserGFx3_RakNet::SearchByFilter_Callback(
    const SystemAddress&         senderAddress,
    RakNet::SearchByFilter_Func* callResult
) {
    if (senderAddress != lobby2Client->GetServerAddress()) return;
    ...
    if (callResult->resultCode == REC_SUCCESS) {
        ...
        for (i = 0; i < callResult->roomsOutput.Size(); i++) {
            rargs.Add(true);
            rargs.Add((Double)callResult->roomsOutput[i]
                          ->GetProperty(DefaultRoomColumns::GetColumnName(DefaultRoomColumns::TC_ROOM_ID))
                          ->i);
            char ipAddr[64];
...
void RoomsBrowserGFx3_RakNet::JoinByFilter_Callback(
    const SystemAddress&       senderAddress,
    RakNet::JoinByFilter_Func* callResult
) {
    if (senderAddress != lobby2Client->GetServerAddress()) return;
...
//////////////////////////////////////////////////////////////////................................................
//////////////////////////////////////////////////////////////////................................................
void RoomsBrowserGFx3_RakNet::LeaveRoom_Callback(
    const SystemAddress&    senderAddress,
    RakNet::LeaveRoom_Func* callResult
) {
    FxResponseArgs<1> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    FxDelegate::Invoke2(movie, "c2f_LeaveRoom", rargs);
}
void RoomsBrowserGFx3_RakNet::SendInvite_Callback(
    const SystemAddress&     senderAddress,
    RakNet::SendInvite_Func* callResult
) {
    FxResponseArgs<2> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
...
void RoomsBrowserGFx3_RakNet::StartSpectating_Callback(
    const SystemAddress&          senderAddress,
    RakNet::StartSpectating_Func* callResult
) {
    FxResponseArgs<1> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    FxDelegate::Invoke2(movie, "c2f_StartSpectating", rargs);
}
void RoomsBrowserGFx3_RakNet::StopSpectating_Callback(
    const SystemAddress&         senderAddress,
    RakNet::StopSpectating_Func* callResult
) {
    FxResponseArgs<1> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    FxDelegate::Invoke2(movie, "c2f_StopSpectating", rargs);
}
void RoomsBrowserGFx3_RakNet::GrantModerator_Callback(
    const SystemAddress&         senderAddress,
    RakNet::GrantModerator_Func* callResult
) {
    FxResponseArgs<2> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
...
void RoomsBrowserGFx3_RakNet::SetReadyStatus_Callback(
    const SystemAddress&         senderAddress,
    RakNet::SetReadyStatus_Func* callResult
) {
    FxResponseArgsList rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    rargs.Add(callResult->isReady);
    rargs.Add((Double)callResult->readyUsers.Size());
    for (unsigned int i = 0; i < callResult->readyUsers.Size(); i++) rargs.Add(callResult->readyUsers[i].C_String());
...
void RoomsBrowserGFx3_RakNet::GetReadyStatus_Callback(
    const SystemAddress&         senderAddress,
    RakNet::GetReadyStatus_Func* callResult
) {
    FxResponseArgsList rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    for (unsigned int i = 0; i < callResult->readyUsers.Size(); i++) rargs.Add(callResult->readyUsers[i].C_String());
...
void RoomsBrowserGFx3_RakNet::SetRoomLockState_Callback(
    const SystemAddress&           senderAddress,
    RakNet::SetRoomLockState_Func* callResult
) {
    FxResponseArgs<2> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    if (callResult->roomLockState == RLS_NOT_LOCKED) rargs.Add("NOT_LOCKED"); // Anyone can join or leave
    else if (callResult->roomLockState == RLS_PLAYERS_LOCKED)
        rargs.Add("PLAYERS_LOCKED"); // Anyone can join as spectator or become
                                     // spectator. New players are not allowed. You
                                     // cannot leave spectator.
    else
        rargs.Add("ALL_LOCKED"); // No new players are allowed, and you cannot
                                 // toggle spectator
    FxDelegate::Invoke2(movie, "c2f_SetRoomLockState", rargs);
}
void RoomsBrowserGFx3_RakNet::GetRoomLockState_Callback(
    const SystemAddress&           senderAddress,
    RakNet::GetRoomLockState_Func* callResult
) {
    FxResponseArgs<2> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
    if (callResult->roomLockState == RLS_NOT_LOCKED) rargs.Add("NOT_LOCKED"); // Anyone can join or leave
...
void RoomsBrowserGFx3_RakNet::AreAllMembersReady_Callback(
    const SystemAddress&             senderAddress,
...
void RoomsBrowserGFx3_RakNet::KickMember_Callback(
    const SystemAddress&     senderAddress,
    RakNet::KickMember_Func* callResult
) {
    FxResponseArgs<3> rargs;
    rargs.Add(RoomsErrorCodeDescription::ToEnum(callResult->resultCode));
...
void RoomsBrowserGFx3_RakNet::GetRoomProperties_Callback(
    const SystemAddress&            senderAddress,
    RakNet::GetRoomProperties_Func* callResult
) {
    char               saString[32];
    char               guidString[64];
    FxResponseArgsList rargs;
    ...
    if (callResult->roomDescriptor.roomLockState == RLS_NOT_LOCKED) rargs.Add("NOT_LOCKED"); // Anyone can join or leave
    ...
    rargs.Add(
        NetworkedRoomCreationParameters::SendInvitePermissionToEnum(callResult->roomDescriptor.inviteToRoomPermission)
    );
    rargs.Add(
        NetworkedRoomCreationParameters::SendInvitePermissionToEnum(
            callResult->roomDescriptor.inviteToSpectatorSlotPermission
        )
    );
...
void RoomsBrowserGFx3_RakNet::RoomMemberStartedSpectating_Callback(
    const SystemAddress&                              senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomMemberStoppedSpectating_Callback(
    const SystemAddress&                              senderAddress,
...
void RoomsBrowserGFx3_RakNet::ModeratorChanged_Callback(
    const SystemAddress&                   senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomMemberReadyStatusSet_Callback(
    const SystemAddress&                           senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomLockStateSet_Callback(
    const SystemAddress&                   senderAddress,
    RakNet::RoomLockStateSet_Notification* notification
) {
    FxResponseArgs<1> rargs;
    if (notification->roomLockState == RLS_NOT_LOCKED) rargs.Add("NOT_LOCKED"); // Anyone can join or leave
...
void RoomsBrowserGFx3_RakNet::RoomMemberKicked_Callback(
    const SystemAddress&                   senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomMemberLeftRoom_Callback(
    const SystemAddress&                     senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomMemberJoinedRoom_Callback(
    const SystemAddress&                       senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomInvitationSent_Callback(
    const SystemAddress&                     senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomInvitationWithdrawn_Callback(
    const SystemAddress&                          senderAddress,
...
void RoomsBrowserGFx3_RakNet::RoomDestroyedOnModeratorLeft_Callback(
    const SystemAddress&                               senderAddress,
