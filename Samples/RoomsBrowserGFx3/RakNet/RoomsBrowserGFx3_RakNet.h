...
/// This is data that should be set with
/// RakPeerInterface::SetOfflinePingResponse() It should also be able to write
/// this data to a LobbyServer instance
struct ServerAndRoomBrowserData {
    ...
    /// For the LAN browser, it expects the offline ping response to be packed
    /// using the format from this structure Therefore, to advertise that your
    /// server is available or updated, fill out the parameters in this structure,
    /// then call SetAsOfflinePingResponse() When your server is no longer
    /// available, set the offline ping response to NULL
    void SetAsOfflinePingResponse(RakNet::RakPeerInterface* rakPeer);

    /// Write to a RakNet table data structure, useful when creating the room or
    /// setting the room properties Writes everything EXCEPT the list of players
...
// GFxPlayerTinyD3D9.cpp has an instance of this class, and callls the
// corresponding 3 function This keeps the patching code out of the GFx sample
// as much as possible
class RoomsBrowserGFx3_RakNet : public RoomsBrowserGFx3,
                                public RakNet::Lobby2Callbacks,
                                public PluginInterface2,
                                RakNet::RoomsCallback {
public:
    ...
    virtual void
    SetRoomLockState_Callback(const SystemAddress& senderAddress, RakNet::SetRoomLockState_Func* callResult);
    virtual void
    GetRoomLockState_Callback(const SystemAddress& senderAddress, RakNet::GetRoomLockState_Func* callResult);
    virtual void
    AreAllMembersReady_Callback(const SystemAddress& senderAddress, RakNet::AreAllMembersReady_Func* callResult);
    virtual void KickMember_Callback(const SystemAddress& senderAddress, RakNet::KickMember_Func* callResult);
    virtual void
    GetRoomProperties_Callback(const SystemAddress& senderAddress, RakNet::GetRoomProperties_Func* callResult);
    ...
    virtual void
    ModeratorChanged_Callback(const SystemAddress& senderAddress, RakNet::ModeratorChanged_Notification* notification);
    virtual void RoomMemberReadyStatusSet_Callback(
        const SystemAddress&                           senderAddress,
        RakNet::RoomMemberReadyStatusSet_Notification* notification
    );
    virtual void
    RoomLockStateSet_Callback(const SystemAddress& senderAddress, RakNet::RoomLockStateSet_Notification* notification);
    virtual void
    RoomMemberKicked_Callback(const SystemAddress& senderAddress, RakNet::RoomMemberKicked_Notification* notification);
