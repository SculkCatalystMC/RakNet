...
int FxPlayerTiny::Run() {
    if (!SetupWindow(GString(FXPLAYER_APP_TITLE " ") + FileName)) return 1;
    ...
    // Developers set states on the loader to modify loading and playback
    // behavior. If you need to load files from a custom package, for example, you
    // can create a GFxFileOpener derived class that loads files in a custom way.
    ...
    if (!(pMovie = *pMovieDef->CreateInstance(false))) return 1;

    // Create renderer.
    if (!(pRenderer = *GRendererD3D9::CreateRenderer())) return 1;

    // Configure renderer in "Dependent mode", honoring externally
    // configured device settings.
    if (!pRenderer->SetDependentVideoMode(pDevice, &PresentParams, 0, hWnd)) return 1;
    ...
    while (RakNet::SocketLayer::IsPortInUse(sd.port, sd.hostAddress, sd.socketFamily) == true) sd.port++;
        ...
        if (PeekMessage(&msg, NULL, 0U, 0U, PM_REMOVE)) {
            ...
            if (pRenderer) {
                ...
                if (status == GRendererD3D9::DisplayStatus_NeedsReset) {
                    if (!SUCCEEDED(ResetD3D())) continue;
                }
            }
...
void FxPlayerTiny::AdvanceAndDisplay() {
    ...
    if (pMovie) {
        ...
        pDevice->SetRenderState(D3DRS_FILLMODE, Wireframe ? D3DFILL_WIREFRAME : D3DFILL_SOLID);

        UInt32 time  = timeGetTime();
        Float  delta = ((Float)(time - MovieLastTime)) / 1000.0f;

        // Advance time and display the movie.
        if (!Paused) pMovie->Advance(delta);
        pMovie->Display();
...
void FxPlayerTiny::OnKey(UInt keyCode, bool downFlag) {
    ...
    if (keyCode == VK_MENU && downFlag) ControlKeyDown = false; // to enable Ctrl-Alt-... combinations to work
    ...
    if (ctrl && downFlag) {
        ...
        case 'A':
            AAEnabled = !AAEnabled;
            {
                ...
                if (AAEnabled) rendererFlags |= GFxRenderConfig::RF_EdgeAA;
...
HRESULT FxPlayerTiny::ResetD3D() {
    // Set new mode
    if (FAILED(pDevice->Reset(&PresentParams))) return E_FAIL;
    return S_OK;
}
...
// Static window function called by Windows for message processing.
// Implementation delegates message processing to MemberWndProc.
LRESULT CALLBACK FxPlayerTiny::StaticWindowProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (iMsg == WM_NCCREATE) {
        ...
        if (!papp) return DefWindowProcW(hwnd, iMsg, wParam, lParam);
...
INT WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR lpCmdLine, INT) {
    ...
    int result = 1;
    {
        ...
        LPSTR pfilename = (!lpCmdLine || !lpCmdLine[0]) ? FXPLAYER_FILENAME : lpCmdLine;
