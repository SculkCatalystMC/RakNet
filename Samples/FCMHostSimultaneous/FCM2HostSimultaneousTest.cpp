...
int main() {
    ...
    while (!quit) {
        for (int i = 0; i < NUM_PEERS; i++) {
            for (packet = rakPeer[i]->Receive(); packet;
                 rakPeer[i]->DeallocatePacket(packet), packet = rakPeer[i]->Receive()) {
                ...
                case ID_NEW_INCOMING_CONNECTION:
                    ...
                    printf(
                        "%i. ID_NEW_INCOMING_CONNECTION from %s. guid=%s.\n",
                        i,
                ...
                case ID_CONNECTION_REQUEST_ACCEPTED:
                    // Somebody connected.  We have their IP now
                    printf(
                        "%i. ID_CONNECTION_REQUEST_ACCEPTED from %s. guid=%s.\n",
                        i,
                        packet->systemAddress.ToString(true),
                        packet->guid.ToString()
                    );
        ...
        if (kbhit()) {
            ...
            if (ch == ' ') {
                ...
                bool         weAreHost;
                for (int i = 0; i < NUM_PEERS; i++) {
                    if (rakPeer[i]->IsActive() == false) continue;
                    ...
                    if (weAreHost)
                        printf(
                            "%i. %iP myGuid=%s, hostGuid=%s tcc=%i (Y)\n",
                            i,
                            participantList,
                            rakPeer[i]->GetGuidFromSystemAddress(RakNet::UNASSIGNED_SYSTEM_ADDRESS).ToString(),
                            hostGuid.ToString(),
                            fcm2[i].GetTotalConnectionCount()
                        );
                    else
                        printf(
                            "%i. %iP myGuid=%s, hostGuid=%s tcc=%i (N)\n",
                            i,
                            participantList,
                            rakPeer[i]->GetGuidFromSystemAddress(RakNet::UNASSIGNED_SYSTEM_ADDRESS).ToString(),
                            hostGuid.ToString(),
