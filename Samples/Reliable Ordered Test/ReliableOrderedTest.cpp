...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 32; i++) packetNumber[i] = 0;
    ...
    if (str[0] == 's' || str[0] == 'S') {
        ...
        Gets(str, sizeof(str));
        if (str[0] == 0) sendInterval = 33;
        else sendInterval = atoi(str);
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') break;
                ...
                char              message[2048];
                if (sender) rss = sender->GetStatistics(sender->GetSystemAddressFromIndex(0));
                else rss = receiver->GetStatistics(receiver->GetSystemAddressFromIndex(0));
        ...
        if (sender) {
            ...
            while (doSend && currentTime > nextSend) {
                ...
                if (sender) {
                    ...
                    printf(
                        "Snd: %i, %s, time %" PRINTF_64_BIT_MODIFIER "u, length %i\n",
                        packetNumber[streamNumber] - 1,
                        type,
                        currentTime,
            ...
            while (packet) {
                ...
                case ID_TIMESTAMP:
                    ...
                    if (receivedPacketNumber > packetNumber[streamNumber])
                        ...
                        printf(
                            "Out of order packet! Expecting %i got %i %s (channel %i).\n",
                            packetNumber[streamNumber],
                            receivedPacketNumber,
                            type,
                            streamNumber
                    ...
                    //					printf("Sent=%"
                    // PRINTF_64_BIT_MODIFIER "u Received=%" PRINTF_64_BIT_MODIFIER "u
                    // Diff=%i.\n", receivedTime, currentTime, (int)(currentTime -
                    // receivedTime));
...
// This sleep keeps RakNet responsive
    ...
    if (sender) RakNet::RakPeerInterface::DestroyInstance(sender);
    if (receiver) RakNet::RakPeerInterface::DestroyInstance(receiver);
