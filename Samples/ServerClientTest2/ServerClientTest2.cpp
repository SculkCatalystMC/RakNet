...
public:
    ...
    ~Client() { RakNet::RakPeerInterface::DestroyInstance(peer); }
    void Startup(void) {
...
public:
    ...
    ~Server() { RakNet::RakPeerInterface::DestroyInstance(peer); }
    void Start(void) {
        RakNet::SocketDescriptor socketDescriptor;
    ...
    void Update(RakNet::TimeMS curTime) {
        ...
        while (p) {
            ...
            case ID_NEW_INCOMING_CONNECTION:
                ...
                // 					{
                // 						peer->Send((const char*)
                // p->data, p->length, HIGH_PRIORITY, RELIABLE_ORDERED, 0, p->guid,
                // true); 						break;
                // 					}
            }
            peer->DeallocatePacket(p);
...
int main(void) {
    ...
    for (i = 0; i < RANDOM_DATA_SIZE_1 - 1; i++) randomData1[i + 1] = i;
    randomData2[0] = (char)ID_USER_PACKET_ENUM;
    for (i = 0; i < RANDOM_DATA_SIZE_2 - 1; i++) randomData2[i + 1] = i;
    ...
    if (mode == 1 || mode == 2) {
        ...
        for (i = 0; i < NUM_CLIENTS; i++) clients[i].Startup();
        printf("Started clients\n");
        printf("Connecting clients...\n");
        for (i = 0; i < NUM_CLIENTS; i++) clients[i].Connect();
        printf("Done.\n");
    }
    ...
    while (time < endTime) {
        if (mode == 0 || mode == 2) server.Update(time);
        if (mode == 1 || mode == 2) {
            for (i = 0; i < NUM_CLIENTS; i++) clients[i].Update(time);
        }
    ...
    if (mode == 0 || mode == 2) server.peer->Shutdown(0);
    if (mode == 1 || mode == 2)
        for (i = 0; i < NUM_CLIENTS; i++) clients[i].peer->Shutdown(0);
