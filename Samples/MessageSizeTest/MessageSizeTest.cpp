...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 1; i < 4000; i++) data[i] = i % 256;
    int stride, sum;
    ...
    for (stride = 1; stride < 2000; stride++) {
        ...
        while (RakNet::GetTime() < timeout) {
            ...
            RakSleep(30);
            if (receiveCount == sendCount) break;
        }

        if (sendCount == receiveCount) printf("Stride=%i Sends=%i Receives=%i\n", stride, sendCount, receiveCount);
        else printf("ERROR! Stride=%i Sends=%i Receives=%i\n", stride, sendCount, receiveCount);
    }
