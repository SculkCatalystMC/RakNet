...
int main(void) {
    ...
    if (portstring[0] == 0) strcpy(portstring, "60000");

    // Enumeration data
    puts(
        "Enter offline ping response data (for return by a LAN discovery for "
        "example)"
    );
    puts("Hit enter for none.");
    char enumData[512];
    Gets(enumData, sizeof(enumData));
    if (enumData[0]) server->SetOfflinePingResponse(enumData, (const unsigned int)strlen(enumData) + 1);
    ...
    if (b) puts("Server started, waiting for connections.");
    else {
    ...
    while (1) {
        if (kbhit()) {
            ...
            if (Gets(buff, sizeof(buff)) && (buff[0] == 'q')) break;
            else {
        ...
        case ID_UNCONNECTED_PONG: {
            ...
            // In this sample since the client is not running a game we can save CPU
            // cycles by Stopping the network threads after receiving the pong.
            client->Shutdown(100);
        } break;
        ...
        if (packetFromServer) server->DeallocatePacket(p);
        else client->DeallocatePacket(p);
    }
