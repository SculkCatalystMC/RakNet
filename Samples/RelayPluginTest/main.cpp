...
int main(void) {
    ...
    if (listenPort[0] == 0) strcpy(listenPort, "1234");
        ...
        if (kbhit()) {
            ...
            if (ch == 'a' || ch == 'A') {
                ...
                if (name[0]) {
                    ...
                    relayPlugin->SendToParticipant(
                        peer->GetGUIDFromIndex(0),
                        name,
                        &msgBs,
                        HIGH_PRIORITY,
                        RELIABLE_ORDERED,
        ...
        for (packet = peer->Receive(); packet; peer->DeallocatePacket(packet), packet = peer->Receive()) {
            ...
            if (packet->data[0] == ID_NEW_INCOMING_CONNECTION) {
                ...
                case RPE_MESSAGE_TO_CLIENT_FROM_SERVER: {
                    ...
                    printf(
                        "RPE_MESSAGE_TO_CLIENT_FROM_SERVER from %s, data=%s\n",
                        senderName.C_String(),
                        dataInAsStr.C_String()
                    );
                } break;
                ...
                case RPE_GROUP_MSG_FROM_SERVER: {
                    ...
                    printf(
                        "RPE_GROUP_MSG_FROM_SERVER from %s, data=%s\n",
                        senderName.C_String(),
                        dataInAsStr.C_String()
                    );
                } break;
