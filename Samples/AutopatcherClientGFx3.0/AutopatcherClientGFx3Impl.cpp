...
public:
    virtual bool OnFile(OnFileStruct* onFileStruct) {
        ...
        if (onFileStruct->context.op == PC_HASH_1_WITH_PATCH || onFileStruct->context.op == PC_HASH_2_WITH_PATCH)
            strcpy(buff, "Patched: ");
        else if (onFileStruct->context.op == PC_WRITE_FILE) strcpy(buff, "Written: ");
        else if (onFileStruct->context.op == PC_ERROR_FILE_WRITE_FAILURE) strcpy(buff, "Write Failure: ");
        else if (onFileStruct->context.op == PC_ERROR_PATCH_TARGET_MISSING) strcpy(buff, "Patch target missing: ");
        ...
        else if (onFileStruct->context.op == PC_NOTICE_WILL_COPY_ON_RESTART) strcpy(buff, "Copy pending restart: ");
        else if (onFileStruct->context.op == PC_NOTICE_FILE_DOWNLOADED) strcpy(buff, "Downloaded: ");
        else if (onFileStruct->context.op == PC_NOTICE_FILE_DOWNLOADED_PATCH) strcpy(buff, "Downloaded Patch: ");
        else RakAssert(0);
...
AutopatcherClientGFx3Impl::~AutopatcherClientGFx3Impl() { Shutdown(); }
void AutopatcherClientGFx3Impl::Init(
    const char*        _pathToThisExe,
...
void AutopatcherClientGFx3Impl::Update(void) {
    ...
    while (p) {
        if (p->data[0] == ID_AUTOPATCHER_REPOSITORY_FATAL_ERROR) {
            ...
            FxResponseArgs<1> args2;
            RakNet::RakString completionMsg(
                "Launch \"AutopatcherClientRestarter.exe %s\"\nQuit this application "
                "immediately after to unlock files.\n",
...
void AutopatcherClientGFx3Impl::Shutdown(void) {
    ...
    movie.Clear();
    if (packetizedTCP) packetizedTCP->Stop();
...
void AutopatcherClientGFx3Impl::PressedPatch(const FxDelegateArgs& pparams) {
    ...
    if (fullRescan == false) prt->LoadLastUpdateDate(&lastUpdateDate, appDir);
    else lastUpdateDate = 0;

    transferCallback.autopatcherClient = prt;
    if (prt->autopatcherClient->PatchApplication(
            appName,
            appDir,
            lastUpdateDate,
            prt->serverAddress,
...
void AutopatcherClientGFx3Impl::OpenSite(const FxDelegateArgs& pparams) {
    ...
    if (stricmp(siteType, "help") == 0) {
        ShellExecute(
            NULL,
            "open",
            "http://www.jenkinssoftware.com/raknet/manual/autopatcher.html",
            NULL,
            NULL,
            SW_SHOWNORMAL
        );
