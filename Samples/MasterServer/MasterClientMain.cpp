...
// This is my own internal test program for the master client but serves as a
// good example. Right now it is hardcoded to connect to 127.0.0.1.  You would
// run the master server first, then run this.
...
int main(void) {
    ...
    printf(
        "On the master server and by game clients to download a list of game "
        "servers\n"
    );
    ...
    printf(
        ...
        "to another game, using NAT punch-through with master server, "
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') {
                ...
                if (ip[0] != 0 && str[0] != 0) {
                    ...
                    printf(
                        "Sent connection attempt notification to the server the "
                        "master server\n"
                    );
                } else {
                    printf("Aborting...");
                ...
                if (serverListSize == 0) {
                    ...
                    for (index = 0; index < serverListSize; index++) {
                        ...
                        if (identiferFound) printf("%s:", outputString);
                        else printf("NO_IP:");
                        outputInt = masterClient.GetServerListRuleAsInt(index, "Port", &identiferFound);
                        if (identiferFound) printf("%i ", outputInt);
                        else printf("NO_PORT ");
                        ...
                        if (identiferFound) printf("%i\n", outputInt);
                        else printf("NO_SCORE\n");
                    }
                }
            }
            ch = 0;
