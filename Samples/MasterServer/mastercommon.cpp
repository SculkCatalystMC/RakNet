...
GameServer::~GameServer() { Clear(); }
void GameServer::Clear() {
    unsigned i;
    for (i = 0; i < serverRules.Size(); i++) delete serverRules[i];
    serverRules.Clear();
}
...
GameServerList::~GameServerList() { Clear(); }
void GameServerList::Clear(void) {
    unsigned i;
    for (i = 0; i < serverList.Size(); i++) delete serverList[i];
    serverList.Clear();
}
void GameServerList::SortOnKey(char* key, bool ascending) {
    unsigned i;
    // Set keyindex
    for (i = 0; i < serverList.Size(); i++) serverList[i]->FindKey(key);
...
int EQ(GameServer* left, GameServer* right) {
    if (left->keyIndex == -1 || right->keyIndex == -1) return true;

    if (left->serverRules[left->keyIndex]->stringValue)
        return strcmp(left->serverRules[left->keyIndex]->stringValue, right->serverRules[right->keyIndex]->stringValue)
            == 0;
    else return left->serverRules[left->keyIndex]->intValue == right->serverRules[right->keyIndex]->intValue;
}
int LT(GameServer* left, GameServer* right) {
    if (left->keyIndex == -1) return true;
    if (right->keyIndex == -1) return false;
...
int LTEQ(GameServer* left, GameServer* right) { return LT(left, right) || EQ(left, right); }
int GT(GameServer* left, GameServer* right) {
    if (left->keyIndex == -1) return false;
    if (right->keyIndex == -1) return true;
...
int GTEQ(GameServer* left, GameServer* right) { return GT(left, right) || EQ(left, right); }
...
int GameServerList::GetIndexByPlayerID(PlayerID playerID) {
    ...
    for (i = 0; i < (int)serverList.Size(); i++) {
        if (serverList[i]->connectionIdentifier == playerID) return i;
    }
    return -1;
}
...
void MasterCommon::ClearServerList(void) { gameServerList.Clear(); }
...
unsigned int MasterCommon::GetServerListSize(void) { return gameServerList.serverList.Size(); }
...
bool MasterCommon::IsReservedRuleIdentifier(char* ruleIdentifier) {
    if (strcmp(ruleIdentifier, "Ping") == 0 || strcmp(ruleIdentifier, "IP") == 0 || strcmp(ruleIdentifier, "Port") == 0)
        return true;
...
void MasterCommon::AddDefaultRulesToServer(GameServer* gameServer, PlayerID playerID) {
    ...
    // Every server has NUMBER_OF_DEFAULT_MASTER_SERVER_KEYS keys by default: IP,
    // port, and ping
...
void MasterCommon::HandlePong(Packet* packet) {
    ...
    if (serverIndex >= 0) {
        ...
        if (gameServerList.serverList[serverIndex]->FindKey("Ping")) {
            ...
            ptime.Read(pingTime);
            gameServerList.serverList[serverIndex]
                ->serverRules[gameServerList.serverList[serverIndex]->keyIndex]
                ->intValue = pingTime;
...
bool MasterCommon::UpdateServerRule(GameServer* gameServer, char* ruleIdentifier, char* stringData, int intData) {
    ...
    // Add the rule to our local server.  If it changes the local server, set a
    // flag so we upload the local server on the next update.
...
void MasterCommon::SerializeServer(GameServer* gameServer, BitStream* outputBitStream) {
    ...
    for (serverIndex = 0; serverIndex < gameServer->serverRules.Size(); serverIndex++) {
        ...
        if (IsReservedRuleIdentifier(gameServer->serverRules[serverIndex]->key) == false) numberOfRulesToWrite++;
    }
    ...
    // Write all the rules
    for (serverIndex = 0; serverIndex < gameServer->serverRules.Size(); serverIndex++) {
        if (IsReservedRuleIdentifier(gameServer->serverRules[serverIndex]->key)) continue;
...
GameServer* MasterCommon::DeserializeServer(BitStream* inputBitStream) {
    ...
    for (serverIndex = 0; serverIndex < numberOfRulesToWrite; serverIndex++) {
        ...
        if (IsReservedRuleIdentifier(gameServerRule->key)) delete gameServerRule;
        else gameServer->serverRules.Insert(gameServerRule);
    }
    return gameServer;
}
...
void MasterCommon::UpdateServer(GameServer* destination, GameServer* source, bool deleteSingleRules) {
    ...
    // If (deleteSingleRules) then delete any rules that exist in the old and not
    // in the new
    if (deleteSingleRules) {
        ...
        while (destinationRuleIndex < destination->serverRules.Size()) {
            ...
            } else destinationRuleIndex++;
        }
    }

    // Go through all the rules.
    for (sourceRuleIndex = 0; sourceRuleIndex < source->serverRules.Size(); sourceRuleIndex++) {
        if (IsReservedRuleIdentifier(source->serverRules[sourceRuleIndex]->key)) continue;

        // Add any fields that exist in the new and do not exist in the old
        // Update any fields that exist in both
        UpdateServerRule(
            destination,
            source->serverRules[sourceRuleIndex]->key,
            source->serverRules[sourceRuleIndex]->stringValue,
            source->serverRules[sourceRuleIndex]->intValue
        );
    }
}
...
void MasterCommon::OnAttach(RakPeerInterface* peer) { rakPeer = peer; }