...
// If we ping NUMBER_OF_MISSED_PINGS_TO_DROP without ever a response, that
// server is dropped from the list. This includes the last ping, so actually
// NUMBER_OF_MISSED_PINGS_TO_DROP-1 would be truly missed
#define NUMBER_OF_MISSED_PINGS_TO_DROP 4
// KEEP_ALIVE_PING_FREQUENCY is how often to ping servers to make sure they are
// active
#define KEEP_ALIVE_PING_FREQUENCY 20000
// How many ms must pass per connection before we check average bytes for a
// flood attack
...
public:
    ...
    // ---------------------------------------------------
    // Sorting function
    // ruleIdentifier is a string used by you previously when adding rules via
    // PostRule It can also be "IP" "Port" or "Ping" Set ascending to true to sort
    // from low to high.  Otherwise sorts from high to low.
    void SortServerListOnKey(char* ruleIdentifier, bool ascending);

    // serverIndex should be from 0 to GetServerListSize()-1
    // ruleIdentifier is a string used by you previously when adding rules via
    // PostRule It can also be "IP" "Port" or "Ping". identifier found will return
    // true if the specified rule is found AND you are reading the correct type.
...
protected:
    ...
    // Returns true if the server has been changed.  False if we are adding a rule
    // that is already the same
    ...
    // Add the specified server to the list of servers - or if the server already
    // exists Update the existing server and delete the server passed
