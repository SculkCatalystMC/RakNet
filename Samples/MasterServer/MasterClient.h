...
public:
    ...
    // masterClientPort is the port the master client should use (must be
    // different than gamePort) gamePort is the port your game server or game
    // client is using.
    bool Connect(char* host, int masterServerPort);

    // Disconnect and terminate.  Game servers should stay connected to use NAT
    // punch-through.  Game clients, or game servers that do not need NAT
    // punch-through can disconnect.
    void Disconnect(void);

    // Returns true if connected
    bool IsConnected(void);

    // This will tell the master server that we are trying to connect to the
    // indicated game server. The indicated server will then also try to connect
    // to us, bypassing most NATs and increasing the chance of a successful
    // connection.  You should try connecting to the game server at the same time
    ...
    // ---------------------------------------------------
    // Adds a rule to look for when we query.  This will update existing servers
    // that contain these rules. Do not query for "Ping", "IP", or "Port" as these
    ...
    // Query the master server with our rule set.  To get all servers, call
    // ClearQueryRules() after any prior calls to add AddQueryRule to clear the
    // rule set.
    ...
    // Event if a server has its rules updated (only happens if querying with a
    // rule set)
    ...
    // You should call AdvertiseSystem to the passed IP and port from your game
    // instance
