        ...
        if (time >= gameServerList.serverList[serverIndex]->nextPingTime) {
            if (gameServerList.serverList[serverIndex]->failedPingResponses >= NUMBER_OF_MISSED_PINGS_TO_DROP) {
#ifdef _SHOW_MASTER_SERVER_PRINTF
                printf(
                    "Deleting %s for lack of ping response.\n",
                ...
                if (rakPeer->GetIndexFromPlayerID(gameServerList.serverList[serverIndex]->connectionIdentifier) == -1) {
                    rakPeer->Ping(
                        (char*)
                            rakPeer->PlayerIDToDottedIP(gameServerList.serverList[serverIndex]->connectionIdentifier),
                        gameServerList.serverList[serverIndex]->connectionIdentifier.port,
                        false
                    );
...
#ifdef _SHOW_MASTER_SERVER_PRINTF
                    printf(
                        "Pinging %s. Waiting on %i repl(ies) so far.\n",
                        (char*)
                            rakPeer->PlayerIDToDottedIP(gameServerList.serverList[serverIndex]->connectionIdentifier),
                        gameServerList.serverList[serverIndex]->failedPingResponses
                    );
...
#ifdef _SHOW_MASTER_SERVER_PRINTF
                    printf(
                        "Not pinging %s since they are currently connected.\n",
                        (char*)rakPeer->PlayerIDToDottedIP(gameServerList.serverList[serverIndex]->connectionIdentifier)
                    );
#endif
                }
...
bool MasterServer::OnReceive(RakPeerInterface* peer, Packet* packet) {
    ...
    if (rss) {
        ...
        if (connectionTime > FLOOD_ATTACK_CHECK_DELAY
...
#ifdef _SHOW_MASTER_SERVER_PRINTF
            printf(
                "%s banned for session due to for flood attack\n",
            ...
            serverIndex = 0;
            while (serverIndex < gameServerList.serverList.Size()) {
                if (gameServerList.serverList[serverIndex]->connectionIdentifier.binaryAddress
...
bool MasterServer::PropagateToGame(Packet* packet) const {
    ...
    return packetIdentifier != ID_QUERY_MASTER_SERVER && packetIdentifier != ID_MASTER_SERVER_DELIST_SERVER
        && packetIdentifier != ID_MASTER_SERVER_SET_SERVER && packetIdentifier != ID_RELAYED_CONNECTION_NOTIFICATION;
}
...
void MasterServer::HandleQuery(Packet* packet) {
    ...
    while (compressedString.GetNumberOfUnreadBits() > 0) {
        // Generate a list of the indices of the servers that have one or more of
        // the specified keys.
        ...
        if (IsReservedRuleIdentifier(ruleIdentifier)) continue;

        for (index = 0; index < gameServerList.serverList.Size(); index++) {
            if (gameServerList.serverList[index]->connectionIdentifier == UNASSIGNED_PLAYER_ID) continue;
    ...
    // Write the packet id
    if (queryAll) outputBitStream.Write((unsigned char)ID_MASTER_SERVER_SET_SERVER);
    else outputBitStream.Write((unsigned char)ID_MASTER_SERVER_UPDATE_SERVER);
    if (queryAll) {
        ...
        for (index = 0; index < serversWithKeysList.Size(); index++) {
            ...
            while (compressedString.GetNumberOfUnreadBits() > 0) {
                // Generate a list of the indices of the servers that have one or more
                // of the specified keys.
                stringCompressor->DecodeString(ruleIdentifier, 256, &compressedString);
                if (ruleIdentifier[0] == 0) break;
                if (IsReservedRuleIdentifier(ruleIdentifier)) continue;

                serversWithKeysList[index]->FindKey(ruleIdentifier);
                key = serversWithKeysList[index]->keyIndex;
                if (key >= 0) SerializeRule(serversWithKeysList[index]->serverRules[key], &outputBitStream);
            }
        }
    }
...
void MasterServer::HandleRelayedConnectionNotification(Packet* packet) {
    ...
    if (!stringCompressor->DecodeString(str, 22, &incomingBitStream)) return;
    ...
    outgoingBitStream.Write(packet->playerId.binaryAddress); // This is the public IP, which the
                                                             // sender doesn't necessarily know
    outgoingBitStream.Write(clientGamePort);

    PlayerID targetID;
    rakPeer->IPToPlayerID(str, serverGamePort, &targetID);

    // Given the IP and port of the game system, give me the index into the game
    // server list
    ...
    if (serverIndex >= 0) {
#ifdef _SHOW_MASTER_SERVER_PRINTF
        printf(
            "ID_RELAYED_CONNECTION_NOTIFICATION sent to %s:%i from %s:%i\n",
            str,
            serverGamePort,
            rakPeer->PlayerIDToDottedIP(packet->playerId),
        ...
        rakPeer->Send(
            ...
            RELIABLE,
            0,
            gameServerList.serverList[serverIndex]->originationId,
            false
        );
    } else {
#ifdef _SHOW_MASTER_SERVER_PRINTF
        printf(
            "ID_RELAYED_CONNECTION_NOTIFICATION not sent to %s:%i from "
