...
void MasterClient::ClearQueryRules(void) { ruleIdentifierList.Reset(); }
void MasterClient::QueryMasterServer(void) {
    BitStream outgoingBitStream;
    // Request to the master server for the list of servers that contain at least
...
void MasterClient::ListServer(void) { listServer = true; }
...
void MasterClient::HandleServerListResponse(Packet* packet, bool overwriteExisting) {
    ...
    if (inputBitStream.ReadCompressed(numberOfServers) == false) return;
    ...
    for (serverIndex = 0; serverIndex < numberOfServers; serverIndex++) {
        ...
        if (newServerAdded) {
            // Ping the new server
            rakPeer->Ping(
                (char*)rakPeer->PlayerIDToDottedIP(gameServer->connectionIdentifier),
                gameServer->connectionIdentifier.port,
                false
            );
...
void MasterClient::PostRule(char* ruleIdentifier, char* stringData, int intData) {
    if (ruleIdentifier) {
        if (IsReservedRuleIdentifier(ruleIdentifier)) return;
...
void MasterClient::RemoveRule(char* ruleIdentifier) {
    if (ruleIdentifier) localServerModified |= RemoveServerRule(&localServer, ruleIdentifier);
}
