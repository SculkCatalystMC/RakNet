...
class FullyConnectedMesh2_UserData : public FullyConnectedMesh2 {
    ...
    virtual void WriteVJSUserData(RakNet::BitStream* bsOut, RakNetGUID userGuid) {
        bsOut->Write(RakString("WriteVJSUserData test, userGuid=%s", userGuid.ToString()));
    }
};
...
int main() {
    ...
    while (!quit) {
        for (int peerIndex = 0; peerIndex < NUM_PEERS; peerIndex++) {
            for (packet = rakPeer[peerIndex]->Receive(); packet;
                 rakPeer[peerIndex]->DeallocatePacket(packet), packet = rakPeer[peerIndex]->Receive()) {
                switch (packet->data[0]) {
                case ID_FCM2_VERIFIED_JOIN_START: {
                    printf(
                        "%s: Got ID_FCM2_VERIFIED_JOIN_START from %s. address=",
                        rakPeer[peerIndex]->GetMyGUID().ToString(),
                    ...
                    for (unsigned int i = 0; i < guids.Size(); i++) {
                        ...
                        ConnectionAttemptResult car =
                            rakPeer[peerIndex]->Connect(addresses[i].ToString(false), addresses[i].GetPort(), 0, 0);
                        switch (car) {
                ...
                case ID_FCM2_VERIFIED_JOIN_FAILED:
                    printf(
                        "%s: ID_FCM2_VERIFIED_JOIN_FAILED from %s\n",
                        rakPeer[peerIndex]->GetMyGUID().ToString(),
                        packet->guid.ToString()
                    );
                    break;
                ...
                case ID_FCM2_VERIFIED_JOIN_CAPABLE: {
                    ...
                    printf(
                        "%s: ID_FCM2_VERIFIED_JOIN_CAPABLE from %s\n",
                        rakPeer[peerIndex]->GetMyGUID().ToString(),
                        packet->guid.ToString()
                    );
                    printf("STR: %s\n", testStr.C_String());
                ...
                case ID_FCM2_VERIFIED_JOIN_ACCEPTED: {
                    ...
                    fcm2[peerIndex].GetVerifiedJoinAcceptedAdditionalData(
                        packet,
                        &thisSystemAccepted,
                        systemsAccepted,
                        &additionalData
                    ...
                    if (thisSystemAccepted) {
                        printf(
                            "%s: ID_FCM2_VERIFIED_JOIN_ACCEPTED from %s. systemsAccepted=",
                            rakPeer[peerIndex]->GetMyGUID().ToString(),
                            packet->guid.ToString()
                        );
                ...
                case ID_FCM2_VERIFIED_JOIN_REJECTED:
                    printf(
                        "%s: ID_FCM2_VERIFIED_JOIN_REJECTED from %s\n",
                        rakPeer[peerIndex]->GetMyGUID().ToString(),
                        packet->guid.ToString()
                    );
