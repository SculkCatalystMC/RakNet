...
RAK_THREAD_DECLARATION(ProducerThread) {
    ...
    while (endThreads == false) {
        ...
        if (i & 1) peer1->Send(out, 2, HIGH_PRIORITY, RELIABLE_ORDERED, 0, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
        else peer2->Send(out, 2, HIGH_PRIORITY, RELIABLE_ORDERED, 0, RakNet::UNASSIGNED_SYSTEM_ADDRESS, true);
...
RAK_THREAD_DECLARATION(ConsumerThread) {
    ...
    while (endThreads == false) {
        ...
        if (i & 1) p = peer1->Receive();
        else p = peer2->Receive();
        //	printf("Thread %i done reading...\n", i);

        if (p) {
            if (p->data[0] == ID_USER_PACKET_ENUM) printf("Got data from thread %i\n", p->data[1]);
            if (i & 1) peer1->DeallocatePacket(p);
            else peer2->DeallocatePacket(p);
        }
