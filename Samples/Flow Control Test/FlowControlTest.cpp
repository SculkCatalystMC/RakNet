...
int main(void) {
    ...
    if (rakServer) {
        ...
        if (remoteIP[0] == 0) strcpy(remoteIP, "127.0.0.1");
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (ch == 'q') {
                ...
                if (sendinterval > 1) sendinterval /= 2;
                printf("Send interval is now %i\n", sendinterval);
            }
            if (ch == 'h')
                printf(
                ...
                if (packetSize < 8192) packetSize *= 2;
                printf("Packet size is now %i\n", packetSize);
            } else if (ch == '-') {
                if (packetSize > 1) packetSize /= 2;
                printf("Packet size is now %i\n", packetSize);
            } else if (ch == ' ') {
                if (rakServer) {
                    StatisticsToString(
                        rakServer->GetStatistics(rakServer->GetSystemAddressFromIndex(0)),
                        randomData,
                        1
                    );
                    printf("%s", randomData);
                    ...
                    StatisticsToString(
                        rakClient->GetStatistics(rakClient->GetSystemAddressFromIndex(0)),
                        randomData,
                        1
                    );
                    printf("%s", randomData);
        ...
        // get packets
        if (rakServer) p = rakServer->Receive();
        else p = rakClient->Receive();
        ...
        if (p) {
            ...
            default:
                ...
                if (rakServer)
                    rakServer
                        ->Send((char*)p->data, p->length, HIGH_PRIORITY, RELIABLE_ORDERED, 0, p->systemAddress, true);
            }

            if (rakServer) rakServer->DeallocatePacket(p);
            else rakClient->DeallocatePacket(p);
        }
    ...
    if (rakServer) RakNet::RakPeerInterface::DestroyInstance(rakServer);
    else RakNet::RakPeerInterface::DestroyInstance(rakClient);
