...
class DBResultHandler : public TitleValidationDBCBInterface {
    virtual void AddTitle_CB(AddTitle_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("AddTitle_CB call canceled:\n");
    ...
    virtual void GetTitles_CB(GetTitles_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("GetTitles_CB call canceled:\n");
        else if (callResult->dbQuerySuccess == false) {
            ...
            for (i = 0; i < callResult->titles.Size(); i++) {
                ...
                printf(
                    "%i. defaultAllowUpdateAccountNumber=%i\n",
                    i + 1,
                    callResult->titles[i]->defaultAllowUpdateAccountNumber
                );
                printf(
                    "%i. defaultAllowUpdateAdminLevel=%i\n",
                    i + 1,
                    callResult->titles[i]->defaultAllowUpdateAdminLevel
                );
                printf(
                    "%i. defaultAllowUpdateAccountBalance=%i\n",
                    i + 1,
                    callResult->titles[i]->defaultAllowUpdateAccountBalance
                );
                printf(
                    "%i. defaultAllowClientsUploadActionHistory=%i\n",
                    i + 1,
                    callResult->titles[i]->defaultAllowClientsUploadActionHistory
                );
                printf(
                    "%i. defaultPermissionsStr=%s\n",
                    i + 1,
                    callResult->titles[i]->defaultPermissionsStr.C_String()
                );
    ...
    virtual void UpdateUserKey_CB(UpdateUserKey_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("UpdateUserKey_CB call canceled:\n");
        else if (callResult->dbQuerySuccess == false) {
    ...
    virtual void GetUserKeys_CB(GetUserKeys_PostgreSQLImpl* callResult, bool wasCancelled, void* context) {
        if (wasCancelled) printf("GetUserKeys_CB call canceled:\n");
        else if (callResult->dbQuerySuccess == false) {
...
void main(int argc, char** argv) {
    printf(
        ...
        "Validation specification based on PostgreSQL\n"
    );
    printf(
    ...
    printf(
        "The goal of this class is to allow you to lookup\ngame titles based "
        "on a password, and verify CD keys.\n"
    );
    ...
    // The real functionality of the TitleValidationDB class is contained in the
    // functors defined in TitleValidationDB_PostgreSQL.h/.cpp. However, this
    // class contains some utility functions and members, such as the
    // functionThread and a pointer to the postgreSQL interface.
    TitleValidation_PostgreSQL titleValidationDB;

    // The class internally uses a functionThread so that database queries can
    // happen asynchronously, as opposed to blocking and slowing down the game. If
    // you don't assign one it will create one automatically and start it with one
    // thread.
    ...
    // from the *_PostgreSQLImpl functors found in
    // TitleValidationDB_PostgreSQL.h/.cpp and override the behavior of
    // Functor::HandleResult
