...
struct SteamResults : public RakNet::Lobby2Callbacks {
    virtual void MessageResult(RakNet::Notification_Console_MemberJoinedRoom* message) {
        RakNet::Notification_Console_MemberJoinedRoom_Steam* msgSteam =
            (RakNet::Notification_Console_MemberJoinedRoom_Steam*)message;
        ...
        if (mySteamId < msgSteam->srcMemberId) {
            // Steam's NAT punch is implicit, so it takes a long time to connect. Give
            // it extra time
            unsigned int            sendConnectionAttemptCount          = 24;
            unsigned int            timeBetweenSendConnectionAttemptsMS = 500;
            ConnectionAttemptResult car                                 = rakPeer->Connect(
                msgSteam->remoteSystem.ToString(false),
                msgSteam->remoteSystem.GetPort(),
                0,
                0,
                0,
                0,
...
int main(int argc, char** argv) {
    ...
    if (msg->resultCode != L2RC_PROCESSING && msg->resultCode != L2RC_SUCCESS) {
        ...
        printf(
            "If this fails, steam_appid.txt was probably not in the working "
            "directory.\n"
        );
    ...
    while (!quit) {
        ...
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_ALREADY_CONNECTED:
                ...
                break;
            case ID_REMOTE_DISCONNECTION_NOTIFICATION: // Server telling the clients
                                                       // of another client
                                                       // disconnecting gracefully.
                                                       // You can manually broadcast
                                                       // this in a peer to peer
                                                       // enviroment if you want.
                printf("ID_REMOTE_DISCONNECTION_NOTIFICATION\n");
                break;
            case ID_REMOTE_CONNECTION_LOST: // Server telling the clients of another
                                            // client disconnecting forcefully.  You
                                            // can manually broadcast this in a peer
            ...
            case ID_REMOTE_NEW_INCOMING_CONNECTION: // Server telling the clients of
                                                    ...
                                                    // can manually broadcast this in
                                                    // a peer to peer enviroment if
                                                    // you want.
            ...
            case ID_FCM2_NEW_HOST: {
                if (packet->systemAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("Got new host (ourselves)");
        ...
        if (kbhit()) {
            ...
            case 'b': {
                ...
                RakNet::Console_LeaveRoom_Steam* msg =
                    (RakNet::Console_LeaveRoom_Steam*)messageFactory->Alloc(RakNet::L2MID_Console_LeaveRoom);
            ...
            case 'c': {
                ...
                RakNet::Console_CreateRoom_Steam* msg =
                    (RakNet::Console_CreateRoom_Steam*)messageFactory->Alloc(RakNet::L2MID_Console_CreateRoom);
            ...
            case 'd': {
                ...
                RakNet::Console_JoinRoom_Steam* msg =
                    (RakNet::Console_JoinRoom_Steam*)messageFactory->Alloc(RakNet::L2MID_Console_JoinRoom);
            ...
            case 'e': {
                ...
                RakNet::Console_GetRoomDetails_Steam* msg =
                    (RakNet::Console_GetRoomDetails_Steam*)messageFactory->Alloc(RakNet::L2MID_Console_GetRoomDetails);
                msg->roomId = lobby2Client->GetRoomID();
                lobby2Client->SendMsg(msg);
                messageFactory->Dealloc(msg);
            } break;
            ...
            case 'f': {
                ...
                RakNet::Console_SendRoomChatMessage_Steam* msg =
                    (RakNet::Console_SendRoomChatMessage_Steam*)messageFactory->Alloc(
                        RakNet::L2MID_Console_SendRoomChatMessage
                    );
            ...
            case 'g': {
                ...
                for (unsigned int i = 0; i < roomMembers.Size(); i++) {
                    printf(
                        "%i. %s ID=%" PRINTF_64_BIT_MODIFIER "u\n",
                        i + 1,
                        lobby2Client->GetRoomMemberName(roomMembers[i]),
                        roomMembers[i]
                    );
                }
            } break;
