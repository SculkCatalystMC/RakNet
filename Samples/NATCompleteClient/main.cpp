...
enum SampleResult { PENDING, FAILED, SUCCEEDED };
...
SystemAddress ConnectBlocking(
    ...
    char ipAddr[64];
    if (defaultAddress == 0 || defaultAddress[0] == 0) printf("Enter IP of system %s is running on: ", hostName);
    else printf("Enter IP of system %s, or press enter for default: ", hostName);
    ...
    char port[64];
    if (defaultAddress == 0 || defaultAddress[0] == 0) printf("Enter port of system %s is running on: ", hostName);
    else printf("Enter port of system %s, or press enter for default: ", hostName);
    ...
    virtual void        Init(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (devlist) {
            ...
            if (UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr)) == 1) {
                // 4/16/2012 Why was I doing this? Just to read my external port? That
                // shouldn't be necessary
                /*
                SystemAddress serverAddress=SelectAmongConnectedSystems(rakPeer,
                "NatTypeDetectionServer"); if
                (serverAddress==RakNet::UNASSIGNED_SYSTEM_ADDRESS)
                {
                        serverAddress=ConnectBlocking(rakPeer, "NatTypeDetectionServer",
                DEFAULT_SERVER_ADDRESS, DEFAULT_SERVER_PORT); if
                (serverAddress==RakNet::UNASSIGNED_SYSTEM_ADDRESS)
                        {
                ...
                */
                ...
                if (r != UPNPCOMMAND_SUCCESS) {
                    ...
                    sampleResult = FAILED;
                } else sampleResult = SUCCEEDED;
            } else sampleResult = FAILED;
        } else sampleResult = FAILED;
    }
...
struct NatTypeDetectionFramework : public SampleFramework {
    ...
    NatTypeDetectionFramework() {
        sampleResult = SUPPORT_NAT_TYPE_DETECTION;
        ntdc         = 0;
    }
    ...
    virtual const char* QueryFunction(void) {
        return "Determines router type to avoid NAT punch attempts that "
               "cannot\nsucceed.";
    }
    virtual const char* QuerySuccess(void) {
        return "If our NAT type is Symmetric, we can skip NAT punch to other "
               "symmetric NATs.";
    }
    ...
    virtual void Init(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
            serverAddress =
                ConnectBlocking(rakPeer, "NatTypeDetectionServer", DEFAULT_SERVER_ADDRESS, DEFAULT_SERVER_PORT);
            if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
                printf("Failed to connect to a server.\n");
                sampleResult = FAILED;
                return;
    ...
    virtual void ProcessPacket(Packet* packet) {
        if (packet->data[0] == ID_NAT_TYPE_DETECTION_RESULT) {
            ...
            printf(
                "NAT Type is %s (%s)\n",
                NATTypeDetectionResultToString(r),
            ...
            if (r == RakNet::NAT_TYPE_PORT_RESTRICTED || r == RakNet::NAT_TYPE_SYMMETRIC) {
                ...
                printf(
                    "Note: Your router must support UPNP or have the user manually "
                    "forward ports.\n"
                );
    ...
    virtual void Update(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (sampleResult == PENDING && RakNet::GetTimeMS() > timeout) {
            printf(
                "No response from the server, probably not running "
                "NatTypeDetectionServer plugin.\n"
            );
            sampleResult = FAILED;
        }
    }
...
struct NatPunchthoughClientFramework : public SampleFramework, public NatPunchthroughDebugInterface_Printf {
    ...
    NatPunchthoughClientFramework() {
        sampleResult = SUPPORT_NAT_PUNCHTHROUGH;
        npClient     = 0;
    }
    virtual const char* QueryName(void) { return "NatPunchthoughClientFramework"; }
    virtual bool        QueryRequiresServer(void) { return true; }
    virtual const char* QueryFunction(void) {
        return "Causes two systems to try to connect to each other at the "
               "same\ntime, to get through routers.";
    }
    virtual const char* QuerySuccess(void) {
        return "We can now communicate with the other system, including "
               "connecting.";
    }
    virtual bool QueryQuitOnSuccess(void) { return true; }
    virtual void Init(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
            serverAddress =
                ConnectBlocking(rakPeer, "NatPunchthroughServer", DEFAULT_SERVER_ADDRESS, DEFAULT_SERVER_PORT);
            if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
                printf("Failed to connect to a server.\n");
                sampleResult = FAILED;
                return;
        ...
        char guid[128];
        printf(
            "Enter RakNetGuid of the remote system, which should have already "
    ...
    virtual void ProcessPacket(Packet* packet) {
        if (packet->data[0] == ID_NAT_TARGET_NOT_CONNECTED || packet->data[0] == ID_NAT_TARGET_UNRESPONSIVE
            ...
            else printf("NAT punch success from remote system %s.\n", packet->systemAddress.ToString(true));
...
struct Router2Framework : public SampleFramework {
    // Set to FAILED to skip this test
    Router2Framework() {
        sampleResult = SUPPORT_ROUTER2;
        router2      = 0;
    }
    virtual const char* QueryName(void) { return "Router2Framework"; }
    virtual bool        QueryRequiresServer(void) { return false; }
    virtual const char* QueryFunction(void) {
        return "Connect to a peer we cannot directly connect to using "
    ...
    virtual void        Init(RakNet::RakPeerInterface* rakPeer) {
        ...
        printf(
            "Given your application's bandwidth, how much traffic can be "
                   "forwarded through a single peer?\nIf you use more than half the "
                   "available bandwidth, then this plugin won't work for you.\n"
        );
        ;
    ...
    virtual void Update(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (sampleResult == PENDING && RakNet::GetTimeMS() > timeout) {
            printf(
                "No response from any system, probably not running Router2 "
                "plugin.\n"
            );
            sampleResult = FAILED;
        }
...
struct UDPProxyClientFramework : public SampleFramework, public UDPProxyClientResultHandler {
    ...
    UDPProxyClientFramework() {
        sampleResult = SUPPORT_UDP_PROXY;
        udpProxy     = 0;
    }
    ...
    virtual void Init(RakNet::RakPeerInterface* rakPeer) {
        ...
        if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
            serverAddress =
                ConnectBlocking(rakPeer, "UDPProxyCoordinator", DEFAULT_SERVER_ADDRESS, DEFAULT_SERVER_PORT);
            if (serverAddress == RakNet::UNASSIGNED_SYSTEM_ADDRESS) {
                printf("Failed to connect to a server.\n");
                sampleResult = FAILED;
                return;
        ...
        if (guid[0]) {
            RakNetGUID remoteSystemGuid;
            remoteSystemGuid.FromString(guid);
            udpProxy->RequestForwarding(
                serverAddress,
                UNASSIGNED_SYSTEM_ADDRESS,
                targetGuid,
                UDP_FORWARDER_MAXIMUM_TIMEOUT,
                0
            );
            isListening = false;
        } else {
    ...
    virtual void OnForwardingSuccess(
        ...
        ConnectionAttemptResult car =
            proxyClientPlugin->GetRakPeerInterface()->Connect(proxyIPAddress, proxyPort, 0, 0);
...
void PrintPacketMessages(Packet* packet, RakPeerInterface* rakPeer) {
    ...
    case ID_REMOTE_DISCONNECTION_NOTIFICATION: // Server telling the clients of
                                               ...
                                               // broadcast this in a peer to peer
    ...
    case ID_REMOTE_NEW_INCOMING_CONNECTION: // Server telling the clients of
                                            // another client connecting.  You can
                                            // manually broadcast this in a peer
                                            // to peer enviroment if you want.
    ...
    case ID_CONNECTION_REQUEST_ACCEPTED:
        ...
        printf(
            "ID_CONNECTION_REQUEST_ACCEPTED to %s with GUID %s\n",
            packet->systemAddress.ToString(true),
...
int main(void) {
    ...
    for (i = 0; i < FEATURE_LIST_COUNT; i++) {
        ...
        printf(
            "\n%s\nRequires server: %s\nDescription: %s\n",
            samples[i]->QueryName(),
            samples[i]->QueryRequiresServer() == 1 ? "Yes" : "No",
            samples[i]->QueryFunction()
        );
    }
    ...
    if (hasServer == false)
        printf(
            ...
            "may want to consider using the Lobby2/Steam project. They host "
            "the\nservers for you.\n\n"
        );
