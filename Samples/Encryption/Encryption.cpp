...
void PrintPacketHeader(Packet* packet) {
    ...
    case ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY:
        printf(
            "Connection request failed - Remote system requires secure "
            "connections, pass a public key to RakPeerInterface::Connect()\n"
        );
        break;
    case ID_OUR_SYSTEM_REQUIRES_SECURITY:
        printf(
            "Connection request failed - We passed a public key to "
            "RakPeerInterface::Connect(), but the other system did not have "
            "security turned on\n"
        );
        break;
    ...
    case ID_NEW_INCOMING_CONNECTION: {
        ...
        if (rakPeer1->GetClientPublicKeyFromSystemAddress(packet->systemAddress, client_public_key)) {
            ...
            for (int ii = 0; ii < (int)sizeof(client_public_key); ++ii) printf("%02x ", (cat::u8)client_public_key[ii]);
            printf("\n");
        } else {
            printf(
                "Server: New connected client provided no public key. (This is an "
                "error if you are doing two-way authentication)\n"
            );
        }
    ...
    case ID_DISCONNECTION_NOTIFICATION:
        printf(
            ...
            "disconnected from us.  For the client, this would mean the server "
            "has shutdown.\n"
        );
        break;
    default:
        printf("Got type %i : ", (int)packet->data[0]);
        for (int ii = 0; ii < (int)packet->length; ++ii) printf("%02x ", (cat::u8)packet->data[ii]);
        printf("\n");
        break;
    }
}
...
int main(void) {
    ...
    // Optional: used only for two-way authentication mode (a slower mode not
    // recommended for normal client-server or peer-peer connections)
    char client_public_key[cat::EasyHandshake::PUBLIC_KEY_BYTES];
        ...
        if (str[0] == '1') {
            ...
            if (str[0]) {
                ...
                if (fp) {
                    ...
                    } else printf("Not loading private key.\n");
                } else {
                    printf("ERROR:Failed to open %s.\n", str);
                }
            } else printf("Not loading public keys.\n");

            PrintOptions();
        } else if (str[0] == '3' || str[0] == '4') {
            bool run_test = true;

            // NOTE: Reiterating, normally you should not use two-way authentication
            // for client-server or peer-peer applications as it only makes sense for
            // server-server connections that rarely occur or if you know what you are
            // doing.
            ...
            if (!run_test) {
                ...
                if (CONNECTION_ATTEMPT_STARTED != rakPeer2->Connect(TEST_SERVER_ADDRSTR, TEST_SERVER_PORT, 0, 0, &pk)) {
                    printf(
                        "ERROR: Connect() returned false - invalid public key most "
                        "likely\n"
                    );
                }
                ...
                while (RakNet::GetTimeMS() < time) {
                    ...
                    if (packet) {
                        ...
                        if (doTwoWayAuthentication) {
                            ...
                            RakAssert(
                                rakPeer1
                                    ->GetClientPublicKeyFromSystemAddress(packet->systemAddress, client_public_key_copy)
                                == true
                            )
                        }
                ...
                if (peer1GotMessage == false) printf("ERROR: Host got no packets\n");
