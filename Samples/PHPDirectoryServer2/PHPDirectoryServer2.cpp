...
void PHPDirectoryServer2::SetField(RakNet::RakString columnName, RakNet::RakString value) {
    ...
    if (columnName == GAME_NAME_COMMAND || columnName == GAME_PORT_COMMAND || columnName == LAST_UPDATE_COMMAND) {
...
unsigned int PHPDirectoryServer2::GetFieldCount(void) const { return fields.Size(); }
...
void PHPDirectoryServer2::UploadTable(
    RakNet::RakString uploadPassword,
    ...
    if (autoRepost) nextRepost = RakNet::GetTimeMS() + 50000;
    else nextRepost = 0;
}
    ...
    while (c && *c) {
        ...
        if (*c == '\001') {
            if (isCommand) {
                ...
                isCommand = false;
                if (buff[0] != 0) resultCode = HTTP_RESULT_GOT_TABLE;
            } else {
                buff[buffIndex] = 0;
            ...
            if (buffIndex < 256 - 1) buff[buffIndex++] = *c;
        }
        c++;
    }
...
const DataStructures::Table* PHPDirectoryServer2::GetLastDownloadedTable(void) const { return &lastDownloadedTable; }
void                         PHPDirectoryServer2::SendOperation(void) {
    ...
    for (unsigned i = 0; i < fields.Size(); i++) {
        ...
        value.URLEncode();
        outgoingMessageBody += RakString("\001%s\001%s", fields.GetKeyAtIndex(i).C_String(), value.C_String());
    }
...
void PHPDirectoryServer2::Update(void) {
    ...
    if (nextRepost == 0 || fields.Size() == 0) return;
