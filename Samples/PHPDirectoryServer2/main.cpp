...
/// \file
/// \brief This file is a sample for using HTTPConnection and
/// PHPDirectoryServer2
...
// Allocate rather than create on the stack or the RakString mutex crashes on
// shutdown
TCPInterface*        tcp;
...
ReadResultEnum ReadResult(RakNet::RakString& httpResult) {
    ...
    while (RakNet::GetTimeMS() < endTime) {
        ...
        if (httpConnection->HasRead()) {
            ...
            // If resultCode is not an empty string, then we got something other than
            // a table (such as delete row success notification, or the message is for
            // HTTP only and not for this class).
...
void DownloadTable() { phpDirectoryServer2->DownloadTable("a"); }
...
bool VerifyDownloadMatchesUpload(int requiredRowCount, int testRowIndex) {
    ...
    if (phpDirectoryServer2->GetFieldCount() + 4 != games->GetColumnCount()) {
        printf(
            "TEST FAILED. Expected %i columns, got %i\n",
            phpDirectoryServer2->GetFieldCount() + 4,
            games->GetColumnCount()
        );
    ...
    for (i = 0; i < phpDirectoryServer2->GetFieldCount(); i++) {
        ...
        for (colIndex = 0; colIndex < columns.Size(); colIndex++) {
            if (strcmp(columnName.C_String(), columns[colIndex].columnName) == 0) break;
        }
        ...
        if (strcmp(value.C_String(), row->cells[colIndex]->c) != 0) {
            printf(
                "TEST FAILED. Expected row with value '%s' at index %i for column "
                "%s. Got '%s'.\n",
                value.C_String(),
                i,
                columnName.C_String(),
                row->cells[colIndex]->c
            );
            return false;
        }
    }
...
bool RunTest() {
    ...
    phpDirectoryServer2->SetField(
        "TestField5",
        "A somewhat big long string as these things "
...
void TestPHPDirectoryServer(int argc, char** argv) {
    ...
    printf(
        "Set columns and one row for your game, and upload it to a\nviewable "
        "and downloadable webpage.\n"
    );
    ...
    // 	phpDirectoryServer2 =
    // RakNet::OP_NEW<PHPDirectoryServer2>(_FILE_AND_LINE_);
    ...
    if (argc == 3) {
        ...
        if (website[0] == 0) strcpy(website, "jenkinssoftware.com");

        printf(
            "Enter path to DirectoryServer.php, e.g. "
            "raknet/DirectoryServer.php:\n"
        );
        Gets(pathToPHP, sizeof(pathToPHP));
        if (pathToPHP[0] == 0) strcpy(pathToPHP, "/raknet/DirectoryServer.php");
    }
    ...
    // This creates an HTTP connection using TCPInterface. It allows you to Post
    // messages to and parse messages from webservers. The connection attempt is
    // asynchronous, and is handled automatically as HTTPConnection::Update() is
    // called
    httpConnection->Init(tcp, website);

    // This adds specific parsing functionality to HTTPConnection, in order to
    // communicate with DirectoryServer.php
    ...
    // The destructor of each of these references the other, so delete in this
    // order
...
void TestGet(void) {
    printf(
        "This is NOT a reliable way to download from a website. Use libcurl "
        "instead.\n"
    );
...
int main(int argc, char** argv) {
    ...
    // Start the TCP thread. This is used for general TCP communication, whether
    // it is for webpages, sending emails, or telnet
    tcp->Start(0, 64);
