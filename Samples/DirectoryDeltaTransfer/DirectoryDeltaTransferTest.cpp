...
int main(void) {
    ...
    // directoryDeltaTransfer is the main plugin that does the work for this
    // sample.
    RakNet::DirectoryDeltaTransfer directoryDeltaTransfer;
    // The fileListTransfer plugin is used by the DirectoryDeltaTransfer plugin
    // and must also be registered (you could use this yourself too if you wanted,
    // of course).
    ...
    printf(
        "This sample demonstrates the plugin to incrementally transfer "
        "compressed\n"
    );
    printf("deltas of directories.  In essence, it's a simple autopatcher.\n");
    printf(
        "Unlike the full autopatcher, it has no dependencies.  It is suitable "
        "for\n"
    );
    ...
    printf(
        "Enter listen port. Enter for default. If running two instances on "
        "the\nsame computer, use 0 for the client.\n"
    );
    unsigned short localPort;
    char           str[256];
    Gets(str, sizeof(str));
    if (str[0] == 0) localPort = 60000;
    else localPort = atoi(str);
    ...
    while (1) {
        ...
        rakPeer->GetStatistics(rakPeer->GetSystemAddressFromIndex(0));
        //	if (statistics->messagesOnResendQueue>0 ||
        statistics->internalOutputQueueSize>0) if
        (rakPeer->GetSystemAddressFromIndex(0)!=RakNet::UNASSIGNED_SYSTEM_ADDRESS)
                {
        ...
        */
        ...
        if (tcp1.HasLostConnection() != RakNet::UNASSIGNED_SYSTEM_ADDRESS) printf("ID_DISCONNECTION_NOTIFICATION\n");
        ...
        if (kbhit()) {
            ...
            if (ch == 's') {
                ...
                Gets(str, sizeof(str));
                if (str[0] == 0) strcpy(str, "C:/Temp");
                directoryDeltaTransfer.SetApplicationDirectory(str);
                printf(
                    "This directory will be prefixed to upload and download "
                    "subdirectories.\n"
                );
            } else if (ch == 'a') {
                ...
                printf(
                    "This directory may be any uploaded directory, or a subdir "
                    "therein.\n"
                );
                ...
                setId = directoryDeltaTransfer.DownloadFromSubdirectory(
                    subdir,
                    outputSubdir,
                    true,
                    sysAddrZero,
                ...
                if (setId == (unsigned short)-1) printf("Download failed.  Host unreachable.\n");
                ...
                Gets(str, sizeof(str));
                if (str[0] == 0) remotePort = 60000;
                else remotePort = atoi(str);
#ifdef USE_TCP
