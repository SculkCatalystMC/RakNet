...
public:
    ...
    virtual void OnFileProgress(FileProgressStruct* fps) {
        printf(
            "OnFileProgress: %i partCount=%i partTotal=%i (%i%%) %i/%i %s "
            "%ib/%ib %ib/%ib total\n",
...
class TestFileListProgress : public RakNet::FileListProgress {
    virtual void OnFilePush(
        ...
        bool                  done,
        RakNet::SystemAddress targetSystem,
...
int main() {
    ...
    printf(
        "the FileListTransferPlugin. Incremental sends will read and send "
        "the.\n"
    );
    printf("file only as needed, rather than putting the whole file in memory.\n");
    printf(
        "This is to support sending large files to many users at the same "
        "time.\n"
    );
    ...
    // Run incremental reads in a thread so the read does not block the main
    // thread
    ...
    char str[256];
    Gets(str, sizeof(str));
    if (str[0] == 0) strcpy(str, "D:\\RakNet\\Lib\\RakNetLibStaticDebug.lib");
    file     = str;
    fileCopy = file + "_copy";
    // Reference this file, rather than add it in memory. Will send 1000 byte
    // chunks. The reason to do this is so the whole file does not have to be in
    // memory at once
    ...
    fileList
        .AddFile(file.C_String(), file.C_String(), 0, fileLength, fileLength, FileListNodeContext(0, 0, 0, 0), true);
    ...
    // When connected, send the file. Since the file is a reference, it will be
    // sent incrementally
    while (1) {
#ifdef USE_TCP
