...
enum { CLIENT_1, CLIENT_2, SERVER_1, SERVER_2, RAKPEER_COUNT };

enum { CLOUD_CLIENT_1, CLOUD_CLIENT_2, CLOUD_CLIENT_COUNT };

enum { CLOUD_SERVER_1, CLOUD_SERVER_2, CLOUD_SERVER_COUNT };
...
class MyCallback : public RakNet::CloudClientCallback {
    ...
    OnSubscriptionNotification(RakNet::CloudQueryRow* result, bool wasUpdated, bool* deallocateRowAfterReturn) {
        if (wasUpdated) printf("OnSubscriptionNotification Updated\n");
        else printf("OnSubscriptionNotification Deleted\n");
    }
};
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (command == 'a' || command == 'A') {
                ...
                cloudClient[CLOUD_CLIENT_1].Post(
                    &dataKey1,
                    (const unsigned char*)"DS1C1S1",
                    (uint32_t)strlen("DS1C1S1") + 1,
                    rakPeer[CLIENT_1]->GetGUIDFromIndex(0)
                );
                ...
                dataKey1.primaryKey   = "ApplicationName";
                dataKey1.secondaryKey = 1;
                cloudClient[CLOUD_CLIENT_2].Post(
                    &dataKey1,
                    (const unsigned char*)"DS1C2S2",
                    (uint32_t)strlen("DS1C2S2") + 1,
                    rakPeer[CLIENT_2]->GetGUIDFromIndex(0)
                );
                ...
                cloudClient[CLOUD_CLIENT_1].Post(
                    &dataKey1,
                    (const unsigned char*)"DS2C2S1",
                    (uint32_t)strlen("DS2C2S1") + 1,
                    rakPeer[CLIENT_1]->GetGUIDFromIndex(0)
                );
        ...
        for (unsigned int rakPeerInstanceIndex = 0; rakPeerInstanceIndex < RAKPEER_COUNT; rakPeerInstanceIndex++) {
            for (packet = rakPeer[rakPeerInstanceIndex]->Receive(); packet;
                 rakPeer[rakPeerInstanceIndex]->DeallocatePacket(packet),
                packet = rakPeer[rakPeerInstanceIndex]->Receive()) {
                ...
                case ID_CLOUD_GET_RESPONSE:
                    cloudClient[rakPeerInstanceIndex]
                        .OnGetReponse(packet, &myCloudClientCallback, &myCloudClientAllocator);
                    break;
                case ID_CLOUD_SUBSCRIPTION_NOTIFICATION:
                    cloudClient[rakPeerInstanceIndex]
                        .OnSubscriptionNotification(packet, &myCloudClientCallback, &myCloudClientAllocator);
                    break;
                default:
                    printf("Packet ID %i\n", packet->data[0]);
                }
