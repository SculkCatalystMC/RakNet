...
// So higher frames per buffer means that there will be a larger latency before
// you hear the sound However, it would lock and unlock the buffer more often,
...
// I think one frame is a set of samples equal to the number of channels.  I'm
// not sure though or how that sample is arranged.
static int
PACallback(void* inputBuffer, void* outputBuffer, unsigned long framesPerBuffer, PaTimestamp outTime, void* userData) {

    if (inputBuffer && !mute) {
        // TODO - if the input data is mostly silence, don't send and save the
        // bandwidth.
...
int main(void) {
    ...
    printf(
        "A sample on how to use RakVoice. You need a microphone for this "
        "sample.\n"
    );
    ...
    printf(
        "and include the files under libspeex, except those that start with "
        "test.\n"
    );
    printf(
        "PortAudio is also included and is used to read and write audio data. "
        " You\n"
    );
    printf(
        "can substitute whatever you want if you do not want to use "
        "portaudio.\n"
    );
    printf("Difficulty: Advanced\n\n");

    // Since voice is peer to peer, we give the option to use the nat punchthrough
    // client if desired.
    ...
    { // Linux fix. Won't compile without it. Because of the goto error above, the
        // scope is ambigious. Make it a block to define that it will not be used
        // after the jump.
        ...
        if (useNatPunchthrough) {
            ...
            if (facilitatorIP[0] == 0) strcpy(facilitatorIP, "natpunch.jenkinssoftware.com");
        ...
        if (useNatPunchthrough == false)
            printf(
                "(Q)uit. (C)onnect. (D)isconnect. C(l)ose voice channels. (M)ute. "
                "' ' for stats.\n"
            );
                ...
                if (p->data[0] == ID_CONNECTION_REQUEST_ACCEPTED) {
                    ...
                    if (p->systemAddress == facilitator) {
                        ...
                        printf("ID_CONNECTION_ATTEMPT_FAILED\n");
                           ...
                           || p->data[0] == ID_RAKVOICE_OPEN_CHANNEL_REPLY) {
