    ...
    while (nextTest != 2) {
        ...
        if (packet) {
            if (packet->data[0] == ID_ADVERTISE_SYSTEM) {
                if (packet->length > 1) printf("Got Advertise system with data: %s\n", packet->data + 1);
                else printf("Got Advertise system with no data\n");
                ...
                if (peer2->IsLocalIP(packet->systemAddress.ToString(false))) printf("ID_UNCONNECTED_PONG from our own");
                else printf("ID_UNCONNECTED_PONG from");
                printf(
                ...
                if (dataLength > 0) {
                    ...
                    RakAssert(
                        (int)strlen("Offline Ping Data") + 1
                        == packet->length - (sizeof(unsigned char) + sizeof(RakNet::TimeMS))
                    );
                }
