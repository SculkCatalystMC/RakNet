...
// These two plugins are just to automatically create a fully connected mesh so
// I don't have to call connect more than once
...
int main(void) {
    ...
    printf(
        "It is used in a peer to peer environment to have a group of\nsystems "
        "signal an event.\n"
    );
    printf(
        "It is useful for changing turns in a turn based game,\nor for lobby "
        "systems where everyone has to set ready before the game starts\n"
    );
    ...
    while (1) {
        ...
        if (ch == 's' || ch == 'S') {
            ch = 0;
            if (readyEventPlugin.SetEvent(0, true)) printf("This system is signaled\n");
            else printf("This system is signaled FAILED\n");
        }
        if (ch == 'u' || ch == 'U') {
            ch = 0;
            if (readyEventPlugin.SetEvent(0, false)) printf("This system is unsignaled\n");
            else printf("This system is unsignaled FAILED\n");
        }
        if (ch == 'c' || ch == 'C') {
            ch = 0;
            printf("Which IP? (Press enter for 127.0.0.1)");
            gets(str);
            if (str[0] == 0) strcpy(str, "127.0.0.1");
            char port[64];
        ...
        if (ch == 'f' || ch == 'F') {
            ...
            printf(
                "Called ForceCompletion()\nIsEventCompleted() will be fixed at "
        ...
        if (ch == ' ') {
            ...
            printf("\n");
            PrintConnections();
            if (readyEventPlugin.IsEventSet(0)) printf("Signaled=True, ");
            else printf("Signaled=False, ");

            if (readyEventPlugin.IsEventCompleted(0)) printf("Completed=True\n");
            else if (readyEventPlugin.IsEventCompletionProcessing(0)) printf("Completed=InProgress\n");
            else printf("Completed=False\n");
...
void PrintConnections() {
    ...
    for (i = 0; i < numberOfSystems; i++) {
        ...
        for (j = 0; j < numberOfSystems; j++) {
            ...
            if (systemAddress != UNASSIGNED_SYSTEM_ADDRESS) printf("%i ", systemAddress.GetPort());
        }
