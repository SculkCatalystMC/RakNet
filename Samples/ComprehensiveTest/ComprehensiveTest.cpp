...
int main(void) {
    ...
    printf(
        "This is just a test app to run a bit of everything to test for "
        "crashes.\n"
    );
    ...
    while (RakNet::GetTimeMS() < endTime) {
        ...
        if (nextAction < .04f) {
            ...
            if ((randomMT() % NUM_PEERS) == 0) target = RakNet::UNASSIGNED_SYSTEM_ADDRESS;
    ...
    for (i = 0; i < NUM_PEERS; i++) RakNet::RakPeerInterface::DestroyInstance(peers[i]);
