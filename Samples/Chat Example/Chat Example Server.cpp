...
// We copy this from Multiplayer.cpp to keep things all in one file for this
// example
...
_CONSOLE_2_SetSystemProcessParams
    ...
    // Record the first client that connects to us so we can pass it to the ping
    // function
    ...
    if (portstring[0] == 0) strcpy(portstring, "1234");

    puts("Starting server.");
    // Starting the server is very simple.  2 players allowed.
    // 0 means we don't care about a connectionValidationInteger, and false
    // for low priority threads
    // I am creating two socketDesciptors, to create two sockets. One using IPV6
    // and the other IPV4
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            char message2[2048];
            // Append Server: to the message so clients know that it ORIGINATED from
            // the server All messages to all clients come from the server either
            // directly or by being relayed from other clients
            message2[0]                = 0;
            ...
            // HIGH_PRIORITY doesn't actually matter here because we don't use any
            // other priority RELIABLE_ORDERED means make sure the message arrives in
            // the right order We arbitrarily pick 0 for the ordering stream
            // RakNet::UNASSIGNED_SYSTEM_ADDRESS means don't exclude anyone from the
            // broadcast true means broadcast the message to everyone connected
            server->Send(
                message2,
        ...
        for (p = server->Receive(); p; server->DeallocatePacket(p), p = server->Receive()) {
            ...
            case ID_NEW_INCOMING_CONNECTION:
                ...
                printf(
                    "ID_NEW_INCOMING_CONNECTION from %s with GUID %s\n",
                    p->systemAddress.ToString(true),
                    p->guid.ToString()
                );
            ...
            default:
                // The server knows the static data of all clients, so we can prefix the
                // message With the name data
                printf("%s\n", p->data);

                // Relay the message.  We prefix the name for other clients.  This
                // demonstrates That messages can be changed on the server before being
                // broadcast Sending is the same as before
