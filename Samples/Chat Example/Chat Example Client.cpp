...
int main(void) {
    ...
    if (serverPort[0] == 0) strcpy(serverPort, "1234");
    ...
    bool b = client->Connect(ip, atoi(serverPort), "Rumpelstiltskin", (int)strlen("Rumpelstiltskin"), &pk)
          == RakNet::CONNECTION_ATTEMPT_STARTED;
#else
    RakNet::ConnectionAttemptResult car =
        client->Connect(ip, atoi(serverPort), "Rumpelstiltskin", (int)strlen("Rumpelstiltskin"));
    ...
    puts(
        ...
        "disconnect. 'connect' to reconnnect. Type to talk."
    );
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (strcmp(message, "connect") == 0) {
                ...
                if (b) puts("Attempting connection");
                else {
