...
bool CloudServerHelperFilter::OnPostRequest(
    ...
    if (clientGuid != serverGuid) {
        if (key.primaryKey == CLOUD_SERVER_CONNECTION_COUNT_PRIMARY_KEY) return false;
    }
    return true;
}
bool CloudServerHelperFilter::OnReleaseRequest(
    RakNetGUID                      clientGuid,
    SystemAddress                   clientAddress,
...
bool CloudServerHelperFilter::OnGetRequest(
    RakNetGUID                        clientGuid,
    SystemAddress                     clientAddress,
    CloudQuery&                       query,
...
bool CloudServerHelperFilter::OnUnsubscribeRequest(
    RakNetGUID                        clientGuid,
    SystemAddress                     clientAddress,
    DataStructures::List<CloudKey>&   cloudKeys,
    DataStructures::List<RakNetGUID>& specificSystems
...
void CloudServerHelper::PrintHelp(void) {
    ...
    printf(
        "Query running servers with CloudClient::Get() on key "
        "CloudServerList,0\n\n"
    );
    printf(
        "Query load with key CloudConnCount,0. Read row data as unsigned "
        "short.\n\n"
    );
    printf("Usage:\n");
    printf(
        "CloudServer.exe DNSHost Username:Password S2SPWD [Port] [ConnIn] "
        "[ConnOut]\n\n"
    );
    printf("Parameters:\n");
    ...
    printf(
        "ConnIn - Max outgoing connections, used for server to server. "
        "Default 64\n\n"
    );
    printf("Example:\n");
    printf(
        "CloudServer.exe test.dnsalias.net test:test qwerty1234 60000 1024 "
        "64\n\n"
    );
}
...
bool CloudServerHelper::StartRakPeer(RakNet::RakPeerInterface* rakPeer) {
    ...
    RakNet::StartupResult    sr = rakPeer->Startup(
        RakNet::CloudServerHelper::allowedIncomingConnections + RakNet::CloudServerHelper::allowedOutgoingConnections,
        &sd,
        1
    );
        ...
        for (packet = rakPeer->Receive(); packet; rakPeer->DeallocatePacket(packet), packet = rakPeer->Receive()) {
            ...
            case ID_CONNECTION_REQUEST_ACCEPTED:
                if (packet->guid == rakPeer->GetMyGUID()) {
                    // Just connected to myself! Host must be pointing to our own IP
                    // address.
...
bool CloudServerHelper_DynDns::SetHostDNSToThisSystemBlocking(void) {
    dynDNS->UpdateHostIPAsynch(dnsHost, 0, dynDNSUsernameAndPassword);
...
MessageID CloudServerHelper::AuthenticateRemoteServerBlocking(
...
void CloudServerHelper::SetupPlugins(
    ...
    // Connect to all systems told about via ConnectionGraph2::AddParticpant(). We
    // are only told about servers that have already been authenticated
    fullyConnectedMesh2->SetConnectOnNewRemoteConnection(true, "");
    // Do not add to the host trracking system all connections, only those
    // designated as servers
    ...
    // Do not add systems to the graph unless first validated as a server through
    // the TwoWayAuthentication plugin
    connectionGraph2->SetAutoProcessNewConnections(false);
}
void CloudServerHelper::OnPacket(
    Packet*                      packet,
    RakPeerInterface*            rakPeer,
...
bool CloudServerHelper::Update(void) { return true; }
CloudServerHelper_DynDns::CloudServerHelper_DynDns(DynDNS* _dynDns) { dynDNS = _dynDns; }
bool CloudServerHelper_DynDns::Update(void) {
    ...
    if (dynDNS->IsRunning()) {
        ...
        if (dynDNS->IsCompleted()) {
            printf("%s.\n", dynDNS->GetCompletedDescription());
            if (dynDNS->WasResultSuccessful() == false) return false;
...
int CloudServerHelper_DynDns::OnJoinCloudResult(
    ...
    if (packet->data[0] == ID_CONNECTION_ATTEMPT_FAILED) {
        ...
        if (SetHostDNSToThisSystemBlocking() == false) return 1;
...
int CloudServerHelper::OnJoinCloudResult(
    ...
    if (result == ID_CONNECTION_REQUEST_ACCEPTED) {
        ...
        */
        ...
        // dnsHost is always public, so if I can connect through it that's my public
        // IP
    ...
    // Force the external server address for queries. Otherwise it would report
    // 127.0.0.1 since the client is on localhost
...
int CloudServerHelper::JoinCloud(
    ...
    packet =
        RakNet::CloudServerHelper::ConnectToRakPeer(rakPeerIpOrDomain, RakNet::CloudServerHelper::rakPeerPort, rakPeer);
    if (packet == 0) return 1;

    int res = OnJoinCloudResult(
        packet,
        rakPeer,
        cloudServer,
        cloudClient,
