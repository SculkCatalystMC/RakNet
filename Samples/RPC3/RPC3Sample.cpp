...
// This has to be a pointer, because it uses UNASSIGNED_NETWORK_ID, initialized
// globally
...
// Shift operators have to be in the namespace RakNet or they might use the
// default one in BitStream.h instead. Error occurs with std::string
namespace RakNet {
// Specialize the << and >> operator to serialize each element of
// NormalizedVector individually This allows us to endian swap each parameter
// (which could not otherwise happen) and also send the data in a compressed
// form
...
public:
    ...
    int b;
    virtual void
    ClassMemberFunc(A* a1, A& a2, C* c1, D* d1, RakNet::BitStream* bs1, RakNet::BitStream& bs2, RakNet::RPC3* rpc3Inst);
};
...
public:
    D() {
        for (int i = 0; i < 10; i++) tenBytes[i] = i;
    }
    char tenBytes[10];
    bool Verify(void) {
        for (int i = 0; i < 10; i++)
            if (tenBytes[i] != i) return false;
        return true;
    }
    virtual void TestSlot(void) { printf("D::TestSlot\n"); }
};

// The number of parameters for a C++ function is limited by
// BOOST_FUSION_INVOKE_MAX_ARITY-1 found in
// boost/fusion/functional/invocation/limits.hpp I define this in RPC3_Boost.h
// to be 10. The default is 6. rpcFromNetwork is automatically filled in from
// the RPC class. Pass 0 when calling locally. Will be set to the plugin
// instance when this function is called from the remote system
void B::ClassMemberFunc(
    A*                 a1,
    A&                 a2,
    C*                 c1,
    D*                 d1,
...
// C and D derive from networkIDObject, so cannot be passed as references. A
// pointer is required to do the object lookup
void C::ClassMemberFunc(
    A*                 a1,
    A&                 a2,
    C*                 c1,
    D*                 d1,
    ...
    if (rpcFromNetwork == 0) {
        // The RakNet::RPC3 * parameter can be passed to Call() if you want to - it
        // is skipped and not serialized or deserialized so it doesn't matter. The
        // point of it is so when this function is called on the remote system, it
        // is set to the instance of the plugin so you can query network parameters
        // from the caller
        //
        // By default, pointers to objects that derive from NetworkIDObject (classes
        // C and D), only transmit the NetworkID of the object. If you also want to
        // dereference the pointer and serialize the object itself, use
        // RakNet::_RPC3::Deref(myVariable) In this case, parameters that both
        // derive from NetworkIDObject and are pointers are the variables c1 and d1
        // c1 will only transmit c1->GetNetworkID() (default behavior)
        // d1 will transmit d1->GetNetworkID() and also bitStream << (*d1) (contents
        // of the pointer)
        //
        rpc3Inst->CallCPP(
            "&C::ClassMemberFunc",
...
// The number of parameters for a C function is limited by
// BOOST_FUSION_INVOKE_MAX_ARITY found in
// boost/fusion/functional/invocation/limits.hpp I define this in RPC3_Boost.h
// to be 9. The default is 6.
void CFunc(
    ...
    // We pass 0 to the rpcFromNetwork when calling this function locally. When it
    // is called by the RPC3 system, it is set to the address of the plugin
    ...
    // the parameter "int intArray[10]" is actually a pointer due to the design of
    // C and C++ The RakNet::_RPC3::PtrToArray() function will tell the RPC3
    // system that this is actually an array of n elements Each element will be
    // endian swapped appropriately
    if (rpcFromNetwork == 0)
...
int main(void) {
    ...
    // Unlike AutoRPC, you don't have to specify if it is C or C++, or the number
    // of parameters
    RPC3_REGISTER_FUNCTION(rpc3Inst, CFunc);
    // Note the & operator as the macro and RPC3::RegisterFunction takes a class
    // pointer
    RPC3_REGISTER_FUNCTION(rpc3Inst, &C::ClassMemberFunc);
    RPC3_REGISTER_FUNCTION(rpc3Inst, &C::ClassMemberFunc2);

    // All slots are called when a signal is sent. This is true whether the slot
    // is local or remote
    ...
    char str[256];
    Gets(str, sizeof(str));
    if (str[0] == 's' || str[0] == 'S') isServer = true;
    else isServer = false;
    ...
    while (1) {
        for (p = rakPeer->Receive(); p; rakPeer->DeallocatePacket(p), p = rakPeer->Receive()) {
            ...
            case ID_NEW_INCOMING_CONNECTION: {
                ...
                int               intArray[10];
                for (int i = 0; i < sizeof(intArray) / sizeof(int); i++) intArray[i] = i;
