...
void Direct3DBase::CreateDeviceResources() {
    ...
    // support. Note the ordering should be preserved. Don't forget to declare
    // your application's minimum required feature level in its description.  All
    // applications are assumed to support 9.1 unless otherwise stated.
...
void Direct3DBase::CreateWindowSizeDependentResources() {
    ...
    swapChainDesc.Scaling               = DXGI_SCALING_STRETCH; // On phone, only stretch and aspect-ratio stretch
                                                                // scaling are allowed.
...
void Direct3DBase::UpdateForWindowSizeChange() {
    if (m_window->Bounds.Width != m_windowBounds.Width || m_window->Bounds.Height != m_windowBounds.Height) {
...
void Direct3DBase::ReleaseResourcesForSuspending() {
    ...
    // entering the background it is a good idea to free memory-intensive objects
    // that will be easy to restore upon reactivation. The swapchain and
    // backbuffer are good candidates here, as they consume a large amount of
    // memory and can be reinitialized quickly.
    m_swapChain        = nullptr;
    m_renderTargetView = nullptr;
    m_depthStencilView = nullptr;
}
...
void Direct3DBase::Present() {
    ...
    // application to sleep until the next VSync. This ensures we don't waste any
...
// Method to convert a length in device-independent pixels (DIPs) to a length in
// physical pixels.
