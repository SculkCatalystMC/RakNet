...
CubeRenderer::CubeRenderer() : m_loadingComplete(false), m_indexCount(0) {}
...
void CubeRenderer::CreateDeviceResources() {
    ...
    auto createVSTask = loadVSTask.then([this](Platform::Array<byte> ^ fileData) {
        ...
        DX::ThrowIfFailed(
            m_d3dDevice
                ->CreateInputLayout(vertexDesc, ARRAYSIZE(vertexDesc), fileData->Data, fileData->Length, &m_inputLayout)
        );
    });
    ...
    auto createCubeTask = (createPSTask && createVSTask).then([this]() {
        ...
        DX::ThrowIfFailed(m_d3dDevice->CreateBuffer(&indexBufferDesc, &indexBufferData, &m_indexBuffer));
    });
...
void CubeRenderer::CreateWindowSizeDependentResources() {
    ...
    XMStoreFloat4x4(
        ...
        XMMatrixTranspose(XMMatrixPerspectiveFovRH(fovAngleY, aspectRatio, 0.01f, 100.0f))
    );
}
