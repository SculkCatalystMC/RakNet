...
Lobby2ClientGFx3Impl::~Lobby2ClientGFx3Impl() { Shutdown(); }
void Lobby2ClientGFx3Impl::Init(
...
void ReadAccountBinaryData(FxResponseArgsList& rargs, RakNet::BitStream* serializedBinaryData) {
...
void Lobby2ClientGFx3Impl::MessageResult(Client_RegisterAccount* message) {
    ...
    // TODO - once the GUI is implemented, comment this out to make sure the
    // implemtor handled not having a valid email address
...
void Lobby2ClientGFx3Impl::MessageResult(System_CreateDatabase* message) {
    // After the database is created (immediately above), create the first title
    // and cd key
...
void Lobby2ClientGFx3Impl::MessageResult(Client_GetAccountDetails* message) {
    ...
    RakNet::BitStream serializedBinaryData(
        (unsigned char*)message->createAccountParameters.binaryData->binaryData,
        message->createAccountParameters.binaryData->binaryDataLength,
        false
    );
...
void Lobby2ClientGFx3Impl::MessageResult(RakNet::Client_GetIgnoreList* message) {
    FxResponseArgsList rargs;
    for (unsigned int i = 0; i < message->ignoredHandles.Size(); i++) rargs.Add(message->ignoredHandles[i].C_String());
...
void Lobby2ClientGFx3Impl::MessageResult(Clans_GetList* message) {
    FxResponseArgsList rargs;
    rargs.Add(Lobby2ResultCodeDescription::ToEnglish(message->resultCode));
    for (unsigned int i = 0; i < message->clanNames.Size(); i++) rargs.Add(message->clanNames[i].C_String());
...
void Lobby2ClientGFx3Impl::OnClosedConnection(
...
void Lobby2ClientGFx3Impl::OnFailedConnectionAttempt(
    Packet*                           packet,
